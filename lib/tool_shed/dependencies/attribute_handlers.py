...
class RepositoryDependencyAttributeHandler(object):
    ...
    def handle_complex_dependency_elem(self, parent_elem, elem_index, elem):
        ...
        if error_message:
            error_message += (
                "  The %s file contains an invalid <repository> tag."
        ...
        if not changeset_revision:
            ...
            if repository:
                ...
                if (
                    ...
                    error_message = (
                        "Invalid latest installable changeset_revision %s "
                ...
                error_message = (
                    "Unable to locate repository with name %s and owner %s.  "
                    % (str(name), str(owner))
                )
    ...
    def handle_sub_elem(self, parent_elem, elem_index, elem):
        ...
        for sub_index, sub_elem in enumerate(elem):
            ...
            if sub_elem.tag == "repository":
                ...
                if message:
                    error_message += (
                        "The %s file contains an invalid <repository> tag.  %s"
