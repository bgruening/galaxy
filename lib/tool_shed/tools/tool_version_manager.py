...
class ToolVersionManager(object):
    ...
    def get_tool_version_association(self, parent_tool_version, tool_version):
        ...
        return (
            ...
            .filter(
                and_(
                    self.app.install_model.ToolVersionAssociation.table.c.parent_id
                    == parent_tool_version.id,
                    self.app.install_model.ToolVersionAssociation.table.c.tool_id
                    == tool_version.id,
                )
            )
            .first()
    ...
    def get_version_lineage_for_tool(self, repository_id, repository_metadata, guid):
        ...
        for changeset in hg_util.reversed_lower_upper_bounded_changelog(
            repo, repository_metadata.changeset_revision, repository.tip(self.app)
        ):
            ctx = repo.changectx(changeset)
            rm = metadata_util.get_repository_metadata_by_changeset_revision(
                self.app, repository_id, str(ctx)
