...
from galaxy.tools import create_tool_from_source, get_tool_source, parameters, Tool
...
class ToolValidator(object):
    ...
    def check_tool_input_params(self, repo_dir, tool_config_name, tool, sample_files):
        ...
        for input_param in tool.input_params:
            if (
                isinstance(input_param, parameters.basic.SelectToolParameter)
                ...
                if options and isinstance(options, dynamic_options.DynamicOptions):
                    ...
                    if (
                        ...
                        if not sample_found:
                            correction_msg = (
                                "This file refers to a file named <b>%s</b>.  "
                                % str(index_file_name)
                            )
                            correction_msg += (
            ...
            if root.find(".hg") < 0:
                for name in files:
                    if name.endswith(".sample"):
                        ...
                        tool_util.copy_sample_file(
                            self.app, relative_path, dest_path=dest_path
                        )
    ...
    def get_list_of_copied_sample_files(self, repo, changeset_revision, dir):
        ...
        for changeset in hg_util.reversed_upper_bounded_changelog(
            ...
            for ctx_file in changeset_ctx.files():
                ...
                if (
                    ctx_file_name.endswith(".sample")
                    and ctx_file_name not in sample_files
                ):
    ...
    def load_tool_from_changeset_revision(
        self, repository_id, changeset_revision, tool_config_filename
    ):
        """
    ...
    def load_tool_from_config(self, repository_id, full_path):
        ...
        try:
            ...
            valid = False
            error_message = (
                'This file requires an entry for "%s" in the tool_data_table_conf.xml file.  Upload a file '
                % str(e)
            )
