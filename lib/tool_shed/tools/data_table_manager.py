...
class ShedToolDataTableManager(object):
    ...
    def generate_repository_info_elem_from_repository(
        self, tool_shed_repository, parent_elem=None, **kwd
    ):
    ...
    def get_tool_index_sample_files(self, sample_files):
        ...
        for s in sample_files:
            ...
            if (
                s.endswith(".loc.sample")
                or s.endswith(".xml.sample")
                or s.endswith(".txt.sample")
            ):
                tool_index_sample_files.append(str(s))
        return tool_index_sample_files

    def handle_missing_data_table_entry(
        self, relative_install_dir, tool_path, repository_tools_tups
    ):
        """
        ...
        if missing_data_table_entry:
            ...
            if sample_tool_data_table_conf:
                ...
                error, message = self.handle_sample_tool_data_table_conf_file(
                    sample_tool_data_table_conf, persist=True
                )
                if error:
    ...
    def get_target_install_dir(self, tool_shed_repository):
        tool_path, relative_target_dir = tool_shed_repository.get_tool_relative_path(
            self.app
        )
        # This is where index files will reside on a per repo/installed version basis.
        target_dir = os.path.join(
            self.app.config.shed_tool_data_path, relative_target_dir
        )
    ...
    def install_tool_data_tables(self, tool_shed_repository, tool_index_sample_files):
        ...
        for sample_file in tool_index_sample_files:
            ...
            if (
                not os.path.exists(target_path_filename)
                ...
                for file_elem in elem.findall("file"):
                    ...
                    if path:
                        file_elem.set(
                            "path",
                            os.path.normpath(
                                os.path.join(target_dir, os.path.split(path)[1])
                            ),
                        )
