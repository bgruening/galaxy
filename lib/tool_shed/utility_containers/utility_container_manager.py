...
class Folder(object):
    ...
    def contains_repository_dependency(self, repository_dependency):
        ...
        for contained_repository_dependency in self.repository_dependencies:
            if (
                contained_repository_dependency.listify
                == listified_repository_dependency
            ):
                return True
        return False
...
class UtilityContainerManager(object):
    ...
    def build_data_managers_folder(self, folder_id, data_managers, label=None):
        ...
        if data_managers:
            ...
            folder = Folder(
                id=folder_id, key=key, label=label, parent=data_managers_root_folder
            )
    ...
    def build_datatypes_folder(self, folder_id, datatypes, label="Datatypes"):
        ...
        if datatypes:
            ...
            folder_id += 1
            folder = Folder(
                id=folder_id, key="datatypes", label=label, parent=datatypes_root_folder
            )
    ...
    def build_invalid_data_managers_folder(
        self, folder_id, data_managers, error_messages=None, label=None
    ):
    ...
    def build_invalid_tools_folder(
        self,
        folder_id,
    ...
    def build_readme_files_folder(
        self, folder_id, readme_files_dict, label="Readme files"
    ):
    ...
    def build_repository_dependencies_folder(
        ...
        if repository_dependencies:
            ...
            repository_dependencies_folder, folder_id, repository_dependency_id = self.populate_repository_dependencies_container(
                repository_dependencies_folder,
                repository_dependencies,
                folder_id,
    ...
    def build_tools_folder(
        ...
        if tool_dicts:
            ...
            folder_id += 1
            folder = Folder(
                id=folder_id, key="tools", label=label, parent=tools_root_folder
            )
            if self.app.name == "galaxy":
        ...
        if tool_dependencies:
            ...
            if self.app.name == "galaxy":
                if new_install or reinstalling:
                    ...
                    folder.description = (
                        "click the name to install the missing dependency"
                    )
                else:
                    folder.description = (
                        "click the name to browse the dependency installation directory"
                    )
    ...
    def build_workflows_folder(
        ...
        if workflows:
            ...
            folder_id += 1
            folder = Folder(
                id=folder_id, key="workflows", label=label, parent=workflows_root_folder
            )
        ...
        if tool_shed_repository_id:
            ...
            if tool_shed_repository:
                if tool_shed_repository.missing_repository_dependencies:
                    installation_status = (
                        "%s, missing repository dependencies" % installation_status
                    )
                elif tool_shed_repository.missing_tool_dependencies:
                    installation_status = (
                        "%s, missing tool dependencies" % installation_status
                    )
    ...
    def handle_repository_dependencies_container_entry(
        ...
        for repository_dependency in rd_value:
            if self.app.name == "galaxy":
                tool_shed_repository_id, installation_status, repository_dependency = self.get_components_from_repository_dependency_for_installed_repository(
                    repository_dependency
                )
            else:
                tool_shed_repository_id = None
            ...
            if can_create_dependency:
                toolshed, repository_name, repository_owner, changeset_revision, prior_installation_required, only_if_compiling_contained_td = common_util.parse_repository_dependency_tuple(
                    repository_dependency
                )
                repository_dependency_id += 1
                repository_dependency = RepositoryDependency(
    ...
    def populate_repository_dependencies_container(
        ...
        for key, value in repository_dependencies.items():
            repository_dependencies_folder, folder_id, repository_dependency_id = self.handle_repository_dependencies_container_entry(
                repository_dependencies_folder,
                key,
                value,
                folder_id,
