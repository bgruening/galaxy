...
def in_tool_dict(
    tool_dict, exact_matches_checked, tool_id=None, tool_name=None, tool_version=None
):
    found = False
    if tool_id and not tool_name and not tool_version:
        ...
        tool_dict_tool_version = tool_dict["version"].lower()
        found = (
            tool_id == tool_dict_tool_id and tool_version == tool_dict_tool_version
        ) or (
            not exact_matches_checked
            and tool_dict_tool_id.find(tool_id) >= 0
        ...
        tool_dict_tool_name = tool_dict["name"].lower()
        found = (
            tool_version == tool_dict_tool_version and tool_name == tool_dict_tool_name
        ) or (
            not exact_matches_checked
            and tool_dict_tool_version.find(tool_version) >= 0
            and tool_dict_tool_name.find(tool_name) >= 0
        ...
        tool_dict_tool_id = tool_dict["id"].lower()
        found = (
            tool_version == tool_dict_tool_version
            and tool_name == tool_dict_tool_name
            and tool_id == tool_dict_tool_id
        ) or (
            not exact_matches_checked
            and tool_dict_tool_version.find(tool_version) >= 0
            and tool_dict_tool_name.find(tool_name) >= 0
...
def in_workflow_dict(workflow_dict, exact_matches_checked, workflow_name):
    ...
    return (workflow_name == workflow_dict_workflow_name) or (
        not exact_matches_checked
        and workflow_dict_workflow_name.find(workflow_name) >= 0
    )
...
def search_ids_names(
    ...
    for i, tool_id in enumerate(tool_ids):
        ...
        if in_tool_dict(
            tool_dict, exact_matches_checked, tool_id=tool_id, tool_name=tool_name
        ):
            match_tuples.append(
                (
...
def search_ids_versions(
    ...
    for i, tool_id in enumerate(tool_ids):
        ...
        if in_tool_dict(
            tool_dict, exact_matches_checked, tool_id=tool_id, tool_version=tool_version
        ):
            match_tuples.append(
                (
                    repository_metadata.repository_id,
                ...
                for tool_dict in tools:
                    if tool_ids and not tool_names and not tool_versions:
                        for tool_id in tool_ids:
                            if in_tool_dict(
                                tool_dict, exact_matches_checked, tool_id=tool_id
                            ):
                                match_tuples.append(
                                    (
                                        repository_metadata.repository_id,
                        ...
                        for tool_name in tool_names:
                            if in_tool_dict(
                                tool_dict, exact_matches_checked, tool_name=tool_name
                            ):
                                match_tuples.append(
                                    (
                                        repository_metadata.repository_id,
                        ...
                        if len(tool_ids) == len(tool_names):
                            ...
                            tool_ids, tool_names = make_same_length(
                                tool_ids, tool_names
                            )
                            match_tuples = search_ids_names(
                                tool_dict,
                        ...
                        if len(tool_ids) == len(tool_versions):
                            ...
                            tool_ids, tool_versions = make_same_length(
                        ...
                        if len(tool_versions) == len(tool_names):
                            ...
                            tool_versions, tool_names = make_same_length(
                                tool_versions, tool_names
                            )
                            match_tuples = search_names_versions(
                                tool_dict,
                                exact_matches_checked,
                        ...
                        if len(tool_versions) == len(tool_names) and len(
                            tool_names
                        ) == len(tool_ids):
                ...
                if workflow_names:
                    ...
                    for workflow_dict in workflows:
                        for workflow_name in workflow_names:
                            if in_workflow_dict(
                                workflow_dict, exact_matches_checked, workflow_name
                            ):
                                match_tuples.append(
                                    (
                                        repository_metadata.repository_id,
