...
def check_for_updates(app, model, repository_id=None):
    ...
    if repository_id is None:
        ...
        for repository in model.context.query(model.ToolShedRepository).filter(
            ...
            ok, updated = check_or_update_tool_shed_status_for_installed_repository(
                app, repository
            )
            if ok:
                success_count += 1
            ...
            if updated:
                updated_count += 1
        message = (
            "Checked the status in the tool shed for %d repositories.  " % success_count
        )
        ...
        if ok:
            if updated:
                message = (
                    "The tool shed status for repository <b>%s</b> has been updated."
                    % escape(str(repository.name))
                )
            else:
                message = (
                    "The status has not changed in the tool shed for repository <b>%s</b>."
                    % escape(str(repository.name))
                )
        else:
            message = (
                "Unable to retrieve status from the tool shed for repository <b>%s</b>."
                % escape(str(repository.name))
            )
            status = "error"
...
def create_or_update_tool_shed_repository(
    ...
    if tool_shed_repository:
        ...
        log.debug(
            "Adding new row for repository '%s' in the tool_shed_repository table, status set to '%s'."
            % (str(name), str(status))
        )
...
def create_repo_info_dict(
    ...
    repository = get_repository_by_name_and_owner(
        app, repository_name, repository_owner
    )
...
def create_repository_admin_role(app, repository):
    ...
    name = get_repository_admin_role_name(
        str(repository.name), str(repository.user.username)
    )
    description = (
        "A user or group member with this role can administer this repository."
    )
    role = app.model.Role(
        name=name, description=description, type=app.model.Role.types.SYSTEM
    )
    sa_session.add(role)
    sa_session.flush()
...
def create_repository(
    ...
    if category_ids:
        ...
        for category_id in category_ids:
            category = sa_session.query(app.model.Category).get(
                app.security.decode_id(category_id)
            )
...
def get_ids_of_tool_shed_repositories_being_installed(app, as_string=False):
    ...
    loading_datatypes_status = (
        app.install_model.ToolShedRepository.installation_status.LOADING_PROPRIETARY_DATATYPES
    )
    for tool_shed_repository in app.install_model.context.query(
        app.install_model.ToolShedRepository
    ).filter(
        or_(
            ...
            app.install_model.ToolShedRepository.status
            == installing_dependencies_status,
            app.install_model.ToolShedRepository.status == loading_datatypes_status,
        )
    ):
        installing_repository_ids.append(
            app.security.encode_id(tool_shed_repository.id)
        )
    if as_string:
...
def get_installed_repository(
    ...
    if installed_changeset_revision is not None:
        clause_list.append(
            app.install_model.ToolShedRepository.table.c.installed_changeset_revision
            == installed_changeset_revision
        )
...
def get_installed_tool_shed_repository(app, id):
    ...
    rval = [
        get_installed_repository(app=app, repository_id=repo_id)
...
def get_repo_info_dict(app, user, repository_id, changeset_revision):
    ...
    if not repository_metadata:
        ...
        next_downloadable_changeset_revision = metadata_util.get_next_downloadable_changeset_revision(
            app, repository, changeset_revision
        )
        if (
            next_downloadable_changeset_revision
            and next_downloadable_changeset_revision != changeset_revision
        ):
            repository_metadata = metadata_util.get_repository_metadata_by_changeset_revision(
    ...
    if repository_metadata:
        ...
        includes_tools_for_display_in_tool_panel = (
            repository_metadata.includes_tools_for_display_in_tool_panel
        )
        ...
        has_repository_dependencies, has_repository_dependencies_only_if_compiling_contained_td = get_repository_dependency_types(
            repository_dependencies
        )
...
def get_repo_info_tuple_contents(repo_info_tuple):
    ...
    if len(repo_info_tuple) == 6:
        description, repository_clone_url, changeset_revision, ctx_rev, repository_owner, tool_dependencies = (
            repo_info_tuple
        )
        repository_dependencies = None
...
def get_repositories_by_category(
    ...
    query = (
        ...
        .join(
            ...
            app.model.Repository.id
            == app.model.RepositoryCategoryAssociation.repository_id,
        )
    ...
    for repository in resultset:
        ...
        for changeset, changehash in repository.installable_revisions(app):
            ...
            metadata = metadata_util.get_repository_metadata_by_changeset_revision(
                app, encoded_id, changehash
            )
            repository_dict["metadata"][
                "%s:%s" % (changeset, changehash)
            ] = metadata.to_dict(value_mapper=default_value_mapper)
        if installable:
...
def get_repository_and_repository_dependencies_from_repo_info_dict(app, repo_info_dict):
    ...
    description, repository_clone_url, changeset_revision, ctx_rev, repository_owner, repository_dependencies, tool_dependencies = get_repo_info_tuple_contents(
        repo_info_tuple
    )
...
def get_repository_for_dependency_relationship(
    ...
    if tool_shed is None or name is None or owner is None or changeset_revision is None:
        ...
        message += (
            "required parameters is None: tool_shed: %s, name: %s, owner: %s, changeset_revision: %s "
...
def get_tool_shed_repository_by_id(app, repository_id):
    ...
    return (
        ...
        .filter(
            app.install_model.ToolShedRepository.table.c.id
            == app.security.decode_id(repository_id)
        )
        .first()
    )
...
def get_tool_shed_status_for_installed_repository(app, repository):
    ...
    try:
        ...
        tool_shed_status_dict = encoding_util.tool_shed_decode(
        ...
        log.exception(
            "Error attempting to get tool shed status for installed repository %s",
            str(repository.name),
        )
        return {}
...
def handle_role_associations(app, role, repository, **kwd):
    ...
    if kwd.get("manage_role_associations_button", False):
        ...
        message += (
            "Role <b>%s</b> has been associated with %d users, %d groups and %d repositories.  "
            % (
...
def repository_was_previously_installed(
    app, tool_shed_url, repository_name, repo_info_tuple, from_tip=False
):
    """
...
def set_repository_attributes(
    app, repository, status, error_message, deleted, uninstalled, remove_from_disk=False
):
    if remove_from_disk:
        ...
        if relative_install_dir:
            ...
            try:
                ...
                log.debug(
                    "Removed repository installation directory: %s" % str(clone_dir)
                )
...
def update_repository(app, trans, id, **kwds):
    ...
    if not (
        ...
        message = (
            "You are not the owner of this repository, so you cannot administer it."
        )
        return None, message
    ...
    for key in (
        ...
        if (
            key in kwds
            and kwds[key] is not None
            and kwds[key] != getattr(repository, key)
        ):
    ...
    if "category_ids" in kwds and isinstance(kwds["category_ids"], list):
        ...
        category_associations = sa_session.query(
            app.model.RepositoryCategoryAssociation
        ).filter(
            app.model.RepositoryCategoryAssociation.table.c.repository_id
...
def validate_repository_name(app, name, user):
    ...
    if name in ["repos"]:
        return (
            "The term <b>%s</b> is a reserved word in the tool shed, so it cannot be used as a repository name."
            % name
        )
    ...
    if check_existing is not None:
        if check_existing.deleted:
            return (
                "You own a deleted repository named <b>%s</b>, please choose a different name."
                % escape(name)
            )
        else:
            return (
                "You already own a repository named <b>%s</b>, please choose a different name."
                % escape(name)
            )
    if len(name) < 2:
