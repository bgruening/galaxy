                ...
                if tool_shed_accessible:
                    ...
                    for rd_key, rd_tups in repository_dependencies_dict.items():
                        ...
                        for rd_tup in rd_tups:
                            tool_shed, name, owner, changeset_revision, prior_installation_required, only_if_compiling_contained_td = parse_repository_dependency_tuple(
                                rd_tup
                            )
    ...
    if file_path:
        ...
        for migrated_tool_config in migrated_tool_configs_dict.keys():
            if migrated_tool_config in [file_path, name]:
                missing_tool_configs_dict[name] = migrated_tool_configs_dict[
                    migrated_tool_config
                ]
...
def generate_clone_url_for_installed_repository(app, repository):
    ...
    return util.build_url(
        tool_shed_url, pathspec=["repos", str(repository.owner), str(repository.name)]
    )
...
def generate_clone_url_from_repo_info_tup(app, repo_info_tup):
    ...
    toolshed, name, owner, changeset_revision, prior_installation_required, only_if_compiling_contained_td = parse_repository_dependency_tuple(
        repo_info_tup
    )
...
def handle_tool_shed_url_protocol(app, shed_url):
    ...
    try:
        ...
        if shed_url is not None:
            log.exception(
                "Handled exception removing protocol from URL %s", str(shed_url)
            )
        return shed_url


def parse_repository_dependency_tuple(
    repository_dependency_tuple, contains_error=False
):
    ...
    if contains_error:
        if len(repository_dependency_tuple) == 5:
            tool_shed, name, owner, changeset_revision, error = (
                repository_dependency_tuple
            )
        elif len(repository_dependency_tuple) == 6:
            tool_shed, name, owner, changeset_revision, prior_installation_required, error = (
                repository_dependency_tuple
            )
