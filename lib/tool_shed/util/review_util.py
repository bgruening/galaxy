    ...
    if user:
        for review in repository.reviews:
            for component_review in review.component_reviews:
                if app.security_agent.user_can_browse_component_review(
                    app, repository, component_review, user
                ):
                    return True
    return False
...
def get_component_by_name(app, name):
    ...
    sa_session = app.model.context.current
    return (
        sa_session.query(app.model.Component)
...
def get_component_review_by_repository_review_id_component_id(
    app, repository_review_id, component_id
):
    ...
    return (
        ...
        .filter(
            and_(
                app.model.ComponentReview.table.c.repository_review_id
                == app.security.decode_id(repository_review_id),
                app.model.ComponentReview.table.c.component_id
                == app.security.decode_id(component_id),
            )
        )
        .first()
    )
...
def get_components(app):
    sa_session = app.model.context.current
    return (
        sa_session.query(app.model.Component).order_by(app.model.Component.name).all()
    )
...
def get_review_by_repository_id_changeset_revision_user_id(
    app, repository_id, changeset_revision, user_id
):
    """
    ...
    return (
        ...
        .filter(
            and_(
                app.model.RepositoryReview.repository_id
...
def get_reviews_by_repository_id_changeset_revision(
