...
def create_or_update_tool_dependency(
    app, tool_shed_repository, name, version, type, status, set_status=True
):
    ...
    if tool_dependency:
        ...
        debug_msg = (
            "Creating a new record for version %s of tool dependency %s for revision %s of repository %s.  "
            % (
                str(version),
                str(name),
...
def create_tool_dependency_objects(
    app, tool_shed_repository, relative_install_dir, set_status=True
):
    """
...
def get_tool_dependency(app, id):
    ...
    return app.install_model.context.query(app.install_model.ToolDependency).get(
...
def get_tool_dependency_by_name_type_repository(app, repository, name, type):
    ...
    return (
        ...
        .filter(
            and_(
                app.install_model.ToolDependency.table.c.tool_shed_repository_id
                == repository.id,
...
def parse_package_elem(
    package_elem, platform_info_dict=None, include_after_install_actions=True
):
    """
    ...
    for elem in package_elem:
        if elem.tag == "actions":
            ...
            for actions_elem in elem.findall("actions"):
                if (
                    actions_elem.get("architecture") is not None
                    and actions_elem.get("os") is not None
                ):
                ...
                if child_element.tag == "actions":
                    ...
                    if system and architecture:
                        ...
                        if platform_info_dict:
                            if (
                                platform_info_dict["os"] != system
                                or platform_info_dict["architecture"] != architecture
                            ):
                                continue
                        else:
                        ...
                        if (
                            platform_actions_elements_processed
                    ...
                    if actions_elems_processed == actions_elem_count:
                        ...
                        debug_msg += (
                            "all <actions> tags.  Skipping <%s> element with type %s."
...
def remove_tool_dependency_installation_directory(dependency_install_dir):
    if os.path.exists(dependency_install_dir):
        try:
            ...
            error_message = ""
            log.debug(
                "Removed tool dependency installation directory: %s"
                % str(dependency_install_dir)
            )
        except Exception as e:
            removed = False
            error_message = (
                "Error removing tool dependency installation directory %s: %s"
