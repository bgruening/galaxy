...
def copy_sample_files(
    app, sample_files, tool_path=None, sample_files_copied=None, dest_path=None
):
    """
    ...
    for filename in sample_files:
        ...
        if (
            filename_sans_path not in filenames_not_to_copy
            and filename not in sample_files_copied
        ):
            if tool_path:
...
def generate_message_for_invalid_tools(
    ...
    if not displaying_invalid_tool:
        if metadata_dict:
            message += (
                "Metadata may have been defined for some items in revision '%s'.  "
                % tip_rev
            )
            message += (
                "Correct the following problems if necessary and reset metadata.%s"
                % new_line
            )
        else:
            message += (
                "Metadata cannot be defined for revision '%s' so this revision cannot be automatically "
                % tip_rev
            )
            message += (
                "installed into a local Galaxy instance.  Correct the following problems and reset metadata.%s"
                % new_line
            )
    for itc_tup in invalid_file_tups:
        ...
        if exception_msg.find("No such file or directory") >= 0:
            ...
            correction_msg += (
                "Upload a file named %s%s%s to the repository to correct this error."
            ...
            if as_html:
                ...
                correction_msg = (
                    exception_msg.replace("<br/>", new_line)
...
def get_tool_path_install_dir(
    partial_install_dir, shed_tool_conf_dict, tool_dict, config_elems
):
