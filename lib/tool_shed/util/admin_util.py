...
class Admin(object):
    ...
    def index(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/webapps/tool_shed/admin/index.mako", message=message, status=status
        )
    ...
    def center(self, trans, **kwd):
        ...
        status = kwd.get("status", "done")
        return trans.fill_template(
            "/webapps/tool_shed/admin/center.mako", message=message, status=status
        )
    ...
    def create_role(self, trans, **kwd):
        ...
        if params.get("create_role_button", False):
            if not name or not description:
                ...
                for group in [
                    trans.sa_session.query(trans.app.model.Group).get(x)
                    for x in in_groups
                ]:
    ...
    def rename_role(self, trans, **kwd):
        ...
        if not id:
            ...
            trans.response.send_redirect(
                web.url_for(
                    controller="admin", action="roles", message=message, status="error"
                )
            )
            ...
            if not new_name:
                ...
                if existing_role and existing_role.id != role.id:
                    ...
                    if not (
                        ...
                        trans.sa_session.add(role)
                        trans.sa_session.flush()
                        message = "Role '%s' has been renamed to '%s'" % (
                            old_name,
                            new_name,
                        )
        ...
        return trans.fill_template(
            "/webapps/tool_shed/admin/dataset_security/role/role_rename.mako",
            role=role,
            message=message,
            status=status,
        )
    ...
    def manage_users_and_groups_for_role(self, trans, **kwd):
        ...
        if params.get("role_members_edit_button", False):
            ...
            message = (
                "Role '%s' has been updated with %d associated users and %d associated groups"
        ...
        if trans.webapp.name == "galaxy" and len(role.dataset_actions) < 25:
            ...
            for dp in role.dataset_actions:
                for ldda in trans.sa_session.query(
                    trans.app.model.LibraryDatasetDatasetAssociation
                ).filter(
                    trans.app.model.LibraryDatasetDatasetAssociation.dataset_id
                    == dp.dataset_id
                ):
                    root_found = False
                    folder_path = ""
                    ...
                    library = (
                        ...
                        .filter(
                            trans.app.model.Library.table.c.root_folder_id == folder.id
                        )
                        .first()
                    )
    ...
    def rename_group(self, trans, **kwd):
        ...
        if not id:
            ...
            trans.response.send_redirect(
                web.url_for(
                    controller="admin", action="groups", message=message, status="error"
                )
            )
            ...
            if not new_name:
                ...
                if existing_group and existing_group.id != group.id:
                    ...
                    if group.name != new_name:
                        ...
                        trans.sa_session.add(group)
                        trans.sa_session.flush()
                        message = "Group '%s' has been renamed to '%s'" % (
                            old_name,
                            new_name,
                        )
                    return trans.response.send_redirect(
    ...
    def manage_users_and_roles_for_group(self, trans, **kwd):
        ...
        if params.get("group_roles_users_edit_button", False):
            ...
            message += (
                "Group '%s' has been updated with %d associated roles and %d associated users"
    ...
    def reset_user_password(self, trans, **kwd):
        ...
        if not user_id:
            ...
            trans.response.send_redirect(
                web.url_for(
                    controller="admin", action="users", message=message, status="error"
                )
            )
        ...
        if "reset_user_password_button" in kwd:
            ...
            if not message and not status:
                message = "Passwords reset for %d %s." % (
                    len(user_ids),
                    inflector.cond_plural(len(user_ids), "user"),
                )
                status = "done"
            trans.response.send_redirect(
                web.url_for(
        ...
        return trans.fill_template(
            "/webapps/tool_shed/admin/user/reset_password.mako",
            id=user_id,
            users=users,
            password="",
            confirm="",
    ...
    def mark_user_deleted(self, trans, **kwd):
        ...
        for user_id in ids:
            ...
            for role in user.all_roles():
                print(
                    role,
                    self.app.config.redact_username_during_deletion,
                    self.app.config.redact_email_during_deletion,
                )
        ...
        for user_id in ids:
            ...
            for h in user.active_histories:
                ...
                for hda in h.active_datasets:
                    ...
                    d = trans.sa_session.query(trans.app.model.Dataset).get(
                        hda.dataset_id
                    )
                    # Delete Dataset
                    if not d.deleted:
    ...
    def name_autocomplete_data(self, trans, q=None, limit=None, timestamp=None):
        ...
        for user in (
            ...
            .filter(func.lower(trans.app.model.User.email).like(q.lower() + "%"))
        ):
    ...
    def manage_roles_and_groups_for_user(self, trans, **kwd):
        ...
        if kwd.get("user_roles_groups_edit_button", False):
            ...
            if out_roles:
                out_roles = [
                    trans.sa_session.query(trans.app.model.Role).get(x)
                    for x in util.listify(out_roles)
                ]
            ...
            if in_roles:
                in_roles = [
                    trans.sa_session.query(trans.app.model.Role).get(x)
                    for x in util.listify(in_roles)
                ]
            out_groups = kwd.get("out_groups", [])
            if out_groups:
                out_groups = [
                    trans.sa_session.query(trans.app.model.Group).get(x)
                    for x in util.listify(out_groups)
                ]
            in_groups = kwd.get("in_groups", [])
            if in_groups:
                in_groups = [
                    trans.sa_session.query(trans.app.model.Group).get(x)
                    for x in util.listify(in_groups)
                ]
            if in_roles:
        ...
        for group in (
            ...
            if group in [x.group for x in user.groups]:
                ...
                out_groups.append((group.id, group.name))
        message += (
            "User '%s' is currently associated with %d roles and is a member of %d groups"
            % (user.email, len(in_roles), len(in_groups))
        )
        if not status:
            status = "done"
