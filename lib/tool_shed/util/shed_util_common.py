...
def clean_dependency_relationships(
    trans, metadata_dict, tool_shed_repository, tool_shed_url
):
    """
    ...
    for rrda in tool_shed_repository.required_repositories:
        ...
        r = rd.repository
        if can_eliminate_repository_dependency(
            metadata_dict, tool_shed_url, r.name, r.owner
        ):
...
def count_repositories_in_category(app, category_id):
    ...
    return (
        ...
        .filter(
            app.model.RepositoryCategoryAssociation.table.c.category_id
...
def get_tool_shed_repo_requirements(
    app, tool_shed_url, repositories=None, repo_info_dicts=None
):
    """
...
def get_requirements_from_tools(tools):
    return {
        tool["id"]: galaxy.tool_util.deps.requirements.ToolRequirements.from_list(
            tool["requirements"]
        )
        for tool in tools
    }
...
def get_tool_panel_config_tool_path_install_dir(app, repository):
    ...
    if not shed_config_dict:
        ...
        for shed_config_dict in app.toolbox.dynamic_confs(
            ...
            if (
                repository.dist_to_shed
                and shed_config_dict["config_filename"]
                == app.config.migrated_tools_config
            ) or (
                not repository.dist_to_shed
                and shed_config_dict["config_filename"]
                != app.config.migrated_tools_config
            ):
                break
...
def handle_email_alerts(
    app, host, repository, content_alert_str="", new_repo_alert=False, admin_only=False
):
    """
    ...
    if smtp_server and (new_repo_alert or repository.email_alerts):
        ...
        if new_repo_alert:
            ...
            for user in sa_session.query(app.model.User).filter(
                and_(
                    app.model.User.table.c.deleted == false(),
...
def is_path_within_dependency_dir(app, path):
    ...
    if tool_dependency_dir:
        ...
        allowed = (
            os.path.commonprefix([dependency_path, resolved_path]) == dependency_path
        )
    return allowed
...
def is_path_within_repo(app, path, repository_id):
    ...
    repo_path = os.path.abspath(
        repository_util.get_repository_by_id(app, repository_id).repo_path(app)
    )
    resolved_path = os.path.realpath(path)
