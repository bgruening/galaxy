...
def get_all_dependencies(app, metadata_entry, processed_dependency_links=[]):
    ...
    for dependency_metadata in required_metadata:
        dependency_dict = dependency_metadata.to_dict(
            value_mapper=value_mapper, view="element"
        )
        ...
        repository = db.query(app.model.Repository).get(
            app.security.decode_id(dependency_dict["repository_id"])
        )
...
def get_dependencies_for_metadata_revision(app, metadata):
    ...
    for shed, name, owner, changeset, prior, _ in metadata["repository_dependencies"]:
        ...
        updated_changeset = get_next_downloadable_changeset_revision(
            app, required_repository, changeset
        )
...
def get_latest_changeset_revision(app, repository):
    ...
    changeset_revisions = [
        revision[1] for revision in get_metadata_revisions(app, repository)
    ]
...
def get_latest_repository_metadata(app, decoded_repository_id, downloadable=False):
    ...
    return get_repository_metadata_by_changeset_revision(
        app, app.security.encode_id(repository.id), changeset_revision
    )


def get_metadata_revisions(
    app, repository, sort_revisions=True, reverse=False, downloadable=True
):
    """
    ...
    for repository_metadata in metadata_revisions:
        if (
            repository_metadata.numeric_revision == -1
            or repository_metadata.numeric_revision is None
        ):
            try:
                rev = hg_util.changeset2rev(
                    repo_path, repository_metadata.changeset_revision
                )
...
def get_previous_metadata_changeset_revision(
    app, repository, before_changeset_revision, downloadable=True
):
    """
            ...
            if repository_dependencies_dict is not None:
                ...
                if repository_dependency_tups is not None:
                    ...
                    for repository_dependency_tup in repository_dependency_tups:
                        toolshed, name, owner, changeset_revision, pir, oicct = common_util.parse_repository_dependency_tuple(
...
def get_repository_metadata_by_changeset_revision(app, id, changeset_revision):
    ...
    all_metadata_records = (
        ...
        .filter(
            and_(
                app.model.RepositoryMetadata.table.c.repository_id
                == app.security.decode_id(id),
                app.model.RepositoryMetadata.table.c.changeset_revision
                == changeset_revision,
            )
        )
        .all()
...
def get_repository_metadata_by_id(app, id):
    ...
    sa_session = app.model.context.current
    return sa_session.query(app.model.RepositoryMetadata).get(
        app.security.decode_id(id)
    )
    ...
    if reviewed:
        ...
        for review in repository.reviews:
            if review.changeset_revision in metadata_changeset_revision_hashes:
                rmcr_hashes = [
                    rmr.changeset_revision for rmr in repository_metadata_revisions
                ]
...
def get_updated_changeset_revisions_from_tool_shed(
    ...
    if (
        ...
        message += (
            "required parameters is None: tool_shed_url: %s, name: %s, owner: %s, changeset_revision: %s "
