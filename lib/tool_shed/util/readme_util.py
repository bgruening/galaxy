...
def build_readme_files_dict(
    app, repository, changeset_revision, metadata, tool_path=None
):
    """
    ...
    if app.name == "galaxy":
        ...
        can_use_disk_files = (
            changeset_revision == latest_downloadable_changeset_revision
        )
    readme_files_dict = {}
    if metadata:
...
def get_readme_files_dict_for_display(app, tool_shed_url, repo_info_dict):
    ...
    description, repository_clone_url, changeset_revision, ctx_rev, repository_owner, repository_dependencies, installed_td = repository_util.get_repo_info_tuple_contents(
        repo_info_tuple
    )
    ...
    params = dict(
        name=name, owner=repository_owner, changeset_revision=changeset_revision
    )
