...
class RepoToolModule(ToolModule):
    ...
    def __init__(
        self, trans, repository_id, changeset_revision, tools_metadata, tool_id
    ):
        self.trans = trans
        ...
        if trans.webapp.name == "tool_shed":
            ...
            with ValidationContext.from_app(trans.app) as validation_context:
                ...
                for tool_dict in tools_metadata:
                    if self.tool_id in [tool_dict["id"], tool_dict["guid"]]:
                        repository, self.tool, valid, message = tv.load_tool_from_changeset_revision(
                            repository_id, changeset_revision, tool_dict["tool_config"]
                        )
            ...
            self.tool = trans.app.toolbox.get_tool(
                self.tool_id, tool_version=self.tool_version
            )
    ...
    def from_dict(
        ...
        module = Class(
            trans, repository_id, changeset_revision, tools_metadata, tool_id
        )
    ...
    def from_workflow_step(
        Class, trans, step, repository_id, changeset_revision, tools_metadata
    ):
        module = Class(
            trans, repository_id, changeset_revision, tools_metadata, step.tool_id
        )
        module.state = galaxy.tools.DefaultToolState()
            ...
            for name, tool_output in self.tool.outputs.items():
                if tool_output.format_source is not None:
                    ...
                    for di in data_inputs:
                        ...
                        if (
                            di["name"] is not None
                            and di["name"].split("|")[-1] == tool_output.format_source
                        ):
...
class RepoWorkflowModuleFactory(WorkflowModuleFactory):
    ...
    def from_dict(
        self, trans, repository_id, changeset_revision, step_dict, tools_metadata, **kwd
    ):
    ...
    def from_workflow_step(
        self, trans, repository_id, changeset_revision, tools_metadata, step
    ):
...
def generate_workflow_image(
    trans, workflow_name, repository_metadata_id=None, repository_id=None
):
    """
    ...
    for step in workflow.steps:
        ...
        module = module_factory.from_workflow_step(
            trans, repository_id, changeset_revision, tools_metadata, step
        )
...
def get_workflow_from_dict(
    trans, workflow_dict, tools_metadata, repository_id, changeset_revision
):
    """
    ...
    for step_dict in workflow_dict["steps"].values():
        ...
        module = module_factory.from_dict(
            trans,
            repository_id,
            changeset_revision,
            step_dict,
        ...
        for pja_dict in post_job_actions.values():
            trans.model.PostJobAction(
                pja_dict["action_type"],
                step,
...
def import_workflow(trans, repository, workflow_name):
    ...
    if workflow_dict:
        ...
        if missing_tool_tups:
            ...
            message += (
                "The following tools required by this workflow are missing from this Galaxy instance: %s.  "
                % name_and_id_str
            )
    else:
        workflow = None
        message += (
            "The workflow named %s is not included in the metadata for revision %s of repository %s"
