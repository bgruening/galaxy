...
class UserGrid(grids.Grid):
    ...
    class EmailColumn(grids.GridColumn):
        def filter(self, trans, user, query, column_filter):
            ...
            return query.filter(
                and_(
                    model.Tool.table.c.user_id == model.User.table.c.id,
    ...
    columns = [
        ...
        UserNameColumn(
            "User Name", key="username", attach_popup=False, filterable="advanced"
        ),
...
class RoleGrid(grids.Grid):
    ...
    columns = [
        ...
        DescriptionColumn(
            "Description", key="description", attach_popup=False, filterable="advanced"
        ),
        GroupsColumn("Groups", attach_popup=False),
    ...
    global_actions = [
        grids.GridAction(
            "Add new role", dict(controller="admin", action="roles", operation="create")
        )
    ]
    ...
    operations = [
        grids.GridOperation(
            "Rename",
            condition=(
                lambda item: not item.deleted and not item.is_repository_admin_role
            ),
            allow_multiple=False,
...
class AdminRepositoryGrid(RepositoryGrid):
    ...
    columns = [
        ...
        RepositoryGrid.DeprecatedColumn(
            "Deprecated", key="deprecated", attach_popup=False
        ),
        # Columns that are valid for filtering but are not visible.
...
class RepositoryMetadataGrid(grids.Grid):
    ...
    def build_initial_query(self, trans, **kwd):
        return trans.sa_session.query(model.RepositoryMetadata).join(
