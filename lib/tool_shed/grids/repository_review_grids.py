...
class ComponentGrid(grids.Grid):
    ...
    columns = [
        ...
        DescriptionColumn(
            "Description", key="Component.description", attach_popup=False
        ),
    ]
    default_filter = {}
    global_actions = [
...
class RepositoriesWithReviewsGrid(RepositoryGrid):
        ...
        def get_value(self, trans, grid, repository):
            ...
            if repository.reviews:
                ...
                for review in repository.reviews:
                    ...
                    rval += (
                        '<a href="manage_repository_reviews_of_revision?id=%s&changeset_revision=%s">%s</a><br/>'
                        % (
        ...
        def get_value(self, trans, grid, repository):
            ...
            if repository.reviewers:
                for user in repository.reviewers:
                    rval += (
                        '<a class="view-info" href="repository_reviews_by_user?id=%s">'
                        % trans.security.encode_id(user.id)
                    )
    ...
    def build_initial_query(self, trans, **kwd):
        return (
            ...
            .join(
                (
                    model.RepositoryReview.table,
                    model.RepositoryReview.table.c.repository_id
                    == model.Repository.table.c.id,
                )
            )
            .join(
                (
                    model.User.table,
                    model.User.table.c.id == model.Repository.table.c.user_id,
                )
            )
            .outerjoin(
                (
                    model.ComponentReview.table,
                    model.ComponentReview.table.c.repository_review_id
                    == model.RepositoryReview.table.c.id,
                )
            )
            .outerjoin(
                (
                    model.Component.table,
                    model.Component.table.c.id
                    == model.ComponentReview.table.c.component_id,
                )
            )
        )
...
class RepositoriesReadyForReviewGrid(RepositoriesWithoutReviewsGrid):
    ...
    columns = [
        ...
        RepositoriesWithoutReviewsGrid.DescriptionColumn(
            "Synopsis", key="description", attach_popup=False
        ),
        RepositoriesWithoutReviewsGrid.WithoutReviewsRevisionColumn(
            "Revisions for review"
        ),
        RepositoriesWithoutReviewsGrid.UserColumn(
            "Owner", model_class=model.User, attach_popup=False, key="User.username"
        ),
    ]
    columns.append(
        ...
        return (
            ...
            .filter(
                and_(
                    ...
                    or_(
                        model.RepositoryMetadata.table.c.includes_tools == false(),
                        and_(
                            model.RepositoryMetadata.table.c.includes_tools == true(),
                            model.RepositoryMetadata.table.c.tools_functionally_correct
                            == true(),
                        ),
                    ),
                )
    ...
    def build_initial_query(self, trans, **kwd):
        return (
            ...
            .join(
                (
                    model.User.table,
                    model.User.table.c.id == model.RepositoryReview.table.c.user_id,
                )
            )
            .outerjoin(
                (
...
class RepositoryReviewsByUserGrid(grids.Grid):
    ...
    def build_initial_query(self, trans, **kwd):
        ...
        return (
            trans.sa_session.query(model.RepositoryReview)
            .filter(
                and_(
