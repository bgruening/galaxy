...
def build_approved_select_field(trans, name, selected_value=None, for_component=True):
    ...
    if for_component:
        options.append(
            ("Not applicable", trans.model.ComponentReview.approved_states.NA)
        )
...
def build_changeset_revision_select_field(
    ...
    if downloadable:
        ...
        for review in repository.reviews:
            reviewed_metadata_changeset_revision_hashes.append(
...
def filter_by_latest_downloadable_changeset_revision_that_has_missing_tool_test_components(
    ...
    """
    repository_metadata = get_latest_downloadable_repository_metadata_if_it_includes_tools(
        trans, repository
    )
...
def filter_by_latest_metadata_changeset_revision_that_has_invalid_tools(
    trans, repository
):
    """
    ...
    """
    repository_metadata = get_latest_repository_metadata_if_it_includes_invalid_tools(
        trans, repository
    )
