...
class CategoryGrid(grids.Grid):
    ...
    class RepositoriesColumn(grids.TextColumn):
        def get_value(self, trans, grid, category):
            ...
            if (
                filter
                == trans.app.repository_grid_filter_manager.filters.CERTIFIED_LEVEL_ONE
            ):
                return trans.app.repository_registry.certified_level_one_viewable_repositories_and_suites_by_category.get(
                    category_name, 0
                )
            elif (
                filter
                == trans.app.repository_grid_filter_manager.filters.CERTIFIED_LEVEL_ONE_SUITES
            ):
                ...
                return trans.app.repository_registry.viewable_suites_by_category.get(
                    category_name, 0
                )
            else:
                # The value filter is None.
                return trans.app.repository_registry.viewable_repositories_and_suites_by_category.get(
                    category_name, 0
                )
    ...
    columns = [
        ...
        DescriptionColumn(
            "Description", key="Category.description", attach_popup=False
        ),
        RepositoriesColumn(
            "Repositories", model_class=model.Repository, attach_popup=False
        ),
    ]
    # Override these
    default_filter = {}
...
class RepositoryGrid(grids.Grid):
    ...
    class HeadsColumn(grids.GridColumn):
        ...
        def get_value(self, trans, grid, repository):
            ...
            if multiple_heads:
                ...
                heads_str = ""
            for ctx in heads:
                heads_str += "%s<br/>" % hg_util.get_revision_label_from_ctx(
                    ctx, include_date=True
                )
            heads_str.rstrip("<br/>")
            if multiple_heads:
                heads_str += "</font>"
    ...
    class MetadataRevisionColumn(grids.GridColumn):
        ...
        def get_value(self, trans, grid, repository):
            ...
            select_field = grids_util.build_changeset_revision_select_field(
                trans, repository, downloadable=False
            )
        ...
        def get_value(self, trans, grid, repository):
            ...
            if repository.categories:
                for rca in repository.categories:
                    rval += (
                        '<li><a href="browse_repositories?operation=repositories_by_category&id=%s">%s</a></li>'
    ...
    class EmailColumn(grids.TextColumn):
        def filter(self, trans, user, query, column_filter):
            ...
            return query.filter(
                and_(
                    model.Repository.table.c.user_id == model.User.table.c.id,
    ...
    class EmailAlertsColumn(grids.TextColumn):
        def get_value(self, trans, grid, repository):
            if (
                trans.user
                and repository.email_alerts
                and trans.user.email in json.loads(repository.email_alerts)
            ):
                return "yes"
            return ""
    ...
    def build_initial_query(self, trans, **kwd):
        ...
        if (
            ...
            return (
                ...
                .filter(
                    or_(*trans.app.repository_registry.certified_level_one_clause_list)
                )
...
class DockerImageGrid(RepositoryGrid):
    columns = [
        ...
        RepositoryGrid.DescriptionColumn(
            "Synopsis", key="description", attach_popup=False
        ),
...
class EmailAlertsRepositoryGrid(RepositoryGrid):
    ...
    global_actions = [
        grids.GridAction(
            "User preferences",
            dict(controller="user", action="index", cntrller="repository"),
        )
    ]
...
class MatchedRepositoryGrid(grids.Grid):
    ...
    def build_initial_query(self, trans, **kwd):
        ...
        if match_tuples:
            for match_tuple in match_tuples:
                ...
                clause_list.append(
                    "%s=%d and %s='%s'"
                    % (
                        model.RepositoryMetadata.table.c.repository_id,
                        int(repository_id),
        ...
        return trans.sa_session.query(model.RepositoryMetadata).filter(
            model.RepositoryMetadata.id < 0
        )
...
class MyWritableRepositoriesGrid(RepositoryGrid):
    ...
    def build_initial_query(self, trans, **kwd):
        ...
        # Return an empty query.
        return trans.sa_session.query(model.Repository).filter(
            model.Repository.table.c.id < 0
        )
...
class RepositoriesInCategoryGrid(RepositoryGrid):
    ...
    columns = [
        ...
        RepositoryGrid.EmailColumn(
            "Email", model_class=model.User, key="email", visible=False
        ),
    ]
    columns.append(
        grids.MulticolFilterColumn(
            ...
            if category_id:
                ...
                if category:
                    return (
                        ...
                        .filter(
                            or_(
                                *trans.app.repository_registry.certified_level_one_clause_list
                            )
                        )
                        .join(model.User.table)
                        .outerjoin(model.RepositoryCategoryAssociation.table)
...
class RepositoriesMissingToolTestComponentsGrid(RepositoryGrid):
    ...
    def build_initial_query(self, trans, **kwd):
        ...
        for repository in trans.sa_session.query(model.Repository).filter(
            ...
            changeset_revision = grids_util.filter_by_latest_downloadable_changeset_revision_that_has_missing_tool_test_components(
                trans, repository
            )
...
class MyWritableRepositoriesMissingToolTestComponentsGrid(
    ...
    def build_initial_query(self, trans, **kwd):
        ...
        for repository in trans.sa_session.query(model.Repository).filter(
            ...
            if allow_push:
                allow_push_usernames = allow_push.split(",")
                if username in allow_push_usernames:
                    user_clause_list.append(
                        model.Repository.table.c.id == repository.id
                    )
        if user_clause_list:
...
class RepositoryDependenciesGrid(RepositoryMetadataGrid):
                    ...
                    if rd_dict:
                        ...
                        for rd_tup in sorted_rd_tups:
                            ...
                            if required_repository and not required_repository.deleted:
                                required_repository_id = trans.security.encode_id(
                                ...
                                rd_line += (
                                    '<a href="browse_repository_dependencies?operation=view_or_manage_repository&id=%s">'
                                    % (required_repository_metadata_id)
                                )
                            rd_line += (
                                "Repository <b>%s</b> revision <b>%s</b> owned by <b>%s</b>"
                                % (
                                    escape_html(name),
                                    escape_html(owner),
                                    escape_html(changeset_revision),
                                )
                            )
    ...
    def build_initial_query(self, trans, **kwd):
        return (
            ...
            .filter(
                and_(
                    model.RepositoryMetadata.table.c.has_repository_dependencies
                    == true(),
                    model.Repository.table.c.deleted == false(),
                    model.Repository.table.c.deprecated == false(),
                )
            ...
            if repository_metadata:
                ...
                if metadata:
                    ...
                    if datatype_dicts:
                        ...
                        sorted_datatype_tups = sorted(
                            datatype_tups, key=lambda datatype_tup: datatype_tup[0]
                        )
                        for datatype_tup in sorted_datatype_tups:
                            extension, datatype = datatype_tup[:2]
                            datatype_str = (
                                '<a href="browse_datatypes?operation=view_or_manage_repository&id=%s">'
                                % trans.security.encode_id(repository_metadata.id)
                            )
                            datatype_str += "<b>%s:</b> %s" % (
                                escape_html(extension),
                                escape_html(datatype),
                            )
                            datatype_str += "</a>"
...
class ToolDependenciesGrid(RepositoryMetadataGrid):
    ...
    def build_initial_query(self, trans, **kwd):
        return (
            ...
            .filter(
                and_(
                    model.RepositoryMetadata.table.c.includes_tool_dependencies
                    == true(),
                    model.Repository.table.c.deleted == false(),
                    model.Repository.table.c.deprecated == false(),
                )
            ...
            if repository_metadata:
                ...
                if metadata:
                    ...
                    if tool_dicts:
                        ...
                        sorted_tool_tups = sorted(
                            tool_tups, key=lambda tool_tup: tool_tup[0]
                        )
...
class ValidCategoryGrid(CategoryGrid):
    class RepositoriesColumn(grids.TextColumn):
        def get_value(self, trans, grid, category):
            ...
            if (
                ...
                return trans.app.repository_registry.viewable_valid_suites_by_category.get(
                    category_name, 0
                )
            else:
                # The value filter is None.
    ...
    columns = [
        CategoryGrid.NameColumn(
            ...
            link=(
                lambda item: dict(
                    operation="valid_repositories_by_category", id=item.id
                )
            ),
            attach_popup=False,
        ),
        CategoryGrid.DescriptionColumn(
            "Description", key="Category.description", attach_popup=False
        ),
        # Columns that are valid for filtering but are not visible.
        RepositoriesColumn(
            "Valid repositories", model_class=model.Repository, attach_popup=False
        ),
    ]
    # Override these
    default_filter = {}
...
class ValidRepositoryGrid(RepositoryGrid):
    ...
    class InstallableRevisionColumn(grids.GridColumn):
        ...
        def get_value(self, trans, grid, repository):
            ...
            select_field = grids_util.build_changeset_revision_select_field(
                trans, repository, downloadable=True
            )
            if len(select_field.options) > 1:
                tmpl = "<select name='%s'>" % select_field.name
                for o in select_field.options:
