...
class RepositoryMetadataManager(metadata_generator.MetadataGenerator):
    def __init__(
        ...
        super(RepositoryMetadataManager, self).__init__(
            app,
            repository,
            changeset_revision,
    ...
    def clean_repository_metadata(self, changeset_revisions):
        ...
        for repository_metadata in (
            ...
            .filter(
                self.app.model.RepositoryMetadata.table.c.repository_id
                == self.repository.id
            )
            .order_by(
            ...
            if (
                changeset_revision in changeset_revisions_checked
                or changeset_revision not in changeset_revisions
            ):
                self.sa_session.delete(repository_metadata)
                self.sa_session.flush()

    def compare_changeset_revisions(
        self, ancestor_changeset_revision, ancestor_metadata_dict
    ):
        """
        ...
        no_repository_dependencies = (
            not ancestor_repository_dependencies and not current_repository_dependencies
        )
        no_tool_dependencies = (
            not ancestor_tool_dependencies and not current_tool_dependencies
        )
        ...
        workflow_comparison = self.compare_workflows(
        ...
        repository_dependency_is_subset = (
            repository_dependency_comparison in self.SUBSET_VALUES
        )
            ...
            for ancestor_datatype in ancestor_datatypes:
                ...
                for current_datatype in current_datatypes:
                    if (
                        ...
                        and current_datatype.get("mimetype", None)
                        == ancestor_datatype_mimetype
                    ):
    ...
    def compare_repository_dependencies(
        self, ancestor_repository_dependencies, current_repository_dependencies
    ):
        """
    ...
    def compare_tool_dependencies(
        self, ancestor_tool_dependencies, current_tool_dependencies
    ):
        """
        ...
        if len(ancestor_tool_dependencies) <= len(current_tool_dependencies):
            for (
                ancestor_td_key,
                ancestor_requirements_dict,
            ) in ancestor_tool_dependencies.items():
            ...
            for ancestor_workflow_tup in ancestor_workflows:
                ...
                for current_workflow_tup in current_workflows:
                    ...
                    if (
                        ...
                        and len(current_workflow_dict["steps"])
                        == num_ancestor_workflow_steps
                    ):
                        found_in_current = True
                        break
                if not found_in_current:
    ...
    def create_or_update_repository_metadata(self, changeset_revision, metadata_dict):
        ...
        if repository_metadata:
            ...
            repository_metadata.has_repository_dependencies = (
                has_repository_dependencies
            )
    ...
    def different_revision_defines_tip_only_repository_dependency(
        ...
        """
        rd_tool_shed, rd_name, rd_owner, rd_changeset_revision, rd_prior_installation_required, rd_only_if_compiling_contained_td = common_util.parse_repository_dependency_tuple(
            rd_tup
        )
        ...
        for repository_dependency in repository_dependencies:
            ...
            if (
                cleaned_rd_tool_shed == cleaned_tool_shed
                and rd_name == name
                and rd_owner == owner
            ):
    ...
    def get_query_for_setting_metadata_on_repositories(
        self, my_writable=False, order=True
    ):
        """
            ...
            if clause_list:
                if order:
                    return (
                        ...
                        .order_by(
                            self.app.model.Repository.table.c.name,
    ...
    def new_metadata_required_for_utilities(self):
        ...
        repository_dependencies_required = self.new_repository_dependency_metadata_required(
            repository_metadata
        )
    ...
    def new_repository_dependency_metadata_required(self, repository_metadata):
        ...
        if repository_metadata:
            ...
            if "repository_dependencies" in metadata:
                saved_repository_dependencies = metadata["repository_dependencies"][
                    "repository_dependencies"
                ]
                ...
                if new_repository_dependencies_metadata:
                    ...
                    for saved_repository_dependency in saved_repository_dependencies:
                        if (
                            ...
                            not in new_repository_dependencies
                        ):
                            # In some cases, the only difference between a dependency definition in the lists
                            # is the changeset_revision value.  We'll check to see if this is the case, and if
                            # the defined dependency is a repository that has metadata set only on its tip.
                    ...
                    if "tools" in metadata:
                        ...
                        for new_tool_metadata_dict in self.metadata_dict["tools"]:
                            for saved_tool_metadata_dict in metadata["tools"]:
                                ...
                                if (
                                    new_tool_metadata_dict["id"]
                                    == saved_tool_metadata_dict["id"]
                                ):
                                    if (
                                        new_tool_metadata_dict["version"]
                                        != saved_tool_metadata_dict["version"]
                                    ):
                                        return True
    ...
    def reset_all_metadata_on_repository_in_tool_shed(self):
        ...
        for changeset in self.repository.get_changesets_for_setting_metadata(self.app):
            ...
            if cloned_ok:
                log.debug(
                    "Generating metadata for changset revision: %s", str(ctx.rev())
                )
    ...
    def reset_metadata_on_selected_repositories(self, **kwd):
        ...
        if repository_ids:
            ...
            if unsuccessful_count:
                message += (
                    "Error setting metadata on %d %s - see the paster log for details.  "
                    % (
                        unsuccessful_count,
    ...
    def set_repository_metadata(self, host, content_alert_str="", **kwd):
        ...
        if self.metadata_dict:
            ...
            if (
                "tools" in self.metadata_dict
                and repository_metadata
                and status != "error"
            ):
                ...
                self.add_tool_versions(
                    encoded_id, repository_metadata, changeset_revisions
                )
        elif len(repo) == 1 and not self.invalid_file_tups:
            message = (
                "Revision <b>%s</b> includes no Galaxy utilities for which metadata can "
    ...
    def set_repository_metadata_due_to_new_tip(
        self, host, content_alert_str=None, **kwd
    ):
