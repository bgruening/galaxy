...
class MetadataGenerator(object):
        ...
        if self.app.name == "galaxy":
            ...
            if shed_config_dict is None:
                if self.repository is not None:
                    self.shed_config_dict = self.repository.get_shed_config_dict(
                        self.app
                    )
                else:
            ...
            if metadata_dict is None:
                ...
                self.metadata_dict = {
                    "shed_config_filename": self.shed_config_dict.get(
    ...
    def generate_data_manager_metadata(
        ...
        relative_data_manager_dir = util.relpath(
            os.path.split(data_manager_config_filename)[0], repo_dir
        )
        ...
        if data_manager_tool_path:
            relative_data_manager_dir = os.path.join(
                relative_data_manager_dir, data_manager_tool_path
            )
        for i, data_manager_elem in enumerate(root.findall("data_manager")):
            ...
            if data_manager_id is None:
                log.error(
                    'Data Manager entry is missing id attribute in "%s".'
                    % (data_manager_config_filename)
                )
            ...
            guid = self.generate_guid_for_object(
                DataManager.GUID_TYPE, data_manager_id, version
            )
            data_tables = []
            if tool_file is None:
                log.error(
                    'Data Manager entry is missing tool_file attribute in "%s".'
                    % (data_manager_config_filename)
                )
                invalid_data_managers.append(
                    {
                ...
                for data_table_elem in data_manager_elem.findall("data_table"):
                    ...
                    if data_table_name is None:
                        log.error(
                            'Data Manager data_table entry is missing name attribute in "%s".'
                            % (data_manager_config_filename)
                        )
                        invalid_data_managers.append(
                            {
                ...
                if bad_data_table:
                    continue
            data_manager_metadata_tool_file = os.path.normpath(
                os.path.join(relative_data_manager_dir, tool_file)
            )
            tool_metadata_tool_file = os.path.join(
                repo_files_directory, data_manager_metadata_tool_file
            )
            tool = tools.get(tool_metadata_tool_file, None)
            if tool is None:
                log.error(
                    "Unable to determine tools metadata for '%s'."
                    % (data_manager_metadata_tool_file)
                )
                invalid_data_managers.append(
                    {"index": i, "error_message": "Unable to determine tools metadata"}
                )
                continue
    ...
    def generate_datatypes_metadata(
        self, tv, repository_files_dir, datatypes_config, metadata_dict
    ):
            ...
            for elem in registration.findall("datatype"):
                ...
                for sub_elem in elem:
                    if sub_elem.tag == "converter":
                        ...
                        tool_config_path = hg_util.get_config_from_disk(
                            tool_config, repository_files_dir
                        )
                        ...
                        if tool is None:
                            ...
                            guid = suc.generate_tool_guid(
                                self.repository_clone_url, tool
                            )
                        converter_dict = dict(
    ...
    def generate_environment_dependency_metadata(
        self, elem, valid_tool_dependencies_dict
    ):
        """
        ...
        for env_elem in elem:
            ...
            if env_name:
                ...
                if "set_environment" in valid_tool_dependencies_dict:
                    valid_tool_dependencies_dict["set_environment"].append(
                        requirements_dict
                    )
                else:
                    valid_tool_dependencies_dict["set_environment"] = [
                        requirements_dict
                    ]
    ...
    def generate_metadata_for_changeset_revision(self):
        ...
        if self.app.name == "galaxy":
            # Shed related tool panel configs are only relevant to Galaxy.
            metadata_dict = {
                "shed_config_filename": self.shed_config_dict.get("config_filename")
            }
        else:
            metadata_dict = {}
        readme_files = []
        ...
        with ValidationContext.from_app(
            app=self.app, work_dir=work_dir
            ...
            datatypes_config = hg_util.get_config_from_disk(
                suc.DATATYPES_CONFIG_FILENAME, files_dir
            )
            if datatypes_config:
                metadata_dict = self.generate_datatypes_metadata(
                    tv, files_dir, datatypes_config, metadata_dict
                )
                ...
                if root.find(".hg") < 0 and root.find("hgrc") < 0:
                    ...
                    for name in files:
                        ...
                        if name == rt_util.REPOSITORY_DEPENDENCY_DEFINITION_FILENAME:
                            path_to_repository_dependencies_config = os.path.join(
                                root, name
                            )
                        ...
                        elif looks_like_a_tool(
                            os.path.join(root, name),
                            invalid_names=self.NOT_TOOL_CONFIGS,
                        ):
                            full_path = str(
                                os.path.abspath(os.path.join(root, name))
                            )  # why the str, seems very odd
                        ...
                        # Find all exported workflows.
                            ...
                            if os.path.getsize(os.path.abspath(relative_path)) > 0:
                                ...
                                if workflow_text:
                                    ...
                                    try:
                                        exported_workflow_dict = json.loads(
                                            workflow_text
                                        )
                                    except Exception:
                                        log.exception(
                                ...
                                if (
                                    ...
                                    and exported_workflow_dict["a_galaxy_workflow"]
                                    == "true"
                                ):
        ...
        data_manager_config = hg_util.get_config_from_disk(
        ...
        tool_dependencies_config = hg_util.get_config_from_disk(
            rt_util.TOOL_DEPENDENCY_DEFINITION_FILENAME, files_dir
        )
        if tool_dependencies_config:
            ...
            if error_message:
                self.invalid_file_tups.append(
                    (rt_util.TOOL_DEPENDENCY_DEFINITION_FILENAME, error_message)
                )
        ...
        if package_name and package_version:
            ...
            for sub_elem in elem:
                if sub_elem.tag == "readme":
                    ...
                    repository_dependency_tup, repository_dependency_is_valid, error_message = self.handle_repository_elem(
                        repository_elem=sub_elem, only_if_compiling_contained_td=False
                    )
                            ...
                            if in_actions_group:
                                ...
                                for action_elem in actions_elem:
                                    if action_elem.tag == "package":
                                        ...
                                        ae_package_version = action_elem.get(
                                            "version", None
                                        )
    ...
    def generate_repository_dependency_metadata(
        self, repository_dependencies_config, metadata_dict
    ):
        """
        ...
        if xml_is_valid:
            invalid_repository_dependencies_dict = dict(
                description=root.get("description")
            )
            invalid_repository_dependency_tups = []
            valid_repository_dependencies_dict = dict(
                description=root.get("description")
            )
    ...
    def generate_tool_metadata(self, tool_config, tool, metadata_dict):
        ...
        for tool_requirement in tool.requirements:
            ...
            version = (
                str(tool_requirement.version) if tool_requirement.version else None
            )
        ...
        if tool.tests:
            for ttb in tool.tests:
                ...
                for output in ttb.outputs:
                    ...
                    outputs.append(
                        (name, basic_util.strip_path(file_name) if file_name else None)
                    )
    ...
    def generate_tool_dependency_metadata(
        ...
        if original_repository_metadata:
            ...
            original_valid_tool_dependencies_dict = original_repository_metadata.get(
        ...
        def _check_elem_for_dep(elems):
            ...
            for elem in elems:
                if elem.tag == "package":
                    rvs.valid_tool_dependencies_dict, rvs.invalid_tool_dependencies_dict, repository_dependency_tup, repository_dependency_is_valid, message = self.generate_package_dependency_metadata(
                        elem,
                        rvs.valid_tool_dependencies_dict,
                        rvs.invalid_tool_dependencies_dict,
                    )
                    if repository_dependency_is_valid:
                        if (
                            repository_dependency_tup
                            and repository_dependency_tup
                            not in valid_repository_dependency_tups
                        ):
                            # We have a valid complex repository dependency.
                            valid_repository_dependency_tups.append(
                                repository_dependency_tup
                            )
                    else:
                        if (
                            repository_dependency_tup
                            and repository_dependency_tup
                            not in invalid_repository_dependency_tups
                        ):
                            ...
                            invalid_repository_dependency_tups.append(
                                repository_dependency_tup
                            )
        ...
        if rvs.valid_tool_dependencies_dict:
            if original_valid_tool_dependencies_dict:
                ...
                updated_tool_dependency_names, deleted_tool_dependency_names = irm.handle_existing_tool_dependencies_that_changed_in_update(
                    self.repository,
    ...
    def generate_workflow_metadata(
        self, relative_path, exported_workflow_dict, metadata_dict
    ):
        """
    ...
    def get_relative_path_to_repository_file(
        self, root, name, relative_install_dir, work_dir, shed_config_dict
    ):
        if self.resetting_all_metadata_on_repository:
            ...
            relative_path_to_file = os.path.join(
                relative_install_dir, stripped_path_to_file
            )
        else:
            ...
            if (
                ...
                relative_path_to_file = relative_path_to_file[
                    len(shed_config_dict.get("tool_path")) + 1 :
                ]
        return relative_path_to_file

    def get_sample_files_from_disk(
        self, repository_files_dir, tool_path=None, relative_install_dir=None
    ):
        work_dir = ""
        if self.resetting_all_metadata_on_repository:
                ...
                for name in files:
                    if name.endswith(".sample"):
                        if self.resetting_all_metadata_on_repository:
                            ...
                            stripped_path_to_sample_file = full_path_to_sample_file.replace(
                                work_dir, ""
                            )
                            if stripped_path_to_sample_file.startswith("/"):
                                stripped_path_to_sample_file = stripped_path_to_sample_file[
                                    1:
                                ]
                            ...
                            if tool_path and relative_install_dir:
                                if relative_path_to_sample_file.startswith(
                                    ...
                                    relative_path_to_sample_file = relative_path_to_sample_file[
                                        len(tool_path) + 1 :
                                    ]
                        sample_file_metadata_paths.append(relative_path_to_sample_file)
        return sample_file_metadata_paths, sample_file_copy_paths

    def handle_repository_elem(
        self, repository_elem, only_if_compiling_contained_td=False
    ):
        """
        ...
        prior_installation_required = str(
            repository_elem.get("prior_installation_required", False)
        )
        repository_dependency_tup = [
            toolshed,
            name,
        ...
        if self.app.name == "galaxy":
            ...
            if repository:
                ...
                text = metadata_util.get_updated_changeset_revisions_from_tool_shed(
                    self.app, toolshed, name, owner, changeset_revision
                )
                if text:
            ...
            if suc.tool_shed_is_this_tool_shed(toolshed):
                try:
                    ...
                    error_message += (
                        "changeset revision %s because the owner is invalid."
                        % changeset_revision
                    )
                    log.debug(error_message)
                    is_valid = False
                ...
                try:
                    ...
                    error_message += (
                        "changeset revision %s because the name is invalid.  "
                        % changeset_revision
                    )
                    log.debug(error_message)
                    is_valid = False
                ...
                if changeset_revision is None and tip_ctx.rev() >= 0:
                    ...
                    if not found:
                        ...
                        error_message += (
                            "changeset revision %s because the changeset revision is invalid.  "
                            % changeset_revision
                        )
                        log.debug(error_message)
                        is_valid = False
    ...
    def update_repository_dependencies_metadata(
        self, metadata, repository_dependency_tups, is_valid, description
    ):
        if is_valid:
