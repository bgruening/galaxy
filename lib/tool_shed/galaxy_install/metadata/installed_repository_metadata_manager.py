...
class InstalledRepositoryMetadataManager(metadata_generator.MetadataGenerator):
    ...
    def build_repository_ids_select_field(
        self, name="repository_ids", multiple=True, display="checkboxes"
    ):
    ...
    def get_query_for_setting_metadata_on_repositories(self, order=True):
        ...
        if order:
            return (
                self.app.install_model.context.query(
                    self.app.install_model.ToolShedRepository
                )
                .filter(
                    self.app.install_model.ToolShedRepository.table.c.uninstalled
                    == false()
                )
                .order_by(
    ...
    def reset_all_metadata_on_installed_repository(self):
        ...
        if self.relative_install_dir:
            ...
            if self.metadata_dict != original_metadata_dict:
                ...
                log.debug(
                    "Metadata has been reset on repository %s." % self.repository.name
                )
            else:
                log.debug(
                    "Metadata did not need to be reset on repository %s."
