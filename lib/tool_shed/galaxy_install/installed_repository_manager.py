...
class InstalledRepositoryManager(object):
    ...
    def activate_repository(self, repository):
        ...
        if repository.includes_tools_for_display_in_tool_panel:
            ...
            if repository.includes_data_managers:
                tp, data_manager_relative_install_dir = repository.get_tool_relative_path(
                    self.app
                )
                # Hack to add repository.name here, which is actually the root of the installed repository
                data_manager_relative_install_dir = os.path.join(
                    data_manager_relative_install_dir, repository.name
                )
        ...
        if repository.includes_datatypes:
            if tool_path:
                repository_install_dir = os.path.abspath(
            ...
            if installed_repository_dict:
                ...
                if converter_path is not None:
                    cdl.load_installed_datatype_converters(
                        installed_repository_dict, deactivate=False
                    )
                display_path = installed_repository_dict.get("display_path")
                if display_path is not None:
                    cdl.load_installed_display_applications(
                        installed_repository_dict, deactivate=False
                    )
    ...
    def add_entry_to_installed_repository_dependencies_of_installed_repositories(
        ...
        if (
            repository_tup
            not in self.installed_repository_dependencies_of_installed_repositories
        ):
            debug_msg = (
                "Adding an entry for revision %s of repository %s owned by %s "
                % (installed_changeset_revision, name, owner)
            )
            debug_msg += (
                "to installed_repository_dependencies_of_installed_repositories."
            )
            log.debug(debug_msg)
            self.installed_repository_dependencies_of_installed_repositories[
                repository_tup
        ...
        for required_repository_tup in repository_dependency_tups:
            ...
            if (
                ...
                in self.installed_dependent_repositories_of_installed_repositories
            ):
                self.installed_dependent_repositories_of_installed_repositories[
                    required_repository_tup
                ].append(repository_tup)
            else:
                self.installed_dependent_repositories_of_installed_repositories[
                    required_repository_tup
                ] = [repository_tup]

    def add_entry_to_installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies(
        ...
        if (
            tool_dependency_tup
            not in self.installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies
        ):
            ...
            installed_runtime_dependent_tool_dependency_tups = self.get_runtime_dependent_tool_dependency_tuples(
    ...
    def add_entry_to_installed_tool_dependencies_of_installed_repositories(
        ...
        if (
            repository_tup
            not in self.installed_tool_dependencies_of_installed_repositories
        ):
            tool_shed, name, owner, installed_changeset_revision = repository_tup
            debug_msg = (
                "Adding an entry for revision %s of repository %s owned by %s "
    ...
    def add_entry_to_runtime_tool_dependencies_of_installed_tool_dependencies(
        ...
        if (
            tool_dependency_tup
            not in self.runtime_tool_dependencies_of_installed_tool_dependencies
        ):
            tool_shed_repository_id, name, version, type = tool_dependency_tup
            debug_msg = "Adding an entry for version %s of %s %s " % (
                version,
            ...
            log.debug(debug_msg)
            runtime_dependent_tool_dependency_tups = self.get_runtime_dependent_tool_dependency_tuples(
                tool_dependency, status=None
            )
            self.runtime_tool_dependencies_of_installed_tool_dependencies[
                tool_dependency_tup
    ...
    def get_containing_repository_for_tool_dependency(self, tool_dependency_tup):
        tool_shed_repository_id, name, version, type = tool_dependency_tup
        return self.app.install_model.context.query(
            self.app.install_model.ToolShedRepository
        ).get(tool_shed_repository_id)
    ...
    def get_dependencies_for_repository(
        ...
        description, repository_clone_url, changeset_revision, ctx_rev, repository_owner, repository_dependencies, tool_dependencies = repository_util.get_repo_info_tuple_contents(
            repo_info_tuple
        )
        ...
        if repository_dependencies:
            ...
            if not updating and repository and repository.metadata:
                installed_rd, missing_rd = self.get_installed_and_missing_repository_dependencies(
                    repository
                )
            else:
                installed_rd, missing_rd = self.get_installed_and_missing_repository_dependencies_for_new_or_updated_install(
                    repo_info_tuple
                )
            # Discover all repository dependencies and retrieve information for installing them.
            all_repo_info_dict = rdim.get_required_repo_info_dicts(
                tool_shed_url, util.listify(repo_info_dict)
            )
            has_repository_dependencies = all_repo_info_dict.get(
            ...
            includes_tool_dependencies = all_repo_info_dict.get(
            ...
            if required_repo_info_dicts:
                ...
                for rid in required_repo_info_dicts:
                    for name, repo_info_tuple in rid.items():
                        description, repository_clone_url, changeset_revision, ctx_rev, repository_owner, rid_repository_dependencies, rid_tool_dependencies = repository_util.get_repo_info_tuple_contents(
                            repo_info_tuple
                        )
                ...
                if required_tool_dependencies:
                    ...
                    required_installed_td, required_missing_td = self.get_installed_and_missing_tool_dependencies_for_repository(
                        required_tool_dependencies
                    )
    ...
    def get_installed_and_missing_repository_dependencies(self, repository):
        ...
        if has_repository_dependencies:
            ...
            for tsr in repository.repository_dependencies:
                prior_installation_required = self.set_prior_installation_required(
                    repository, tsr
                )
                only_if_compiling_contained_td = self.set_only_if_compiling_contained_td(
                    repository, tsr
                )
                rd_tup = [
                    tsr.tool_shed,
                ...
                if (
                    ...
                    if repository.missing_tool_dependencies:
                        if not self.repository_dependency_needed_only_for_compiling_tool_dependency(
                            repository, tsr
                        ):
            ...
            if installed_rd_tups or missing_rd_tups:
                ...
                root_key = container_util.generate_repository_dependencies_key_for_repository(
        ...
        if repository_dependencies:
            ...
            for key, rd_tups in repository_dependencies.items():
                ...
                for rd_tup in rd_tups:
                    ...
                    tmp_clone_url = common_util.generate_clone_url_from_repo_info_tup(
                        self.app, rd_tup
                    )
                    tmp_repo_info_tuple = (
                        None,
        ...
        if tool_dependencies_dict:
            ...
            for td_key, val in tmp_tool_dependencies_dict.items():
                ...
                tool_dependency_status = (
                    self.install_model.ToolDependency.installation_status.NEVER_INSTALLED
                )
    ...
    def get_repository_install_dir(self, tool_shed_repository):
        for tree in self.tool_trees:
            ...
            if tool_path:
                ts = common_util.remove_port_from_tool_shed_url(
                    str(tool_shed_repository.tool_shed)
                )
    ...
    def get_runtime_dependent_tool_dependency_tuples(
        self, tool_dependency, status=None
    ):
        """
        ...
        if required_env_shell_file_path is not None:
            for td in self.app.install_model.context.query(
                ...
                if status is None or td.status == status:
                    ...
                    if env_shell_file_path is not None:
                        ...
                        if (
                            ...
                            and contents.find(required_env_shell_file_path) >= 0
                        ):
                            td_tuple = self.get_tool_dependency_tuple_for_installed_repository_manager(
                                td
                            )
                            runtime_dependent_tool_dependency_tups.append(td_tuple)
        return runtime_dependent_tool_dependency_tups

    def get_tool_dependency_tuple_for_installed_repository_manager(
        self, tool_dependency
    ):
        ...
        return (
            tool_dependency.tool_shed_repository_id,
            str(tool_dependency.name),
    ...
    def handle_existing_tool_dependencies_that_changed_in_update(
        ...
        for (
            original_dependency_key,
            original_dependency_val_dict,
        ) in original_dependency_dict.items():
            if original_dependency_key not in new_dependency_dict:
                ...
                if updated_tool_dependency:
                    ...
                    deleted_tool_dependency_names.append(
                        original_dependency_val_dict["name"]
                    )
    ...
    def handle_repository_install(self, repository):
        ...
        self.add_entry_to_installed_repository_dependencies_of_installed_repositories(
            repository
        )
        ...
        self.add_entry_to_installed_tool_dependencies_of_installed_repositories(
            repository
        )
        for tool_dependency in repository.tool_dependencies:
    ...
    def handle_repository_uninstall(self, repository):
        ...
        for tool_dependency in repository.tool_dependencies:
            ...
            for (
                ...
                if (
                    tool_dependency_tup
                    in installed_runtime_dependent_tool_dependency_tups
                ):
                    # Remove the tool_dependency from the list.
                    installed_runtime_dependent_tool_dependency_tups.remove(
                        tool_dependency_tup
                    )
            ...
            self.installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies = altered_installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies
            # Remove the entry for this tool_dependency from self.runtime_tool_dependencies_of_installed_tool_dependencies.
            self.remove_entry_from_runtime_tool_dependencies_of_installed_tool_dependencies(
                tool_dependency
            )
        ...
        self.remove_entry_from_installed_tool_dependencies_of_installed_repositories(
            repository
        )
        ...
        self.remove_entry_from_installed_repository_dependencies_of_installed_repositories(
            repository
        )
        # Remove this repository's entry from self.repository_dependencies_of_installed_repositories.
        self.remove_entry_from_repository_dependencies_of_installed_repositories(
            repository
        )
    ...
    def handle_tool_dependency_install(self, repository, tool_dependency):
        ...
        if tool_dependency.is_installed:
            ...
            if (
                repository_tup
                in self.installed_tool_dependencies_of_installed_repositories
            ):
                self.installed_tool_dependencies_of_installed_repositories[
                    repository_tup
                ].append(tool_dependency_tup)
            else:
                self.installed_tool_dependencies_of_installed_repositories[
                    repository_tup
                ] = [tool_dependency_tup]
    ...
    def load_dependency_relationships(self):
        ...
        for repository in self.context.query(
            self.app.install_model.ToolShedRepository
        ).filter(
            self.app.install_model.ToolShedRepository.table.c.status
            == self.app.install_model.ToolShedRepository.installation_status.INSTALLED
        ):
            # Populate self.repository_dependencies_of_installed_repositories.
            self.add_entry_to_repository_dependencies_of_installed_repositories(
                repository
            )
            # Populate self.installed_repository_dependencies_of_installed_repositories.
            self.add_entry_to_installed_repository_dependencies_of_installed_repositories(
        ...
        for tool_dependency in self.context.query(
            self.app.install_model.ToolDependency
        ).filter(
            self.app.install_model.ToolDependency.table.c.status
            == self.app.install_model.ToolDependency.installation_status.INSTALLED
        ):
            # Populate self.runtime_tool_dependencies_of_installed_tool_dependencies.
            self.add_entry_to_runtime_tool_dependencies_of_installed_tool_dependencies(
    ...
    def load_proprietary_datatypes(self):
        ...
        for tool_shed_repository in (
            ...
            .filter(
                and_(
                    self.install_model.ToolShedRepository.table.c.includes_datatypes
                    == true(),
    ...
    def load_proprietary_converters_and_display_applications(self, deactivate=False):
        ...
        for installed_repository_dict in self.installed_repository_dicts:
            if installed_repository_dict["converter_path"]:
                cdl.load_installed_datatype_converters(
                    installed_repository_dict, deactivate=deactivate
                )
    ...
    def uninstall_repository(self, repository, remove_from_disk=True):
        ...
        if repository.includes_tools_for_display_in_tool_panel:
            ...
            tpm.remove_repository_contents(
                repository, shed_tool_conf, uninstall=remove_from_disk
            )
        if repository.includes_data_managers:
        ...
        if repository.includes_datatypes:
            ...
            if installed_repository_dict:
                ...
                if converter_path is not None:
                    cdl.load_installed_datatype_converters(
                        installed_repository_dict, deactivate=True
                    )
                display_path = installed_repository_dict.get("display_path")
                if display_path is not None:
                    cdl.load_installed_display_applications(
        ...
        if remove_from_disk:
            try:
                ...
                log.debug(
                    "Removed repository installation directory: %s"
                    % str(repository_install_dir)
                )
                removed = True
            except Exception as e:
                log.debug(
                    "Error removing repository installation directory %s: %s"
                    % (str(repository_install_dir), str(e))
                )
                if isinstance(e, OSError) and not os.path.exists(
                    repository_install_dir
                ):
                    removed = True
                    log.debug(
                        "Repository directory does not exist on disk, marking as uninstalled."
                    )
                else:
                    removed = False
            if removed:
                ...
                tool_dependencies_to_uninstall = (
                    repository.tool_dependencies_installed_or_in_error
                )
                tool_dependencies_to_uninstall.extend(
                    repository.tool_dependencies_being_installed
                )
    ...
    def purge_repository(self, repository):
        ...
        if repository.is_new:
            ...
            if repository.tool_versions:
                for tool_version in repository.tool_versions:
                    if tool_version.parent_tool_association:
                        ...
                        ) in tool_version.parent_tool_association:
                            try:
                    ...
                    if tool_version.child_tool_association:
                        for (
                            tool_version_association
                        ) in tool_version.child_tool_association:
                            try:
                                sa_session.delete(tool_version_association)
                                sa_session.flush()
                            except Exception as e:
            ...
            for orphan_repository_dependency in sa_session.query(
                self.app.install_model.RepositoryDependency
            ).filter(
                self.app.install_model.RepositoryDependency.table.c.tool_shed_repository_id
                == repository.id
            ):
            ...
            sa_session.flush()
            message = (
                "The repository named <b>%s</b> with status <b>%s</b> has been purged.<br/>"
                % (str(repository.name), str(repository.status))
            )
            message += (
                "Total associated tool_version records purged: %d<br/>"
                % purged_tool_versions
            )
            message += (
                "Total associated tool_dependency records purged: %d<br/>"
                % purged_tool_dependencies
            )
            message += (
    ...
    def remove_entry_from_installed_repository_dependencies_of_installed_repositories(
        ...
        for (
            r_tup,
            v_tups,
        ) in self.installed_dependent_repositories_of_installed_repositories.items():
            if repository_tup in v_tups:
                debug_msg = (
                    "Removing entry for revision %s of repository %s owned by %s "
                    % (installed_changeset_revision, name, owner)
                )
                r_tool_shed, r_name, r_owner, r_installed_changeset_revision = r_tup
                debug_msg += (
                    "from the dependent list for revision %s of repository %s owned by %s "
                    % (r_installed_changeset_revision, r_name, r_owner)
                )
                debug_msg += (
                    "in installed_repository_dependencies_of_installed_repositories."
                )
                log.debug(debug_msg)
        ...
        if (
            repository_tup
            in self.installed_repository_dependencies_of_installed_repositories
        ):
            debug_msg = (
                "Removing entry for revision %s of repository %s owned by %s "
                % (installed_changeset_revision, name, owner)
            ...
            debug_msg += (
                "from installed_repository_dependencies_of_installed_repositories."
            )
            log.debug(debug_msg)
            del self.installed_repository_dependencies_of_installed_repositories[
                repository_tup
            ]

    def remove_entry_from_installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies(
        self, tool_dependency
    ):
        ...
        if (
            ...
            log.debug(debug_msg)
            del self.installed_runtime_dependent_tool_dependencies_of_installed_tool_dependencies[
                tool_dependency_tup
            ]

    def remove_entry_from_installed_tool_dependencies_of_installed_repositories(
        self, repository
    ):
        ...
        if repository_tup in self.installed_tool_dependencies_of_installed_repositories:
            ...
            log.debug(debug_msg)
            del self.installed_tool_dependencies_of_installed_repositories[
                repository_tup
            ]
    ...
    def remove_entry_from_runtime_tool_dependencies_of_installed_tool_dependencies(
        self, tool_dependency
    ):
        ...
        if (
            tool_dependency_tup
            in self.runtime_tool_dependencies_of_installed_tool_dependencies
        ):
            tool_shed_repository_id, name, version, type = tool_dependency_tup
            debug_msg = (
                "Removing entry for version %s of %s %s from runtime_tool_dependencies_of_installed_tool_dependencies."
                % (version, type, name)
            )
            log.debug(debug_msg)
            del self.runtime_tool_dependencies_of_installed_tool_dependencies[
                tool_dependency_tup
            ]
    ...
    def remove_entry_from_tool_dependencies_of_installed_repositories(self, repository):
        ...
        if repository_tup in self.tool_dependencies_of_installed_repositories:
            tool_shed, name, owner, installed_changeset_revision = repository_tup
            debug_msg = (
                "Removing entry for revision %s of repository %s owned by %s from tool_dependencies_of_installed_repositories."
                % (installed_changeset_revision, name, owner)
            )
            log.debug(debug_msg)
    ...
    def repository_dependency_needed_only_for_compiling_tool_dependency(
        self, repository, repository_dependency
    ):
        for (
            rd_tup
        ) in repository.tuples_of_repository_dependencies_needed_for_compiling_td:
            ...
            cleaned_tool_shed = common_util.remove_protocol_and_port_from_tool_shed_url(
                tool_shed
            )
            ...
            if (
                ...
                and (
                    repository_dependency.installed_changeset_revision
                    == changeset_revision
    ...
    def set_prior_installation_required(self, repository, required_repository):
        ...
        required_rd_tups_that_must_be_installed = (
            repository.requires_prior_installation_of
        )
    ...
    def update_existing_tool_dependency(
        self, repository, original_dependency_dict, new_dependencies_dict
    ):
        """
        ...
        if tool_dependency and tool_dependency.can_update:
            ...
            if removed_from_disk:
                ...
                if (
                    new_dependency_name
                    and new_dependency_type
                    and new_dependency_version
                ):
                    ...
                    tool_dependency.status = (
                        self.app.install_model.ToolDependency.installation_status.UNINSTALLED
                    )
