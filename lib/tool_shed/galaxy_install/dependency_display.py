...
class DependencyDisplayer(object):
    ...
    def add_installation_directories_to_tool_dependencies(self, tool_dependencies):
        ...
        for dependency_key, requirements_dict in tool_dependencies.items():
            ...
            if self.app.config.tool_dependency_dir:
                ...
                root_dir = (
                    "<set your tool_dependency_dir in your Galaxy configuration file>"
                )
            ...
            if error_from_tuple:
                ...
                if invalid_repository_dependencies_dict is not None:
                    ...
                    for repository_dependency_tup in invalid_repository_dependencies:
                        toolshed, name, owner, changeset_revision, prior_installation_required, only_if_compiling_contained_td, error = common_util.parse_repository_dependency_tuple(
                ...
                if repository_dependencies_dict is not None:
                    ...
                    if rd_tups is not None:
                        ...
                        for rd_tup in rd_tups:
                            tool_shed, name, owner, changeset_revision, pir, oicct = common_util.parse_repository_dependency_tuple(
                                rd_tup
                            )
                            if util.asbool(pir):
                                pir_str = "True"
    ...
    def generate_message_for_orphan_tool_dependencies(self, repository, metadata_dict):
                ...
                for td_key, requirements_dict in tool_dependencies.items():
                    if td_key == "set_environment":
                        ...
                        if self.tool_dependency_is_orphan(type, name, version, tools):
                            ...
                            package_orphans_str += (
                                "<b>* name:</b> %s, <b>type:</b> %s, <b>version:</b> %s<br/>"
                                % (str(name), str(type), str(version))
                            )
        if has_orphan_package_dependencies:
            message += (
                "The settings for <b>name</b>, <b>version</b> and <b>type</b> from a "
            )
            message += "contained tool configuration file's <b>requirement</b> tag does not match "
            message += (
                "the information for the following tool dependency definitions in the "
            )
            message += (
                "<b>tool_dependencies.xml</b> file, so these tool dependencies have no "
            )
        ...
        if has_orphan_set_environment_dependencies:
            message += (
                "The settings for <b>name</b> and <b>type</b> from a contained tool "
            )
    ...
    def merge_containers_dicts_for_new_install(self, containers_dicts):
        ...
        if containers_dicts:
            ...
            try:
                ...
                for old_container_dict in containers_dicts:
                    ...
                    if old_container_tool_dependencies_root:
                        if tool_dependencies_root_folder is None:
                            ...
                            td_list = [
                                td.listify
                                for td in tool_dependencies_folder.tool_dependencies
                            ]
                            ...
                            for (
                                ...
                                old_container_tool_dependencies_folder.tool_dependencies
                            ):
                ...
                log.debug(
                    "Exception in merge_containers_dicts_for_new_install: %s" % str(e)
                )
            finally:
                lock.release()
    ...
    def populate_containers_dict_from_repository_metadata(
        ...
        if metadata:
            ...
            installed_repository_dependencies, missing_repository_dependencies = self.app.installed_repository_manager.get_installed_and_missing_repository_dependencies(
                repository
            )
            ...
            repository_installed_tool_dependencies, repository_missing_tool_dependencies = self.get_installed_and_missing_tool_dependencies_for_installed_repository(
            ...
            for rid in required_repo_info_dicts:
                for name, repo_info_tuple in rid.items():
                    ...
                    if tool_dependencies:
                        ...
                        required_repository, installed_changeset_revision = repository_util.repository_was_previously_installed(
                            self.app,
                            tool_shed_url,
                            name,
                            repo_info_tuple,
                        ...
                        if required_repository:
                            required_repository_installed_tool_dependencies, required_repository_missing_tool_dependencies = self.get_installed_and_missing_tool_dependencies_for_installed_repository(
                                required_repository, tool_dependencies
                            )
                            if required_repository_installed_tool_dependencies:
                                # Add the install_dir attribute to the tool_dependencies.
                                required_repository_installed_tool_dependencies = self.add_installation_directories_to_tool_dependencies(
                                ...
                                for (
                                    ...
                                    required_repository_installed_tool_dependencies.items()
                                ):
                                    if (
                                        td_key
                                        not in repository_installed_tool_dependencies
                                    ):
                                        repository_installed_tool_dependencies[
                                            td_key
                                        ] = td_dict
                            if required_repository_missing_tool_dependencies:
                                # Add the install_dir attribute to the tool_dependencies.
                                required_repository_missing_tool_dependencies = self.add_installation_directories_to_tool_dependencies(
                                ...
                                for (
                                    ...
                                    required_repository_missing_tool_dependencies.items()
                                ):
                                    if (
                                        td_key
                                        not in repository_missing_tool_dependencies
                                    ):
                                        repository_missing_tool_dependencies[
                                            td_key
                                        ] = td_dict
