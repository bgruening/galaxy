...
def generate_deprecated_repository_img_str(include_mouse_over=False):
    if include_mouse_over:
        deprecated_tip_str = (
            'class="icon-button" title="This repository is deprecated in the Tool Shed"'
        )
    else:
        deprecated_tip_str = ""
    return '<img src="%s/images/icon_error_sml.gif" %s/>' % (
        url_for("/static"),
        deprecated_tip_str,
    )
...
def generate_includes_workflows_img_str(include_mouse_over=False):
    if include_mouse_over:
        deprecated_tip_str = (
            'class="icon-button" title="This repository contains exported workflows"'
        )
    else:
        deprecated_tip_str = ""
    return '<img src="%s/images/fugue/gear.png" %s/>' % (
        url_for("/static"),
        deprecated_tip_str,
    )
...
def generate_latest_revision_img_str(include_mouse_over=False):
    ...
    return '<img src="%s/june_2007_style/blue/ok_small.png" %s/>' % (
        url_for("/static"),
        latest_revision_tip_str,
    )
...
def generate_revision_updates_img_str(include_mouse_over=False):
    ...
    return '<img src="%s/images/icon_warning_sml.gif" %s/>' % (
        url_for("/static"),
        revision_updates_tip_str,
    )
...
def generate_revision_upgrades_img_str(include_mouse_over=False):
    ...
    return '<img src="%s/images/up.gif" %s/>' % (
        url_for("/static"),
        revision_upgrades_tip_str,
    )
...
def generate_unknown_img_str(include_mouse_over=False):
    if include_mouse_over:
        unknown_tip_str = (
            'class="icon-button" title="Unable to get information from the Tool Shed"'
        )
    else:
        unknown_tip_str = ""
    return '<img src="%s/june_2007_style/blue/question-octagon-frame.png" %s/>' % (
        url_for("/static"),
        unknown_tip_str,
    )
...
class InstalledRepositoryGrid(grids.Grid):
    ...
    class StatusColumn(grids.TextColumn):
        def get_value(self, trans, grid, tool_shed_repository):
            return repository_util.get_tool_shed_repository_status_label(
    ...
    columns = [
        ...
        DeletedColumn(
            label="Status", key="deleted", visible=False, filterable="advanced"
        ),
    ]
    columns.append(
    ...
    operations = [
        ...
        grids.GridOperation(
            ...
            target="center",
            url_args=dict(
                controller="admin_toolshed", action="install_latest_repository_revision"
            ),
        ),
        grids.GridOperation(
            label="Install",
        ...
        grids.GridOperation(
            ...
            target="center",
            url_args=dict(
                controller="admin_toolshed", action="deactivate_or_uninstall_repository"
            ),
        ),
        grids.GridOperation(
            label="Reset to install",
            condition=(
                lambda item: (
                    item.status
                    == tool_shed_install.ToolShedRepository.installation_status.ERROR
    ...
    def legend(self):
        legend_str = (
            "%s&nbsp;&nbsp;Updates are available in the Tool Shed for this revision<br/>"
            % generate_revision_updates_img_str()
        )
        legend_str += (
            "%s&nbsp;&nbsp;A newer installable revision is available for this repository<br/>"
            % generate_revision_upgrades_img_str()
        )
        legend_str += (
            "%s&nbsp;&nbsp;This is the latest installable revision of this repository<br/>"
            % generate_latest_revision_img_str()
        )
        legend_str += (
            "%s&nbsp;&nbsp;This repository is deprecated in the Tool Shed<br/>"
            % generate_deprecated_repository_img_str()
        )
        legend_str += (
            "%s&nbsp;&nbsp;This repository contains exported workflows<br/>"
            % generate_includes_workflows_img_str()
        )
        legend_str += (
            "%s&nbsp;&nbsp;Unable to get information from the Tool Shed<br/>"
