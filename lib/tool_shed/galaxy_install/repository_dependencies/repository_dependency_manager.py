...
from galaxy.util import asbool, build_url, smart_str, unicodify, url_get
...
class RepositoryDependencyInstallManager(object):
    ...
    def build_repository_dependency_relationships(
        self, repo_info_dicts, tool_shed_repositories
    ):
        """
    ...
    def create_repository_dependency_objects(
        ...
        for repo_info_dict in all_repo_info_dicts:
            ...
            if (
                self.is_in_repo_info_dicts(repo_info_dict, repo_info_dicts)
                or install_repository_dependencies
            ):
                for name, repo_info_tuple in repo_info_dict.items():
                    ...
                    repository_db_record, installed_changeset_revision = repository_util.repository_was_previously_installed(
                    ...
                    if repository_db_record:
                        if repository_db_record.status in [
                            ...
                            debug_msg += (
                                ...
                                % (
                                    str(installed_changeset_revision),
                                    str(repository_db_record.status),
                                )
                            )
                            log.debug(debug_msg)
                            can_update_db_record = False
                    ...
                    if can_update_db_record:
                        ...
                        if (
                            ...
                            not in all_created_or_updated_tool_shed_repositories
                        ):
                        ...
                        if (
                            ...
                            or self.is_in_repo_info_dicts(
                                repo_info_dict, repo_info_dicts
                            )
                        ):
                            if (
                                tool_shed_repository
                                not in created_or_updated_tool_shed_repositories
                            ):
    ...
    def get_or_create_tool_shed_repository(
        self, tool_shed, name, owner, changeset_revision
    ):
        """
        ...
        if not repository:
            ...
            ctx_rev = suc.get_ctx_rev(
                self.app, tool_shed_url, name, owner, changeset_revision
            )
    ...
    def get_repository_dependency_by_repository_id(
        self, install_model, decoded_repository_id
    ):
        return (
                            ...
                            if required_repo_info_dicts:
                                for required_repo_info_dict in required_repo_info_dicts:
                                    ...
                                    all_repo_info_dicts_keys = [
                                        next(iter(d)) for d in all_repo_info_dicts
                                    ]
                                    if (
                                        required_repo_info_dict_key
                                        not in all_repo_info_dicts_keys
                                    ):
    ...
    def reset_previously_installed_repository(self, repository):
        ...
        repository.status = (
            self.app.install_model.ToolShedRepository.installation_status.NEW
        )
