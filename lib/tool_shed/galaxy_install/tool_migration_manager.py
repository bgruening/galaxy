...
class ToolMigrationManager(object):
    def __init__(
        ...
        if install_dependencies and app.config.tool_dependency_dir is None:
            ...
            message += (
                'for "tool_dependency_dir" set in your galaxy.ini file.  Set this '
            )
            ...
            if tree is None:
                ...
                if tool_panel_configs:
                    ...
                    tool_shed_accessible, missing_tool_configs_dict = common_util.check_for_missing_tools(
                        app, tool_panel_configs, latest_migration_script_number
                    )
                else:
                    ...
                    if missing_tool_configs_dict:
                        ...
                        for repository_elem in root:
                            ...
                            if self.__is_valid_repository_tag(repository_elem):
                                ...
                                tool_shed_accessible, repository_dependencies_dict = common_util.get_repository_dependencies(
                                    app,
                                    self.tool_shed_url,
                                    name,
                                ...
                                created_tool_shed_repositories = self.create_or_update_tool_shed_repository_records(
                                    name,
                                    changeset_revision,
                                ...
                                ordered_tool_shed_repositories = self.order_repositories_for_installation(
                                ...
                                ) in ordered_tool_shed_repositories:
                                    is_repository_dependency = self.__is_repository_dependency(
                                        name, changeset_revision, tool_shed_repository
                                    )
    ...
    def create_or_update_tool_shed_repository_record(
        self, name, owner, changeset_revision, description=None
    ):
        ...
        relative_clone_dir = os.path.join(
            self.tool_shed, "repos", owner, name, changeset_revision
        )
        ...
        if not self.__iscloned(clone_dir):
            repository_clone_url = os.path.join(
                self.tool_shed_url, "repos", owner, name
            )
            ctx_rev = suc.get_ctx_rev(
                self.app, self.tool_shed_url, name, owner, changeset_revision
            )
    ...
    def create_or_update_tool_shed_repository_records(
        ...
        tool_shed_repository = self.create_or_update_tool_shed_repository_record(
            name, self.repository_owner, changeset_revision, description=description
        )
                    ...
                    if section_elem.tag == "tool":
                        ...
                        if tool_config == proprietary_tool_config:
                            ...
                            tool_sections.append(
                                ToolSection(
                                    ensure_tool_conf_item(proprietary_tool_panel_elem)
                                )
                            )
                            if not is_displayed:
                                is_displayed = True
    ...
    def get_guid(self, repository_clone_url, relative_install_dir, tool_config):
        if self.shed_config_dict.get("tool_path"):
            relative_install_dir = os.path.join(
                self.shed_config_dict["tool_path"], relative_install_dir
            )
    ...
    def get_prior_install_required_dict(
        self, tool_shed_repositories, repository_dependencies_dict
    ):
        """
        ...
        tsr_ids = [
            tool_shed_repository.id for tool_shed_repository in tool_shed_repositories
        ]
                ...
                if util.asbool(prior_installation_required):
                    for tsr in tool_shed_repositories:
                        if (
                            tsr.name == name
                            and tsr.owner == owner
                            and tsr.changeset_revision == changeset_revision
                        ):
    ...
    def handle_repository_contents(
        self,
        tool_shed_repository,
        ...
        if not is_repository_dependency:
            for tool_elem in repository_elem:
                ...
                guid = self.get_guid(
                    repository_clone_url, relative_install_dir, tool_config
                )
                ...
                if is_displayed:
                    tool_panel_dict_for_tool_config = self.tpm.generate_tool_panel_dict_for_tool_config(
                    ...
                    log.error(
                        ...
                        % (
                            guid,
                            tool_config,
                            ", ".join(self.proprietary_tool_confs or []),
                        )
                    )
        ...
        if "tools" in irmm_metadata_dict:
            ...
            tool_util.copy_sample_files(
                self.app, tool_index_sample_files, tool_path=self.tool_path
            )
            ...
            if repository_tools_tups:
                ...
                repository_tools_tups = tdtm.handle_missing_data_table_entry(
                    relative_install_dir, self.tool_path, repository_tools_tups
                )
        ...
        if "datatypes" in irmm_metadata_dict:
            ...
            tool_shed_repository.status = (
                self.app.install_model.ToolShedRepository.installation_status.LOADING_PROPRIETARY_DATATYPES
            )
    ...
    def install_repository(
        ...
        if cloned_ok and is_installed:
            ...
            if cloned_ok and not is_installed:
                ...
                log.error(
                    "Error attempting to clone repository %s: %s",
    ...
    def order_repositories_for_installation(
        self, tool_shed_repositories, repository_dependencies_dict
    ):
        """
        ...
        prior_install_required_dict = self.get_prior_install_required_dict(
            tool_shed_repositories, repository_dependencies_dict
        )
    ...
    def __has_tool_dependencies(self, metadata_dict):
        ...
        if (
            "tool_dependencies" in metadata_dict
    ...
    def __is_repository_dependency(
        self, name, changeset_revision, tool_shed_repository
    ):
        """Determine if the provided tool shed repository is a repository dependency."""
        if (
            str(tool_shed_repository.name) != str(name)
            or str(tool_shed_repository.owner) != str(self.repository_owner)
            or str(tool_shed_repository.changeset_revision) != str(changeset_revision)
        ):
            return True
        return False
