...
class SyncDatabase(object):
    def sync_database_with_file_system(
        ...
        if (
            ...
            == app.install_model.ToolDependency.installation_status.INSTALLING
        ):
            ...
            if (
                ...
                in tool_dependency_installation_directory_contents
            ):
                ...
                if app.config.running_functional_tests:
                    ...
                    error_message += (
                        "missing database record it is now being set to Error."
                    )
                    tool_dependency.status = (
        ...
        if proceed_with_install:
            ...
            if package_install_version == "1.0":
                ...
                if not actions_elem_tuples:
                    proceed_with_install = False
                    error_message = (
                        "Version %s of the %s package cannot be installed because "
    ...
    def process_tag_set(
        ...
        if package_name and package_version:
            dependencies_ignored = (
                not self.app.toolbox.dependency_manager.uses_tool_shed_dependencies()
            )
...
class Repository(RecipeTag, SyncDatabase):
    ...
    def create_tool_dependency_with_initialized_env_sh_file(
        ...
        if tool_dependencies_config:
            required_td_tree, error_message = xml_util.parse_xml(
                tool_dependencies_config
            )
            if required_td_tree:
                ...
                for required_td_elem in required_td_root:
                    ...
                    if required_td_elem.tag == "package":
                        ...
                        required_tool_dependency = tool_dependency_util.get_tool_dependency_by_name_version_type_repository(
                            self.app,
                            required_repository,
                        ...
                        if (
                            required_td_package_name == package_name
                            and required_td_package_version == package_version
                        ):
                            ...
                            if return_code:
                                error_message = (
                                    "Error defining env.sh file for package %s, return_code: %s"
                                ...
                                required_tool_dependency is not None
                                and required_tool_dependency.in_error_state
                            ):
                                error_message = (
    ...
    def get_required_repository_package_env_sh_path(
        self, package_name, package_version, required_repository
    ):
    ...
    def handle_complex_repository_dependency_for_package(
        ...
        required_repository = repository_util.get_repository_for_dependency_relationship(
            self.app,
            tool_shed_url,
        ...
        if required_repository:
            ...
            if can_install_tool_dependency:
                ...
                if required_repository.is_deactivated_or_installed:
                    ...
                    if not repo_files_dir:
                        ...
                        raise Exception(message)
                    tool_dependencies_config = repository_util.get_absolute_path_to_file_in_repository(
            ...
            message = (
                "Unable to locate required tool shed repository named %s owned by %s with revision %s."
                % (
    ...
    def process_tag_set(
        ...
        for rd_tool_dependency in rd_tool_dependencies:
            if (
                ...
                log.error(
                    "Error installing tool dependency for required repository: %s"
                    % str(rd_tool_dependency.error_message)
                )
        return tool_dependency, proceed_with_install, action_elem_tuples
...
class SetEnvironment(RecipeTag):
    ...
    def set_environment(
        self, elem, tool_shed_repository, attr_tups_of_dependencies_for_install
    ):
        """
        ...
        for env_var_elem in elems:
            ...
            if attr_tup in attr_tups_of_dependencies_for_install:
                ...
                if env_var_version == "1.0":
                    ...
                    if return_code:
                        error_message = (
                            "Error creating env.sh file for tool dependency %s, return_code: %s"
