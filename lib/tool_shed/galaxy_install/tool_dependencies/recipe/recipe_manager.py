...
class StepManager(object):
    ...
    def execute_step(
        ...
        if actions:
            ...
            tool_dependency, filtered_actions, dir = step_handler.execute_step(
    ...
    def prepare_step(
        ...
        if action_elem is not None:
            step_handler = self.get_step_handler_by_type(action_type)
            action_dict = step_handler.prepare_step(
                tool_dependency=tool_dependency,
...
class TagManager(object):
    ...
    def process_tag_set(
        ...
        tool_dependency, proceed_with_install, action_elem_tuples = tag_handler.process_tag_set(
            tool_shed_repository,
            tool_dependency,
            package_elem,
            package_name,
    ...
    def load_tag_handlers(self):
        tag_handlers = dict(
            environment_variable=tag_handler.SetEnvironment(self.app),
