...
class Download(object):
    def url_download(
        ...
        if "sha256sum" in checksums or "#sha256#" in download_url:
            downloaded_checksum = (
                hashlib.sha256(open(file_path, "rb").read()).hexdigest().lower()
            )
            ...
            if downloaded_checksum != expected:
                raise Exception(
                    "Given sha256 checksum does not match with the one from the downloaded file (%s != %s)."
                    % (downloaded_checksum, expected)
                )

        if "md5sum" in checksums or "#md5#" in download_url or "#md5=" in download_url:
            downloaded_checksum = (
                hashlib.md5(open(file_path, "rb").read()).hexdigest().lower()
            )
            ...
            if downloaded_checksum != expected:
                raise Exception(
                    "Given md5 checksum does not match with the one from the downloaded file (%s != %s)."
                    % (downloaded_checksum, expected)
                )

        if extract:
            if tarfile.is_tarfile(file_path) or (
                zipfile.is_zipfile(file_path) and not file_path.endswith(".jar")
            ):
...
class AssertDirectoryExecutable(RecipeStep):
    ...
    def execute_step(
        ...
        if not self.assert_directory_executable(full_path=full_path):
            ...
            error_message = (
                "The path %s is not a directory or is not executable by the owner."
                % str(full_path)
            )
...
class AssertDirectoryExists(RecipeStep):
    ...
    def execute_step(
        ...
        if not self.assert_directory_exists(full_path=full_path):
            status = self.app.install_model.ToolDependency.installation_status.ERROR
            error_message = "The path %s is not a directory or does not exist." % str(
                full_path
            )
            tool_dependency = tool_dependency_util.set_tool_dependency_attributes(
                self.app, tool_dependency, status=status, error_message=error_message
...
class AssertFileExecutable(RecipeStep):
    ...
    def execute_step(
        ...
        if not self.assert_file_executable(full_path=full_path):
            status = self.app.install_model.ToolDependency.installation_status.ERROR
            error_message = (
                "The path %s is not a file or is not executable by the owner."
                % str(full_path)
            )
            tool_dependency = tool_dependency_util.set_tool_dependency_attributes(
                self.app, tool_dependency, status=status, error_message=error_message
...
class Autoconf(RecipeStep):
    ...
    def execute_step(
        ...
        with settings(warn_only=True):
            ...
            if "prefix=" in configure_opts:
                ...
                pre_cmd = (
                    "./configure --prefix=$INSTALL_DIR %s && make && make install"
                    % configure_opts
                )
            cmd = install_environment.build_command(
                basic_util.evaluate_template(pre_cmd, install_environment)
            )
            install_environment.handle_command(
    ...
    def prepare_step(
        ...
        if action_elem.text:
            configure_opts = basic_util.evaluate_template(
                action_elem.text, install_environment
            )
...
class ChangeDirectory(RecipeStep):
    ...
    def execute_step(
        ...
        if dir_valid(target_dir):
            ...
            target_dir = os.path.realpath(
                os.path.normpath(os.path.join(current_dir, action_dict["directory"]))
            )
            if dir_valid(target_dir):
                dir = target_dir
            else:
    ...
    def prepare_step(
        ...
        if action_elem.text:
            action_dict["directory"] = basic_util.evaluate_template(
                action_elem.text, install_environment
            )
        return action_dict
    ...
    def execute_step(
        ...
        for target_file, mode in action_dict["change_modes"]:
            if os.path.exists(target_file):
                ...
                log.debug(
                    "Invalid file %s specified, ignoring %s action.",
                    target_file,
                    self.type,
                )
        return tool_dependency, None, None
...
class DownloadBinary(Download, RecipeStep):
    ...
    def download_binary(self, url, work_dir, checksums):
        ...
        self.url_download(
            work_dir, downloaded_filename, url, extract=False, checksums=checksums
        )
    ...
    def execute_step(
        ...
        try:
            ...
            log.debug(
                "Attempting to download from %s to %s", url, str(target_directory)
            )
        ...
        if downloaded_filename and os.path.exists(
            os.path.join(work_dir, downloaded_filename)
        ):
            if target_directory:
                target_directory = os.path.realpath(
                    os.path.normpath(
                        os.path.join(install_environment.install_dir, target_directory)
                    )
                )
                # Make sure the target directory is not outside of $INSTALL_DIR.
                if target_directory.startswith(
                    os.path.realpath(install_environment.install_dir)
                ):
                    full_path_to_dir = os.path.abspath(
                        os.path.join(install_environment.install_dir, target_directory)
                    )
                else:
    ...
    def prepare_step(
        ...
        if len(url_template_elems) > 1:
            ...
            url_template_elem = tool_dependency_util.get_download_url_for_platform(
                url_template_elems, platform_info_dict
            )
        else:
            url_template_elem = url_template_elems[0]
        action_dict["url"] = Template(url_template_elem.text.strip()).safe_substitute(
            platform_info_dict
        )
...
class DownloadByUrl(Download, RecipeStep):
    ...
    def execute_step(
        ...
        dir = self.url_download(
            work_dir, downloaded_filename, url, extract=True, checksums=checksums
        )
        if is_binary:
            log_file = os.path.join(
                install_environment.install_dir, basic_util.INSTALLATION_LOG
            )
...
class DownloadFile(Download, RecipeStep):
    ...
    def execute_step(
        ...
        log.debug("Attempting to download via url: %s", url)
        self.url_download(
            work_dir, filename, url, extract=action_dict["extract"], checksums=checksums
        )
        if initial_download:
            dir = os.path.curdir
            return tool_dependency, filtered_actions, dir
...
class MakeInstall(RecipeStep):
    ...
    def prepare_step(
        ...
        # make; make install; allow providing make options
        if action_elem.text:
            make_opts = basic_util.evaluate_template(
                action_elem.text, install_environment
            )
...
class MoveDirectoryFiles(RecipeStep):
    ...
    def prepare_step(
        ...
        for move_elem in action_elem:
            move_elem_text = basic_util.evaluate_template(
                move_elem.text, install_environment
            )
            if move_elem_text:
...
class MoveFile(RecipeStep):
    ...
    def prepare_step(
        ...
        # </action>
        action_dict["source"] = basic_util.evaluate_template(
...
class RegexReplace(RecipeStep):
    ...
    def execute_step(
        ...
        if os.path.isabs(action_dict["filename"]):
            ...
            if not (
                filename.startswith(current_dir)
                or filename.startswith(install_environment.install_dir)
            ):
                return tool_dependency, None, None
        else:
            filename = os.path.abspath(
                os.path.join(current_dir, action_dict["filename"])
            )
    ...
    def prepare_step(
        ...
        """
        action_dict["filename"] = basic_util.evaluate_template(
            action_elem.get("filename"), install_environment
        )
        action_dict["regex"] = basic_util.evaluate_template(
            action_elem.find("regex").text, install_environment
        )
        action_dict["replacement"] = basic_util.evaluate_template(
            action_elem.find("replacement").text, install_environment
        )
        return action_dict
...
class SetEnvironment(RecipeStep):
    ...
    def execute_step(
        ...
        root_dir_dict = dict(
            action="set_to",
            name="%s_ROOT_DIR"
            % re.sub(r"[^A-Z0-9_]", "_", tool_dependency.name.upper()),
        ...
        if "$ENV[" in env_var_value and "]" in env_var_value:
            ...
            for env_cmd in set_prior_environment_commands:
                ...
                if env_cmd.startswith(inherited_env_var_name):
                    ...
                    log.info(
                        "Replacing %s with %s in env.sh for this repository.",
                        to_replace,
                        env_val,
                    )
            ...
            if not found:
                ...
                log.debug(
                    "Environment variable %s not found, removing from set_environment.",
                    inherited_env_var_name,
                )
                env_var_value = env_var_value.replace(
                    to_replace, "$%s" % inherited_env_var_name
                )
        ...
        for env_elem in action_elem:
            if env_elem.tag == "environment_variable":
                env_var_dict = env_manager.create_env_var_dict(
                    elem=env_elem, install_environment=install_environment
                )
                if env_var_dict:
...
class SetupPerlEnvironment(Download, RecipeStep):
    ...
    def execute_step(
        ...
        if env_shell_file_paths is None:
            log.debug(
                "Missing Perl environment, make sure your specified Perl installation exists."
            )
            if initial_download:
                return tool_dependency, filtered_actions, dir
            return tool_dependency, None, None
            ...
            install_environment.add_env_shell_file_paths(env_shell_file_paths)
        log.debug(
            "Handling setup_perl_environment for tool dependency %s with install_environment.env_shell_file_paths:\n%s"
                ...
                for perl_package_dict in perl_packages:
                    ...
                    if perl_package.find("://") != -1:
                        ...
                        if os.path.exists(os.path.join(tmp_work_dir, "Makefile.PL")):
                            ...
                            log.debug(
                                "No Makefile.PL or Build.PL file found in %s. Skipping installation of %s."
    ...
    def prepare_step(
        ...
        action_dict = env_manager.get_env_shell_file_paths_from_setup_environment_elem(
            all_env_shell_file_paths, action_elem, action_dict
        )
        perl_packages = []
        for env_elem in action_elem:
...
class SetupREnvironment(Download, RecipeStep):
    ...
    def execute_step(
        ...
        if env_shell_file_paths is None:
            ...
            install_environment.add_env_shell_file_paths(env_shell_file_paths)
        log.debug(
            "Handling setup_r_environment for tool dependency %s with install_environment.env_shell_file_paths:\n%s"
            % (str(tool_dependency.name), str(install_environment.env_shell_file_paths))
        )
        tarball_names = []
        for r_package_dict in action_dict["r_packages"]:
            ...
            self.url_download(
                work_dir, filename, url, extract=False, checksums=checksums
            )
        dir = os.path.curdir
        current_dir = os.path.abspath(os.path.join(work_dir, dir))
        with lcd(current_dir):
...
class SetupRubyEnvironment(Download, RecipeStep):
    ...
    def execute_step(
        ...
        if env_shell_file_paths is None:
            log.debug(
                "Missing Ruby environment, make sure your specified Ruby installation exists."
            )
            if initial_download:
                return tool_dependency, filtered_actions, dir
            return tool_dependency, None, None
            ...
            install_environment.add_env_shell_file_paths(env_shell_file_paths)
        log.debug(
            "Handling setup_ruby_environment for tool dependency %s with install_environment.env_shell_file_paths:\n%s"
            % (str(tool_dependency.name), str(install_environment.env_shell_file_paths))
        )
        dir = os.path.curdir
        current_dir = os.path.abspath(os.path.join(work_dir, dir))
            ...
            with settings(warn_only=True):
                ...
                for ruby_package_tup_dict in ruby_package_tups:
                    ...
                    if os.path.isfile(gem):
                        ...
                        self.url_download(
                            work_dir, gem_name, url, extract=False, checksums=checksums
                        )
                        cmd = """PATH=$PATH:$RUBY_HOME/bin; export PATH; GEM_HOME=$INSTALL_DIR; export GEM_HOME;
                                gem install --local %s %s""" % (
                            gem_name,
    ...
    def prepare_step(
        ...
        for env_elem in action_elem:
            if env_elem.tag == "package":
                ...
                ruby_package_tups.append(
                    dict(package=tup, checksums=self.get_elem_checksums(env_elem))
                )
...
class SetupPythonEnvironment(Download, RecipeStep):
    ...
    def execute_step(
        ...
        if env_shell_file_paths is None:
            log.debug(
                "Missing Python environment, make sure your specified Python installation exists."
            )
            if initial_download:
                return tool_dependency, filtered_actions, dir
            return tool_dependency, None, None
            ...
            install_environment.add_env_shell_file_paths(env_shell_file_paths)
        log.debug(
            "Handling setup_python_environment for tool dependency %s with install_environment.env_shell_file_paths:\n%s"
            % (str(tool_dependency.name), str(install_environment.env_shell_file_paths))
        )
        dir = os.path.curdir
        current_dir = os.path.abspath(os.path.join(work_dir, dir))
            ...
            with settings(warn_only=True):
                ...
                for python_package_tup_dict in python_package_tups:
                    ...
                    package_path = os.path.join(
                        install_environment.tool_shed_repository_install_dir, package
                    )
                ...
                env_file_builder.append_line(
                    ...
                    action="prepend_to",
                    value=os.path.join(
                        install_environment.install_dir, "lib", "python"
                    ),
                )
                env_file_builder.append_line(
                    name="PYTHONPATH",
...
class SetupVirtualEnv(Download, RecipeStep):
    ...
    def execute_step(
        ...
        venv_src_directory = os.path.abspath(
            os.path.join(self.app.config.tool_dependency_dir, "__virtualenv_src")
        )
        ...
        if os.path.exists(os.path.join(install_environment.install_dir, requirements)):
            ...
            requirements_path = os.path.join(
                install_environment.install_dir, "requirements.txt"
            )
        ...
        setup_command = "%s %s/virtualenv.py --no-site-packages '%s'" % (
            python_cmd,
            venv_src_directory,
            venv_directory,
        )
                ...
                while True:
                    ...
                    if line:
                        line_install_command = "python '%s' install %s --log '%s'" % (
                            ...
                            line,
                            os.path.join(
                                install_environment.install_dir,
                                "pip_install_%s.log" % (line),
                            ),
                        )
        ...
        if return_code:
            log.error(
                "Failed to do setup_virtualenv install, exit code='%s'", return_code
            )
            # would it be better to try to set env variables anway, instead of returning here?
            return tool_dependency, None, None
        site_packages_directory, site_packages_directory_list = self.__get_site_packages_directory(
            install_environment, self.app, tool_dependency, python_cmd, venv_directory
        )
        env_file_builder.append_line(
            name="PATH", action="prepend_to", value=os.path.join(venv_directory, "bin")
        )
        if site_packages_directory is None:
            ...
            env_file_builder.append_line(
                name="PYTHONPATH", action="prepend_to", value=site_packages_directory
            )
        ...
        if not os.path.exists(venv_dir):
            with install_environment.use_tmp_dir() as work_dir:
                ...
                try:
                    ...
                    log.error(
                        "Failed to download virtualenv: url_download( '%s', '%s', '%s' ) threw an exception",
                        work_dir,
                        downloaded_filename,
                        VIRTUALENV_URL,
                    )
    ...
    def prepare_step(
        ...
        action_dict["use_requirements_file"] = asbool(
            action_elem.get("use_requirements_file", True)
        )
    ...
    def __get_site_packages_directory(
        self, install_environment, app, tool_dependency, python_cmd, venv_directory
    ):
        ...
        # Use raw strings so that python won't automatically unescape the quotes before passing the command
        # to subprocess.Popen.
        for site_packages_command in [
            r"""%s -c 'import site; site.getsitepackages()[0]'"""
            ...
            output = install_environment.handle_command(
                tool_dependency=tool_dependency,
...
class ShellCommand(RecipeStep):
    ...
    def prepare_step(
        ...
        action_elem_text = basic_util.evaluate_template(
            action_elem.text, install_environment
        )
        if action_elem_text:
...
class TemplateCommand(RecipeStep):
    ...
    def prepare_step(
        ...
        if language == "cheetah":
            ...
            log.debug(
                "Unsupported template language '%s'. Not proceeding." % str(language)
            )
            raise Exception(
