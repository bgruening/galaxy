...
class EnvManager(object):
    ...
    def create_env_var_dict(self, elem, install_environment):
        ...
        if elem.text and elem.text.find("REPOSITORY_INSTALL_DIR") >= 0:
            if (
                tool_shed_repository_install_dir
                and elem.text.find("$REPOSITORY_INSTALL_DIR") != -1
            ):
                env_var_text = elem.text.replace(
                    "$REPOSITORY_INSTALL_DIR", tool_shed_repository_install_dir
                )
                return dict(
                    name=env_var_name, action=env_var_action, value=env_var_text
                )
            else:
                env_var_text = elem.text.replace(
                    "$REPOSITORY_INSTALL_DIR", tool_dependency_install_dir
                )
                return dict(
                    name=env_var_name, action=env_var_action, value=env_var_text
                )
        if elem.text and elem.text.find("INSTALL_DIR") >= 0:
            if tool_dependency_install_dir:
                env_var_text = elem.text.replace(
                    "$INSTALL_DIR", tool_dependency_install_dir
                )
                return dict(
                    name=env_var_name, action=env_var_action, value=env_var_text
                )
    ...
    def get_env_shell_file_paths(self, elem):
        ...
        if toolshed and repository_name and repository_owner and changeset_revision:
                ...
                for sub_elem in elem:
                    ...
                    if (
                        ...
                        for tool_dependency in repository.tool_dependencies:
                            if (
                                tool_dependency.type == tool_dependency_type
                                and tool_dependency.name == tool_dependency_name
                                and tool_dependency.version == tool_dependency_version
                            ):
                                break
                        if tool_dependency:
                            ...
                            if env_shell_file_path:
                                ...
                                error_message += (
                                    "type %s, name %s, version %s for repository %s"
                                    % (
                        ...
                        error_message = (
                            "Skipping invalid tool dependency definition: type %s, name %s, version %s."
                            % (
                                str(tool_dependency_type),
                                str(tool_dependency_name),
                                str(tool_dependency_version),
            ...
            error_message = (
                "Skipping invalid set_environment_for_install definition: toolshed %s, name %s, owner %s, changeset_revision %s."
                % (
                    str(toolshed),
                    str(repository_name),
                    str(repository_owner),
