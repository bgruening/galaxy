...
class DataManagerHandler(object):
    ...
    def data_manager_config_elems_to_xml_file(self, config_elems, config_filename):
        ...
        if data_managers_path:
            root_str = (
                '<?xml version="1.0"?><data_managers tool_path="%s"></data_managers>'
                % data_managers_path
            )
        else:
    ...
    def remove_from_data_manager(self, repository):
        ...
        if metadata_dict and "data_manager" in metadata_dict:
            ...
            if tree:
                root = tree.getroot()
                assert root.tag == "data_managers", (
                    "The file provided (%s) for removing data managers from is not a valid data manager xml file."
                    % (shed_data_manager_conf_filename)
                )
                guids = [
                    data_manager_dict.get("guid")
                    for data_manager_dict in metadata_dict.get("data_manager", {})
                    .get("data_managers", {})
                    .values()
                    ...
                    if elem.tag == "data_manager":
                        ...
                        if guid in guids:
                            ...
                            if tool_elem is not None:
                                installed_changeset_revision_elem = tool_elem.find(
