...
class InstallToolDependencyManager(object):
    ...
    def install_and_build_package(
        self, install_environment, tool_dependency, actions_dict
    ):
    ...
    def install_and_build_package_via_fabric(
        ...
        try:
            ...
            log.exception(
                "Error installing tool dependency %s version %s.",
    ...
    def install_specified_tool_dependencies(
        self,
        ...
        attr_tups_of_dependencies_for_install = [
            (td.name, td.version, td.type) for td in tool_dependencies
        ]
        installed_packages = []
        ...
        if tree is None:
            log.debug(
                "The received tool_dependencies.xml file is likely invalid: %s"
                % str(error_message)
            )
        ...
        for elem in root:
            if elem.tag == "set_environment":
                ...
                if version != "1.0":
                    raise Exception(
                        "The <set_environment> tag must have a version attribute with value 1.0"
                    )
                ...
                if index is not None:
                    ...
                    if tool_dependency.type == "package" and proceed_with_install:
                        try:
                            ...
                            error_message = (
                                "Error installing tool dependency %s version %s: %s"
                        ...
                        if tool_dependency and tool_dependency.status in [
                            ...
                            if self.app.config.manage_dependency_relationships:
                                ...
                                self.app.installed_repository_manager.handle_tool_dependency_install(
                                    tool_shed_repository, tool_dependency
                                )
        return installed_packages
    ...
    def install_via_fabric(
        ...
        **kwd
        ...
        if actions_elem is not None:
            ...
            elems = []
        step_manager = StepManager(self.app)
        tool_shed_repository_install_dir = self.get_tool_shed_repository_install_dir(
            tool_shed_repository
        )
        install_environment = InstallEnvironment(
            self.app, tool_shed_repository_install_dir, install_dir
        )
    ...
    def install_package(
        ...
        if tool_dependencies and package_name and package_version:
            ...
            for tool_dependency in tool_dependencies:
                if package_name == str(tool_dependency.name) and package_version == str(
                                        ...
                                        if actions_elem.tag == "actions":
                                            ...
                                            if not binary_installed:
                                                ...
                                                if os.path.exists(
                                                    ...
                                                    installation_directory_contents = os.listdir(
                                                        installation_directory
                                                    )
                                                    if installation_directory_contents:
                                                        removed, error_message = tool_dependency_util.remove_tool_dependency(
                                                            self.app, tool_dependency
                                                        )
                                                        if removed:
                                                            can_install_from_source = (
                                                                True
                                                            )
                                                        else:
                                                            log.debug(
...
class InstallRepositoryManager(object):
    ...
    def __get_install_info_from_tool_shed(
        self, tool_shed_url, name, owner, changeset_revision
    ):
        params = dict(
            name=str(name), owner=str(owner), changeset_revision=str(changeset_revision)
        )
    ...
    def __handle_repository_contents(
        ...
        """
        shed_config_dict = self.app.toolbox.get_shed_config_dict_by_filename(
            shed_tool_conf
        )
        ...
        if "tools" in irmm_metadata_dict:
            ...
            tool_util.copy_sample_files(
                self.app, tool_index_sample_files, tool_path=tool_path
            )
    ...
    def initiate_repository_installation(self, installation_dict):
        # The following installation_dict entries are all required.
        created_or_updated_tool_shed_repositories = installation_dict[
            "created_or_updated_tool_shed_repositories"
        ]
        ...
        includes_tools_for_display_in_tool_panel = installation_dict[
            "includes_tools_for_display_in_tool_panel"
        ]
        install_repository_dependencies = installation_dict[
            "install_repository_dependencies"
        ]
        install_resolver_dependencies = installation_dict[
        ...
        tool_panel_section_mapping = installation_dict.get(
            "tool_panel_section_mapping", {}
        )
        tool_path = installation_dict["tool_path"]
        tool_shed_url = installation_dict["tool_shed_url"]
        # Handle contained tools.
        if includes_tools_for_display_in_tool_panel and (
            new_tool_panel_section_label or tool_panel_section_id
        ):
        ...
        if includes_tools_for_display_in_tool_panel and (
            tool_panel_section_mapping is not None
        ):
            for tool_guid in tool_panel_section_mapping:
                if tool_panel_section_mapping[tool_guid]["action"] == "create":
                    new_tool_panel_section_name = tool_panel_section_mapping[tool_guid][
                        "tool_panel_section"
                    ]
                    log.debug(
                        'Creating tool panel section "%s" for tool %s'
                        % (new_tool_panel_section_name, tool_guid)
                    )
                    self.tpm.handle_tool_panel_section(
                        self.app.toolbox,
                        None,
                        tool_panel_section_mapping[tool_guid]["tool_panel_section"],
                    )
        encoded_repository_ids = [
            self.app.security.encode_id(tsr.id)
        ...
        for tsr_id in tsr_ids:
            tsr = self.install_model.context.query(
                self.install_model.ToolShedRepository
            ).get(tsr_id)
            tool_shed_repositories.append(tsr)
        clause_list = []
        for tsr_id in tsr_ids:
            clause_list.append(
                self.install_model.ToolShedRepository.table.c.id == tsr_id
            )
        query = self.install_model.context.query(
            self.install_model.ToolShedRepository
    ...
    def __initiate_and_install_repositories(
        ...
        install_tool_dependencies = install_options.get(
        ...
        tool_panel_section_mapping = install_options.get(
            "tool_panel_section_mapping", {}
        )
        ...
        created_or_updated_tool_shed_repositories, tool_panel_section_keys, repo_info_dicts, filtered_repo_info_dicts = self.handle_tool_shed_repositories(
            installation_dict
        )
    ...
    def install_repositories(self, tsr_ids, decoded_kwd, reinstalling):
        ...
        # required order.
        ordered_tsr_ids, ordered_repo_info_dicts, ordered_tool_panel_section_keys = self.order_components_for_installation(
            tsr_ids, repo_info_dicts, tool_panel_section_keys=tool_panel_section_keys
        )
        for tsr_id in ordered_tsr_ids:
            repository = self.install_model.context.query(
                self.install_model.ToolShedRepository
    ...
    def install_tool_shed_repository(
        ...
        relative_install_dir = os.path.join(
            relative_clone_dir, tool_shed_repository.name
        )
        ...
        if cloned_ok:
            ...
            if "tools" in metadata and install_resolver_dependencies:
                ...
                if self.app.config.use_cached_dependency_manager:
                    [
                        self.app.toolbox.dependency_manager.build_cache(r)
    ...
    def order_components_for_installation(
        self, tsr_ids, repo_info_dicts, tool_panel_section_keys
    ):
        """
            ...
            if tsr_id not in ordered_tsr_ids:
                ...
                for prior_install_required_id in prior_install_required_ids:
                    if prior_install_required_id not in ordered_tsr_ids:
                        ...
                        prior_repo_info_dict, prior_tool_panel_section_key = self.get_repository_components_for_installation(
                        ...
                        ordered_tool_panel_section_keys.append(
    ...
    def update_tool_shed_repository_status(
        self, tool_shed_repository, status, error_message=None
    ):
        """
    ...
    def __assert_can_install_dependencies(self):
        if self.app.config.tool_dependency_dir is None:
            no_tool_dependency_dir_message = (
                "Tool dependencies can be automatically installed only if you set "
            )
            no_tool_dependency_dir_message += (
                "the value of your 'tool_dependency_dir' setting in your Galaxy "
            )
            no_tool_dependency_dir_message += (
...
class RepositoriesInstalledException(exceptions.RequestParameterInvalidException):
    def __init__(self):
        super(RepositoriesInstalledException, self).__init__(
