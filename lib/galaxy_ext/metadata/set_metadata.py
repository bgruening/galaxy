...
assert sys.version_info[:2] >= (2, 7), (
    "Python version must be at least 2.7, this is: %s" % sys.version
)
...
def set_metadata_portable():
    ...
    galaxy.model.metadata.MetadataTempFile.tmp_dir = (
    ...
    try:
        ...
        raise Exception(
            "Failed to find metadata/params.json from cwd [%s]"
            % tool_job_working_directory
        )
    ...
    for output_name, output_dict in outputs.items():
        ...
        filename_results_code = os.path.join(
            "metadata/metadata_results_%s" % output_name
        )
        ...
        try:
            ...
            files_path = os.path.abspath(
                os.path.join(tool_job_working_directory, extra_files_dir_name)
            )
            dataset.dataset.external_extra_files_path = files_path
            file_dict = tool_provided_metadata.get_dataset_meta(
                output_name, dataset.dataset.id
            )
            if "ext" in file_dict:
            ...
            json.dump(
                (False, str(e)), open(filename_results_code, "wt+")
