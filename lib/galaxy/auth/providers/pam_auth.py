...
class PAM(AuthProvider):
    ...
    def authenticate(self, email, username, password, options):
        ...
        if not options["redact_username_in_logs"]:
            log.debug(
                "use username: {} use email {} email {} username {}".format(
                    ...
                    options.get("login-use-email", False),
                    email,
                    username,
                )
            )
        ...
        authenticated = False
        if use_helper:
            authentication_helper = options.get(
                "authentication-helper-script", "/bin/false"
            ).strip()
            log.debug(
                "PAM auth: external helper script: {}".format(authentication_helper)
            )
            if not authentication_helper.startswith("/"):
                ...
                if status != 0 and error != "":
                    log.debug(
                        "PAM auth: external authentication script had errors: status {} error {}".format(
                            status, error
                        )
                    )
        ...
        if authenticated:
            log.debug(
                "PAM authentication successful for {}".format(
                    "redacted" if options["redact_username_in_logs"] else pam_username
                )
            )
            return True, auto_register_email, auto_register_username
        else:
            log.debug(
                "PAM authentication failed for {}".format(
                    "redacted" if options["redact_username_in_logs"] else pam_username
                )
            )
            return False, "", ""
