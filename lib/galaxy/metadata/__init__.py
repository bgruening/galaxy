...
class MetadataCollectionStrategy(object):
    ...
    @abc.abstractmethod
    def external_metadata_set_successfully(
        self, dataset, name, sa_session, working_directory
    ):
    ...
    def _load_metadata_from_path(
        ...
        def path_rewriter(path):
            if not path:
                return path
            normalized_remote_metadata_directory = (
                remote_metadata_directory
                and os.path.normpath(remote_metadata_directory)
            )
            normalized_path = os.path.normpath(path)
            if remote_metadata_directory and normalized_path.startswith(
                normalized_remote_metadata_directory
            ):
                return normalized_path.replace(
                    normalized_remote_metadata_directory, working_directory, 1
                )
            return path

        dataset.metadata.from_JSON_dict(
            metadata_output_path, path_rewriter=path_rewriter
        )
    ...
    def _metadata_results_from_file(self, dataset, filename_results_code):
        ...
        if not rval:
            log.debug(
                "setting metadata externally failed for %s %s: %s"
                % (dataset.__class__.__name__, dataset.id, rstring)
            )
        return rval
...
class PortableDirectoryMetadataGenerator(MetadataCollectionStrategy):
    ...
    def setup_external_metadata(
        ...
        assert (
            job_metadata
        ), "setup_external_metadata must be supplied with job_metadata path"
        kwds = kwds or {}
        ...
        for name, dataset in datasets_dict.items():
            ...
            outputs[name] = {
                "filename_override": _get_filename_override(
                    output_fnames, dataset.file_name
                )
            }
    ...
    def load_metadata(
        ...
        metadata_output_path = os.path.join(
            working_directory, "metadata", "metadata_out_%s" % name
        )
        self._load_metadata_from_path(
            dataset, metadata_output_path, working_directory, remote_metadata_directory
        )

    def external_metadata_set_successfully(
        self, dataset, name, sa_session, working_directory
    ):
        metadata_results_path = os.path.join(
            working_directory, "metadata", "metadata_results_%s" % name
        )
        try:
...
class JobExternalOutputMetadataWrapper(MetadataCollectionStrategy):
    ...
    def _get_output_filenames_by_dataset(self, dataset, sa_session):
        if isinstance(dataset, galaxy.model.HistoryDatasetAssociation):
            return (
                sa_session.query(galaxy.model.JobExternalOutputMetadata)
                .filter_by(
                    job_id=self.job_id,
    ...
    def setup_external_metadata(
                ...
                def _metadata_path(what):
                    return abspath(
                        tempfile.NamedTemporaryFile(
                            dir=tmp_dir, prefix="metadata_%s_%s_" % (what, key)
                        ).name
                    )

                filename_in, filename_out, filename_results_code, filename_kwds, filename_override_metadata = _initialize_metadata_inputs(
                    dataset, _metadata_path, tmp_dir, kwds
                )
        ...
        args = '"%s" "%s" %s %s' % (
            metadata_path_on_compute(datatypes_config),
            job_metadata,
        ...
        if include_command:
            ...
            with os.fdopen(fd, "w") as f:
                f.write(SET_METADATA_SCRIPT)
            return 'python "%s" %s' % (
                metadata_path_on_compute(metadata_script_file),
                args,
            )
        else:
            # return args to galaxy_ext.metadata.set_metadata required to build
    ...
    def external_metadata_set_successfully(
        ...
        return self._metadata_results_from_file(
            dataset, metadata_files.filename_results_code
        )

    def cleanup_external_metadata(self, sa_session):
        log.debug("Cleaning up external metadata files")
        for metadata_files in (
            sa_session.query(galaxy.model.Job).get(self.job_id).external_output_metadata
        ):
            ...
            for key, fname in [
                ...
                try:
                    ...
                    log.debug(
                        "Failed to cleanup external metadata file (%s) for %s: %s"
                        % (key, dataset_key, e)
                    )
    ...
    def load_metadata(
        ...
        output_filename = self._get_output_filenames_by_dataset(
            dataset, sa_session
        ).filename_out
        self._load_metadata_from_path(
            dataset, output_filename, working_directory, remote_metadata_directory
        )
...
def _initialize_metadata_inputs(dataset, path_for_part, tmp_dir, kwds):
    ...
    # create the file on disk, so it cannot be reused by tempfile (unlikely, but possible)
    json.dump(
        (False, "External set_meta() not called"), open(filename_results_code, "wt+")
    )
    ...
    for meta_key, spec_value in dataset.metadata.spec.items():
        if (
            isinstance(spec_value.param, FileParameter)
            and dataset.metadata.get(meta_key, None) is not None
        ):
            metadata_temp = MetadataTempFile()
            metadata_temp.tmp_dir = tmp_dir
            shutil.copy(
                dataset.metadata.get(meta_key, None).file_name, metadata_temp.file_name
            )
...
def _assert_datatypes_config(datatypes_config):
    if datatypes_config is None:
        raise Exception(
            "In setup_external_metadata, the received datatypes_config is None."
        )
