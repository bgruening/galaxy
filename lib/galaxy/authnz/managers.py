...
class AuthnzManager(object):
        ...
        try:
            ...
            for child in root:
                if child.tag != "Setter":
                    ...
                    continue
                if (
                    "Property" not in child.attrib
                    or "Value" not in child.attrib
                    or "Type" not in child.attrib
                ):
                    log.error(
                        "Could not find the node attributes `Property` and/or `Value` and/or `Type`;"
                        " found these attributes: `{}`; skipping this node.".format(
                            child.attrib
                        )
                    )
                    continue
                try:
                    func = getattr(
                        importlib.import_module("__builtin__"), child.get("Type")
                    )
                except AttributeError:
                    log.error(
                        "The value of attribute `Type`, `{}`, is not a valid built-in type;"
                        " skipping this node"
            ...
            raise ParseError(
                "Invalid configuration at `{}`: {} -- unable to continue.".format(
                    config_file, e
                )
            )
        ...
        try:
            ...
            for child in root:
                ...
                if "name" not in child.attrib:
                    log.error(
                        "Could not find a node attribute 'name'; skipping the node '{}'.".format(
                            child.tag
                        )
                    )
                    continue
    ...
    def _parse_custos_config(self, config_xml):
        ...
        if config_xml.find("well_known_oidc_config_uri") is not None:
            rtv["well_known_oidc_config_uri"] = config_xml.find(
    ...
    def _get_authnz_backend(self, provider):
        ...
        if unified_provider_name in self.oidc_backends_config:
            ...
            identity_provider_class = self._get_identity_provider_class(
                self.oidc_backends_implementation[provider]
            )
            try:
                return (
                    True,
                ...
                log.exception(
                    "An error occurred when loading {}".format(
                        identity_provider_class.__name__
                    )
                )
                return False, str(e), None
        else:
            msg = "The requested identity provider, `{}`, is not a recognized/expected provider.".format(
                provider
            )
            log.debug(msg)
        ...
        if cloudauthz.provider == "aws":
            ...
            try:
                ...
                msg = (
                    ...
                    "account.".format(
                        self._unify_provider_name(cloudauthz.authn.provider),
                        cloudauthz.authn.uid,
                    )
                )
                log.debug(
                    ...
                    "Error Message: `{}`".format(
                        user_id, cloudauthz.id, msg, e.response.text
                    )
                )
    ...
    def can_user_assume_authn(trans, authn_id):
        ...
        if qres.user_id != trans.user.id:
            msg = (
                "The request authentication with ID `{}` is not accessible to user with ID "
                "`{}`.".format(
                    trans.security.encode_id(authn_id),
    ...
    def authenticate(self, provider, trans):
        ...
        try:
            ...
            return (
                True,
                "Redirecting to the `{}` identity provider for authentication".format(
                    provider
                ),
                backend.authenticate(trans),
            )
        except Exception:
            msg = "An error occurred when authenticating a user on `{}` identity provider".format(
                provider
            )
            log.exception(msg)
            return False, msg, None
    ...
    def callback(self, provider, state_token, authz_code, trans, login_redirect_url):
        try:
            ...
            return (
                True,
                message,
                backend.callback(state_token, authz_code, trans, login_redirect_url),
            )
        except Exception:
            msg = "An error occurred when handling callback from `{}` identity provider".format(
                provider
            )
            log.exception(msg)
    ...
    def disconnect(self, provider, trans, disconnect_redirect_url=None):
        try:
            ...
            msg = "An error occurred when disconnecting authentication with `{}` identity provider for user `{}`".format(
                provider, trans.user.username
            )
            log.exception(msg)
            return False, msg, None

    def get_cloud_access_credentials(
        self, cloudauthz, sa_session, user_id, request=None
    ):
        """
        ...
        """
        config = self._extend_cloudauthz_config(
            cloudauthz, request, sa_session, user_id
        )
        try:
            ca = CloudAuthz()
            log.info(
    ...
    def get_cloud_access_credentials_in_file(
        self, new_file_path, cloudauthz, sa_session, user_id, request=None
    ):
        """
        This method leverages CloudAuthz (https://github.com/galaxyproject/cloudauthz)
        to request a cloud-based resource provider (e.g., Amazon AWS, Microsoft Azure)
        ...
        credentials = self.get_cloud_access_credentials(
            cloudauthz, sa_session, user_id, request
        )
        log.info(
