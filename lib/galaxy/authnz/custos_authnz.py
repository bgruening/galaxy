...
class CustosAuthnz(IdentityProvider):
    ...
    def disconnect(self, provider, trans, disconnect_redirect_url=None):
        try:
            ...
            provider_tokens = [
                token
                for token in user.custos_auth
                if token.provider == self.config["provider"]
            ]
            if len(provider_tokens) == 0:
                raise Exception(
                    "User is not associated with provider {}".format(
                        self.config["provider"]
                    )
                )
            if len(provider_tokens) > 1:
                raise Exception(
                    "User is associated more than once with provider {}".format(
                        self.config["provider"]
                    )
                )
            ...
            return (
                False,
                "Failed to disconnect provider {}: {}".format(provider, str(e)),
                None,
            )
    ...
    def _create_oauth2_session(self, state=None, scope=None):
        ...
        if (
            ...
            log.warning(
                "Setting OAUTHLIB_INSECURE_TRANSPORT to '1' to allow plain HTTP (non-SSL) callback"
            )
    ...
    def _get_userinfo(self, oauth2_session):
        ...
        return oauth2_session.get(
            userinfo_endpoint, verify=self._get_verify_param()
        ).json()

    def _get_custos_authnz_token(self, sa_session, user_id, provider):
        return (
            sa_session.query(CustosAuthnzToken)
            .filter_by(external_user_id=user_id, provider=provider)
            .one_or_none()
        )
    ...
    def _load_config_for_provider_and_realm(self, provider, realm):
        self.config[
            "well_known_oidc_config_uri"
        ] = self._get_well_known_uri_for_provider_and_realm(provider, realm)
        well_known_oidc_config = self._load_well_known_oidc_config(
            self.config["well_known_oidc_config_uri"]
        )
        self.config["authorization_endpoint"] = well_known_oidc_config[
    ...
    def _get_well_known_uri_for_provider_and_realm(self, provider, realm):
        ...
        if provider == "custos":
            return "{}/realms/{}/.well-known/openid-configuration".format(
    ...
    def _load_well_known_oidc_config(self, well_known_uri):
        try:
            return requests.get(well_known_uri, verify=self._get_verify_param()).json()
        except Exception:
            log.error(
                "Failed to load well-known OIDC config URI: {}".format(well_known_uri)
            )
            raise
