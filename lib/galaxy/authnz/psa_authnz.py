...
class PSAAuthnz(IdentityProvider):
    ...
    def _setup_idp(self, oidc_backend_config):
        ...
        if oidc_backend_config.get("prompt") is not None:
            self.config[setting_name("AUTH_EXTRA_ARGUMENTS")][
                "prompt"
            ] = oidc_backend_config.get("prompt")
    ...
    def callback(self, state_token, authz_code, trans, login_redirect_url):
        ...
        strategy.session_set(
            BACKENDS_NAME[self.config["provider"]] + "_state", state_token
        )
        backend = self._load_backend(strategy, self.config["redirect_uri"])
        redirect_url = do_complete(
            backend,
            login=lambda backend, user, social_user: self._login_user(
    ...
    def disconnect(
        self, provider, trans, disconnect_redirect_url=None, association_id=None
    ):
        on_the_fly_config(trans.sa_session)
...
def disconnect(
    ...
    **kwargs
    ...
    """
    user_authnz = (
        strategy.trans.sa_session.query(user_storage)
        .filter(
            user_storage.table.c.user_id == user.id,
            user_storage.table.c.provider == name,
        )
        .first()
    )
    if user_authnz is None:
        return {
            "success": False,
            "message": "Not authenticated by any identity providers.",
        }
    # option A
