...
def get_dynamic_post_processing_actions( config_post_actions ):
    """
        Extract informations about all post processing actions from the galaxy.ini config file.
    """
    actions = list()
    for action_name in config_post_actions:
        if ":" in action_name:
            ...
            module = __import__( 'galaxy.jobs.actions.dynamic', globals(), fromlist=['temp_module'] )
        ...
        if split:
            ...
            log.error( 'No description specified in the __doc__ string for %s.' % action_name )
        if len(split) > 1:
            description = split[1].strip()
        else:
            description = ''

        actions.append( dict( actionpath=action_name, short_desc=sdesc, desc=description, cl=class_object ) )
    return actions
...
class DynamicAction(DefaultJobAction):
    ...
    def execute(cls, app, sa_session, action, job, replacement_dict):
        for dataset_assoc in job.output_datasets:
            if action.output_name == '' or dataset_assoc.name == action.output_name:
                # Extract the action id from the select form id.
                # Colons are encoded as underscrores and underscores are not allowed in class names
                action_identifier = [action.action_arguments['process'].replace('-', ':')]
                action = get_dynamic_post_processing_actions( action_identifier )
                # Get the action class
                action_class = action[0]['cl']
                dynamic_action = action_class(dataset_assoc=dataset_assoc, app=app, job=job)
                dynamic_action.run()

    @classmethod
    def get_config_form(cls, trans):
        dynamic_actions = get_dynamic_post_processing_actions(trans.app.config.dynamic_post_processing_actions)
    ...
    @classmethod
    def get_short_str(cls, pja):
        return "Apply dynamic post processing action on '%s'." % pja.output_name
...
class ActionBox(object):
    ...
    public_actions = ['RenameDatasetAction', 'ChangeDatatypeAction',
                      'ColumnSetAction', 'EmailAction', 'DynamicAction',
