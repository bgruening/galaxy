...
DETECTED_JOB_STATE = Bunch(
    OK="ok", OUT_OF_MEMORY_ERROR="oom_error", GENERIC_ERROR="generic_error"
)
...
def check_output_regex(
    job_id_tag, regex, stream, stream_name, job_messages, max_error_level
):
    """
...
def check_output(
    stdio_regexes, stdio_exit_codes, stdout, stderr, tool_exit_code, job_id_tag
):
    """
    ...
    try:
        ...
        if len(stdio_regexes) > 0 or len(stdio_exit_codes) > 0:
            ...
            if tool_exit_code is not None:
                for stdio_exit_code in stdio_exit_codes:
                    if (
                        tool_exit_code >= stdio_exit_code.range_start
                        ...
                        job_messages.append(reason)
                        max_error_level = max(
                            max_error_level, stdio_exit_code.error_level
                        )
        ...
        if state != DETECTED_JOB_STATE.OK:
            ...
            log.debug(
                "job failed, detected state %s, standard error is - [%s]"
                % (state, peak)
            )
    except Exception:
        log.exception(
            "Job state check encountered unexpected exception; assuming execution successful"
        )
