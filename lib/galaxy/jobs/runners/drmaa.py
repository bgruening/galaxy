...
class DRMAAJobRunner(AsynchronousJobRunner):
    ...
    def __init__(self, app, nworkers, **kwargs):
        ...
        runner_param_specs = {
            "drmaa_library_path": dict(
                map=str, default=os.environ.get("DRMAA_LIBRARY_PATH", None)
            )
        }
        for retry_exception in RETRY_EXCEPTIONS_LOWER:
            runner_param_specs[retry_exception + "_state"] = dict(
                map=str,
                valid=lambda x: x in (model.Job.states.OK, model.Job.states.ERROR),
                default=model.Job.states.OK,
            )
            runner_param_specs[retry_exception + "_retries"] = dict(
    ...
    def queue_job(self, job_wrapper):
        ...
        script = self.get_job_file(
            job_wrapper, exit_code_path=ajs.exit_code_file, shell=job_wrapper.shell
        )
        try:
        ...
        if job_wrapper.get_state() == model.Job.states.DELETED:
            log.debug(
                "(%s) Job deleted by user before it entered the queue" % galaxy_id_tag
            )
        ...
        if external_runjob_script is None:
            ...
            if pwent is None:
                if not allow_guests:
                    fail_msg = (
                        "User %s is not mapped to any real user, and not permitted to start jobs."
                        % job_wrapper.user
                    )
                    job_wrapper.fail(fail_msg)
                    return
            ...
            log.debug(
                "(%s) submitting with credentials: %s [uid: %s]"
                % (galaxy_id_tag, pwent[0], pwent[2])
            )
            filename = self.store_jobtemplate(job_wrapper, jt)
            self.userid = pwent[2]
            external_job_id = self.external_runjob(
                external_runjob_script, filename, pwent[2]
            )
        ...
        if drmaa_state == drmaa.JobState.FAILED:
            ...
            if external_metadata:
                self._handle_metadata_externally(
                    ajs.job_wrapper, resolve_requirements=True
                )
            if ajs.job_wrapper.get_state() != model.Job.states.DELETED:
    ...
    def check_watched_item(self, ajs, new_watched):
        ...
        state = None
        try:
            assert external_job_id not in (None, "None"), "(%s/%s) Invalid job id" % (
                galaxy_id_tag,
                external_job_id,
            )
            ...
            if self.runner_params[state_param] == model.Job.states.OK:
                log.warning(
                    "(%s/%s) job will now be finished OK",
                    galaxy_id_tag,
                    external_job_id,
                )
                self.work_queue.put((self.finish_job, ajs))
                ...
                log.warning(
                    "(%s/%s) job will now be errored", galaxy_id_tag, external_job_id
                )
                self.work_queue.put((self.fail_job, ajs))
            else:
                raise Exception(
            ...
            log.warning(
                "(%s/%s) unable to communicate with DRM: %s",
                galaxy_id_tag,
                external_job_id,
                e,
            )
            ...
            log.exception(
                "(%s/%s) unable to check job status: %s"
    ...
    def stop_job(self, job_wrapper):
        ...
        try:
            ...
            log.info(
                "(%s/%s) Removed from DRM queue at user's request" % (job.id, ext_id)
            )
    ...
    def store_jobtemplate(self, job_wrapper, jt):
        ...
        with open(filename, "w+") as fp:
            json.dump(jt, fp)
        log.debug(
            "(%s) Job script for external submission is: %s"
    ...
    def external_runjob(self, external_runjob_script, jobtemplate_filename, username):
        ...
        if exitcode != 0:
            ...
            log.exception(
                "External_runjob failed (exit code %s). Child process reported error: %s"
