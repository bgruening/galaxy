...
def eval_condition(condition, job_state):
    runner_state = (
        getattr(job_state, "runner_state", None) or JobState.runner_states.UNKNOWN_ERROR
    )
    ...
    condition_locals = {
        ...
        "memory_limit_reached": runner_state
        == JobState.runner_states.MEMORY_LIMIT_REACHED,
        "tool_detected_failure": runner_state
...
def _handle_resubmit_definitions(resubmit_definitions, app, job_runner, job_state):
    ...
    for resubmit in resubmit_definitions:
        ...
        new_destination = job_state.job_wrapper.job_runner_mapper.cache_job_destination(
            new_destination
        )
        # Reset job state
        ...
        if resubmit.get("handler", None):
            log.debug(
                "%s Job reassigned to handler %s", job_log_prefix, resubmit["handler"]
            )
        ...
        info = (
            "This job was resubmitted to the queue because %s on its "
...
class _ExpressionContext(object):
    ...
    def safe_eval(self, condition):
        ...
        if self._lazy_context is None:
            runner_state = (
                getattr(self._job_state, "runner_state", None)
                or JobState.runner_states.UNKNOWN_ERROR
            )
            attempt = 1
            now = datetime.utcnow()
            last_running_state = None
            ...
            if last_queued_state:
                seconds_since_queued = (
                    now - last_queued_state.create_time
                ).total_seconds()

            self._lazy_context = {
                "walltime_reached": runner_state
                == JobState.runner_states.WALLTIME_REACHED,
                "memory_limit_reached": runner_state
                == JobState.runner_states.MEMORY_LIMIT_REACHED,
                "unknown_error": JobState.runner_states.UNKNOWN_ERROR,
