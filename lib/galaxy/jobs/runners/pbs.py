...
CLUSTER_ERROR_MESSAGE = (
    "Job cannot be completed due to a cluster error, please retry it later: %s"
)
...
class PBSJobRunner(AsynchronousJobRunner):
    ...
    def __init__(self, app, nworkers):
        ...
        if (
            app.config.pbs_application_server
            and app.config.outputs_to_working_directory
        ):
            raise Exception(
    ...
    def url_to_destination(self, url):
        ...
        log.debug(
            "Converted URL '%s' to destination runner=pbs, params=%s" % (url, params)
        )
    ...
    def parse_destination_params(self, params):
        ...
        for arg, value in params.items():
            try:
                ...
                log.warning(
                    "Unrecognized long argument in destination params: %s" % arg
                )
    ...
    def __args_to_attrs(self, args):
        ...
        for arg, value in args.items():
            if arg == "l":
                ...
                for j, (res, val) in enumerate(
                    [a.split("=", 1) for a in resource_attrs]
                ):
    ...
    def queue_job(self, job_wrapper):
        ...
        if not self.prepare_job(
            job_wrapper, include_metadata=not (self.app.config.pbs_stage_path)
        ):
            return
        ...
        pbs_options = []
        if (
            "-q" in job_destination.params
            and "destination" not in job_destination.params
        ):
        ...
        job_destination.params["destination"] = "%s@%s" % (
            pbs_queue_name or "",
            pbs_server_name,
        )
        ...
        if c <= 0:
            ...
            log.error(
                "Connection to PBS server for submit failed: %s: %s" % (errno, text)
            )
            return
        ...
        if self.app.config.pbs_application_server:
            ...
            stagein = self.get_stage_in_out(
                job_wrapper.get_input_fnames() + output_files, symlink=True
            )
        ...
        attrs.append(
            dict(
                ...
                value=str(
                    "%s_%s_%s"
                    % (job_wrapper.job_id, job_wrapper.tool.id, job_wrapper.user)
                ),
            )
        )
        ...
        while tries < 5:
            ...
            errno, text = pbs.error()
            log.warning(
                "(%s) pbs_submit failed (try %d/5), PBS error %d: %s"
    ...
    def check_watched_items(self):
        ...
        for pbs_job_state in self.watched:
            ...
            try:
                ...
                try:
                    ...
                    if errno == 15001:
                        ...
                        log.debug(
                            "(%s/%s) PBS job has left queue" % (galaxy_job_id, job_id)
                        )
            ...
            if status.job_state != old_state:
                log.debug(
                    "(%s/%s) PBS job state changed from %s to %s"
            ...
            if status.job_state == "R" and status.get("resources_used", False):
                ...
                try:
                    ...
                    log.debug(
                        "(%s/%s) PBS job has completed successfully"
                        % (galaxy_job_id, job_id)
                    )
    ...
    def check_single_job(self, pbs_server_name, job_id):
        ...
        if c <= 0:
            log.debug(
                "connection to PBS server %s for state check failed" % pbs_server_name
            )
            return None
    ...
    def fail_job(self, pbs_job_state):
        ...
        if pbs_job_state.job_wrapper.cleanup_job == "always":
            self.cleanup(
                (
                    pbs_job_state.output_file,
                    pbs_job_state.error_file,
                    pbs_job_state.exit_code_file,
                    pbs_job_state.job_file,
                )
            )
        ...
        for fname in fnames:
            if os.access(fname, os.R_OK):
                ...
                if symlink and self.app.config.pbs_stage_path:
                    stage_name = os.path.join(
                        self.app.config.pbs_stage_path, os.path.split(fname)[1]
                    )
                else:
                    stage_name = fname
                stage += "%s@%s:%s" % (
                    stage_name,
                    self.app.config.pbs_dataset_server,
                    fname,
                )
        return stage
    ...
    def stop_job(self, job_wrapper):
        ...
        try:
            ...
            if c <= 0:
                log.debug(
                    "(%s) Connection to PBS server for job delete failed" % job_tag
                )
                return
