...
SLURM_TOP_WARNING_RES = (SLURM_CGROUP_RE,)

# These messages are returned to the user
OUT_OF_MEMORY_MSG = (
    "This job was terminated because it used more memory than it was allocated."
)
PROBABLY_OUT_OF_MEMORY_MSG = (
    "This job was cancelled probably because it used more memory than it was allocated."
)
...
class SlurmJobRunner(DRMAAJobRunner):
    ...
    def _complete_terminal_job(self, ajs, drmaa_state, **kwargs):
        def _get_slurm_state_with_sacct(job_id, cluster):
            ...
            if p.returncode != 0:
                ...
                if stderr == "SLURM accounting storage is disabled":
                    log.warning(
                        "SLURM accounting storage is not properly configured, unable to run sacct"
                    )
                    return
                raise Exception(
            ...
            if drmaa_state == self.drmaa_job_states.FAILED:
                ...
                if slurm_state == "NOT_FOUND":
                    ...
                    ajs.job_wrapper.change_state(
                        ...
                        info="Job was resubmitted due to node failure",
                    )
                    try:
            ...
            if drmaa_state == self.drmaa_job_states.DONE:
                ...
                with open(ajs.error_file, "r+") as f:
                    ...
                    for line in lines:
                        ...
                        if any(
                            _ in stripped_line
                            for _ in SLURM_MEMORY_LIMIT_EXCEEDED_PARTIAL_WARNINGS
                        ):
                            log.debug(
                                '(%s/%s) Job completed, removing SLURM exceeded memory warning: "%s"',
                                ajs.job_wrapper.get_id_tag(),
                                ajs.job_id,
                                stripped_line,
                            )
        ...
        return super(SlurmJobRunner, self)._complete_terminal_job(
...
def _remove_spurious_top_lines(rfh, ajs, maxlines=3):
    ...
    if bad:
        ...
        for line in bad:
            log.debug(
                '(%s/%s) Job completed, removing SLURM spurious warning: "%s"',
                ajs.job_wrapper.get_id_tag(),
                ajs.job_id,
                line,
            )
