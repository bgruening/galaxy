...
class ChronosJobRunner(AsynchronousJobRunner):
    ...
    RUNNER_PARAM_SPEC = {
        "chronos": {"map": str},
        "insecure": {"map": lambda x: x in ["true", "True", "TRUE"], "default": True},
        "username": {"map": str},
        "password": {"map": str},
        "owner": {"map": str},
    }
    ...
    DESTINATION_PARAMS_SPEC = {
        "docker_cpu": {"default": 0.1, "map_name": "cpus", "map": float},
        "docker_memory": {"default": 128, "map_name": "mem", "map": int},
        "docker_disk": {"default": 256, "map_name": "disk", "map": int},
        "volumes": {
            "default": None,
        ...
        "max_retries": {"default": 2, "map_name": "retries", "map": int},
    }
    ...
    def queue_job(self, job_wrapper):
        ...
        if not self.prepare_job(
            job_wrapper, include_metadata=False, modify_command_for_container=False
        ):
    ...
    def recover(self, job, job_wrapper):
        msg = (
            "(name!r/runner!r) is still in {state!s} state, adding to"
        ...
        if job.state == model.Job.states.RUNNING:
            LOGGER.debug(
                msg.format(
                    name=job.id, runner=job.job_runner_external_id, state="running"
                )
            )
            ...
            LOGGER.debug(
                msg.format(
                    name=job.id, runner=job.job_runner_external_id, state="queued"
                )
            )
