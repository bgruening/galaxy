...
class KubernetesJobRunner(AsynchronousJobRunner):
    ...
    def __init__(self, app, nworkers, **kwargs):
        ...
        runner_param_specs = dict(
            ...
            k8s_timeout_seconds_job_deletion=dict(
                map=int, valid=lambda x: int > 0, default=30
            ),
        ...
        if (
            "k8s_use_service_account" in self.runner_params
            and self.runner_params["k8s_use_service_account"]
        ):
    ...
    def setup_volumes(self):
        volume_claims = dict(
            volume.split(":")
            for volume in self.runner_params["k8s_persistent_volume_claims"].split(",")
        )
        mountable_volumes = [
            {"name": claim_name, "persistentVolumeClaim": {"claimName": claim_name}}
            for claim_name in volume_claims
        ]
        self.runner_params["k8s_mountable_volumes"] = mountable_volumes
        volume_mounts = [
            {"name": claim_name, "mountPath": mount_path}
            for claim_name, mount_path in volume_claims.items()
        ]
    ...
    def queue_job(self, job_wrapper):
        ...
        if job.exists() and not self._galaxy_instance_id:
            ...
            while job.exists():
                ...
                if (
                    elapsed_seconds
                    > self.runner_params["k8s_timeout_seconds_job_deletion"]
                ):
                    log.debug(
    ...
    def __get_pull_policy(self):
        if "k8s_pull_policy" in self.runner_params:
            if self.runner_params["k8s_pull_policy"] in [
                "Always",
                "IfNotPresent",
                "Never",
            ]:
    ...
    def __get_supplemental_group(self):
        if "k8s_supplemental_group_id" in self.runner_params:
            try:
                ...
                log.warning(
                    "Supplemental group passed for Kubernetes runner needs to be an integer, value "
    ...
    def __get_fs_group(self):
        if "k8s_fs_group_id" in self.runner_params:
            try:
                ...
                log.warning(
                    "FS group passed for Kubernetes runner needs to be an integer, value "
    ...
    def __get_galaxy_instance_id(self):
        ...
        if "k8s_galaxy_instance_id" in self.runner_params:
            if re.match(
                r"(?!-)[a-z\d-]{1,20}(?<!-)$",
                self.runner_params["k8s_galaxy_instance_id"],
            ):
    ...
    def __get_k8s_job_spec_template(self, ajs):
        ...
        k8s_spec_template = {
            "metadata": {
                "labels": {
                    "app": self.__produce_unique_k8s_job_name(
        ...
        if self._supplemental_group and self._supplemental_group > 0:
            k8s_spec_template["spec"]["securityContext"] = dict(
                supplementalGroups=[self._supplemental_group]
            )
                    ...
                    envs.append(
                        {
                            ...
                            "value": str(
                                ByteSize(limits["memory"]).to_unit("M", as_string=False)
                            ),
                        }
                    )
                if "cpu" in limits:
                    envs.append(
                        {
                            "name": "GALAXY_SLOTS",
                            "value": str(int(math.ceil(float(limits["cpu"])))),
                        }
                    )
    ...
    def __get_memory_request(self, job_wrapper):
        ...
        if "requests_memory" in job_destinantion.params:
            return self.__transform_memory_value(
                job_destinantion.params["requests_memory"]
            )
        return None
    ...
    def __get_memory_limit(self, job_wrapper):
        ...
        if "limits_memory" in job_destinantion.params:
            return self.__transform_memory_value(
                job_destinantion.params["limits_memory"]
            )
        return None
    ...
    def check_watched_item(self, job_state):
        ...
        if len(jobs.response["items"]) == 1:
            ...
            log.error(
                "No Jobs are available under expected selector app=%s", job_state.job_id
            )
    ...
    def _handle_job_failure(self, job, job_state, reason=None):
        with open(job_state.error_file, "a") as error_file:
            if reason == "OOM":
                error_file.write(
                    "Job killed after running out of memory. Try with more memory.\n"
                )
                ...
                job_state.fail_message = (
                    "More pods failed than allowed. See stdout for pods details."
                )
        job_state.running = False
        self.mark_as_failed(job_state)
    ...
    def __job_failed_due_to_low_memory(self, job_state):
        ...
        pods = Pod.objects(self._pykube_api).filter(
            selector="app=%s" % job_state.job_id
        )
        pod = Pod(self._pykube_api, pods.response["items"][0])

        if (
            pod.obj["status"]["phase"] == "Failed"
            and pod.obj["status"]["containerStatuses"][0]["state"]["terminated"][
                "reason"
            ]
            == "OOMKilled"
        ):
    ...
    def stop_job(self, job_wrapper):
        ...
        try:
            ...
            log.debug(
                "(%s/%s) Terminated at user's request"
