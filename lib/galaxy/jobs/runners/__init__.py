...
from galaxy.jobs.runners.util.job_script import job_script, write_script
...
JOB_RUNNER_PARAMETER_UNKNOWN_MESSAGE = (
    "Invalid job runner parameter for this plugin: %s"
)
JOB_RUNNER_PARAMETER_MAP_PROBLEM_MESSAGE = (
    "Job runner parameter '%s' value '%s' could not be converted to the correct type"
)
JOB_RUNNER_PARAMETER_VALIDATION_FAILED_MESSAGE = (
    "Job runner parameter %s failed validation"
)
...
class BaseJobRunner(object):
    DEFAULT_SPECS = dict(
        recheck_missing_job_retries=dict(
    ...
    def _init_worker_threads(self):
        ...
        for i in range(self.nworkers):
            worker = threading.Thread(
                name="%s.work_thread-%d" % (self.runner_name, i), target=self.run_next
            )
            worker.daemon = True
    ...
    def shutdown(self):
        ...
        """
        log.info(
            "%s: Sending stop signal to %s job worker threads",
            self.runner_name,
            len(self.work_threads),
        )
        ...
        if join_timeout > 0:
            log.info(
                "Waiting up to %d seconds for job worker threads to shutdown...",
                join_timeout,
            )
            start = time.time()
            ...
            for thread in self._alive_worker_threads(cycle=True):
                ...
                try:
                    ...
                    log.exception(
                        "Caught exception attempting to shutdown job worker thread %s:",
                        thread.name,
                    )
    ...
    def prepare_job(
        ...
        if job_state == model.Job.states.DELETED:
            log.debug(
                "(%s) Job deleted by user before it entered the %s queue"
                % (job_id, self.runner_name)
            )
            ...
            log.info(
                "(%s) Job is in state %s, skipping execution" % (job_id, job_state)
            )
    ...
    def build_command_line(
        ...
        if not container and job_wrapper.requires_containerization:
            raise Exception(
                "Failed to find a container when required, contact Galaxy admin."
            )
    ...
    def get_work_dir_outputs(
        self, job_wrapper, job_working_directory=None, tool_working_directory=None
    ):
        """
            ...
            if joda and job_tool:
                ...
                if hda_tool_output and hda_tool_output.from_work_dir:
                    ...
                    source_file = os.path.join(
                        tool_working_directory, hda_tool_output.from_work_dir
                    )
    ...
    def _handle_metadata_externally(self, job_wrapper, resolve_requirements=False):
        ...
        if (
            job_wrapper.get_state()
            not in [model.Job.states.ERROR, model.Job.states.DELETED]
            and job_wrapper.output_paths
        ):
            ...
            external_metadata_script = "%s %s %s" % (
                lib_adjust,
                venv,
            ...
            if resolve_requirements:
                ...
                if dependency_shell_commands:
                    ...
                    external_metadata_script = "%s&&%s" % (
                        dependency_shell_commands,
                        external_metadata_script,
                    )
            log.debug(
                "executing external set_meta script for job %d: %s"
            ...
            log.debug(
                "execution of external set_meta for job %d finished"
                % job_wrapper.job_id
            )
    ...
    def fail_job(self, job_state, exception=False):
        ...
        if not job_state.runner_state_handled:
            job_state.job_wrapper.fail(
                getattr(job_state, "fail_message", "Job failed"), exception=exception
            )
    ...
    def _finish_or_resubmit_job(
        self, job_state, job_stdout, job_stderr, job_id=None, external_job_id=None
    ):
        ...
        try:
            ...
            tool_stdout_path = os.path.join(
                job_wrapper.working_directory, "tool_stdout"
            )
            tool_stderr_path = os.path.join(
                job_wrapper.working_directory, "tool_stderr"
            )
            ...
            if job_not_ok:
                ...
                if (
                    ...
                    == DETECTED_JOB_STATE.OUT_OF_MEMORY_ERROR
                ):
            ...
            log.exception(
                "(%s/%s) Job wrapper finish method failed"
                % (job_id or "", external_job_id or "")
            )
...
class JobState(object):
    ...
    def __init__(self, job_wrapper, job_destination):
        ...
        if self.job_wrapper:
            self.redact_email_in_job_name = (
                self.job_wrapper.app.config.redact_email_in_job_name
            )
    ...
    def set_defaults(self, files_dir):
        if self.job_wrapper is not None:
            ...
            self.job_name = "".join(
                x if x in (string.ascii_letters + string.digits + "_") else "_"
                for x in job_name
            )
    ...
    def read_exit_code(self):
        ...
        try:
            ...
            log.warning(
                "(%s) Exit code '%s' invalid. Using 0." % (galaxy_id_tag, exit_code_str)
            )
            exit_code = 0
            ...
            if self.running and (self.check_count % 20 == 0):
                if runtime is None:
                    runtime = datetime.datetime.now() - (
                        self.start_time or datetime.datetime.now()
                    )
                self.check_count = 0
...
class AsynchronousJobRunner(BaseJobRunner, Monitors):
    ...
    def _init_monitor_thread(self):
        ...
        super(AsynchronousJobRunner, self)._init_monitor_thread(
            name=name, target=self.monitor, start=True, config=self.app.config
        )
    ...
    def finish_job(self, job_state):
        ...
        while which_try < self.app.config.retry_job_output_collection + 1:
            try:
                with open(job_state.output_file, "rb") as stdout_file, open(
                    job_state.error_file, "rb"
                ) as stderr_file:
                ...
                if which_try == self.app.config.retry_job_output_collection:
                    stdout = ""
                    stderr = (
                        job_state.runner_states.JOB_OUTPUT_NOT_RETURNED_FROM_CLUSTER
                    )
                    log.error(
                        "(%s/%s) %s: %s"
