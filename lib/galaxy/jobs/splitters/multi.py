...
def do_split(job_wrapper):
    ...
    if len(illegal_inputs) > 0:
        raise Exception(
            "Inputs have conflicting parallelism attributes: %s" % str(illegal_inputs)
        )
    ...
    for input in parent_job.input_datasets:
        if input.dataset is None:
            ...
            if input.name in split_inputs:
                type_to_input_map.setdefault(input.dataset.datatype, []).append(
                    input.name
                )
                ...
                log_error = (
                    "The input '%s' does not define a method for implementing parallelism"
                    % str(input.name)
                )
    ...
    if len(type_to_input_map) > 1:
        log_error = (
            "The multi splitter does not support splitting inputs of more than one type"
        )
        log.error(log_error)
        raise Exception(log_error)
    ...
    for input in parent_job.input_datasets:
        if input.name in split_inputs:
            ...
            if len(this_input_files) > 1:
                log_error = (
                    "The input '%s' is composed of multiple files - splitting is not allowed"
                    % str(input.name)
                )
                log.error(log_error)
                raise Exception(log_error)
    ...
    try:
        input_type.split(
            input_datasets, get_new_working_directory_name, parallel_settings
        )
    ...
    tasks = []
    prepare_files = (
        os.path.join(util.galaxy_directory(), "extract_dataset_parts.sh") + " %s"
    )
    for dir in task_dirs:
    ...
    try:
        ...
        for index, output in enumerate(outputs):
            output_file_name = str(
                output_paths[index]
            )  # Use false_path if set, else real path.
            ...
            if output in merge_outputs:
                ...
                if output_files:
                    ...
                    output_type.merge(
                        output_files, output_file_name, **extra_merge_args
                    )
                ...
                log_error = (
                    "The output '%s' does not define a method for implementing parallelism"
                    % output
                )
                log.exception(log_error)
                raise Exception(log_error)
