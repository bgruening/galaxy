...
def set_basic_defaults(job_wrapper):
    ...
    parallelism.attributes["merge_outputs"] = next(
        iter(job_wrapper.get_output_hdas_and_fnames())
    )


def do_split(job_wrapper):
    if (
        len(job_wrapper.get_input_fnames()) > 1
        or len(job_wrapper.get_output_fnames()) > 1
    ):
        log.error(
            "The basic splitter is not capable of handling jobs with multiple inputs or outputs."
        )
        raise Exception(
            "Job Splitting Failed, the basic splitter only handles tools with one input and one output"
        )
