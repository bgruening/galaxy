...
class TransferManager(object):
    ...
    def __init__(self, app):
        ...
        self.command = [
            "python",
            os.path.abspath(os.path.join(os.getcwd(), "scripts", "transfer.py")),
        ]
    ...
    def new(self, path=None, **kwd):
        ...
        transfer_job = self.app.model.TransferJob(
            state=self.app.model.TransferJob.states.NEW, params=kwd
        )
    ...
    def get_state(self, transfer_jobs, via_socket=False):
                ...
                try:
                    ...
                    if not valid:
                        ...
                        raise Exception(
                            dict(
                                code=128,
                                message="Did not receive valid response from transfer daemon for state",
                            )
                        )
                    ...
                    rval.append(
                        dict(transfer_job_id=tj.id, state=tj.state, error=error)
                    )
            else:
                self.sa_session.refresh(tj)
        ...
        for tj_state in rval:
            if tj_state["state"] in self.app.model.TransferJob.terminal_states:
                log.debug(
                    "Transfer job %s is in terminal state: %s"
                    % (tj_state["transfer_job_id"], tj_state["state"])
                )
            elif (
                tj_state["state"] == self.app.model.TransferJob.states.PROGRESS
                and "percent" in tj_state
            ):
                log.debug(
                    "Transfer job %s is %s%% complete"
    ...
    def __restarter(self):
        ...
        while self.running:
            ...
            for tj in self.sa_session.query(self.app.model.TransferJob).filter(
                self.app.model.TransferJob.state
                == self.app.model.TransferJob.states.RUNNING
            ):
                if not tj.pid:
                    continue
                ...
                try:
                    ...
                    if tj.state == tj.states.RUNNING:
                        log.error(
                            "Transfer job %s is marked as running but pid %s appears to be dead."
                            % (tj.id, tj.pid)
                        )
                        dead.append(tj)
            if dead:
