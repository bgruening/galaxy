...
FORM_TYPES = dict(
    [
        (f_type.lower(), f_descript)
        for f_type, f_descript in FormDefinition.types.items()
    ]
)
...
class FormDefinitionFactory(object):
    ...
    def new(
        ...
        """
        assert form_type in self.form_types, (
            "Invalid FormDefinition type ( %s not in %s )"
    ...
    def from_elem(self, elem, form_definition_current=None):
        ...
        if layouts_elem:
            for layout_elem in layouts_elem.findall("grid"):
                ...
                assert (
                    layout_name and layout_name not in layout
                ), "Layout grid element requires a unique name."
        ...
        if fields_elem is not None:
            for field_elem in fields_elem.findall("field"):
                ...
                assert field_type in self.field_type_factories, (
                    "Invalid form field type ( %s )." % field_type
                )
                fields.append(
                    self.field_type_factories[field_type].from_elem(field_elem, layout)
                )
...
class FormDefinitionFieldFactory(object):
    ...
    def from_elem(self, elem, layout=None):
        ...
        if field_layout:
            assert (
                layout and field_layout in layout
            ), "Invalid layout specified: %s not in %s" % (field_layout, layout)
            field_layout = str(
...
class FormDefinitionTextFieldFactory(FormDefinitionFieldFactory):
    ...
    def from_elem(self, elem, layout=None):
        ...
        """
        rval = super(FormDefinitionTextFieldFactory, self).from_elem(
            elem, layout=layout
        )
        rval["type"] = self.__get_stored_field_type(
            string_as_bool(elem.get("area", "false"))
        )
        return rval
...
class FormDefinitionPasswordFieldFactory(FormDefinitionFieldFactory):
    ...
    def from_elem(self, elem, layout=None):
        ...
        Return FormDefinition field created from an xml element.
        """
        rval = super(FormDefinitionPasswordFieldFactory, self).from_elem(
            elem, layout=layout
        )
        rval["type"] = self.__get_stored_field_type()
        return rval
...
class FormDefinitionAddressFieldFactory(FormDefinitionFieldFactory):
    ...
    def from_elem(self, elem, layout=None):
        ...
        Return FormDefinition field created from an xml element.
        """
        rval = super(FormDefinitionAddressFieldFactory, self).from_elem(
            elem, layout=layout
        )
        rval["type"] = self.__get_stored_field_type()
        return rval
...
class FormDefinitionWorkflowFieldFactory(FormDefinitionFieldFactory):
    ...
    def from_elem(self, elem, layout=None):
        ...
        Return FormDefinition field created from an xml element.
        """
        rval = super(FormDefinitionWorkflowFieldFactory, self).from_elem(
            elem, layout=layout
        )
        rval["type"] = self.__get_stored_field_type()
        return rval
...
class FormDefinitionWorkflowMappingFieldFactory(FormDefinitionFieldFactory):
    ...
    def from_elem(self, elem, layout=None):
        ...
        Return FormDefinition field created from an xml element.
        """
        rval = super(FormDefinitionWorkflowMappingFieldFactory, self).from_elem(
            elem, layout=layout
        )
        rval["type"] = self.__get_stored_field_type()
        return rval
...
class FormDefinitionHistoryFieldFactory(FormDefinitionFieldFactory):
    ...
    def from_elem(self, elem, layout=None):
        ...
        Return FormDefinition field created from an xml element.
        """
        rval = super(FormDefinitionHistoryFieldFactory, self).from_elem(
            elem, layout=layout
        )
        rval["type"] = self.__get_stored_field_type()
        return rval
...
class FormDefinitionSelectFieldFactory(FormDefinitionFieldFactory):
    ...
    def from_elem(self, elem, layout=None):
        ...
        Return FormDefinition field created from an xml element.
        """
        rval = super(FormDefinitionSelectFieldFactory, self).from_elem(
            elem, layout=layout
        )
        rval["type"] = self.__get_stored_field_type(
            string_as_bool(elem.get("checkboxes", "false"))
        )
        # load select options
        rval["selectlist"] = []
