...
try:
    ...
    docker = Bunch(
        types=Bunch(
            ContainerSpec=None, RestartPolicy=None, Resources=None, Placement=None
        )
    )
...
class DockerSwarmInterface(DockerInterface):
    ...
    def services_clean(self):
        ...
        if completed_services:
            cleaned_service_ids.extend(
                self.service_rm([x.id for x in completed_services])
            )
        ...
        if terminal_services:
            cleaned_service_ids.extend(
                self.service_rm([x.id for x in terminal_services])
            )
        return filter(
            lambda x: x.id in cleaned_service_ids,
            completed_services + terminal_services,
        )
...
class DockerSwarmCLIInterface(DockerSwarmInterface, DockerCLIInterface):
    ...
    def nodes(self, id=None, name=None):
        for node_dict in self.node_ls(id=id, name=name):
            ...
            if self._node_prefix and not node_name.startswith(self._node_prefix):
                continue
            task_list = filter(
                lambda x: x["NAME"].startswith(self._name_prefix), self.node_ps(node_id)
            )
    ...
    def service_create(self, command, image=None, **kwopts):
        if (
            "service_create_image_constraint" in self._conf
...
class DockerSwarmAPIInterface(DockerSwarmInterface, DockerAPIInterface):
    ...
    resources_option_map = {
        "cpus": {
            "params": ("cpu_limit", "cpu_reservation"),
            "map": lambda x: int(x * 1000000000),
        },
    ...
    def create_random_port_spec(port):
        return {"Protocol": "tcp", "PublishedPort": None, "TargetPort": port}
    ...
    def service_create(self, command, image=None, **kwopts):
        ...
        log.debug(
            "Creating docker service with image '%s' for command: %s", image, command
        )
        ...
        # create specs
        service_mode = self._create_docker_api_spec(
            "service_mode", docker.types.ServiceMode, kwopts
        )
        endpoint_spec = self._create_docker_api_spec(
            "endpoint_spec", docker.types.EndpointSpec, kwopts
        )
        task_template = self._create_docker_api_spec(
            "task_template", docker.types.TaskTemplate, kwopts
        )
        self.set_kwopts_name(kwopts)
        log.debug(
            "Docker service task template:\n%s", safe_dumps_formatted(task_template)
        )
        log.debug(
            "Docker service endpoint specification:\n%s",
            safe_dumps_formatted(endpoint_spec),
        )
        log.debug("Docker service mode:\n%s", safe_dumps_formatted(service_mode))
        log.debug(
            "Docker service creation parameters:\n%s", safe_dumps_formatted(kwopts)
        )
