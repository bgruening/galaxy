...
class DockerInterface(ContainerInterface):
    ...
    def _default_image(self):
        assert (
            self._conf.image is not None
        ), "No default image for this docker interface"
...
class DockerCLIInterface(DockerInterface):
    ...
    def validate_config(self):
        log.warning(
            "The `docker_cli` interface is deprecated and will be removed in Galaxy 18.09, please use `docker`"
        )
    ...
    def ps(self, id=None, name=None):
        return self._run_docker(
            subcommand="ps", args=self._filter_by_id_or_name(id, name)
        )
    ...
    def inspect(self, container_id):
        try:
            ...
            if (
                exc.stdout == "[]"
                and exc.stderr
                == "Error: no such object: {container_id}".format(
    ...
    def image_inspect(self, image):
        try:
            ...
            if (
                exc.stdout == "[]"
                and exc.stderr == "Error: no such image: {image}".format(image=image)
            ):
...
class DockerAPIClient(object):
    ...
    def _init_client():
        ...
        DockerAPIClient._client = docker.APIClient(
            *DockerAPIClient._client_args, **kwargs
        )
        log.info(
            "Initialized Docker API client for server: %s",
            kwargs.get("base_url", "localhost"),
        )
        ...
        for tries in range(1, max_tries + 1):
            ...
            try:
                ...
                if exc is not None:
                    ...
                    if reinit:
                        log.warning(
                            "Reinitializing Docker API client due to connection-oriented failure"
                        )
                    ...
                    if success_test is not None:
                        log.info(
                            "Testing if %s() succeeded despite the exception", qualname
                        )
                        r = success_test()
                    if r:
                        log.warning(
                            "The request appears to have succeeded, will not retry. Response is: %s",
                            str(r),
                        )
                        return r
                    elif tries >= max_tries:
                        log.error("Maximum number of attempts (%s) exceeded", max_tries)
                        if "response" in exc and DockerAPIClient._nonfatal_error(
...
class DockerAPIInterface(DockerInterface):
    ...
    def validate_config(self):
        assert (
            docker is not None
        ), "Docker module could not be imported, DockerAPIInterface unavailable"
    ...
    def _create_docker_api_spec(self, option_map_name, spec_class, kwopts):
        ...
        def _kwopt_to_arg(map_spec, key, value, param=None):
            ...
            if isinstance(map_spec.get("param"), int):
                ...
                spec_kwopts.update(
                    DockerAPIInterface._kwopt_to_params(map_spec, key, value)
                )
        ...
        for kwopt in filter(
            lambda k: not k.startswith("_") and k in option_map, kwopts.keys()
        ):
        ...
        for _sub_k in filter(
            lambda k: k.startswith("_") and "spec_class" in option_map[k],
            option_map.keys(),
        ):
            ...
            if (
                _sub_v is not None
                or map_spec.get("required")
                or isinstance(map_spec.get("param"), int)
            ):
    ...
    def _create_host_config(self, kwopts):
        ...
        return self._create_docker_api_spec(
            "host_config", self._client.create_host_config, kwopts
        )
    ...
    def ps(self, id=None, name=None, running=True):
        return self._client.containers(
            all=not running, filters=self._filter_by_id_or_name(id, name)
        )
    ...
    def run(self, command, image=None, **kwopts):
        ...
        log.debug(
            "Creating docker container with image '%s' for command: %s", image, command
        )
    ...
    def inspect(self, container_id):
        try:
            ...
            raise ContainerNotFound(
                "Invalid container id: %s" % container_id, container_id=container_id
            )
    ...
    def image_inspect(self, image):
        try:
            ...
            raise ContainerImageNotFound(
                "%s not pulled, cannot get digest" % image, image=image
            )
