...
class ApplicationStackMessage(dict):
    ...
    def _merge_class_tuples(self):
        ...
        for name in names:
            setattr(
                self.__class__,
                name.lstrip("_"),
                property(lambda self, name=name: self._get_list_from_mro(name)),
            )
    ...
    def bind_default_handler(self, obj, name):
        ...
        assert self.default_handler is not None, (
            "%s has no default handler method, cannot bind" % self.__class__.__name__
        )
...
class TaskMessage(ParamMessage):
    ...
    def default_handler(self, msg):
        ...
        assert name in dir(
            self
        ), "{cls} has no method _handle_{task}_msg, cannot handle message: {msg}".format(
