...
from galaxy.util import asbool, safe_makedirs, safe_str_cmp, smart_str, unicodify
...
class WebApplication(base.WebApplication):
    ...
    def __init__(self, galaxy_app, session_cookie="galaxysession", name=None):
        ...
        self.set_transaction_factory(
            lambda e: self.transaction_chooser(e, galaxy_app, session_cookie)
        )
        # Mako support
        ...
        for fname in os.listdir(controller_dir):
            if not (fname.startswith("_")) and fname.endswith(".py"):
                ...
                for key in dir(module):
                    ...
                    if (
                        inspect.isclass(T)
                        and T is not BaseUIController
                        and issubclass(T, BaseUIController)
                    ):
            ...
            if not (fname.startswith("_")) and fname.endswith(".py"):
                ...
                for key in dir(module):
                    ...
                    if (
                        ...
                        and issubclass(T, BaseAPIController)
                    ):
...
class GalaxyWebTransaction(
    base.DefaultWebTransaction,
    context.ProvidesAppContext,
    context.ProvidesUserContext,
        ...
        if self.galaxy_session:
            ...
            if config.session_duration:
                ...
                if self.galaxy_session.last_action:
                    expiration_time = (
                        self.galaxy_session.last_action
                        + datetime.timedelta(minutes=config.session_duration)
                    )
                else:
                    expiration_time = now
                    self.galaxy_session.last_action = now - datetime.timedelta(
                        seconds=1
                    )
    ...
    def setup_i18n(self):
        ...
        if "HTTP_ACCEPT_LANGUAGE" in self.environ:
            ...
            for locale in client_locales:
                try:
                    locales.append(
                        Locale.parse(locale.split(";")[0].strip(), sep="-").language
                    )
    ...
    def set_cors_headers(self):
        ...
        def is_allowed_origin(origin):
            ...
            for allowed_origin in self.app.config.allowed_origin_hostnames:
                if allowed_origin == "*" or matches_allowed_origin(
    ...
    def set_cookie(self, value, name="galaxysession", path="/", age=90, version="1"):
        ...
        self.response.cookies[name]["expires"] = time.strftime(
        ...
        try:
            ...
            log.warning(
                "Error setting httponly attribute in cookie '%s': %s" % (name, e)
            )
    ...
    def _authenticate_api(self, session_cookie):
        ...
        """
        api_key = self.request.params.get("key", None) or self.request.headers.get(
            "x-api-key", None
        )
        ...
        if api_key_supplied and self._check_master_api_key(api_key):
            ...
            try:
                provided_key = (
                    self.sa_session.query(self.app.model.APIKeys)
                    .filter(self.app.model.APIKeys.table.c.key == api_key)
                    .one()
                )
    ...
    def _ensure_valid_session(self, session_cookie, create=True):
        ...
        if secure_id:
            ...
            if session_key:
                ...
                galaxy_session = (
                    self.sa_session.query(self.app.model.GalaxySession)
                    .filter(
                        and_(
                            self.app.model.GalaxySession.table.c.session_key
                            == session_key,
        ...
        if self.app.config.use_remote_user:
            remote_user_email = self.environ.get(
                self.app.config.remote_user_header, None
            )
            if galaxy_session:
                if remote_user_email and galaxy_session.user is None:
                    # No user, associate
                    galaxy_session.user = self.get_or_create_remote_user(
                        remote_user_email
                    )
            ...
            if (
                galaxy_session and galaxy_session.user is None
            ) and user_for_new_session is None:
                raise Exception(
                    "Remote Authentication Failure - user is unknown and/or not supplied."
                )
        else:
            if (
                galaxy_session is not None
                and galaxy_session.user
                and galaxy_session.user.external
            ):
                ...
                galaxy_session is not None
                and galaxy_session.user is not None
                and galaxy_session.user.deleted
            ):
                invalidate_existing_session = True
                log.warning(
                    "User '%s' is marked deleted, invalidating session"
                    % galaxy_session.user.email
                )
    ...
    def _ensure_logged_in_user(self, environ, session_cookie):
        ...
        if session_cookie == "galaxysession" and self.galaxy_session.user is None:
            ...
            if (
                self.app.datatypes_registry.get_display_sites("ucsc")
                and self.request.path == display_as
            ):
                try:
            ...
            if self.request.path.startswith(external_display_path):
                ...
                try:
                    if (
                        self.app.datatypes_registry.display_applications.get(
                            request_path_split[-5]
                        )
                        and request_path_split[-4]
                        in self.app.datatypes_registry.display_applications.get(
                            request_path_split[-5]
                        ).links
            ...
            if self.request.path not in allowed_paths:
                login_url = url_for(
                    controller="root", action="login", redirect=self.request.path
                )
    ...
    def get_or_create_remote_user(self, remote_user_email):
        ...
        if user:
            ...
            for char in [
                x
                for x in username
                if x not in string.ascii_lowercase + string.digits + "-" + "."
            ]:
            ...
            if (
                ...
                while (
                    self.sa_session.query(self.app.model.User)
                    .filter_by(username=(username + "-" + str(i)))
                    .first()
                ):
                    i += 1
                username += "-" + str(i)
    ...
    def __update_session_cookie(self, name="galaxysession"):
        ...
        """
        self.set_cookie(
            self.security.encode_guid(self.galaxy_session.session_key),
            name=name,
    ...
    def check_user_library_import_dir(self, user):
        if getattr(self.app.config, "user_library_import_dir_auto_creation", False):
            ...
            try:
                safe_makedirs(
                    os.path.join(self.app.config.user_library_import_dir, user.email)
                )
    ...
    def _associate_user_history(self, user, prev_galaxy_session=None):
        ...
        if (
            ...
            and (
                prev_galaxy_session.current_history.user is None
    ...
    def handle_user_logout(self, logout_all=False):
        ...
        if logout_all and galaxy_user_id is not None:
            for other_galaxy_session in self.sa_session.query(
    ...
    def show_error_message(
        self, message, refresh_frames=[], use_panels=False, active_view=""
    ):
        """
    ...
    def fill_template_string(self, template_string, context=None, **kwargs):
        ...
        """
        template = Template(
            source=template_string, searchList=[context or kwargs, dict(caller=self)]
        )
        return str(template)
...
def build_url_map(app, global_conf, local_conf):
    ...
    from galaxy.web.framework.middleware.static import (
        CacheableStaticURLParser as Static,
    )
