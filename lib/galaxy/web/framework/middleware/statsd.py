...
class StatsdMiddleware(object):
    ...
    def __init__(
        self, application, statsd_host, statsd_port, statsd_prefix, statsd_influxdb
    ):
        if not statsd:
            raise ImportError(
                "Statsd middleware configured, but no statsd python module found. "
        ...
        self.statsd_client = statsd.StatsClient(
            statsd_host, statsd_port, prefix=statsd_prefix
        )
    ...
    def __call__(self, environ, start_response):
        ...
        page = environ.get("controller_action_key", None) or environ.get(
            "PATH_INFO", "NOPATH"
        ).strip("/").replace("/", ".")
