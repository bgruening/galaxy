    ...
    def __call__(self, environ, start_response):
        ...
        if self.display_servers and "REMOTE_ADDR" in environ:
            ...
            if host in self.display_servers:
                environ[self.remote_user_header] = "remote_display_server@%s" % (
        ...
        if self.remote_user_header in environ:
            ...
            if self.maildomain and "@" not in environ[self.remote_user_header]:
                environ[self.remote_user_header] = "%s@%s" % (
                    environ[self.remote_user_header],
                    self.maildomain,
                )
        ...
        if environ.get(self.remote_user_header, None):
            ...
            if not path_info.startswith("/user"):
                ...
                [path_info.startswith(prefix) for prefix in user_accessible_paths]
            ):
            ...
            log.debug(
                "Unable to identify user.  %s not found" % self.remote_user_header
            )
