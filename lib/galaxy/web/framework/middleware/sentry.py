...
RAVEN_IMPORT_MESSAGE = (
    "The Python raven package is required to use this " "feature, please install it"
)
...
class Sentry(object):
    ...
    def handle_slow_request(self, environ, dt):
        ...
        cak = environ.get("controller_action_key", None) or environ.get(
            "PATH_INFO", "NOPATH"
        ).strip("/").replace("/", ".")
        event_id = self.client.captureMessage(
            ...
            level="warning",
            tags={"type": "sloreq", "action_key": cak},
        )
