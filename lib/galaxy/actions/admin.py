...
class AdminActions(object):
    ...
    def _create_quota(self, params, decode_id=None):
        ...
        if not params.name or not params.description:
            ...
            self.sa_session.query(self.app.model.Quota)
            .filter(self.app.model.Quota.table.c.name == params.name)
            .first()
        ):
            ...
            params.default != "no"
            and params.default
            not in self.app.model.DefaultQuotaAssociation.types.__dict__.values()
        ):
            ...
            if params.default != "no":
                ...
                in_users = [
                    self.sa_session.query(self.app.model.User).get(
                ...
                message = (
                    "Quota '%s' has been created with %d associated users and %d associated groups."
    ...
    def _rename_quota(self, quota, params):
        if not params.name:
            ...
            and self.sa_session.query(self.app.model.Quota)
            .filter(self.app.model.Quota.table.c.name == params.name)
            .first()
        ):
    ...
    def _manage_users_and_groups_for_quota(self, quota, params, decode_id=None):
        if quota.default:
            ...
            message = (
                "Quota '%s' has been updated with %d associated users and %d associated groups."
                % (quota.name, len(in_users), len(in_groups))
            )
            return message
    ...
    def _edit_quota(self, quota, params):
        ...
        if not params.amount:
            ...
            message = "Quota '%s' is now '%s'." % (
                quota.name,
                quota.operation + quota.display_amount,
            )
            return message
    ...
    def _purge_quota(self, quota, params=None):
        ...
        if len(names) == 1:
            raise ActionInputError(
                "Quota '%s' has not been deleted, so it cannot be purged." % (names[0])
            )
        elif len(names) > 1:
            raise ActionInputError(
                "Quotas have not been deleted so they cannot be undeleted: "
