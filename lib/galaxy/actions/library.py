...
class LibraryActions(object):
    ...
    def _upload_dataset(
        self, trans, library_id, folder_id, replace_dataset=None, **kwd
    ):
        ...
        for file in files:
            ...
            uploaded_datasets.append(
                self._make_library_uploaded_dataset(
                    trans, params, name, file, "server_dir", library_bunch
                )
            )
    ...
    def _get_path_paste_uploaded_datasets(
        self, trans, params, library_bunch, response_code, message
    ):
        ...
        (
            ...
            _response_code,
            _message,
        ) = self._get_path_files_and_folders(params, preserve_dirs)
        if _response_code:
    ...
    def _paths_list(self, params):
        return [
            (l.strip(), os.path.abspath(l.strip()))
    ...
    def _make_library_uploaded_dataset(
        self, trans, params, name, path, type, library_bunch, in_folder=None
    ):
        link_data_only = params.get("link_data_only", "copy_files")
        ...
        uploaded_dataset.purge_source = getattr(
    ...
    def _create_folder(self, trans, parent_id, library_id, **kwd):
        ...
        new_folder = trans.app.model.LibraryFolder(
            name=kwd.get("name", ""), description=kwd.get("description", "")
        )
    ...
    def _check_access(self, trans, is_admin, item, current_user_roles):
        ...
        if isinstance(item, trans.model.HistoryDatasetAssociation):
            ...
            if not item:
                ...
                not trans.app.security_agent.can_access_dataset(
                    current_user_roles, item.dataset
                )
                and item.history.user == trans.user
            ):
                message = (
                    "You do not have permission to access the history dataset with id (%s)."
                    % str(item.id)
                )
                can_access = False
        else:
            ...
            if not item:
                ...
                message = (
                    "You do not have permission to access the %s with id (%s)."
    ...
    def _check_add(self, trans, is_admin, item, current_user_roles):
        ...
        if not (
            is_admin
            or trans.app.security_agent.can_add_library_item(current_user_roles, item)
        ):
