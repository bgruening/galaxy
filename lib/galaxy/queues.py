...
def all_control_queues_for_declare(application_stack):
    ...
    return [
        Queue("control.%s" % server_name, galaxy_exchange, routing_key="control.*")
...
def control_queues_from_config(config):
    ...
    process_name = "{server_name}@{hostname}".format(
        server_name=config.server_name, hostname=hostname
    )
    exchange_queue = Queue(
        "control.%s" % process_name,
        galaxy_exchange,
        routing_key="control.%s" % process_name,
    )
    non_exchange_queue = Queue(
        "control.%s" % process_name, routing_key="control.%s" % process_name
    )
