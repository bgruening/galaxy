...
def validate_email(trans, email, user=None, check_dup=True, allow_empty=False):
    ...
    if not (VALID_EMAIL_RE.match(email)):
        ...
        message = (
            "Email address cannot be more than %d characters in length." % EMAIL_MAX_LEN
        )
    elif (
        check_dup
        and trans.sa_session.query(trans.app.model.User).filter_by(email=email).first()
    ):
...
def validate_publicname(trans, publicname, user=None):
    ...
    if len(publicname) < PUBLICNAME_MIN_LEN:
        return "Public name must be at least %d characters in length." % (
            PUBLICNAME_MIN_LEN
        )
    if len(publicname) > PUBLICNAME_MAX_LEN:
        return "Public name cannot be more than %d characters in length." % (
            PUBLICNAME_MAX_LEN
        )
...
def transform_publicname(trans, publicname, user=None):
    ...
    if user and user.username == publicname:
        ...
        publicname = publicname.ljust(PUBLICNAME_MIN_LEN + 1, FILL_CHAR)[
            :PUBLICNAME_MAX_LEN
        ]
        if (
