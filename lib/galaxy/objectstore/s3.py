...
class CloudConfigMixin(object):
    def _config_to_dict(self):
        return {
            "auth": {"access_key": self.access_key, "secret_key": self.secret_key},
            "bucket": {"name": self.bucket, "use_reduced_redundancy": self.use_rr},
            "connection": {
                "host": self.host,
                "port": self.port,
...
class S3ObjectStore(ObjectStore, CloudConfigMixin):
        ...
        for i in range(5):
            try:
                ...
                try:
                    ...
                    log.exception(
                        "Could not get bucket '%s', attempt %s/5", bucket_name, i + 1
                    )
                    time.sleep(2)
            ...
            if os.path.exists(source_file):
                ...
                if from_string:
                    ...
                    if mb_size < 10 or (not self.multipart):
                        ...
                        multipart_upload(
                            self.s3server, self.bucket, key.name, source_file, mb_size
                        )
    ...
    def file_ready(self, obj, **kwargs):
        ...
        if self._in_cache(rel_path):
            if os.path.getsize(self._get_cache_path(rel_path)) == self._get_size_in_s3(
                rel_path
            ):
                return True
            log.debug(
