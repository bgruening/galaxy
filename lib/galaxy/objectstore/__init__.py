...
from galaxy.util import directory_hash_id, force_symlink, umask_fix_perms
from galaxy.util.odict import odict
from galaxy.util.path import safe_makedirs, safe_relpath
...
class ObjectStore(object):
    ...
    def __init__(self, config, config_dict={}, **kwargs):
        ...
        self.store_by = config_dict.get("store_by", None) or getattr(
        ...
        extra_dirs.update(
            dict((e["type"], e["path"]) for e in config_dict.get("extra_dirs", []))
        )
    ...
    def size(
        self, obj, extra_dir=None, extra_dir_at_root=False, alt_name=None, obj_dir=False
    ):
        """
        ...
        **kwargs
        ...
        if not dir_only:
            assert obj_id is not None, (
                "The effective dataset identifier consumed by object store [%s] must be set before a path can be constructed."
                % (self.store_by)
            )
            path = os.path.join(
        ...
        if file_name and self.exists(obj, **kwargs):
            try:
                if preserve_symlinks and os.path.islink(file_name):
                    force_symlink(
                        os.readlink(file_name), self.get_filename(obj, **kwargs)
                    )
                else:
                    path = self.get_filename(obj, **kwargs)
                ...
                log.critical(
                    ...
                    % (file_name, self._get_filename(obj, **kwargs), ex)
                )
                raise ex
...
class NestedObjectStore(ObjectStore):
    ...
    def update_from_file(self, obj, **kwargs):
        ...
        return self._call_method(
            "update_from_file", obj, ObjectNotFound, True, **kwargs
        )
...
class DistributedObjectStore(NestedObjectStore):
    ...
    def __init__(self, config, config_dict, fsmon=False):
        ...
        self.sleeper = None
        if fsmon and (
            self.global_max_percent_full
            or [_ for _ in self.max_percent_full.values() if _ != 0.0]
        ):
    ...
    def from_xml(clazz, config, config_xml, fsmon=False):
        ...
        if config_xml is None:
            ...
            log.debug(
                "Loading backends for distributed object store from %s",
                distributed_config,
            )
    ...
    def to_dict(self):
        ...
        for backend_id, backend in self.backends.items():
            ...
            backend_as_dict["weight"] = len(
                [i for i in self.original_weighted_backend_ids if i == backend_id]
            )
...
class HierarchicalObjectStore(NestedObjectStore):
    ...
    def __init__(self, config, config_dict, fsmon=False):
        ...
        for order, backend_def in enumerate(config_dict["backends"]):
            backends[order] = build_object_store_from_config(
                config, config_dict=backend_def, fsmon=fsmon
            )
...
def build_object_store_from_config(
    config, fsmon=False, config_xml=None, config_dict=None
):
    """
        ...
        if os.path.exists(config_file):
            if config_file.endswith(".xml") or config_file.endswith(".xml.sample"):
                ...
                config_xml = ElementTree.parse(
                    config.object_store_config_file
                ).getroot()
    ...
    if from_object == "xml":
        return objectstore_class.from_xml(
            config=config, config_xml=config_xml, **objectstore_constructor_kwds
        )
    else:
        return objectstore_class(
            config=config, config_dict=config_dict, **objectstore_constructor_kwds
        )
...
class ObjectStorePopulator(object):
    ...
    def set_object_store_id(self, data):
        ...
        self.object_store_id = (
            data.dataset.object_store_id
        )  # these will be the same thing after the first output
