...
def parse_config_xml(config_xml):
    ...
    try:
        ...
        log.exception(
            "Malformed PithosObjectStore Configuration XML -- " "unable to continue"
        )
        raise
    return r
...
class PithosObjectStore(ObjectStore):
        ...
        **kwargs
        ...
        if alt_name:
            if not safe_relpath(alt_name):
                log.warning(
                    "alt_name would locate path outside dir: {0}".format(alt_name)
                )
                raise ObjectInvalid("The requested object is invalid")
        ...
        if not self.exists(obj, **kwargs):
            ...
            if dir_only:
                ...
                rel_path = os.path.join(
                    rel_path, alt_name if alt_name else "dataset_{0}.dat".format(obj.id)
                )
    ...
    def delete(self, obj, **kwargs):
        ...
        try:
            ...
            log.exception(
                "Could not delete {path} from Pithos, {err}".format(path=path, err=ce)
            )
        return False
