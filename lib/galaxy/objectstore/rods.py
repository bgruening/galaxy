...
IRODS_IMPORT_MESSAGE = (
    "The Python irods package is required to use this " "feature, please install it"
)
...
class IRODSObjectStore(DiskObjectStore):
    ...
    def __init__(self, config, file_path=None, extra_dirs=None):
        ...
        if config.irods_root_collection_path is None or (
            ...
            assert (
                rods_home != ""
            ), "Unable to initialize iRODS Object Store: rodsHome cannot be determined and irods_root_collection_path in Galaxy config is unset or not absolute."
        ...
        if self.root_collection.getId() == -1:
            ...
            assert (
                self.root_collection.createCollection(
                    os.path.basename(self.root_collection_path)
                )
                == 0
            ), ("iRODS root collection creation failed: %s" % self.root_collection_path)
            self.root_collection = irods.irodsCollection(
                self.rods_conn, self.root_collection_path
            )
            assert (
                self.root_collection.getId() != -1
            ), "iRODS root collection creation claimed success but still does not exist"
    ...
    def __get_cache_path(self, obj, **kwargs):
        ...
        return os.path.join(
            self.cache_path,
            path_basename(self.__get_rods_path(obj, strip_dat=False, **kwargs)),
        )
    ...
    def __get_rods_handle(self, obj, mode="r", **kwargs):
        if kwargs.get("dir_only", False):
            return irods.irodsCollection(
                self.rods_conn, self.__get_rods_path(obj, **kwargs)
            )
        else:
            return irods.irodsOpen(
                self.rods_conn, self.__get_rods_path(obj, **kwargs), mode
            )
    ...
    def __mkcolls(self, rods_path):
        ...
        assert rods_path.startswith(self.root_collection_path + "/"), (
            "__mkcolls(): Creating collections outside the root collection is not allowed (requested path was: %s)"
            % rods_path
        )
        mkcolls = []
        ...
        while c.getId() == -1:
            assert c.getCollName().startswith(self.root_collection_path + "/"), (
                "__mkcolls(): Attempted to move above the root collection: %s"
                % c.getCollName()
            )
        ...
        for collname in reversed(mkcolls):
            ...
            assert status == 0, (
                "__mkcolls(): Failed to create collection: %s" % collname
            )
    ...
    def create(self, obj, **kwargs):
        if not self.exists(obj, **kwargs):
            ...
            if not dir_only:
                ...
                doi.dataSize = (
                    0
                )  # 0 actually means "unknown", although literally 0 would be preferable
                irods.addKeyVal(
                    doi.condInput, irods.DEST_RESC_NAME_KW, self.default_resource
                )
    ...
    def delete(self, obj, entire_dir=False, **kwargs):
        assert (
            "dir_only" not in kwargs
        ), "delete(): `dir_only` parameter is invalid here"
        rods_path = self.__get_rods_path(obj, **kwargs)
        # __get_rods_path prepends self.root_collection_path but we are going
        # to ensure that it's valid anyway for safety's sake
        assert rods_path.startswith(self.root_collection_path + "/"), (
            "ERROR: attempt to delete object outside root collection (path was: %s)"
            % rods_path
        )
        if entire_dir:
            # TODO
        ...
        try:
            ...
            log.warning(
                "delete(): operation failed: object does not exist: %s", rods_path
            )
    ...
    def get_filename(self, obj, **kwargs):
        ...
        while not os.path.exists(cached_path):
            ...
            time.sleep(5)
            log.debug(
                "get_filename(): waiting on incoming '%s' for %s %s",
                incoming_path,
                obj.__class__.__name__,
                obj.id,
            )
    ...
    def update_from_file(self, obj, file_name=None, create=False, **kwargs):
        assert (
            "dir_only" not in kwargs
        ), "update_from_file(): `dir_only` parameter is invalid here"
...
def rods_connect():
    ...
    conn, err = irods.rcConnect(
        env.rodsHost, env.rodsPort, env.rodsUserName, env.rodsZone
    )
    ...
    assert status == 0, (
        "connect(): getting password with obfGetPw() failed (%s): %s"
        % (status, irods.strerror(status))
    )
    status = irods.clientLoginWithObfPassword(conn, pw)
    assert status == 0, (
        "connect(): logging in with clientLoginWithObfPassword() failed (%s): %s"
        % (status, irods.strerror(status))
    )
    return env, conn
