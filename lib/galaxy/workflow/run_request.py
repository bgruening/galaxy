...
def _normalize_inputs(steps, inputs, inputs_by):
    ...
    for step in steps:
        ...
        if not inputs_key:
            message = (
                "Workflow cannot be run because an expected input step '%s' has no input dataset."
                % step.id
            )
...
def _normalize_step_parameters(
    steps, param_map, legacy=False, already_normalized=False
):
...
def _flatten_step_params(param_dict, prefix=""):
    ...
    for key in list(param_dict.keys()):
        ...
        if isinstance(value, dict) and (
            not ("src" in value and "id" in value) and key != "__POST_JOB_ACTIONS__"
        ):
...
def build_workflow_run_configs(trans, workflow, payload):
    ...
    if "inputs" in payload and "ds_map" in payload:
        raise exceptions.RequestParameterInvalidException(
            "Cannot specify both legacy ds_map and input attributes."
        )
    ...
    for index, param_map in enumerate(expanded_params):
        history = _get_target_history(
            trans, workflow, payload, expanded_param_keys, index
        )
        ...
        for key, input_dict in normalized_inputs.items():
            ...
            if "src" not in input_dict:
                raise exceptions.RequestParameterInvalidException(
                    "Not input source type defined for input '%s'." % input_dict
                )
            if "id" not in input_dict:
                raise exceptions.RequestParameterInvalidException(
                    "Not input id defined for input '%s'." % input_dict
                )
            if "content" in input_dict:
                raise exceptions.RequestParameterInvalidException(
                    "Input cannot specify explicit 'content' attribute %s'."
                    % input_dict
                )
            ...
            try:
                if input_source == "ldda":
                    ...
                    dataset = (
                        trans.sa_session.query(app.model.Dataset)
                        .filter(app.model.Dataset.uuid == input_id)
                        .first()
                    )
                    if dataset is None:
                        ...
                        raise exceptions.RequestParameterInvalidException(
                            "Input cannot find UUID: %s." % input_id
                        )
                    assert (
                        trans.user_is_admin
                        or trans.app.security_agent.can_access_dataset(
                    ...
                    raise exceptions.RequestParameterInvalidException(
                        "Unknown workflow input source '%s' specified." % input_source
                    )
                ...
                raise exceptions.ItemAccessibilityException(
                    "Invalid workflow input '%s' specified" % input_id
                )
        ...
        if resource_params:
            ...
            resource_parameters = resource_mapper_function(
                trans=trans, stored_workflow=None, workflow=workflow
            )
            for resource_parameter in resource_parameters:
                if resource_parameter.get("type") == "select":
                    ...
                    if name in resource_params:
                        ...
                        if not valid_option:
                            raise exceptions.RequestParameterInvalidException(
                                "Invalid value for parameter '%s' found." % name
                            )
...
def workflow_run_config_to_request(trans, run_config, workflow):
    ...
    for step in workflow.steps:
        ...
        if step.type == "subworkflow":
            ...
            subworkflow_invocation = workflow_run_config_to_request(
                trans, subworkflow_run_config, step.subworkflow
            )
    ...
    for name, value in replacement_dict.items():
        add_parameter(name=name, value=value, type=param_types.REPLACEMENT_PARAMETERS)
    ...
    for step_id, param_dict in run_config.param_map.items():
        add_parameter(
            name=step_id, value=json.dumps(param_dict), type=param_types.STEP_PARAMETERS
        )
...
def workflow_request_to_run_config(work_request_context, workflow_invocation):
    ...
    for input_association in workflow_invocation.input_dataset_collections:
        inputs[
            input_association.workflow_step_id
        ] = input_association.dataset_collection
