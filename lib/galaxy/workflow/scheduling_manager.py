...
DEFAULT_SCHEDULER_ID = (
    "default"
)  # well actually this should be called DEFAULT_DEFAULT_SCHEDULER_ID...
...
class WorkflowSchedulingManager(ConfiguresHandlers):
    ...
    def __startup_recovery(self):
        ...
        for invocation_id in model.WorkflowInvocation.poll_unhandled_workflow_ids(
            ...
            workflow_invocation = sa_session.query(model.WorkflowInvocation).get(
                invocation_id
            )
...
class WorkflowRequestMonitor(Monitors):
    ...
    def __active_invocation_ids(self, scheduler_id):
        ...
        return model.WorkflowInvocation.poll_active_workflow_ids(
            sa_session, scheduler=scheduler_id, handler=handler
        )
