...
class WorkflowCanvas(object):
    ...
    def finish(self):
        ...
        text_style_layer = self.canvas.g(
            style="font-family: Helvetica, Arial, FreeSans, Sans, sans, sans-serif;"
        )
    ...
    def add_boxes(self, step_dict, width, name_fill):
        ...
        self.boxes.append(
            svgwrite.shapes.Rect(
                (x - MARGIN, y), (width, 30), fill=name_fill, stroke="#000000"
            )
        )
        box_height = (
            len(step_dict["data_inputs"]) + len(step_dict["data_outputs"])
        ) * LINE_SPACING + MARGIN
        # Draw separator line.
    ...
    def add_text(self, module_data_inputs, module_data_outputs, step, module_name):
        ...
        self.text.append(
            svgwrite.text.Text(module_name, (x, y + 20), style="font-size:14px")
        )
        y += 45
        count = 0
        in_pos = self.in_pos
        ...
        for di in module_data_inputs:
            ...
            self.text.append(
                svgwrite.text.Text(di["label"], (x, cur_y), style="font-size:10px")
            )
            count += 1
        ...
        for do in module_data_outputs:
            ...
            self.text.append(
                svgwrite.text.Text(do["name"], (x, cur_y), style="font-size:10px")
            )
            count += 1
    ...
    def add_connection(self, step_dict, conn, output_dict):
        ...
        self.text.append(
            svgwrite.shapes.Circle(
                center=(
                    out_conn_pos[0] + self.widths[output_dict["id"]] - MARGIN,
