...
def get_resource_mapper_function(app):
    ...
    if mapper is None:
        ...
        return functools.partial(
            raw_function, workflow_resource_params=workflow_resource_params
        )
    else:
        workflow_resource_params = _read_defined_parameter_definitions(config)
        ...
        if "by_group" in mapper_definition:
            ...
            return functools.partial(
                _resource_parameters_by_group,
                by_group=by_group,
                workflow_resource_params=workflow_resource_params,
            )
        else:
            raise Exception(
                "Currently workflow parameter mapper definitions require a by_group definition."
            )
...
def validate_by_group_workflow_parameters_mapper(by_group, workflow_resource_params):
    ...
    try:
        if "default" not in by_group:
            raise Exception(
                "'workflow_resource_params_mapper' YAML file is malformed, 'default' attribute not found!"
            )
        default_group = by_group["default"]
        if "groups" not in by_group:
            raise Exception(
                "'workflow_resource_params_mapper' YAML file is malformed, 'groups' attribute not found!"
            )
                ...
                if type(attrib) is dict:
                    if "name" not in attrib:
                        raise Exception(
                            ...
                            "'name' attribute not found in attribute of group '"
                            + group
                            + "'!"
                        )
                    ...
                    if "options" not in attrib:
                        raise Exception(
                            "'workflow_resource_params_mapper' YAML file is malformed, "
                            "'options' attribute not found in attribute of group '"
                            + group
                            + "'!"
                        )
...
def _import_resource_mapping_function(qualified_function_path):
    ...
    try:
        ...
        raise Exception(
            "Failed to find workflow resource mapper module %s" % full_module_name
        )
    ...
    if hasattr(module, function_name):
        ...
        raise Exception(
            "Failed to find workflow resource mapper function %s.%s"
            % (full_module_name, function_name)
        )
