...
from galaxy.managers import annotatable, datasets, secured, taggable, users
...
class HDAManager(
    ...
    def get_owned_ids(self, object_ids, history=None):
        ...
        """
        filters = [
            self.model_class.id.in_(object_ids),
            self.model_class.history_id == history.id,
        ]
    ...
    def has_been_resubmitted(self, hda):
        ...
        query = self._job_state_history_query(hda).filter(
            model.JobStateHistory.state == job_states.RESUBMITTED
        )
...
class HDASerializer(  # datasets._UnflattenedMetadataDatasetAssociationSerializer,
        ...
        self.serializers.update(
            {
                ...
                "resubmitted": lambda i, k, **c: self.hda_manager.has_been_resubmitted(
                    i
                ),
                ...
                "download_url": lambda i, k, **c: self.url_for(
                ...
                "accessible": lambda i, k, user=None, **c: self.manager.is_accessible(
                    i, user, **c
                ),
    ...
    def serialize_old_display_applications(self, hda, key, trans=None, **context):
        ...
        for display_app in hda.datatype.get_display_types():
            target_frame, display_links = display_link_fn(
                hda, display_app, self.app, trans.request.base
            )
    ...
    def serialize_urls(self, hda, key, **context):
        ...
        urls = {
            "purge": url_for(
                controller="dataset", action="purge_async", dataset_id=encoded_id
            ),
            "display": url_for(
                controller="dataset",
                action="display",
                dataset_id=encoded_id,
                preview=True,
            ),
            ...
            "download": url_for(
                ...
                action="display",
                dataset_id=encoded_id,
                to_ext=hda.extension,
            ),
            "report_error": url_for(
                controller="dataset", action="errors", id=encoded_id
            ),
            "rerun": url_for(controller="tool_runner", action="rerun", id=encoded_id),
            "show_params": url_for(
                controller="dataset", action="show_params", dataset_id=encoded_id
            ),
        ...
        self.deserializers.update(
            {
                ...
                # remapped
                "genome_build": lambda i, k, v, **c: self.deserialize_genome_build(
                    i, "dbkey", v
                ),
                "misc_info": lambda i, k, v, **c: self.deserialize_basestring(
                    i, "info", v, convert_none_to_empty=True
                ),
            }
        )
...
class HDAFilterParser(
    datasets.DatasetAssociationFilterParser,
