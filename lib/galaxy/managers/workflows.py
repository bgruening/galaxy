...
class WorkflowsManager(object):
    ...
    def get_stored_workflow(self, trans, workflow_id):
        ...
        if util.is_uuid(workflow_id):
            ...
            workflow_query = trans.sa_session.query(
                trans.app.model.StoredWorkflow
            ).filter(
                and_(
                    trans.app.model.StoredWorkflow.latest_workflow_id
                    == trans.app.model.Workflow.id,
    ...
    def check_security(
        self, trans, has_workflow, check_ownership=True, check_accessible=True
    ):
    ...
    def get_invocation(self, trans, decoded_invocation_id):
        ...
        self.check_security(
            trans, workflow_invocation, check_ownership=True, check_accessible=False
        )
    ...
    def cancel_invocation(self, trans, decoded_invocation_id):
        ...
        if cancelled:
            ...
            raise exceptions.MessageException(
                "Cannot cancel an inactive workflow invocation."
            )
    ...
    def update_invocation_step(
        self, trans, decoded_workflow_invocation_step_id, action
    ):
        if action is None:
        ...
        if not workflow_invocation.active:
            raise exceptions.RequestParameterInvalidException(
                "Attempting to modify the state of an completed workflow invocation."
            )
    ...
    def build_invocations_query(
        self, trans, stored_workflow_id=None, history_id=None, user_id=None
    ):
        ...
        if stored_workflow_id is not None:
            stored_workflow = sa_session.query(model.StoredWorkflow).get(
                stored_workflow_id
            )
        ...
        if user_id is not None:
            invocations_query = invocations_query.join(model.History).filter(
        ...
        return [
            ...
            if self.check_security(
                trans, inv, check_ownership=True, check_accessible=False
            )
        ]
    ...
    def serialize_workflow_invocation(self, invocation, **kwd):
        ...
        as_dict = invocation.to_dict(
            view, step_details=step_details, legacy_job_state=legacy_job_state
        )
    ...
    def serialize_workflow_invocations(self, invocations, **kwd):
        if "view" not in kwd:
            kwd["view"] = "collection"
        return list(
            map(lambda i: self.serialize_workflow_invocation(i, **kwd), invocations)
        )


CreatedWorkflow = namedtuple(
...
class WorkflowContentsManager(UsesAnnotations):
    ...
    def normalize_workflow_format(self, trans, as_dict):
        ...
        if (
            workflow_class == "GalaxyWorkflow"
            or "$graph" in as_dict
            or "yaml_content" in as_dict
        ):
    ...
    def build_workflow_from_raw_description(
        ...
        if create_stored_workflow:
            ...
            if data["annotation"]:
                ...
                self.add_item_annotation(
                    trans.sa_session, stored.user, stored, annotation
                )
            workflow_tags = data.get("tags", [])
            trans.app.tag_handler.set_tags_from_list(
                user=trans.user, item=stored, new_tags_list=workflow_tags
            )
        ...
        return CreatedWorkflow(
            stored_workflow=stored, workflow=workflow, missing_tools=missing_tool_tups
        )

    def update_workflow_from_raw_description(
        self, trans, stored_workflow, raw_workflow_description, **kwds
    ):
        ...
        workflow, missing_tool_tups = self._workflow_from_raw_description(
            trans, raw_workflow_description, name=stored_workflow.name, **kwds
        )
        ...
        if missing_tool_tups:
            ...
            for missing_tool_tup in missing_tool_tups:
                errors.append(
                    "Step %i: Requires tool '%s'."
                    % (int(missing_tool_tup[3]) + 1, missing_tool_tup[0])
                )
    ...
    def _workflow_from_raw_description(
        self, trans, raw_workflow_description, name, **kwds
    ):
        data = raw_workflow_description.as_dict
        ...
        for step_dict in self.__walk_step_dicts(data):
            module, step = self.__module_from_dict(
                trans, steps, steps_by_external_id, step_dict, **kwds
            )
    ...
    def workflow_to_dict(self, trans, stored, style="export", version=None):
        ...
        if style == "editor":
            ...
            wf_dict = self._workflow_to_dict_instance(
                stored, workflow=workflow, legacy=True
            )
        elif style == "instance":
            wf_dict = self._workflow_to_dict_instance(
                stored, workflow=workflow, legacy=False
            )
        elif style == "run":
            ...
            raise exceptions.RequestParameterInvalidException(
                "Unknown workflow style [%s]" % style
            )
        if version:
        ...
        with open(workflow_path, "w") as f:
            if workflow_path.endswith(".ga"):
                wf_dict = self._workflow_to_dict_export(
                    trans, stored_workflow, workflow=workflow
                )
    ...
    def _workflow_to_dict_run(self, trans, stored, workflow):
        ...
        if missing_tools:
            ...
            raise exceptions.MessageException(
                "Following tools missing: %s" % ", ".join(missing_tools)
            )
        workflow.annotation = self.get_item_annotation_str(
            trans.sa_session, trans.user, workflow
        )
        ...
        for step in workflow.steps:
            ...
            ] = step.module.get_replacement_parameters(step)
        ...
        return {
            ...
            "history_id": trans.app.security.encode_id(trans.history.id)
            if trans.history
            else None,
            "name": stored.name,
            "steps": step_models,
    ...
    def _workflow_resource_parameters(self, trans, stored, workflow):
        ...
        """
        return self._resource_mapper_function(
            trans=trans, stored_workflow=stored, workflow=workflow
        )

    def _workflow_to_dict_editor(
        self, trans, stored, workflow, tooltip=True, is_subworkflow=False
    ):
        ...
        for step in workflow.steps:
            ...
            # Get user annotation.
            annotation_str = (
                self.get_item_annotation_str(trans.sa_session, trans.user, step) or ""
            )
            ...
            if tooltip:
                step_dict["tooltip"] = module.get_tooltip(
                    ...
                    if pja.action_type == "ChangeDatatypeAction":
                        for output in step_dict["outputs"]:
                            if output["name"] == pja.output_name:
                                output["force_datatype"] = [
                                    pja.action_arguments["newtype"]
                                ]
            ...
            for input_value in input_connections:
                ...
                for input_step_data_output in input_step["outputs"]:
                    if input_step_data_output["name"] == input_value["output_name"]:
                        ...
                        # outer workflow.
                        if collection_type:
                            if current_data_input.get(
                                "input_type"
                            ) == "dataset" and current_data_input.get("multiple"):
                                ...
                                if ":" in collection_type:
                                    ...
                                    collection_type = ":".join(
                                        collection_type.rsplit(":")[:-1]
                                    )
                                else:
                                    # We've reduced a list or a pair
                                    collection_type = None
                            elif (
                                current_data_input.get("input_type")
                                == "dataset_collection"
                            ):
                                current_collection_types = current_data_input[
                                    "collection_types"
                                ]
                                if not current_collection_types:
                                    ...
                                    for accepted_collection_type in current_data_input[
                                        "collection_types"
                                    ]:
                                        ...
                                        if collection_type.endswith(
                                            accepted_collection_type
                                        ):
                                            _outer_map_over = collection_type[
                                                : -(len(accepted_collection_type) + 1)
                                            ]
                                            if len(_outer_map_over.split(":")) < len(
                                                outer_map_over.split(":")
                                            ):
                            ...
                            if collection_type and len(
                                collection_type.split(":")
                            ) >= len(max_map_over.split(":")):
        ...
        for order_index in sorted(steps):
            ...
            if step["type"] == "tool" and not step.get("errors"):
                ...
                for step_data_output in step["outputs"]:
                    if (
                        step_data_output.get("collection_type_source")
                        and step_data_output["collection_type"] is None
                    ):
                        collection_type_source = step_data_output[
                            "collection_type_source"
                        ]
                        for input_connection in step["input_connections"].get(
                            collection_type_source, []
                        ):
                            input_step = steps[input_connection["id"]]
                            for input_step_data_output in input_step["outputs"]:
                                if (
                                    input_step_data_output["name"]
                                    == input_connection["output_name"]
                                ):
                                    step_data_output[
                                        "collection_type"
                                    ] = input_step_data_output.get("collection_type")
                    if map_over:
                        ...
                        if step_data_output.get("collection_type"):
                            collection_type = "%s:%s" % (
                                map_over,
                                step_data_output["collection_type"],
                            )
    ...
    def _workflow_to_dict_export(self, trans, stored=None, workflow=None):
        ...
        if stored is not None:
            annotation_str = (
                self.get_item_annotation_str(trans.sa_session, trans.user, stored) or ""
            )
        ...
        data[
            "a_galaxy_workflow"
        ] = "true"  # Placeholder for identifying galaxy workflow
        ...
        for step in workflow.steps:
            ...
            for name, val in step_state.items():
                ...
                if input_type == RuntimeValue:
                    input_dicts.append(
                        {
                            "name": name,
                            "description": "runtime parameter for tool %s"
                            % module.get_name(),
                        }
                    )
            ...
            for workflow_output in step.unique_workflow_outputs:
                workflow_output_dict = dict(
                    ...
                    if workflow_output.uuid is not None
                    else None,
                )
            ...
            if type(module) is ToolModule:
                for output in module.get_data_outputs():
                    step_dict["outputs"].append(
                        {"name": output["name"], "type": output["extensions"][0]}
                    )
            ...
            if step.type is None or step.type == "tool":
                ...
                if module.tool:
                    ...
                    visit_input_values(
                        module.tool.inputs, module.state.inputs, callback
                    )
    ...
    def _workflow_to_dict_instance(self, stored, workflow, legacy=True):
        ...
        item["inputs"] = inputs
        item["annotation"] = self.get_item_annotation_str(
            sa_session, stored.user, stored
        )
        steps = {}
    ...
    def __module_from_dict(self, trans, steps, steps_by_external_id, step_dict, **kwds):
        ...
        if annotation:
            ...
            self.add_item_annotation(
                trans.sa_session, trans.get_user(), step, annotation
            )
            ...
            for workflow_output in workflow_outputs:
                ...
                if output_name in found_output_names:
                    raise exceptions.ObjectAttributeInvalidException(
                        "Duplicate workflow outputs with name [%s] found." % output_name
                    )
                if not output_name:
                    raise exceptions.ObjectAttributeInvalidException(
                ...
                m = step.create_or_update_workflow_output(
                    output_name=output_name, uuid=uuid, label=label
                )
    ...
    def __load_subworkflow_from_step_dict(
        self, trans, step_dict, subworkflow_id_map, **kwds
    ):
        ...
        if embedded_subworkflow:
            subworkflow = self.__build_embedded_subworkflow(
                trans, embedded_subworkflow, **kwds
            )
    ...
    def __build_embedded_subworkflow(self, trans, data, **kwds):
        ...
        subworkflow = self.build_workflow_from_raw_description(
            ...
            fill_defaults=kwds.get("fill_defaults", False),
        ).workflow
        return subworkflow
            ...
            for input_name, conn_list in step.temp_input_connections.items():
                ...
                for conn_dict in conn_list:
                    ...
                    if external_id not in steps_by_external_id:
                        raise KeyError(
                            "Failed to find external id %s in %s"
                            % (external_id, steps_by_external_id.keys())
                        )
