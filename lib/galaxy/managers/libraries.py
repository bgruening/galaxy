...
class LibraryManager(object):
    ...
    def list(self, trans, deleted=False):
        ...
        if is_admin:
            ...
            library_add_action = (
                trans.app.security_agent.permitted_actions.LIBRARY_ADD.action
            )
            library_modify_action = (
                trans.app.security_agent.permitted_actions.LIBRARY_MODIFY.action
            )
            library_manage_action = (
                trans.app.security_agent.permitted_actions.LIBRARY_MANAGE.action
            )
    ...
    def check_accessible(self, trans, library):
        ...
        """
        if not trans.app.security_agent.can_access_library(
            trans.get_current_user_roles(), library
        ):
            raise exceptions.ObjectNotFound(
                "Library with the id provided was not found."
            )
    ...
    def get_library_dict(self, trans, library, prefetched_ids=None):
        ...
        """
        restricted_library_ids = (
            prefetched_ids.get("restricted_library_ids", None)
            if prefetched_ids
            else None
        )
        allowed_library_add_ids = (
            prefetched_ids.get("allowed_library_add_ids", None)
            if prefetched_ids
            else None
        )
        allowed_library_modify_ids = (
            prefetched_ids.get("allowed_library_modify_ids", None)
            if prefetched_ids
            else None
        )
        allowed_library_manage_ids = (
            prefetched_ids.get("allowed_library_manage_ids", None)
            if prefetched_ids
            else None
        )
        ...
        if not trans.user_is_admin:
            if prefetched_ids:
                library_dict["can_user_add"] = (
                    ...
                    if (
                        allowed_library_add_ids
                        and library.id in allowed_library_add_ids
                    )
                    else False
                )
                library_dict["can_user_modify"] = (
                    ...
                    if (
                        allowed_library_modify_ids
                        and library.id in allowed_library_modify_ids
                    )
                    else False
                )
                library_dict["can_user_manage"] = (
                    ...
                    if (
                        allowed_library_manage_ids
                        and library.id in allowed_library_manage_ids
                    )
                    else False
                )
            else:
                ...
                library_dict[
                    "can_user_add"
                ] = trans.app.security_agent.can_add_library_item(
    ...
    def get_current_roles(self, trans, library):
        ...
        add_library_item_role_list = [
            (add_role.name, trans.security.encode_id(add_role.id))
            for add_role in self.get_add_roles(trans, library)
        ]
        return dict(
    ...
    def set_permission_roles(
        self, trans, library, access_roles, modify_roles, manage_roles, add_roles
    ):
        """
