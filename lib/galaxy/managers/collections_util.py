...
ERROR_MESSAGE_INVALID_PARAMETER_FOUND = (
    "Found invalid parameter %s in element identifier description %s."
)
ERROR_MESSAGE_DUPLICATED_IDENTIFIER_FOUND = (
    "Found duplicated element identifier name %s."
)
...
def validate_input_element_identifiers(element_identifiers):
    ...
    """
    log.debug(
        "Validating %d element identifiers for collection creation."
...
def get_collection_elements(collection, name=""):
    ...
    for element in collection.elements:
        ...
        if element.is_collection:
            subnames, subhdas = get_collection(
                element.child_collection, name=full_element_name
            )
            names.extend(subnames)
            hdas.extend(subhdas)
        else:
...
def dictify_dataset_collection_instance(
    dataset_collection_instance, parent, security, view="element", fuzzy_count=None
):
    ...
    if view in ["element", "element-reference"]:
        collection = dataset_collection_instance.collection
        rank_fuzzy_counts = gen_rank_fuzzy_counts(
            collection.collection_type, fuzzy_count
        )
        elements, rest_fuzzy_counts = get_fuzzy_count_elements(
            collection, rank_fuzzy_counts
        )
        if view == "element":
        ...
        dict_value["elements"] = [
            element_func(_, rank_fuzzy_counts=rest_fuzzy_counts) for _ in elements
        ]

    security.encode_all_ids(
        dict_value, recursive=True
    )  # TODO: Use Kyle's recursive formulation of this.
    return dict_value
...
def dictify_element_reference(element, rank_fuzzy_counts=None):
    ...
    if element_object is not None:
        object_details = dict(
            id=element_object.id, model_class=element_object.__class__.__name__
        )
        if element.child_collection:
            ...
            object_details["elements"] = [
                dictify_element_reference(_, rank_fuzzy_counts=rest_fuzzy_counts)
                for _ in elements
            ]
...
def dictify_element(element, rank_fuzzy_counts=None):
    ...
    if element_object is not None:
        ...
        if element.child_collection:
            ...
            child_collection = element.child_collection
            object_details["elements"] = [
                dictify_element(_, rank_fuzzy_counts=rest_fuzzy_counts)
                for _ in elements
            ]
...
def gen_rank_fuzzy_counts(collection_type, fuzzy_count=None):
    ...
    if fuzzy_count is None:
        ...
        list_fuzzy_count_mult = math.floor(
            (fuzzy_count * 1.0) / paired_fuzzy_count_mult
        )
        list_rank_fuzzy_count = (
            int(math.floor(math.pow(list_fuzzy_count_mult, 1.0 / list_count)) + 1)
            if list_count > 0
            else 1.0
        )
        ...
        rank_fuzzy_counts = [
            pair_rank_fuzzy_count if rt == "paired" else list_rank_fuzzy_count
