...
class LibraryDatasetsManager(datasets.DatasetAssociationManager):
    ...
    def get(self, trans, decoded_library_dataset_id, check_accessible=True):
        ...
        try:
            ld = (
                ...
                .filter(
                    trans.app.model.LibraryDataset.table.c.id
                    == decoded_library_dataset_id
                )
                .one()
            )
        ...
        for key, val in payload.items():
            ...
            if key in ("file_ext"):
                ...
                if datatype is None and val not in ("auto",):
                    raise RequestParameterInvalidException(
                        "This Galaxy does not recognize the datatype of: %s" % (val)
                    )
                validated_payload[key] = val
    ...
    def check_accessible(self, trans, ld):
        ...
        if not trans.app.security_agent.can_access_library_item(
            trans.get_current_user_roles(), ld, trans.user
        ):
    ...
    def serialize(self, trans, ld):
        ...
        rval["can_user_modify"] = (
            trans.user_is_admin
            or trans.app.security_agent.can_modify_library_item(current_user_roles, ld)
        )
        ...
        rval["can_user_manage"] = (
            trans.user_is_admin
            or trans.app.security_agent.can_manage_dataset(
