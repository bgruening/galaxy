    ...
    def get(self, trans, history_id, bucket_name, objects, authz_id, input_args=None):
        ...
        try:
            ...
            raise ItemAccessibilityException(
                "Could not get the bucket `{}`: {}".format(bucket_name, str(e))
            )
        ...
        for obj in objects:
            try:
                ...
                raise MessageException(
                    "The following error occurred while getting the object {}: {}".format(
                        obj, str(e)
                    )
                )
            if key is None:
                log.exception(
                    ...
                    "invalid or not authorized to read the bucket/object.".format(
                        obj, trans.user.id
                    )
                )
            ...
            if not history:
                raise ObjectNotFound(
                    "History with ID `{}` not found.".format(
    ...
    def send(
        ...
        for hda in history.datasets:
            if (
                ...
                or hda.purged
                or hda.state != "ok"
                or hda.creating_job.tool_id == SEND_TOOL
            ):
                continue
            if dataset_ids is None or hda.dataset.id in dataset_ids:
                try:
                    ...
                    if not d2c:
                        log.debug(
                            "Failed to get the `send` tool per user `{}` request.".format(
                                trans.user.id
                            )
                        )
                        failed.append(
                    ...
                    failed.append(
                        json.dumps({"object": object_label, "error": err_msg})
                    )
        return sent, failed
