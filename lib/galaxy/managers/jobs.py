...
from galaxy.util import defaultdict, ExecutionTimer
...
class JobManager(object):
    ...
    def get_accessible_job(self, trans, decoded_job_id):
        job = (
            ...
            .filter(trans.app.model.Job.id == decoded_job_id)
            .first()
        )
        if job is None:
...
class JobSearch(object):
    ...
    def by_tool_input(
        self, trans, tool_id, tool_version, param=None, param_dump=None, job_state="ok"
    ):
        ...
        def populate_input_data_input_id(path, key, value):
            ...
            if key == "id":
                ...
                input_data[path_key].append(
                    {"src": src, "id": value, "identifier": identifier}
                )
    ...
    def __search(
        ...
        conditions = [and_(model.Job.tool_id == tool_id, model.Job.user == user)]
        ...
        for k, input_list in input_data.items():
            for type_values in input_list:
                ...
                if t == "hda":
                    ...
                    conditions.append(
                        and_(
                            ...
                            or_(
                                and_(
                                    or_(
                                        a.dataset_version.in_([0, b.version]),
                    ...
                    conditions.append(
                        and_(model.Job.id == a.job_id, a.name == k, a.ldda_id == v)
                    )
                    ...
                    conditions.append(
                        and_(
                            ...
                            or_(
                                ...
                                and_(
                                    or_(
                                        c.copied_from_history_dataset_collection_association_id
                                        == b.id,
        ...
        for k, v in wildcard_param_dump.items():
            ...
            conditions.append(
                and_(
                    model.Job.id == a.job_id, a.name == k, a.value.like(wildcard_value)
                )
            )
        ...
        for job in query.all():
            ...
            if len(job) > 1:
                ...
                for src, requested_id, used_id in zip(
                    data_types, requested_ids, current_jobs_data_ids
                ):
                ...
                for k, v in new_param_dump.items():
                    ...
                    job_parameter_conditions.append(
                        and_(a.name == k, a.value == json.dumps(v, sort_keys=True))
                    )
            else:
    ...
    for job_source_id, job_source_type in zip(job_source_ids, job_source_types):
        if job_source_type == "Job":
            ...
            raise RequestParameterInvalidException(
                "Invalid job source type %s found." % job_source_type
            )
        ...
        if job_source_type == "Job":
            rval.append(
                summarize_jobs_to_dict(
                    sa_session, sa_session.query(model.Job).get(decode(job_source_id))
                )
            )
        else:
            rval.append(
                summarize_jobs_to_dict(
                    sa_session,
                    sa_session.query(model.ImplicitCollectionJobs).get(
                        decode(job_source_id)
                    ),
                )
            )
