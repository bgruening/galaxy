...
from galaxy.managers import hdas, histories, lddas
...
from galaxy.util import odict, validation

log = logging.getLogger(__name__)

ERROR_INVALID_ELEMENTS_SPECIFICATION = (
    "Create called with invalid parameters, must specify element identifiers."
)
...
class DatasetCollectionManager(object):
    ...
    def precreate_dataset_collection_instance(
        ...
        dataset_collection = self.precreate_dataset_collection(
            structure, allow_unitialized_element=implicit_output_name is not None
        )
    ...
    def precreate_dataset_collection(self, structure, allow_unitialized_element=True):
        ...
        if not has_structure and allow_unitialized_element:
            ...
            dataset_collection.collection_type = (
            ...
            for index, (identifier, substructure) in enumerate(structure.children):
                ...
                element = model.DatasetCollectionElement(
                    element=element, element_identifier=identifier, element_index=index
                )
    ...
    def _create_instance_for_collection(
        ...
        if isinstance(parent, model.History):
            dataset_collection_instance = self.model.HistoryDatasetCollectionAssociation(
                collection=dataset_collection, name=name
            )
            if implicit_inputs:
            ...
            dataset_collection_instance = self.model.LibraryDatasetCollectionAssociation(
                collection=dataset_collection, folder=parent, name=name
            )
        ...
        if elements is None:
            ...
            if has_subcollections:
                ...
                self.__recursively_create_collections_for_elements(
                    trans, elements, hide_source_items, copy_elements=copy_elements
                )
    ...
    def _element_identifiers_to_elements(
        ...
        if collection_type_description.has_subcollections():
            ...
            self.__recursively_create_collections_for_identifiers(
                trans, element_identifiers, hide_source_items, copy_elements
            )
        new_collection = False
        for element_identifier in element_identifiers:
            if (
                element_identifier.get("src") == "new_collection"
                and element_identifier.get("collection_type") == ""
            ):
                new_collection = True
    ...
    def _append_tags(
        self, dataset_collection_instance, implicit_inputs=None, tags=None
    ):
        tags = tags or {}
    ...
    def set_collection_elements(self, dataset_collection, dataset_instances):
        ...
        type_plugin = collection_type_description.rank_type_plugin()
        builder.set_collection_elements(
            dataset_collection, type_plugin, dataset_instances
        )
    ...
    def delete(self, trans, instance_type, id, recursive=False, purge=False):
        ...
        if recursive:
            for dataset in dataset_collection_instance.collection.dataset_instances:
                self.hda_manager.error_unless_owner(
                    dataset, user=trans.get_user(), current_history=trans.history
                )
    ...
    def _validate_and_parse_update_payload(self, payload):
        ...
        for key, val in payload.items():
            ...
            if key in ("deleted", "visible"):
                ...
                validated_payload[
                    key
                ] = validation.validate_and_sanitize_basestring_list(key, val)
    ...
    def __recursively_create_collections_for_identifiers(
        self, trans, element_identifiers, hide_source_items, copy_elements
    ):
    ...
    def __recursively_create_collections_for_elements(
        self, trans, elements, hide_source_items, copy_elements
    ):
    ...
    def __load_elements(
        self, trans, element_identifiers, hide_source_items=False, copy_elements=False
    ):
    ...
    def __load_element(
        self, trans, element_identifier, hide_source_items, copy_elements
    ):
        ...
        if src_type == "hda":
            ...
            if copy_elements:
                element = self.hda_manager.copy(
                    hda, history=trans.history, hide_copy=True
                )
            else:
                element = hda
            if hide_source_items and self.hda_manager.get_owned(
                hda.id, user=trans.user, current_history=trans.history
            ):
                hda.visible = False
            self.tag_handler.apply_item_tags(
                user=trans.user, item=element, tags_str=tag_str
            )
        elif src_type == "ldda":
            element = self.ldda_manager.get(trans, encoded_id, check_accessible=True)
            element = element.to_history_dataset_association(
                trans.history, add_to_history=True
            )
            self.tag_handler.apply_item_tags(
                user=trans.user, item=element, tags_str=tag_str
            )
    ...
    def get_dataset_collection(self, trans, encoded_id):
        ...
        collection = trans.sa_session.query(trans.app.model.DatasetCollection).get(
            collection_id
        )
        return collection
    ...
    def apply_rules(self, hdca, rule_set, handle_dataset):
        ...
        initial_data, initial_sources = self.__init_rule_data(
            elements, collection_type_description
        )
    ...
    def _build_elements_from_rule_data(
        self, collection_type_description, rule_set, data, sources, handle_dataset
    ):
        ...
        for data_index, row_data in enumerate(data):
            ...
            for i, identifier_column in enumerate(identifier_columns):
                ...
                if i + 1 == len(identifier_columns):
                    ...
                    collection_type_at_depth = (
                        collection_type_at_depth.child_collection_type_description()
                    )
                    found = False
                    ...
                    if not found:
                        ...
                        sub_collection[
                            "collection_type"
                        ] = collection_type_at_depth.collection_type
    ...
    def __init_rule_data(
        self, elements, collection_type_description, parent_identifiers=None
    ):
    ...
    def __get_library_collection_instance(
        ...
        collection_instance = trans.sa_session.query(
            trans.app.model.LibraryDatasetCollectionAssociation
        ).get(instance_id)
        if check_accessible:
            if not trans.app.security_agent.can_access_library_item(
                ...
                raise ItemAccessibilityException(
                    "LibraryDatasetCollectionAssociation is not accessible to the current user",
                    type="error",
                )
        return collection_instance
