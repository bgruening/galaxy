...
class UserManager(base.ModelManager, deletable.PurgableManagerMixin):
    ...
    def by_email_like(
        self, email_with_wildcards, filters=None, order_by=None, **kwargs
    ):
        """
    ...
    def _admin_emails(self):
        ...
        """
        return [
            email.strip() for email in self.app.config.get("admin_users", "").split(",")
        ]
    ...
    def tags_used(self, user, tag_models=None):
        ...
        for tag_model in tag_models:
            subq = (
                self.session()
                .query(tag_model.user_tname, tag_model.user_value)
    ...
    def change_password(
        self, trans, password=None, confirm=None, token=None, id=None, current=None
    ):
        """
        ...
        if token:
            token_result = trans.sa_session.query(
                self.app.model.PasswordResetToken
            ).get(token)
                ...
                if trans.galaxy_session:
                    for other_galaxy_session in trans.sa_session.query(
                        ...
                        and_(
                            ...
                            self.app.model.GalaxySession.table.c.id
    ...
    def __get_activation_token(self, trans, email):
        ...
        user = (
            ...
            .filter(self.app.model.User.table.c.email == email)
            .first()
        )
        activation_token = user.activation_token
        if activation_token is None:
            activation_token = util.hash_util.new_secure_hash(
        ...
        if message:
            ...
            if prt:
                ...
                try:
                    ...
                    log.debug(body)
                    return (
                        "Failed to submit email. Please contact the administrator: %s"
                        % str(e)
                    )
            else:
    ...
    def get_reset_token(self, trans, email):
        ...
        if not reset_user:
            ...
            reset_user = (
                trans.sa_session.query(self.app.model.User)
                .filter(
                    func.lower(self.app.model.User.table.c.email) == func.lower(email)
                )
                .first()
            )
        if reset_user:
...
class UserDeserializer(base.ModelDeserializer):
    ...
    def add_deserializers(self):
        ...
        self.deserializers.update({"username": self.deserialize_username})
