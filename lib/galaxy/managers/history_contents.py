...
class HistoryContentsManager(containers.ContainerManagerMixin):
    ...
    # ---- interface
    def contained(
        self, container, filters=None, limit=None, offset=None, order_by=None, **kwargs
    ):
        """
    ...
    def state_counts(self, history):
        ...
        """
        filters = [
            base.ModelFilterParser.parsed_filter(
                "orm", sql.column("deleted") == false()
            ),
            base.ModelFilterParser.parsed_filter(
                "orm", sql.column("visible") == true()
            ),
        ]
    ...
    def _union_of_contents(self, container, expand_models=True, **kwargs):
        ...
        id_map = dict(
            (
                (self.contained_class_type_name, []),
        ...
        **kwargs
        ...
        for orm_filter in filters:
            if orm_filter.filter_type == "orm_function":
                contained_query = contained_query.filter(
                    orm_filter.filter(self.contained_class)
                )
                subcontainer_query = subcontainer_query.filter(
                    orm_filter.filter(self.subcontainer_class)
                )
    ...
    def _contents_common_query_for_contained(self, history_id, user_id):
        ...
        if history_id:
            ...
            subquery = subquery.filter(
                component_class.history_id == model.History.table.c.id,
...
class HistoryContentsSerializer(
...
class HistoryContentsFilters(
                ...
                if op == "in":
                    ...
                    for state in states:
                        if state not in valid_states:
                            self.raise_filter_err(
                                attr, op, state, "invalid state in filter"
                            )
    ...
    def parse_type_id_list(self, type_id_list_string, sep=","):
        ...
        """
        return [
            self.decode_type_id(type_id) for type_id in type_id_list_string.split(sep)
        ]
