...
class DataManagerToolAction(DefaultToolAction):
    ...
    def execute(self, tool, trans, **kwds):
        ...
        if (
            ...
            and len(rval) == 2
            and isinstance(rval[0], trans.app.model.Job)
        ):
            assoc = trans.app.model.DataManagerJobAssociation(
                job=rval[0], data_manager_id=tool.data_manager_id
            )
            trans.sa_session.add(assoc)
            trans.sa_session.flush()
        else:
            log.error(
                "Got bad return value from DefaultToolAction.execute(): %s" % (rval)
            )
        return rval
