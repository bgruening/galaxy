...
class DefaultToolAction(object):
        ...
        def visitor(input, value, prefix, parent=None, **kwargs):
            ...
            if isinstance(input, DataToolParameter):
                    ...
                    for (
                        ...
                        if not new_data or new_data.datatype.matches_any(
                            ...
                            raise Exception(
                                "A path for explicit datatype conversion has not been found: %s --/--> %s"
                                % (
                                    input_datasets[prefix + input.name].extension,
                                    conversion_extensions,
                                )
                            )
                    target_dict = parent
                ...
                if replace_collection:
                    collection_type_description = trans.app.dataset_collections_service.collection_type_descriptions.for_collection_type(
    ...
    def _check_access(self, tool, trans):
        assert tool.allow_user_access(
            trans.user
        ), "User (%s) is not allowed to access this tool." % (trans.user)
    ...
    def _remap_job_on_rerun(
        ...
        try:
            ...
            assert old_job.tool_id == current_job.tool_id, (
    ...
    def __remap_parameters(self, job_to_remap, jtid, jtod, out_data):
        input_values = dict(
            [(p.name, json.loads(p.value)) for p in job_to_remap.parameters]
        )
