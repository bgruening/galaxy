...
class SetMetadataToolAction(ToolAction):
    ...
    def execute(
        ...
        **kwargs
        ...
        trans.log_event(
            "Added set external metadata job to the job queue, id: %s" % str(job.id),
            tool_id=job.tool_id,
        )
        return job, odict
    ...
    def execute_via_app(
        ...
        app.object_store.create(
            job, base_dir="job_work", dir_only=True, extra_dir=str(job.id)
        )
        job_working_dir = app.object_store.get_filename(
            job, base_dir="job_work", dir_only=True, extra_dir=str(job.id)
        )
        ...
        output_datatasets_dict = {dataset_name: dataset}
        cmd_line = external_metadata_wrapper.setup_external_metadata(
            ...
            job_metadata=os.path.join(
                job_working_dir, "working", tool.provided_metadata_file
            ),
        ...
        job.state = (
            start_job_state
        )  # job inputs have been configured, restore initial job state
        sa_session.flush()
