...
from galaxy.util import find_instance_nested, unicodify
...
class ToolEvaluator(object):
    ...
    def set_compute_environment(self, compute_environment, get_special=None):
        ...
        self.unstructured_path_rewriter = (
            compute_environment.unstructured_path_rewriter()
        )
        ...
        request_context = WorkRequestContext(
            app=self.app, user=job.history and job.history.user, history=job.history
        )
        ...
        incoming.update(
            model.User.user_template_environment(job.history and job.history.user)
        )
    ...
    def build_param_dict(
        ...
        self.__populate_wrappers(
            param_dict, input_datasets, input_dataset_paths, job_working_directory
        )
        self.__populate_input_dataset_wrappers(
            param_dict, input_datasets, input_dataset_paths
        )
        self.__populate_output_dataset_wrappers(
            param_dict, output_datasets, output_paths, job_working_directory
        )
        self.__populate_output_collection_wrappers(
            param_dict, output_collections, output_paths, job_working_directory
        )
    ...
    def __populate_wrappers(
        self, param_dict, input_datasets, input_dataset_paths, job_working_directory
    ):
            ...
            if isinstance(input, DataToolParameter) and input.multiple:
                ...
                for (
                    ...
                    conv_ext, converted_dataset = input_values[
                        input.name
                    ].find_conversion_destination(conversion_datatypes)
    ...
    def __populate_input_dataset_wrappers(
        self, param_dict, input_datasets, input_dataset_paths
    ):
        ...
        for name, data in input_datasets.items():
            ...
            if not isinstance(
                ...
                wrapper_kwds = dict(
                    datatypes_registry=self.app.datatypes_registry, tool=self, name=name
                )
                if data:
    ...
    def __populate_output_collection_wrappers(
        self, param_dict, output_collections, output_paths, job_working_directory
    ):
        ...
        for name, out_collection in output_collections.items():
            ...
            for element_identifier, output_def in output_def.outputs.items():
                if not output_def.implicit:
                    ...
                    log.info(
                        "Updating param_dict for %s with %s"
                        % (output_def.name, dataset_wrapper)
                    )

    def __populate_output_dataset_wrappers(
        self, param_dict, output_datasets, output_paths, job_working_directory
    ):
        output_dataset_paths = dataset_path_rewrites(output_paths)
        for name, hda in output_datasets.items():
            ...
            param_dict[name].files_path = os.path.abspath(
                os.path.join(job_working_directory, file_name)
            )
    ...
    def build(self):
        ...
        try:
            ...
            global_tool_errors.add_error(
                self.tool.config_file, "Building Config Files", e
            )
            raise e
        try:
            ...
            # capture and log parsing errors
            global_tool_errors.add_error(
                self.tool.config_file, "Building Param File", e
            )
            raise e
        try:
            ...
            # capture and log parsing errors
            global_tool_errors.add_error(
                self.tool.config_file, "Building Command Line", e
            )
            raise e
        try:
            ...
            global_tool_errors.add_error(
                self.tool.config_file, "Building Environment Variables", e
            )
            raise e
    ...
    def __build_environment_variables(self):
        ...
        for environment_variable_def in self.tool.environment_variables:
            ...
            os.close(fd)
            self.__write_workdir_file(
                config_filename, environment_variable_template, param_dict
            )
        ...
        if tmp_dir:
            for tmp_directory_var in self.tool.tmp_directory_vars:
                environment_variable = dict(
                    name=tmp_directory_var, value='"%s"' % tmp_dir, raw=True
                )
                environment_variables.append(environment_variable)
        ...
        return (
            json.dumps(
                wrapped_json.json_wrap(
                    self.tool.inputs, self.param_dict, handle_files=handle_files
                )
            ),
            False,
        )
