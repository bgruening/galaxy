...
class DataManagers(object):
    def __init__(self, app, xml_filename=None):
        ...
        if self.app.config.shed_data_manager_config_file:
            self.load_from_xml(
                self.app.config.shed_data_manager_config_file, store_tool_path=True
            )
    ...
    def load_from_xml(self, xml_filename, store_tool_path=True):
        ...
        for data_manager_elem in root.findall("data_manager"):
            if not self.load_manager_from_elem(
                data_manager_elem, tool_path=self.tool_path
            ):
    ...
    def load_manager_from_elem(
        ...
        try:
            ...
            log.error(
                "Error loading data_manager '%s':\n%s"
                % (e, util.xml_to_string(data_manager_elem))
            )
            return None
        if add_manager:
    ...
    def add_manager(self, data_manager):
        if data_manager.id in self.data_managers:
            log.warning(
                "A data manager has been defined twice: %s " % (data_manager.id)
            )
                ...
                for data_table_name in data_manager.data_tables.keys():
                    ...
                    for other_data_manager in self.data_managers.values():
                        if data_table_name in other_data_manager.data_tables:
                            ...
                            break
                    if (
                        remove_data_table_tracking
                        and data_table_name in self.managed_data_tables
                    ):
...
class DataManager(object):
    ...
    def load_from_element(self, elem, tool_path):
        assert elem.tag == "data_manager", (
            'A data manager configuration must have a "data_manager" tag as the root. "%s" is present'
            % (elem.tag)
        )
        ...
        if path is None:
            ...
            if (
                hasattr(self.data_managers.app, "tool_cache")
                ...
                if tool_shed_repository is None:
                    ...
                    try:
                        tool_path = (
                            util.parse_xml(elem.get("shed_conf_file"))
                            .getroot()
                            .get("tool_path", tool_path)
                        )
                    except Exception as e:
                        log.error(
        ...
        for data_table_elem in elem.findall("data_table"):
            ...
            assert (
                data_table_name is not None
            ), "A name is required for a data table entry"
                    ...
                    if value_translation_elems is not None:
                        for value_translation_elem in value_translation_elems:
                            ...
                            if value_translation is not None:
                                ...
                                if (
                                    data_table_name
                                    not in self.value_translation_by_data_table_column
                                ):
                                    self.value_translation_by_data_table_column[
                                        data_table_name
                                    ] = {}
                                ...
                                if value_translation_type == "function":
                                    ...
                                    assert (
                                        ...
                                        == DEFAULT_VALUE_TRANSLATION_TYPE
                                    ), ValueError(
                                ...
                                self.value_translation_by_data_table_column[
                                    data_table_name
                                ][data_table_coumn_name].append(value_translation)
    ...
    def id(self):
        return (
            self.guid or self.declared_id
        )  # if we have a guid, we will use that as the data_manager id

    def load_tool(
        self,
        tool_filename,
        guid=None,
        data_manager_id=None,
        tool_shed_repository_id=None,
    ...
    def process_result(self, out_data):
        ...
        for output_name, output_dataset in out_data.items():
            try:
                ...
                log.warning(
                    'Error reading DataManagerTool json for "%s": %s' % (output_name, e)
                )
                continue
        ...
        for data_table_name in self.data_tables.keys():
            ...
            if data_table_name in self.output_ref_by_data_table:
                for data_table_column, output_ref in self.output_ref_by_data_table[
                    ...
                    assert (
                        output_ref_dataset is not None
                    ), "Referenced output was not found."
            ...
            for data_table_row in data_table_values:
                ...
                for (
                    ...
                    if name in output_ref_values:
                        ...
                        data_table_value[name] = self.process_value_translation(
        ...
        if self.undeclared_tables and data_tables_dict:
            ...
            log.debug(
                "Attempting to add entries for undeclared tables: %s.",
                ", ".join(data_tables_dict.keys()),
            )
            for ref_file in out_data.values():
                if ref_file.extra_files_path_exists():
                    util.move_merge(
                        ref_file.extra_files_path,
                        self.data_managers.app.config.galaxy_data_manager_data_path,
                    )
            ...
            for data_table_name, data_table_values in data_tables_dict.items():
                ...
                for data_table_row in data_table_values:
                    ...
                    data_table.add_entry(
                        data_table_value, persist=True, entry_source=self
                    )
                send_control_task(
                    self.data_managers.app,
                    "reload_tool_data_tables",
    ...
    def process_value_translation(self, data_table_name, column_name, **kwd):
        ...
        if (
            data_table_name in self.value_translation_by_data_table_column
            and column_name
            in self.value_translation_by_data_table_column[data_table_name]
        ):
            for value_translation in self.value_translation_by_data_table_column[
                data_table_name
            ][column_name]:
