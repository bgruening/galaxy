...
class UploadDataset(Group):
        ...
        if dataset_name is None:
            ...
            for composite_file in context.get("files", []):
                if not composite_file.get("ftp_files", ""):
                    filenames.append(
                        (composite_file.get("file_data") or {}).get("filename", "")
                    )
                else:
    ...
    def get_file_type(self, context, parent_context=None):
        ...
        if file_type == "":
            if parent_context:
                file_type = parent_context.get(
                    self.file_type_name, self.default_file_type
                )
            else:
    ...
    def title_by_index(self, trans, index, context):
        ...
        for i, (composite_name, composite_file) in enumerate(
            d_type.writable_files.items()
        ):
            if i == index:
                rval = composite_name
    ...
    def get_uploaded_datasets(
        self, trans, context, override_name=None, override_info=None
    ):
        def get_data_file_filename(
            data_file, override_name=None, override_info=None, purge=True
        ):
        ...
        def get_url_paste_urls_or_filename(
            group_incoming, override_name=None, override_info=None
        ):
            ...
            if url_paste_file is not None:
                ...
                if start_of_url(url_paste):
                    ...
                    dataset_name = (
                        "Pasted Entry"
                    )  # we need to differentiate between various url pastes here
                    if override_name:
                        dataset_name = override_name
        ...
        def get_one_filename(context):
            ...
            if file_bunch.path:
                ...
                url_paste is not None and url_paste.strip()
                ...
                ftp_files is not None and trans.user is not None
                        ...
                        for ftp_filename in ftp_files:
                            if ftp_filename == filename:
                                ...
                                if not os.path.islink(os.path.join(dirpath, filename)):
                                    ...
                                    purge = getattr(
                                        trans.app.config, "ftp_upload_purge", True
                                    )
                                    file_bunch = get_data_file_filename(
                                        ftp_data_file,
                                        override_name=name,
        ...
        def get_filenames(context):
            ...
            if ftp_files is not None:
                ...
                if trans.user is None:
                    log.warning(
                        "Anonymous user passed values in ftp_files: %s" % ftp_files
                    )
                    ftp_files = []
            ...
            for ftp_file in ftp_files:
                if ftp_file not in valid_files:
                    log.warning(
                        "User passed an invalid file path in ftp_files: %s" % ftp_file
                    )
                    continue
                    # TODO: warning to the user (could happen if file is already imported)
                ftp_data_file = {
                    "local_filename": os.path.abspath(
                        os.path.join(user_ftp_dir, ftp_file)
                    ),
                    "filename": os.path.basename(ftp_file),
                }
                purge = getattr(trans.app.config, "ftp_upload_purge", True)
                file_bunch = get_data_file_filename(
                    ftp_data_file, override_name=name, override_info=info, purge=purge
                )
                if file_bunch.path:
                    file_bunch.to_posix_lines = to_posix_lines
                    file_bunch.auto_decompress = auto_decompress
        ...
        if d_type.composite_type is not None or force_composite:
            ...
            if (
                dataset.primary_file is None
            ):  # remove this before finish, this should create an empty dataset
                raise Exception(
            ...
            for i, group_incoming in enumerate(groups_incoming[writable_files_offset:]):
                ...
                if (
                    ...
                    and not writable_files[
                        list(writable_files.keys())[keys.index(key)]
                    ].optional
                ):
                    dataset.warnings.append(
                        "A required composite file (%s) was not specified." % (key)
                    )
            ...
            for i, file_contexts in enumerate(context[self.name]):
                ...
                for dataset in datasets:
                    ...
                    d_type = self.get_datatype(
                        trans, context[self.name][i], parent_context=context
                    )
                    dataset.file_type = override_file_type
                    dataset.datatype = d_type
                    dataset.ext = self.get_datatype_ext(
                        trans, context[self.name][i], parent_context=context
                    )
                    dataset.dbkey = self.get_dbkey(
                        context[self.name][i], parent_context=context
                    )
...
class Conditional(Group):
    ...
    def __init__(self):
        ...
        self.cases = []
        self.value_ref = None
        self.value_ref_in_group = (
            True
        )  # When our test_param is not part of the conditional Group, this is False
