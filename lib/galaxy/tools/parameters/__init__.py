...
def visit_input_values(
    ...
    for input in inputs.values():
        if isinstance(input, Repeat) or isinstance(input, UploadDataset):
            ...
            case_error = (
                None
                if get_current_case(input, values) >= 0
                else "The selected case is unavailable/invalid."
            )
            callback_helper(
                input.test_param,
                values,
...
def params_to_incoming(incoming, inputs, input_values, app, name_prefix=""):
    ...
    for input in inputs.values():
        if isinstance(input, Repeat) or isinstance(input, UploadDataset):
            ...
            new_name_prefix = name_prefix + input.name + "|"
            incoming[new_name_prefix + input.test_param.name] = values[
                input.test_param.name
            ]
            params_to_incoming(
                incoming, input.cases[current].inputs, values, app, new_name_prefix
            )
        elif isinstance(input, Section):
            values = input_values[input.name]
            new_name_prefix = name_prefix + input.name + "|"
