...
class ToolBox(BaseGalaxyToolBox):
    ...
    def __init__(self, config_filenames, tool_root_dir, app):
        ...
        super(ToolBox, self).__init__(
            config_filenames=config_filenames, tool_root_dir=tool_root_dir, app=app
        )
    ...
    def create_tool(self, config_file, **kwds):
        try:
            ...
            raise e
        return self._create_tool_from_source(
            tool_source, config_file=config_file, **kwds
        )
    ...
    def create_dynamic_tool(self, dynamic_tool, **kwds):
        ...
        tool_source = get_tool_source_from_representation(
            tool_format=tool_format, tool_representation=tool_representation
        )
        kwds["dynamic"] = True
    ...
    def get_tool_components(
        ...
        for tool_id in tool_ids:
            if get_loaded_tools_by_lineage:
                ...
                tools = toolbox.get_tool(
                    tool_id, tool_version=tool_version, get_all_versions=True
                )
            if tools:
                tool = toolbox.get_tool(
                    tool_id, tool_version=tool_version, get_all_versions=False
                )
                if len(tools) > 1:
                    tool_version_select_field = self.__build_tool_version_select_field(
                        tools, tool.id, set_selected
                    )
                break
    ...
    def _get_tool_shed_repository(
        self, tool_shed, name, owner, installed_changeset_revision
    ):
        ...
        for option_tup in options:
            ...
            if selected:
                select_field.add_option(
                    "version %s" % option_tup[0], option_tup[1], selected=True
                )
            else:
...
class DefaultToolState(object):
    ...
    def decode(self, values, tool, app):
        ...
        self.rerun_remap_job_id = (
            values.pop("__rerun_remap_job_id__")
...
class Tool(Dictifiable):
    ...
    def requires_galaxy_python_environment(self):
        ...
        if preserve_python_environment == "always":
            ...
            preserve_python_environment == "legacy_and_local" and self.tool_shed is None
        ):
            return True
        else:
    ...
    def __get_job_tool_configuration(self, job_params=None):
            ...
            for job_tool_config in self.job_tool_configurations:
                if job_tool_config.params:
                    ...
                    for param, value in job_params.items():
                        if (
                            param not in job_tool_config.params
                            or job_tool_config.params[param] != value
                        ):
                            break
                    else:
        ...
        assert rval is not None, (
            "Could not get a job tool configuration for Tool %s with job_params %s, this is a bug"
            % (self.id, job_params)
        )
        return rval
    ...
    def get_job_destination(self, job_params=None):
        ...
        """
        return self.app.job_config.get_destination(
            self.__get_job_tool_configuration(job_params=job_params).destination
        )
    ...
    def parse(self, tool_source, guid=None, dynamic=False):
        ...
        if (
            profile >= packaging.version.parse("16.04")
        ...
        if not dynamic and not self.name:
            raise Exception(
                "Missing tool 'name' for tool with id '%s' at '%s'"
                % (self.id, tool_source)
            )
        ...
        if not self.version:
            if self.profile < 16.04:
                ...
                raise Exception(
                    "Missing tool 'version' for tool with id '%s' at '%s'"
                    % (self.id, tool_source)
                )
        ...
        self.display_interface = tool_source.parse_display_interface(
            default=self.display_interface
        )

        self.require_login = tool_source.parse_require_login(self.require_login)

        request_param_translation_elem = (
            tool_source.parse_request_param_translation_elem()
        )
        if request_param_translation_elem is not None:
            ...
            self.input_translator = ToolInputTranslator.from_element(
                request_param_translation_elem
            )
        else:
        ...
        if version_command is not None:
            ...
            if version_cmd_interpreter:
                ...
                abs_executable = os.path.abspath(
                    os.path.join(self.tool_dir, executable)
                )
                command_line = self.version_string_cmd.replace(
                    executable, abs_executable, 1
                )
        ...
        if hasattr(self.app, "job_config"):
            self.job_tool_configurations = self.app.job_config.get_job_tool_configurations(
                self_ids
            )
        ...
        self.options = dict(
            sanitize=tool_source.parse_sanitize(), refresh=tool_source.parse_refresh()
        )
        ...
        if action is None:
            ...
            if getattr(self.tool_action, "requires_js_runtime", False):
                try:
                    ...
                    message = (
                        REQUIRES_JS_RUNTIME_MESSAGE % self.tool_id or self.tool_uuid
                    )
                    raise Exception(message)
        # Tests
    ...
    def tool_provided_metadata(self, job_wrapper):
        meta_file = os.path.join(
            job_wrapper.tool_working_directory, self.provided_metadata_file
        )
        return parse_tool_provided_metadata(
            meta_file,
            provided_metadata_style=self.provided_metadata_style,
    ...
    def parse_inputs(self, tool_source):
        ...
        if pages.inputs_defined:
            if hasattr(pages, "input_elem"):
                ...
                self.check_values = string_as_bool(
                    input_elem.get("check_values", self.check_values)
                )
                self.nginx_upload = string_as_bool(
                    input_elem.get("nginx_upload", self.nginx_upload)
                )
                ...
                if self.nginx_upload and self.app.config.nginx_upload_path:
                    ...
                    self.action = (
                        self.app.config.nginx_upload_path + "?nginx_redir=",
    ...
    def parse_input_elem(self, page_source, enctypes, context=None):
        ...
        for input_source in page_source.parse_input_sources():
            ...
            if input_type == "repeat":
                ...
                assert group.min <= group.max, ValueError(
                    "Min repeat count must be less-than-or-equal to the max."
                )
                ...
                group.value_ref_in_group = input_source.get_bool(
                ...
                if value_from:
                    ...
                    for case_value, case_inputs in group.value_from(
                        context, group, self
                    ).items():
                        case = ConditionalWhen()
                        case.value = case_value
                        if case_inputs:
                            page_source = XmlPageSource(
                                ElementTree.XML("<when>%s</when>" % case_inputs)
                            )
                            case.inputs = self.parse_input_elem(
                                page_source, enctypes, context
                            )
                        else:
                            case.inputs = odict()
                    ...
                    group.test_param = self.parse_param_elem(
                        test_param_input_source, enctypes, context
                    )
                    if group.test_param.optional:
                        log.warning(
                            "Tool with id %s declares a conditional test parameter as optional, this is invalid and will be ignored."
                            % self.id
                        )
                        group.test_param.optional = False
                    possible_cases = list(
                        group.test_param.legal_values
                    ...
                    for (
                        value,
                        case_inputs_source,
                    ) in input_source.parse_when_input_sources():
                        case = ConditionalWhen()
                        case.value = value
                        case.inputs = self.parse_input_elem(
                            case_inputs_source, enctypes, context
                        )
                        group.cases.append(case)
                        try:
                            ...
                            log.warning(
                                "Tool %s: a when tag has been defined for '%s (%s) --> %s', but does not appear to be selectable."
                                % (
                                    self.id,
                                    group.name,
                                    group.test_param.name,
                                    case.value,
                                )
                            )
                    for unspecified_case in possible_cases:
                        log.warning(
                            "Tool %s: a when tag has not been defined for '%s (%s) --> %s', assuming empty inputs."
                            % (
                                self.id,
                                group.name,
                                group.test_param.name,
                ...
                group.default_file_type = elem.get(
                    "default_file_type", group.default_file_type
                )
    ...
    def parse_param_elem(self, input_source, enctypes, context):
        ...
        for name in param.get_dependencies():
            ...
            if name not in context:
                log.error(
                    "Could not find dependency '%s' of parameter '%s' in tool %s"
    ...
    def __inititalize_help(self):
        ...
        if help_text is not None:
            try:
                if help_text.find(".. image:: ") >= 0 and (
                    self.tool_shed_repository or self.repository_id
                ):
                ...
                log.exception(
                    "Exception in parse_help, so images may not be properly displayed"
                )
            try:
                ...
                log.exception(
                    "Exception while parsing help for tool with id '%s'", self.id
                )
            ...
            if hasattr(tool_source, "root"):
                ...
                try:
                    ...
                    log.exception(
                        "Exception while parsing multi-page help for tool with id '%s'",
                        self.id,
                    )
    ...
    def expand_incoming(self, trans, incoming, request_context):
        ...
        if "rerun_remap_job_id" in incoming:
            try:
                rerun_remap_job_id = trans.app.security.decode_id(
                    incoming["rerun_remap_job_id"]
                )
            except Exception as exception:
                log.error(str(exception))
                raise exceptions.MessageException(
                    "Failure executing tool (attempting to rerun invalid job)."
                )
        ...
        for expanded_incoming in expanded_incomings:
            ...
            if not self.check_values:
                ...
                populate_state(
                    request_context, self.inputs, expanded_incoming, params, errors
                )
        ...
        log.debug(
            "Validated and populated state for tool request %s" % validation_timer
        )
    ...
    def handle_input(self, trans, incoming, history=None, use_cached_job=False):
        ...
        request_context = WorkRequestContext(
            app=trans.app, user=trans.user, history=history or trans.history
        )
        all_params, all_errors, rerun_remap_job_id, collection_info = self.expand_incoming(
            trans=trans, incoming=incoming, request_context=request_context
        )
        ...
        if any(all_errors):
            ...
            raise exceptions.MessageException(
                ", ".join(msg for msg in err_data.values()), err_data=err_data
            )
        else:
            ...
            raise_execution_exception = (
                not execution_tracker.successful_jobs and len(all_params) > 0
            )
    ...
    def handle_single_execution(
        ...
        if isinstance(out_data, odict):
            ...
            if isinstance(out_data, string_types):
                ...
                message = (
                    "Failure executing tool (invalid data returned from tool execution)"
                )
            return False, message
    ...
    def params_with_missing_data_table_entry(self):
        ...
        for input_param in self.input_params:
            if isinstance(input_param, SelectToolParameter) and input_param.is_dynamic:
                ...
                if (
                    options
                    and options.missing_tool_data_table_name
    ...
    def execute(self, trans, incoming={}, set_output_hid=True, history=None, **kwargs):
        ...
        try:
            ...
            if job is not None:
                job.mark_failed(
                    info=exc.err_msg, blurb=exc.err_code.default_error_message
                )
                job_id = job.id
    ...
    def check_and_update_param_values(
        self, values, trans, update_values=True, workflow_building_mode=False
    ):
        """
                    ...
                    try:
                        ...
                        if not prefixed_name.startswith("__"):
                            messages[prefixed_name] = (
                                ...
                                else "%s Using default: '%s'." % (error, value)
                            )
                        parent[input.name] = value
                    except Exception:
                        messages[prefixed_name] = (
                            "Attempt to replace invalid value for '%s' failed."
                            % (prefixed_label)
                        )
                else:
    ...
    def installed_tool_dependencies(self):
        if self.tool_shed_repository:
            installed_tool_dependencies = (
                self.tool_shed_repository.tool_dependencies_installed_or_in_error
            )
        else:
    ...
    def tool_requirements_status(self):
        ...
        return self._view.get_requirements_status(
            {self.id: self.tool_requirements}, self.installed_tool_dependencies
        )
    ...
    def build_redirect_url_params(self, param_dict):
        ...
        redirect_url_params = fill_template(
            self.redirect_url_params, context=param_dict
        )
        # Remove newlines
    ...
    def call_hook(self, hook_name, *args, **kwargs):
        ...
        try:
            ...
            e.args = (
                "Error in '%s' hook '%s', original message: %s"
                % (self.name, hook_name, original_message),
            )
            raise
    ...
    def discover_outputs(
        ...
        """
        tool = self
        permission_provider = output_collect.PermissionProvider(
            inp_data, tool.app.security_agent, job
        )
        ...
        collected = output_collect.collect_primary_datasets(
            job_context, out_data, input_ext
        )
        output_collect.collect_dynamic_outputs(job_context, out_collections)
    ...
    def to_archive(self):
            ...
            for help_line in tool_help.split("\n"):
                ...
                if matches is not None:
                    ...
                    filesystem_path = os.path.abspath(
                        os.path.join(self.app.config.root, "static", tool_help_image)
                    )
                    if os.path.exists(filesystem_path):
                        tarball_files.append((filesystem_path, tarball_path))
                        image_found = True
                        tool_xml = tool_xml.replace(
                            "${static_path}/%s" % tarball_path, tarball_path
                        )
        ...
        if image_found:
            ...
            tool_tup = (
                os.path.abspath(new_tool_config),
                os.path.split(tool.config_file)[-1],
            )
            temp_files.append(os.path.abspath(new_tool_config))
        else:
            tool_tup = (
                os.path.abspath(tool.config_file),
                os.path.split(tool.config_file)[-1],
            )
        ...
        for external_file in tool.get_externally_referenced_paths(
            os.path.abspath(tool.config_file)
        ):
            external_file_abspath = os.path.abspath(
                os.path.join(tool_path, external_file)
            )
            ...
            for test in tests:
                ...
                for input in test.inputs:
                    for input_value in test.inputs[input]:
                        ...
                        input_path = os.path.abspath(
                            os.path.join("test-data", input_filename)
                        )
                ...
                for label, filename, _ in test.outputs:
                    output_filepath = os.path.abspath(
                        os.path.join("test-data", filename)
                    )
                ...
                if hasattr(param.options, "tool_data_table"):
                    ...
                    if hasattr(data_table, "filenames"):
                        ...
                        for data_table_filename in data_table.filenames:
                            ...
                            if not data_table.filenames[data_table_filename][
                                "from_shed_config"
                            ]:
                                tar_file = (
                                    data_table.filenames[data_table_filename][
                                        "filename"
                                    ]
                                    + ".sample"
                                )
    ...
    def to_dict(self, trans, link_details=False, io_details=False):
        ...
        if trans.user_is_admin:
            config_file = (
                None if not self.config_file else os.path.abspath(self.config_file)
            )
        ...
        if link_details:
            ...
            if not isinstance(self, DataSourceTool):
                ...
                link = self.app.url_for(
                    controller="tool_runner",
                    action="data_source_redirect",
                    tool_id=self.id,
                )
        ...
        if io_details:
            tool_dict["inputs"] = [
                input.to_dict(trans) for input in self.inputs.values()
            ]
            tool_dict["outputs"] = [
                output.to_dict(app=self.app) for output in self.outputs.values()
            ]

        tool_dict["panel_section_id"], tool_dict[
            "panel_section_name"
        ] = self.get_panel_section()
    ...
    def to_json(self, trans, kwd={}, job=None, workflow_building_mode=False):
        ...
        if (
            ...
            try:
                ...
                raise exceptions.MessageException(
                    "[history_id=%s] Failed to retrieve history. %s."
                    % (history_id, str(e))
                )
        ...
        if self.help:
            tool_help = self.help.render(
                static_path=self.app.url_for("/static"),
                host_url=self.app.url_for("/", qualified=True),
            )
    ...
    def populate_model(
        self, request_context, inputs, state_inputs, group_inputs, other_values=None
    ):
        """
            ...
            if input.type == "repeat":
                ...
                if "test_param" in tool_dict:
                    ...
                    test_param["value"] = input.test_param.value_to_basic(
                        group_state.get(
                            test_param["name"],
                            input.test_param.get_initial_value(
                                request_context, other_values
                            ),
                        ),
                        self.app,
                    )
                    test_param["text_value"] = input.test_param.value_to_display_text(
                        test_param["value"]
                    )
                ...
                try:
                    initial_value = input.get_initial_value(
                        request_context, other_values
                    )
                    tool_dict = input.to_dict(
                        request_context, other_values=other_values
                    )
                    tool_dict["value"] = input.value_to_basic(
                        state_inputs.get(input.name, initial_value),
                        self.app,
                        use_security=True,
                    )
                    tool_dict["default_value"] = input.value_to_basic(
                        initial_value, self.app, use_security=True
                    )
                    tool_dict["text_value"] = input.value_to_display_text(
                        tool_dict["value"]
                    )
                except Exception:
                    tool_dict = input.to_dict(request_context)
                    log.exception(
                        "tools::to_json() - Skipping parameter expansion '%s'",
                        input.name,
                    )
                    pass
    ...
    def _compare_tool_version(self, job):
        ...
        try:
            ...
            if (
                self.id != tool_id and self.old_id != tool_id
            ) or self.version != tool_version:
                if self.id == tool_id:
                    if tool_version:
                        message = (
                            'This job was run with tool version "%s", which is not available. '
                            % tool_version
                        )
                        if len(tools) > 1:
                    ...
                    new_tool_shed_url = "%s/%s/" % (
                        tool.sharable_url,
                        tool.changeset_revision,
                    )
                    old_tool_shed_url = common_util.get_tool_shed_url_from_tool_shed_registry(
                        self.app, tool_id.split("/repos/")[0]
                    )
                    old_tool_shed_url = "%s/view/%s/%s/" % (
                        old_tool_shed_url,
                        tool.repository_owner,
                        tool.repository_name,
                    )
                    message = (
                        'This job was run with <a href="%s" target="_blank">tool id "%s"</a>, version "%s", which is not available. '
                        % (old_tool_shed_url, tool_id, tool_version)
                    )
                    if len(tools) > 1:
                        message += (
                            'You can re-run the job with the selected <a href="%s" target="_blank">tool id "%s"</a> or choose another derivation of the tool. '
                            % (new_tool_shed_url, self.id)
                        )
                    else:
                        message += (
                            'You can re-run the job with <a href="%s" target="_blank">tool id "%s"</a>, which is a derivation of the original tool. '
                            % (new_tool_shed_url, self.id)
                        )
...
class ExpressionTool(Tool):
    ...
    def parse_outputs(self, tool_source):
        ...
        if len(self.output_collections) != 0:
            message = (
                "Expression tools may not declare output collections at this time."
            )
            raise Exception(message)
        for output in self.outputs.values():
            if not hasattr(output, "from_expression"):
                message = (
                    "Expression tools may not declare output datasets at this time."
                )
                raise Exception(message)

    def exec_before_job(self, app, inp_data, out_data, param_dict=None):
        super(ExpressionTool, self).exec_before_job(
            app, inp_data, out_data, param_dict=param_dict
        )
        local_working_directory = param_dict["__local_working_directory__"]
        expression_inputs_path = os.path.join(
            local_working_directory, ExpressionTool.EXPRESSION_INPUTS_NAME
        )
    ...
    def parse_environment_variables(self, tool_source):
        ...
        """
        environmnt_variables_raw = super(
            ExpressionTool, self
        ).parse_environment_variables(tool_source)
...
class DataSourceTool(OutputParameterJSONTool):
    ...
    def _build_GALAXY_URL_parameter(self):
        return ToolParameter.build(
            self,
            ElementTree.XML(
                '<param name="GALAXY_URL" type="baseurl" value="/tool_runner?tool_id=%s" />'
                % self.id
            ),
        )
...
class AsyncDataSourceTool(DataSourceTool):
    ...
    def _build_GALAXY_URL_parameter(self):
        return ToolParameter.build(
            self,
            ElementTree.XML(
                '<param name="GALAXY_URL" type="baseurl" value="/async/%s" />' % self.id
            ),
        )
...
class SetMetadataTool(Tool):
    ...
    def exec_after_process(self, app, inp_data, out_data, param_dict, job=None):
        ...
        for name, dataset in inp_data.items():
            ...
            if external_metadata.external_metadata_set_successfully(
                dataset, name, sa_session, working_directory=working_directory
            ):
                external_metadata.load_metadata(
                    dataset, name, sa_session, working_directory=working_directory
                )
            else:
    ...
    def job_failed(self, job_wrapper, message, exception=False):
        ...
        if job:
            ...
            return self.exec_after_process(
                job_wrapper.app, inp_data, {}, job_wrapper.get_param_dict(), job=job
            )
...
class DataManagerTool(OutputParameterJSONTool):
    ...
    def exec_after_process(self, app, inp_data, out_data, param_dict, job=None, **kwds):
        assert self.allow_user_access(
            job.user
        ), "You must be an admin to access this tool."
        # run original exec_after_process
        super(DataManagerTool, self).exec_after_process(
            app, inp_data, out_data, param_dict, job=job, **kwds
        )
        ...
        assert data_manager is not None, (
            "Invalid data manager (%s) requested. It may have been removed before the job completed."
            % (data_manager_id)
        )
    ...
    def get_default_history_by_trans(self, trans, create=False):
        def _create_data_manager_history(user):
            history = trans.app.model.History(
                name="Data Manager History (automatically created)", user=user
            )
            data_manager_association = trans.app.model.DataManagerHistoryAssociation(
    ...
    def allow_user_access(self, user, attempting_access=True):
        ...
        """
        if super(DataManagerTool, self).allow_user_access(
            user
        ) and self.app.config.is_admin_user(user):
            return True
...
class UnzipCollectionTool(DatabaseOperationTool):
    ...
    def produce_outputs(
        self, trans, out_data, output_collections, incoming, history, tags=None
    ):
        ...
        forward, reverse = (
            forward_o.copy(copy_tags=tags),
            reverse_o.copy(copy_tags=tags),
        )
...
class ZipCollectionTool(DatabaseOperationTool):
    ...
    def produce_outputs(
        self, trans, out_data, output_collections, incoming, history, **kwds
    ):
...
class ExtractDatasetCollectionTool(DatabaseOperationTool):
    ...
    def produce_outputs(
        ...
        extracted_o = extracted.copy(
            copy_tags=tags, new_name=extracted_element.element_identifier
        )
        ...
        for input_num, input_list in enumerate(input_lists):
            for dce in input_list.collection.elements:
                ...
                if element_identifier not in identifiers_map:
                    ...
                    raise Exception(
                        "Duplicate collection element identifiers found for [%s]"
                        % element_identifier
                    )
            ...
            for dce in input_list.collection.elements:
                ...
                if valid:
                    ...
                    if dupl_actions == "suffix_every":
                        ...
                        dupl_actions == "suffix_conflict_rest"
                        and len(appearances) > 1
                        and appearances[0] != copy
                    ):
                        add_suffix = True
...
class FilterDatasetsTool(DatabaseOperationTool):
    ...
    def produce_outputs(
        ...
        new_elements = self._get_new_elements(
            history=history, elements_to_copy=elements_to_copy
        )
        self._add_datasets_to_history(history, itervalues(new_elements))
        output_collections.create_collection(
            next(iter(self.outputs.values())), "output", elements=new_elements
    ...
    def produce_outputs(
        ...
        def add_elements(collection, prefix=""):
            for dce in collection.elements:
                ...
                identifier = (
                    "%s%s%s" % (prefix, join_identifier, dce_identifier)
                    if prefix
                    else dce_identifier
                )
...
class SortTool(DatabaseOperationTool):
    ...
    def produce_outputs(
        ...
        if sorttype == "alpha":
            ...
            presort_elements = [
                (int(re.sub("[^0-9]", "", dce.element_identifier)), dce)
                for dce in elements
            ]
        if presort_elements:
            sorted_elements = [
                x[1] for x in sorted(presort_elements, key=lambda x: x[0])
            ]
        if sorttype == "file":
            ...
            if data_lines == len(elements):
                ...
                try:
                    sorted_elements = [
                        old_elements_dict[line.strip()] for line in open(hda.file_name)
                    ]
                except KeyError:
                    hdca_history_name = "%s: %s" % (hdca.hid, hdca.name)
                    message = (
                        "List of element identifiers does not match element identifiers in collection '%s'"
                        % hdca_history_name
                    )
                    raise Exception(message)
            else:
    ...
    def produce_outputs(
        ...
        def add_copied_value_to_new_elements(new_label, dce_object):
            ...
            if new_label in new_elements:
                raise Exception(
                    "New identifier [%s] appears twice in resulting collection, these values must be unique."
                    % new_label
                )
        ...
        if strict and len(hdca.collection.elements) != len(new_labels):
            raise Exception(
                "Relabel mapping file contains incorrect number of identifiers"
            )
        if how_type == "tabular":
            ...
            new_labels_dict = {
                source: new_label for source, new_label in source_new_label
            }
            for i, dce in enumerate(hdca.collection.elements):
                ...
                if not new_label:
                    raise Exception(
                        "Failed to find new label for identifier [%s]"
                        % element_identifier
                    )
            ...
            if getattr(dce.element_object, "history_content_type", None) == "dataset":
                ...
                if new_tags:
                    if how in ("add", "remove") and dce.element_object.tags:
                        ...
                        old_tags = set(
                            tag
                            for tag in tags_manager.get_tags_str(
                                dce.element_object.tags
                            ).split(",")
                            if tag
                        )
                        if how == "add":
                        ...
                        new_tags = old_tags
                    tags_manager.add_tags_from_list(
                        user=history.user, item=copied_value, new_tags_list=new_tags
                    )
            else:
                ...
                for new_element, old_element in zip(
                    copied_value.dataset_elements, dce.element_object.dataset_elements
                ):
                    ...
                    if how in ("add", "remove"):
                        old_tags = set(
                            tag
                            for tag in tags_manager.get_tags_str(
                                old_element.element_object.tags
                            ).split(",")
                            if tag
                        )
                        if new_tags:
                    ...
                    tags_manager.add_tags_from_list(
                        user=history.user,
                        item=new_element.element_object,
...
class FilterFromFileTool(DatabaseOperationTool):
    ...
    def produce_outputs(
        ...
        for i, dce in enumerate(hdca.collection.elements):
            ...
            passes_filter = (
                in_filter_file
                if how_filter == "remove_if_absent"
                else not in_filter_file
            )
