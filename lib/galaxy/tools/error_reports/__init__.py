...
class ErrorReports(object):
    ...
    def __plugins_dict(self):
        ...
        return plugin_config.plugins_dict(
            galaxy.tools.error_reports.plugins, "plugin_type"
        )
...
class ErrorPlugin(object):
    ...
    def submit_report(
        self, dataset, job, tool, user=None, user_submission=False, **kwargs
    ):
        if user_submission:
            assert self._can_access_dataset(dataset, user), Exception(
        ...
        for plugin in self.plugins:
            if user_submission == plugin.user_submission:
                try:
                    ...
                    log.debug(
                        "Bug report plugin %s generated response %s", plugin, response
                    )
