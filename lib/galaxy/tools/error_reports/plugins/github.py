...
class GithubPlugin(BaseGitPlugin):
    ...
    def __init__(self, **kwargs):
        ...
        self.git_default_repo_only = string_as_bool(
            kwargs.get("github_default_repo_only", True)
        )
        ...
        try:
            ...
            repo = self.github.get_repo(
                "%s/%s" % (self.git_default_repo_owner, self.git_default_repo_name)
            )
    ...
    def submit_report(self, dataset, job, tool, **kwargs):
        ...
        if self.github:
            ...
            if github_projecturl not in self.git_project_cache:
                self.git_project_cache[github_projecturl] = self.github.get_repo(
                    "%s" % github_projecturl
                )
            ...
            label = self.get_label(
                "%s/%s" % (unicodify(job.tool_id), unicodify(job.tool_version)),
                gh_project,
                issue_cache_key,
            )
            ...
            if error_title not in self.issue_cache[issue_cache_key]:
                # Create a new issue.
                self._create_issue(
                    issue_cache_key, error_title, error_message, gh_project, label=label
                )
            else:
    ...
    def get_label(self, label, git_project, issue_cache_key):
        ...
        if label not in self.label_cache[issue_cache_key]:
            self.label_cache[issue_cache_key][label] = git_project.create_label(
                name=label, color="ffffff"
            )
