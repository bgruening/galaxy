...
class GitLabPlugin(BaseGitPlugin):
    ...
    def __init__(self, **kwargs):
        self.app = kwargs["app"]
        self.redact_user_details_in_bugreport = (
            self.app.config.redact_user_details_in_bugreport
        )
        ...
        self.git_default_repo_only = string_as_bool(
            kwargs.get("gitlab_default_repo_only", True)
        )
        ...
        try:
            ...
            log.error(
                "GitLab error reporting - Please install python-gitlab to submit bug reports to GitLab."
            )
            self.gitlab = None
    ...
    def submit_report(self, dataset, job, tool, **kwargs):
        ...
        if self.gitlab:
            ...
            try:
                log.info(
                    "GitLab error reporting - submit report - job tool id: %s - job tool version: %s - tool tool_shed: %s"
                    % (job.tool_id, job.tool_version, tool.tool_shed)
                )
                ...
                log.info(
                    "GitLab error reporting - Determine ToolShed Repository URL: %s",
                    ts_repourl,
                )

                # Determine the GitLab project URL and the issue cache key
                gitlab_projecturl = (
                    urlparse.urlparse(ts_repourl).path[1:]
                    if (ts_repourl and not self.git_default_repo_only)
                    else "/".join(
                        [self.git_default_repo_owner, self.git_default_repo_name]
                    )
                )
                ...
                if error_title not in self.issue_cache[issue_cache_key]:
                    # Create a new issue.
                    self._create_issue(
                        issue_cache_key, error_title, error_message, gl_project
                    )
                else:
                ...
                log.error(
                    "GitLab error reporting - Could not connect to GitLab. Exception information: "
                    + e.message
                )
                return ("Internal Error.", "danger")
            ...
            log.error(
                "GitLab error reporting - No connection to GitLab. Cannot report error to GitLab."
            )
            return ("Internal Error.", "danger")

    def _create_issue(
        self, issue_cache_key, error_title, error_mesage, project, **kwargs
    ):
        # Create the issue on GitLab
        issue = project.issues.create(
            {"title": error_title, "description": error_mesage}
        )
