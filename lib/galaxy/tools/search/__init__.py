        ...
        for tool_id in tool_cache._new_tool_ids:
            ...
            if tool:
                add_doc_kwds = self._create_doc(
                    tool_id=tool_id, tool=tool, index_help=index_help
                )
    ...
    def search(
        ...
        if tool_enable_ngram_search is True or tool_enable_ngram_search == "True":
            ...
            for query in ngrams:
                ...
                curr_hits = searcher.search(
                    parser.parse("*" + query + "*"), limit=float(tool_search_limit)
                )
                for i, curr_hit in enumerate(curr_hits):
                    ...
                    for prev_hit in hits_with_score:
                        ...
                        if curr_hit["id"] == prev_hit:
                            ...
                            hits_with_score[prev_hit] = (
                                curr_hits.score(i) + hits_with_score[prev_hit]
                            )
            ...
            hits = searcher.search(
                parser.parse("*" + q + "*"), limit=float(tool_search_limit)
            )
