...
def assert_has_text(output, text):
    ...
    assert (
        output.find(text) >= 0
    ), "Output file did not contain expected text '%s' (output '%s')" % (text, output)
...
def assert_has_text_matching(output, expression):
    ...
    assert match is not None, (
        "No text matching expression '%s' was found in output file." % expression
    )
...
def assert_has_line_matching(output, expression):
    ...
    assert match is not None, (
        "No line matching expression '%s' was found in output file." % expression
    )
