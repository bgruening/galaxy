    ...
    if filename is not None:
        ...
        if keep_outputs_dir:
            ...
            try:
                ...
                error_log_msg = "Could not save output file %s to %s: " % (
                    temp_name,
                    ofn,
                )
            ...
            if compare == "diff":
                ...
                if abs(s1 - s2) > int(delta):
                    raise AssertionError(
                        "Files %s=%db but %s=%db - compare by size (delta=%s) failed"
                        % (temp_name, s1, local_name, s2, delta)
                    )
...
def _bam_to_sam(local_name, temp_name):
    temp_local = tempfile.NamedTemporaryFile(
        suffix=".sam", prefix="local_bam_converted_to_sam_"
    )
    fd, temp_temp = tempfile.mkstemp(
        suffix=".sam", prefix="history_bam_converted_to_sam_"
    )
    os.close(fd)
    try:
...
def _verify_checksum(data, checksum_type, expected_checksum_value):
    if checksum_type not in ["md5", "sha1", "sha256", "sha512"]:
        raise Exception(
            "Unimplemented hash algorithm [%s] encountered." % checksum_type
        )
    ...
    if not filecmp.cmp(file1, file2, shallow=False):
        ...
        try:
            ...
            if file1.endswith(".pdf") or file2.endswith(".pdf"):
                ...
                history_data = [
                    unicodify(l)
                    for l in get_fileobj(
                        file2, mode="rb", compressed_formats=compressed_formats
                    )
                ]
                local_file = [
                    unicodify(l)
                    for l in get_fileobj(
                        file1, mode="rb", compressed_formats=compressed_formats
                    )
                ]
            else:
        ...
        diff = list(
            difflib.unified_diff(local_file, history_data, "local_file", "history_data")
        )
...
def files_re_match_multiline(file1, file2, attributes=None):
    ...
    assert re.match(
        local_file, history_data, re.MULTILINE
...
def files_contains(file1, file2, attributes=None):
    ...
    for contains in local_file:
        ...
        if line_diff_count > lines_diff:
            raise AssertionError(
                "Failed to find '%s' in history data. (lines_diff=%i):\n"
                % (contains, lines_diff)
            )
