...
APP_DESCRIPTION = (
    """Application to target for operation (i.e. galaxy, tool_shed, or reports))"""
)
...
EXTRA_SERVER_MESSAGE = (
    "Additional server section after [%s] encountered [%s], will be ignored."
)
MISSING_FILTER_TYPE_MESSAGE = (
    "Missing filter type for section [%s], it will be ignored."
)
UNHANDLED_FILTER_TYPE_MESSAGE = (
    "Unhandled filter type encountered [%s] for section [%s]."
)
NO_APP_MAIN_MESSAGE = (
    "No app:main section found, using application defaults throughout."
)
...
class _DeprecatedAction(_OptionAction):
    def lint(self, args, app_desc, key, value):
        print(
            "Option [%s] has been deprecated, this will likely be dropped in future releases of Galaxy."
            % key
        )
...
OPTION_ACTIONS = {
    ...
    "trust_ipython_notebook_conversion": _RenameAction(
...
def _arg_parser():
    ...
    parser.add_argument(
        "--dry-run", default=False, action="store_true", help=DRY_RUN_DESCRIPTION
    )
...
def _validate(args, app_desc):
    ...
    c = Core(source_file=path, schema_files=[name])
    c.validate()
...
def _build_sample_yaml(args, app_desc):
    ...
    if description:
        ...
        f.write(as_comment)
    _write_sample_section(
        args, f, "uwsgi", Schema(UWSGI_OPTIONS), as_comment=False, uwsgi_hack=True
    )
...
def _write_sample_section(
    args, f, section_header, schema, as_comment=True, uwsgi_hack=False
):
    ...
    for key, value in schema.app_schema.items():
        ...
        _write_option(
            args, f, key, option_value, as_comment=as_comment, uwsgi_hack=uwsgi_hack
        )
...
def _write_option(args, f, key, option_value, as_comment=False, uwsgi_hack=False):
    ...
    if uwsgi_hack:
        ...
        key_val_str = (
            yaml.dump({key: value}, width=float("inf")).lstrip("{").rstrip("\n}")
        )
