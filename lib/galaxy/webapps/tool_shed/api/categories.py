...
class CategoriesController(BaseAPIController):
    ...
    def create(self, trans, payload, **kwd):
        ...
        if name:
            ...
            if suc.get_category_by_name(self.app, name):
                ...
                category_dict["message"] = "Category '%s' has been created" % str(
                    category.name
                )
    ...
    def get_repositories(self, trans, category_id, **kwd):
        ...
        if category is None:
            category_dict = dict(
                message="Unable to locate category record for id %s." % (str(id)),
                status="error",
            )
            return category_dict
        category_dict = category.to_dict(
        ...
        category_dict["repository_count"] = suc.count_repositories_in_category(
            self.app, category_id
        )
        category_dict["url"] = web.url_for(
            controller="categories",
            action="show",
    ...
    def index(self, trans, deleted=False, **kwd):
        ...
        for category in (
            ...
            category_dict[
                "repositories"
            ] = self.app.repository_registry.viewable_repositories_and_suites_by_category.get(
                category.name, 0
            )
