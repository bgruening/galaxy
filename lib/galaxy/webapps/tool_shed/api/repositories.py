...
class RepositoriesController(BaseAPIController):
    ...
    def add_repository_registry_entry(self, trans, payload, **kwd):
        ...
        if not trans.user_is_admin:
            ...
            response_dict[
                "message"
            ] = "You are not authorized to add entries to this Tool Shed's repository registry."
    ...
    def get_ordered_installable_revisions(self, trans, name=None, owner=None, **kwd):
        ...
        if None not in [name, owner]:
            ...
            if repository is None:
                ...
                return {
                    "status": "error",
                    "message": "No repository named %s found with owner %s"
                    % (name, owner),
                }
        ...
        return [
            revision[1]
            for revision in repository.installable_revisions(
                self.app, sort_revisions=True
            )
        ]
    ...
    def get_repository_revision_install_info(
        ...
        if name and owner and changeset_revision:
            ...
            if repository is None:
                log.debug(
                    "Cannot locate repository %s owned by %s" % (str(name), str(owner))
                )
                return {}, {}, {}
            ...
            if repository_metadata is not None:
                ...
                if "tools" in repository_metadata.metadata:
                    repository_metadata_dict[
                        "valid_tools"
                    ] = repository_metadata.metadata["tools"]
            ...
            debug_msg += (
                "Invalid name %s or owner %s or changeset_revision %s received."
    ...
    def import_capsule(self, trans, payload, **kwd):
        ...
        try:
            ...
            log.debug(
                "Error opening capsule file %s: %s" % (str(capsule_file_name), str(e))
            )
            return {}
        irm = capsule_manager.ImportRepositoryManager(
            self.app, trans.request.host, trans.user, trans.user_is_admin
        )
                ...
                for changeset, changehash in repository.installable_revisions(self.app):
                    ...
                    for tool in tools:
                        if tool["guid"] in tool_ids:
                            repository_found.append(
                                "%d:%s" % (int(changeset), changehash)
                            )
                    metadata = metadata_util.get_current_repository_metadata_for_changeset_revision(
                        self.app, repository, changehash
                    )
                    ...
                    metadata_dict = metadata.to_dict(
                        value_mapper={
                            "id": self.app.security.encode_id,
                    ...
                    if metadata.has_repository_dependencies:
                        ...
                        ] = metadata_util.get_all_dependencies(
                    ...
                    if metadata.includes_tool_dependencies:
                        metadata_dict[
                            "tool_dependencies"
                        ] = repository.get_tool_dependencies(self.app, changehash)
                    else:
    ...
    def _search(self, trans, q, page=1, page_size=10):
        ...
        if len(search_term) < 1:
            raise RequestParameterInvalidException(
                "The search term has to be at least one character long."
            )
    ...
    def repository_ids_for_setting_metadata(self, trans, my_writable=False, **kwd):
        ...
        query = rmm.get_query_for_setting_metadata_on_repositories(
            my_writable=my_writable, order=False
        )
        ...
        for repository in query:
            if (
                ...
                and repository.id not in handled_repository_ids
            ):
    ...
    def reset_metadata_on_repository(self, trans, payload, **kwd):
        ...
        if repository_id is not None:
            ...
            start_time = strftime("%Y-%m-%d %H:%M:%S")
            log.debug(
                "%s...resetting metadata on repository %s"
                % (start_time, str(repository.name))
            )
    ...
    def updates(self, trans, **kwd):
        ...
        if repository:
            ...
            if changeset_revision == repository.tip(trans.app):
                ...
                if repository_metadata is None:
                    if (
                        next_installable_revision
                        and next_installable_revision != changeset_revision
                    ):
                        tool_shed_status_dict["latest_installable_revision"] = "True"
                    else:
            ...
            metadata_revisions = [
                revision[1]
                for revision in metadata_util.get_metadata_revisions(
    ...
    def show_tools(self, trans, id, changeset, **kwd):
        ...
        if repository_metadata is not None:
            ...
            if "tools" in repository_metadata.metadata:
                repository_metadata_dict["valid_tools"] = repository_metadata.metadata[
                    "tools"
                ]
            # Get the repo_info_dict for installing the repository.
            repo_info_dict, includes_tools, includes_tool_dependencies, includes_tools_for_display_in_tool_panel, has_repository_dependencies, has_repository_dependencies_only_if_compiling_contained_td = repository_util.get_repo_info_dict(
    ...
    def metadata(self, trans, id, **kwd):
        ...
        for changeset, changehash in repository.installable_revisions(self.app):
            ...
            if metadata.includes_tool_dependencies and recursive:
                metadata_dict["tool_dependencies"] = repository.get_tool_dependencies(
                    self.app, changehash
                )
            else:
                metadata_dict["tool_dependencies"] = {}
    ...
    def update(self, trans, id, **kwd):
        ...
        repo, message = repository_util.update_repository(
            app=self.app, trans=trans, id=id, **update_kwds
        )
        if repo is None:
        ...
        * POST /api/repositories:
        ...
        if not synopsis:
            raise RequestParameterMissingException(
                "Missing required parameter 'synopsis'."
            )
        ...
        invalid_message = repository_util.validate_repository_name(
            self.app, name, trans.user
        )
        if invalid_message:
    ...
    def create_changeset_revision(self, trans, id, payload, **kwd):
        ...
        if not (
            trans.user_is_admin
            or self.app.security_agent.user_can_administer_repository(
