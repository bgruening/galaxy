...
:       param key: the current Galaxy admin user's API key
        ...
        message = self.__validate(
            trans, email=email, password=password, confirm=password, username=username
        )
        if message:
            ...
            user_dict["url"] = web.url_for(
                controller="users", action="show", id=trans.security.encode_id(user.id)
            )
        return user_dict
