...
class ToolsController(BaseAPIController):
    ...
    def json(self, trans, **kwd):
        ...
        if None in [changeset, tsr_id, guid]:
            message = (
                "Changeset, repository ID, and tool GUID are all required parameters."
            )
        ...
        if found_tool is None:
            message = "Unable to find tool with guid %s in repository %s." % (
                guid,
                repository.name,
            )
        ...
        tool_dict.update(
            {
                ...
                "requirements": [
                    {"name": r.name, "version": r.version} for r in tool.requirements
                ],
