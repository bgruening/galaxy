...
PasswordResetToken.table = Table(
...
UserGroupAssociation.table = Table(
...
UserRoleAssociation.table = Table(
...
GroupRoleAssociation.table = Table(
...
RepositoryRoleAssociation.table = Table(
...
RepositoryMetadata.table = Table(
    "repository_metadata",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
...
ComponentReview.table = Table(
    ...
    Column("update_time", DateTime, default=now, onupdate=now),
    Column(
        "repository_review_id", Integer, ForeignKey("repository_review.id"), index=True
    ),
...
RepositoryRatingAssociation.table = Table(
...
RepositoryCategoryAssociation.table = Table(
...
mapper(
    ...
    User.table,
    properties=dict(
        active_repositories=relation(
            Repository,
            primaryjoin=(
                (Repository.table.c.user_id == User.table.c.id)
                & (not_(Repository.table.c.deleted))
            ),
            order_by=(Repository.table.c.name),
        ),
        galaxy_sessions=relation(
            GalaxySession, order_by=desc(GalaxySession.table.c.update_time)
        ),
        api_keys=relation(
            APIKeys, backref="user", order_by=desc(APIKeys.table.c.create_time)
        ),
    ),
)
...
mapper(Group, Group.table, properties=dict(users=relation(UserGroupAssociation)))
...
mapper(
    ...
    properties=dict(
        repositories=relation(
            RepositoryRoleAssociation,
            primaryjoin=(
                (Role.table.c.id == RepositoryRoleAssociation.table.c.role_id)
                & (
                    RepositoryRoleAssociation.table.c.repository_id
                    == Repository.table.c.id
                )
            ),
        ),
        users=relation(
            UserRoleAssociation,
            primaryjoin=(
                (Role.table.c.id == UserRoleAssociation.table.c.role_id)
                & (UserRoleAssociation.table.c.user_id == User.table.c.id)
            ),
        ),
        groups=relation(
            GroupRoleAssociation,
            primaryjoin=(
                (Role.table.c.id == GroupRoleAssociation.table.c.role_id)
                & (GroupRoleAssociation.table.c.group_id == Group.table.c.id)
            ),
        ),
    ),
)
...
mapper(
    ...
    properties=dict(repository=relation(Repository), role=relation(Role)),
)

mapper(
    UserGroupAssociation,
    UserGroupAssociation.table,
    properties=dict(
        user=relation(User, backref="groups"), group=relation(Group, backref="members")
    ),
)
...
mapper(
    ...
    properties=dict(
        ...
        non_private_roles=relation(
            ...
            primaryjoin=(
                (User.table.c.id == UserRoleAssociation.table.c.user_id)
                & (UserRoleAssociation.table.c.role_id == Role.table.c.id)
                & not_(Role.table.c.name == User.table.c.email)
            ),
        ),
        role=relation(Role),
    ),
...
mapper(
    GroupRoleAssociation,
    GroupRoleAssociation.table,
    properties=dict(group=relation(Group, backref="roles"), role=relation(Role)),
)

mapper(GalaxySession, GalaxySession.table, properties=dict(user=relation(User)))

mapper(
    Tag,
    Tag.table,
    properties=dict(
        children=relation(Tag, backref=backref("parent", remote_side=[Tag.table.c.id]))
    ),
)
...
mapper(
    ...
    properties=dict(
        repositories=relation(
            ...
            primaryjoin=(
                Category.table.c.id == RepositoryCategoryAssociation.table.c.category_id
            ),
            secondaryjoin=(
                RepositoryCategoryAssociation.table.c.repository_id
                == Repository.table.c.id
            ),
        )
    ),
...
mapper(
    ...
    properties=dict(
        ...
        ratings=relation(
            RepositoryRatingAssociation,
            order_by=desc(RepositoryRatingAssociation.table.c.update_time),
            backref="repositories",
        ),
        user=relation(User),
        downloadable_revisions=relation(
            RepositoryMetadata,
            primaryjoin=(
                (Repository.table.c.id == RepositoryMetadata.table.c.repository_id)
                & (RepositoryMetadata.table.c.downloadable == true())
            ),
            order_by=desc(RepositoryMetadata.table.c.update_time),
        ),
        metadata_revisions=relation(
            RepositoryMetadata, order_by=desc(RepositoryMetadata.table.c.update_time)
        ),
        roles=relation(RepositoryRoleAssociation),
        reviews=relation(
            RepositoryReview,
            primaryjoin=(
                (Repository.table.c.id == RepositoryReview.table.c.repository_id)
            ),
        ),
        reviewers=relation(
            User,
...
mapper(
    RepositoryReview,
    RepositoryReview.table,
    properties=dict(
        repository=relation(
            Repository,
            primaryjoin=(
                RepositoryReview.table.c.repository_id == Repository.table.c.id
            ),
        ),
...
mapper(
    ...
    properties=dict(
        repository_review=relation(RepositoryReview),
        component=relation(
            Component,
...
mapper(
    RepositoryCategoryAssociation,
    RepositoryCategoryAssociation.table,
    properties=dict(category=relation(Category), repository=relation(Repository)),
)
...
def init(file_path, url, engine_options={}, create_tables=False):
    ...
    result.hgweb_config_manager = (
        galaxy.webapps.tool_shed.util.hgweb_config.HgWebConfigManager()
    )
    return result
