    ...
    if RepositoryMetadata_table is not None:
        ...
        try:
            ...
            migrate_engine.execute(
                "UPDATE repository_metadata SET test_install_error=%s" % default_false
            )
        except Exception:
            log.exception(
                "Adding test_install_error column to the repository_metadata table failed."
            )
...
def downgrade(migrate_engine):
    ...
    try:
        ...
        log.exception(
            "Dropping column test_install_error from the repository_metadata table failed."
        )
