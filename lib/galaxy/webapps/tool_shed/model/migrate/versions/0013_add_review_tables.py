...
def localtimestamp(migrate_engine):
    if (
        migrate_engine.name in ["postgresql", "postgres"]
...
def downgrade(migrate_engine):
    ...
    if role_id:
        ...
        if group_id:
            ...
            cmd = "DELETE FROM user_group_association WHERE group_id = %d;" % int(
                group_id
            )
            migrate_engine.execute(cmd)
            # Delete all GroupRoleAssociations for the IUC group and the REVIEWER role.
            cmd = (
                "DELETE FROM group_role_association WHERE group_id = %d and role_id = %d;"
                % (int(group_id), int(role_id))
            )
            migrate_engine.execute(cmd)
