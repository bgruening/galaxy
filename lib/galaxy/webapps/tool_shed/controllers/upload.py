        ...
        if kwd.get("upload_button", False):
            ...
            if uploaded_file or uploaded_directory:
                ...
                if istar:
                    ...
                    ok, message, files_to_remove, content_alert_str, undesirable_dirs_removed, undesirable_files_removed = self.upload_directory(
                        trans,
                        rdah,
                        tdah,
                        repository,
                    ...
                    if ok:
                        if upload_point is not None:
                            full_path = os.path.abspath(
                                os.path.join(
                                    repo_dir, upload_point, uploaded_file_filename
                                )
                            )
                        else:
                            full_path = os.path.abspath(
                ...
                if ok:
                    ...
                    if tip == repository.tip(trans.app):
                        ...
                        message = (
                            "The %s <b>%s</b> has been successfully%suploaded to the repository.  "
                            % (source_type, escape(source), uncompress_str)
                        )
                        if istar and (
                            undesirable_dirs_removed or undesirable_files_removed
                        ):
                            items_removed = (
                        ...
                        if istar and remove_repo_files_not_in_tar and files_to_remove:
                            if upload_point is not None:
                                ...
                                message += (
                                    "  %d files were removed from the repository root.  "
                                    % len(files_to_remove)
                                )
        ...
        for root, dirs, files in os.walk(uploaded_directory):
            for uploaded_file in files:
                relative_path = os.path.normpath(
                    os.path.join(
                        os.path.relpath(root, uploaded_directory), uploaded_file
                    )
                )
                if repository.type == rt_util.REPOSITORY_SUITE_DEFINITION:
                    ...
                    ok = (
                        os.path.basename(uploaded_file)
                        not in commit_util.UNDESIRABLE_FILES
                    )
                if ok:
                ...
                if ok:
                    ...
                    if os.path.exists(repo_path):
                        if os.path.isdir(repo_path):
                            ...
                            os.remove(repo_path)
                    shutil.move(
                        os.path.join(uploaded_directory, relative_path), repo_path
                    )
