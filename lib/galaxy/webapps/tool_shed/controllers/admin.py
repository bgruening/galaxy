...
from tool_shed.util import metadata_util, repository_util, shed_util_common as suc
...
class AdminController(BaseUIController, Admin):
        ...
        if "operation" in kwd:
            ...
            if operation == "view_or_manage_repository":
                ...
                if kwd["id"]:
                    ...
                    return trans.response.send_redirect(
                        web.url_for(
                            controller="repository", action="set_email_alerts", **kwd
                        )
                    )
                else:
                    del kwd["operation"]
        ...
        if id:
            ...
            if count:
                message = "Deleted %d repository metadata %s" % (
                    count,
                    inflector.cond_plural(len(ids), "record"),
                )
        else:
    ...
    def manage_categories(self, trans, **kwd):
        if "f-free-text-search" in kwd:
            ...
            return trans.response.send_redirect(
                web.url_for(controller="admin", action="browse_repositories", **kwd)
            )
        if "operation" in kwd:
            operation = kwd["operation"].lower()
            if operation == "create":
                return trans.response.send_redirect(
                    web.url_for(controller="admin", action="create_category", **kwd)
                )
            elif operation == "delete":
                return trans.response.send_redirect(
                    web.url_for(
                        controller="admin", action="mark_category_deleted", **kwd
                    )
                )
            elif operation == "undelete":
                return trans.response.send_redirect(
                    web.url_for(controller="admin", action="undelete_category", **kwd)
                )
            elif operation == "purge":
                return trans.response.send_redirect(
                    web.url_for(controller="admin", action="purge_category", **kwd)
                )
            elif operation == "edit":
                return trans.response.send_redirect(
                    web.url_for(controller="admin", action="edit_category", **kwd)
                )
    ...
    def regenerate_statistics(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/webapps/tool_shed/admin/statistics.mako", message=message, status=status
        )
