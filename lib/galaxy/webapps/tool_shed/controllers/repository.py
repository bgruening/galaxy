...
def get_mercurial_default_options_dict(command):
    ...
    return dict(
        (r[1].replace("-", "_"), r[2]) for r in next(iter(possible.values()))[1][1]
    )
...
class RepositoryController(BaseUIController, ratings_util.ItemRatings):
    ...
    deprecated_repositories_i_own_grid = (
        repository_grids.DeprecatedRepositoriesIOwnGrid()
    )
    ...
    repositories_i_can_administer_grid = (
        repository_grids.RepositoriesICanAdministerGrid()
    )
    repositories_in_category_grid = repository_grids.RepositoriesInCategoryGrid()
    repositories_missing_tool_test_components_grid = (
        repository_grids.RepositoriesMissingToolTestComponentsGrid()
    )
    repositories_with_invalid_tools_grid = (
        repository_grids.RepositoriesWithInvalidToolsGrid()
    )
    ...
    def _redirect_if_necessary(self, trans, **kwd):
        if "operation" in kwd:
            ...
            if operation == "view_or_manage_repository":
                ...
                return trans.response.send_redirect(
                    web.url_for(controller="repository", action="deprecate", **kwd)
                )
    ...
    def browse_categories(self, trans, **kwd):
        ...
        if "f-free-text-search" in kwd:
            ...
            if (
                "id" in kwd
                and "f-free-text-search" in kwd
                and kwd["id"] == kwd["f-free-text-search"]
            ):
                ...
                try:
                    ...
                    repository = repository_util.get_repository_by_name(
                        trans.app, repository_name
                    )
            ...
            return trans.response.send_redirect(
                web.url_for(
                    controller="repository", action="browse_repositories", **kwd
                )
            )
        if "operation" in kwd:
            operation = kwd["operation"].lower()
    ...
    def browse_deprecated_repositories_i_own(self, trans, **kwd):
        ...
        selected_changeset_revision, repository = suc.get_repository_from_refresh_on_change(
            trans.app, **kwd
        )
        if repository:
            return trans.response.send_redirect(
    ...
    @web.expose
    def browse_my_writable_repositories_missing_tool_test_components(
        self, trans, **kwd
    ):
        _redir = self._redirect_if_necessary(trans, **kwd)
        if _redir is not None:
        ...
        if "message" not in kwd:
            message = (
                "This list contains repositories that match the following criteria:<br>"
            )
            message += "<ul>"
            ...
            kwd["message"] = message
            kwd["status"] = "warning"
        return self.my_writable_repositories_missing_tool_test_components_grid(
            trans, **kwd
        )
    ...
    def browse_my_writable_repositories_with_invalid_tools(self, trans, **kwd):
        ...
        if "message" not in kwd:
            ...
            message += "<li>you are authorized to update them</li>"
            message += (
                "<li>the latest metadata revision contains at least 1 invalid tool</li>"
            )
            message += "</ul>"
    ...
    def browse_repositories(self, trans, **kwd):
        ...
        if "operation" in kwd:
            ...
            if operation == "view_or_manage_repository":
                ...
                return trans.response.send_redirect(
                    web.url_for(
                        controller="repository", action="edit_repository", **kwd
                    )
                )
            elif operation == "repositories_by_user":
                return trans.response.send_redirect(
            ...
            if operation == "repositories_by_user":
                ...
                if user_id is None:
                    ...
                    if repository_id:
                        ...
                        user_id = trans.security.encode_id(repository.user.id)
                        user = suc.get_user(trans.app, user_id)
                        self.repositories_by_user_grid.title = (
                            "Repositories owned by %s" % user.username
                        )
                        kwd["user_id"] = user_id
                        return self.repositories_by_user_grid(trans, **kwd)
        selected_changeset_revision, repository = suc.get_repository_from_refresh_on_change(
    ...
    def browse_repository(self, trans, id, **kwd):
        ...
        repository_type_select_field = rt_util.build_repository_type_select_field(
            trans, repository=repository
        )
    ...
    def browse_repository_dependencies(self, trans, **kwd):
        if "operation" in kwd:
            ...
            if operation == "browse_repository":
                return trans.response.send_redirect(
                    web.url_for(
                        controller="repository", action="browse_repository", **new_kwd
                    )
                )
            if operation == "view_or_manage_repository":
                return trans.response.send_redirect(
    ...
    def browse_valid_categories(self, trans, **kwd):
        ...
        if "operation" in kwd:
            operation = kwd["operation"].lower()
            if operation in [
                "valid_repositories_by_category",
                "valid_repositories_by_user",
            ]:
                # Eliminate the current filters if any exist.
                for k, v in list(kwd.items()):
                    if k.startswith("f-"):
    ...
    def check_for_updates(self, trans, **kwd):
        ...
        if from_update_manager:
            ...
            message = (
                "Unable to check for updates due to an invalid Galaxy URL: <b>%s</b>.  "
                % galaxy_url
            )
    ...
    def contact_owner(self, trans, id, **kwd):
        ...
        repository = repository_util.get_repository_in_tool_shed(trans.app, id)
        metadata = metadata_util.get_repository_metadata_by_repository_id_changeset_revision(
            trans.app, id, repository.tip(trans.app), metadata_only=True
        )
        if trans.user and trans.user.email:
            ...
            return trans.show_error_message(
                "You must be logged in to contact the owner of a repository."
            )
    ...
    def create_galaxy_docker_image(self, trans, **kwd):
        ...
        if "operation" in kwd:
            ...
            docker_image_template = string.Template(template).safe_substitute(
                selected_repositories=repository_string
            )
            ...
            trans.response.headers["Content-Disposition"] = (
                'attachment; filename="%s"' % docker_file_name
            )
    ...
    def create_repository(self, trans, **kwd):
        ...
        if kwd.get("create_repository_button", False):
            error = False
            message = repository_util.validate_repository_name(
                trans.app, name, trans.user
            )
            if message:
                error = True
            if not description:
    ...
    def deprecate(self, trans, **kwd):
        ...
        if mark_deprecated:
            ...
            message = (
                "The repository <b>%s</b> has been marked as deprecated."
            ...
            message = (
                "The repository <b>%s</b> has been marked as not deprecated."
                % escape(repository.name)
            )
        trans.response.send_redirect(
            web.url_for(
    ...
    @web.expose
    def display_tool(
        self, trans, repository_id, tool_config, changeset_revision, **kwd
    ):
        status = kwd.get("status", "done")
        render_repository_actions_for = kwd.get(
        ...
        try:
            ...
            message = (
                "Error displaying tool, probably due to a problem in the tool config.  The exception is: %s."
                % str(e)
            )
    ...
    def export(self, trans, repository_id, changeset_revision, **kwd):
        ...
        if kwd.get("export_repository_button", False):
            ...
            if error_message:
                ...
                trans.response.headers[
                    "Content-Disposition"
                ] = 'attachment; filename="%s"' % (repositories_archive_filename)
                opened_archive = open(repositories_archive.name)
                # Make sure the file is removed from disk after the contents have been downloaded.
                os.unlink(repositories_archive.name)
                repositories_archive_path, file_name = os.path.split(
        ...
        rb = relation_builder.RelationBuilder(
            trans.app, repository, repository_metadata, toolshed_base_url
        )
        ...
        repository_dependencies = (
            rb.get_repository_dependencies_for_changeset_revision()
        )
        if repository_dependencies:
            ...
            export_repository_dependencies_check_box = CheckboxField(
                "export_repository_dependencies", value=True
            )
        else:
        ...
        return trans.fill_template(
            "/webapps/tool_shed/repository/export_repository.mako",
            changeset_revision=changeset_revision,
    ...
    def export_via_api(self, trans, **kwd):
        ...
        encoded_repositories_archive_name = kwd.get(
            "encoded_repositories_archive_name", None
        )
        if encoded_repositories_archive_name:
            repositories_archive_name = encoding_util.tool_shed_decode(
                encoded_repositories_archive_name
            )
    ...
    def find_tools(self, trans, **kwd):
        ...
        if "operation" in kwd:
            ...
            if item_id:
                ...
                if operation == "view_or_manage_repository":
                    ...
                    if trans.webapp.name == "tool_shed" and (
                        is_admin or repository.user == trans.user
                    ):
                        a = "manage_repository"
                    else:
                        a = "view_repository"
                    return trans.response.send_redirect(
                        web.url_for(controller="repository", action=a, **kwd)
                    )
                if operation == "install to galaxy":
                    ...
                    for repository_metadata_id in util.listify(item_id):
                        ...
                        encoded_repository_ids.append(
                            trans.security.encode_id(repository_metadata.repository.id)
                        )
        ...
        tool_versions = [
            item.lower() for item in util.listify(kwd.get("tool_version", ""))
        ]
        ...
        if tool_ids or tool_names or tool_versions:
            ...
            if ok:
                ...
                if trans.webapp.name == "galaxy":
                    ...
                    install_url_args = dict(
                        controller="repository", action="find_tools"
                    )
                    operations = [
                        grids.GridOperation(
                            "Install",
                            url_args=install_url_args,
                            allow_multiple=True,
                            async_compatible=False,
                        )
                    ]
                    ...
                    self.matched_repository_grid.title = (
                ...
                status = "error"
        exact_matches_check_box = CheckboxField(
            "exact_matches", value=exact_matches_checked
        )
        return trans.fill_template(
    ...
    def find_workflows(self, trans, **kwd):
        ...
        if "find_workflows_button" in kwd:
            workflow_names = [
                item.lower() for item in util.listify(kwd.get("workflow_name", ""))
            ]
            exact_matches = kwd.get("exact_matches", "")
            exact_matches_checked = CheckboxField.is_checked(exact_matches)
            match_tuples = []
            ...
            if workflow_names:
                ok, match_tuples = search_util.search_repository_metadata(
                    trans.app, exact_matches_checked, workflow_names=workflow_names
                )
            else:
                ok, match_tuples = search_util.search_repository_metadata(
                    trans.app,
            ...
            if ok:
                ...
                if trans.webapp.name == "galaxy":
                    ...
                    self.install_matched_repository_grid.global_actions = global_actions
                    install_url_args = dict(
                        controller="repository", action="find_workflows"
                    )
                    operations = [
                        grids.GridOperation(
                            "Install",
                    ...
                    kwd["message"] = (
                        "workflow name: <b>%s</b><br/>exact matches only: <b>%s</b>"
                        % (
                            escape(basic_util.stringify(workflow_names)),
                            str(exact_matches_checked),
                        )
                    )
                    self.matched_repository_grid.title = (
                        "Repositories with matching workflows"
                    )
                    return self.matched_repository_grid(trans, **kwd)
            else:
                message = "No search performed - each field must contain the same number of comma-separated items."
        ...
        return trans.fill_template(
            "/webapps/tool_shed/repository/find_workflows.mako",
    ...
    def generate_workflow_image(
        self, trans, workflow_name, repository_metadata_id=None
    ):
    ...
    def get_changeset_revision_and_ctx_rev(self, trans, **kwd):
        ...
        def has_galaxy_utilities(repository_metadata):
            ...
            if repository_metadata:
                ...
                if metadata:
                    ...
                    has_repository_dependencies, has_repository_dependencies_only_if_compiling_contained_td = repository_util.get_repository_dependency_types(
                        repository_dependencies
                    )
        ...
        includes_tool_dependencies = has_galaxy_utilities_dict[
            "includes_tool_dependencies"
        ]
        has_repository_dependencies = has_galaxy_utilities_dict[
            "has_repository_dependencies"
        ]
                ...
                for changeset in repo.changelog:
                    ...
                    if update_to_changeset_hash:
                        ...
                        if update_to_repository_metadata:
                            has_galaxy_utilities_dict = has_galaxy_utilities(
                                repository_metadata
                            )
                            includes_data_managers = has_galaxy_utilities_dict[
                                "includes_data_managers"
                            ]
                            includes_datatypes = has_galaxy_utilities_dict[
                                "includes_datatypes"
                            ]
                            includes_tools = has_galaxy_utilities_dict["includes_tools"]
                            includes_tools_for_display_in_tool_panel = has_galaxy_utilities_dict[
                            ...
                            includes_workflows = has_galaxy_utilities_dict[
                                "includes_workflows"
                            ]
                            # We found a RepositoryMetadata record.
                            if changeset_hash == repository.tip(trans.app):
                        ...
                        and changeset_hash == changeset_revision
                    ):
                        # We've found the changeset in the changelog for which we need to get the next update.
                        update_to_changeset_hash = changeset_hash
    ...
    def get_ctx_rev(self, trans, **kwd):
        ...
        repository = repository_util.get_repository_by_name_and_owner(
            trans.app, repository_name, repository_owner
        )
        repo = hg_util.get_repo_for_repository(trans.app, repository=repository)
    ...
    def get_file_contents(self, trans, file_path, repository_id):
        ...
        return suc.get_repository_file_contents(
            trans.app, file_path, repository_id, is_admin
        )
    ...
    def get_latest_downloadable_changeset_revision(self, trans, **kwd):
        ...
        if repository_name is not None and repository_owner is not None:
            ...
            if repository:
                return metadata_util.get_latest_downloadable_changeset_revision(
    ...
    def get_readme_files(self, trans, **kwd):
        ...
        if (
            ...
            and repository_owner is not None
            and changeset_revision is not None
        ):
            repository = repository_util.get_repository_by_name_and_owner(
                trans.app, repository_name, repository_owner
            )
    ...
    def get_repository_information(
        self, trans, repository_ids, changeset_revisions, **kwd
    ):
        """
        ...
        for tup in zip(util.listify(repository_ids), util.listify(changeset_revisions)):
            ...
            repo_info_dict, cur_includes_tools, cur_includes_tool_dependencies, cur_includes_tools_for_display_in_tool_panel, cur_has_repository_dependencies, cur_has_repository_dependencies_only_if_compiling_contained_td = repository_util.get_repo_info_dict(
                trans.app, trans.user, repository_id, changeset_revision
            )
            ...
            if (
                ...
                and not includes_tools_for_display_in_tool_panel
            ):
    ...
    def get_required_repo_info_dict(self, trans, encoded_str=None):
        ...
        repo_info_dict = {}
        if encoded_str:
            encoded_required_repository_str = encoding_util.tool_shed_decode(
                encoded_str
            )
            encoded_required_repository_tups = encoded_required_repository_str.split(
                encoding_util.encoding_sep2
            )
            ...
            if encoded_repository_ids and changeset_revisions:
                repo_info_dict = json.loads(
                    self.get_repository_information(
                        trans, encoded_repository_ids, changeset_revisions
                    )
                )
    ...
    def get_tool_dependencies_config_contents(self, trans, **kwd):
        ...
        tool_dependencies_config = hg_util.get_config_from_disk(
            rt_util.TOOL_DEPENDENCY_DEFINITION_FILENAME, repo_dir
        )
    ...
    def get_updated_repository_information(
        ...
        if metadata:
            ...
            readme_files_dict = readme_util.build_readme_files_dict(
                trans.app, repository, changeset_revision, metadata
            )
        ...
        for name, repo_info_tuple in repo_info_dict.items():
            if (
                ...
                description, reposectory_clone_url, changeset_revision, ctx_rev, repository_owner, repository_dependencies, tool_dependencies = repository_util.get_repo_info_tuple_contents(
                    repo_info_tuple
                )
                for rd_key, rd_tups in repository_dependencies.items():
                    if rd_key in ["root_key", "description"]:
                        continue
                    curr_has_repository_dependencies, curr_has_repository_dependencies_only_if_compiling_contained_td = repository_util.get_repository_dependency_types(
                        rd_tups
                    )
                    if (
                        curr_has_repository_dependencies
                        and not has_repository_dependencies
                    ):
                        has_repository_dependencies = True
                    if (
    ...
    def help(self, trans, **kwd):
        ...
        status = kwd.get("status", "done")
        return trans.fill_template(
            "/webapps/tool_shed/repository/help.mako",
            message=message,
            status=status,
            **kwd
        )
    ...
    def import_capsule(self, trans, **kwd):
        ...
        irm = capsule_manager.ImportRepositoryManager(
            trans.app, trans.request.host, trans.user, trans.user_is_admin
        )
        ...
        if "import_capsule_button" in kwd:
            ...
            return trans.fill_template(
                "/webapps/tool_shed/repository/import_capsule_results.mako",
        ...
        return trans.fill_template(
            "/webapps/tool_shed/repository/import_capsule.mako",
    ...
    def index(self, trans, **kwd):
        ...
        repository_metadata = trans.sa_session.query(
            trans.model.RepositoryMetadata
        ).first()
        ...
        if current_user:
            ...
            if not trans.user_is_admin:
                if current_user.active_repositories:
                    ...
                    for repository in trans.sa_session.query(
                        trans.model.Repository
                    ).filter(trans.model.Repository.table.c.deleted == false()):
                        if trans.app.security_agent.user_can_administer_repository(
                            current_user, repository
                        ):
                            can_administer_repositories = True
                            break
    ...
    def install_repositories_by_revision(self, trans, **kwd):
        ...
        if galaxy_url:
            ...
            message = (
                "Repository installation is not possible due to an invalid Galaxy URL: <b>%s</b>.  "
                % galaxy_url
            )
            message += "You may need to enable third-party cookies in your browser.  "
            status = "error"
    ...
    def load_invalid_tool(
        ...
        with ValidationContext.from_app(trans.app) as validation_context:
            ...
            invalid_file_tups = []
            if tool:
                invalid_file_tups = tv.check_tool_input_params(
                    repository.repo_path(trans.app), tool_config, tool, []
                )
    ...
    def manage_repository(self, trans, id, **kwd):
        ...
        repository.clone_url = common_util.generate_clone_url_for_repository_in_tool_shed(
            trans.user, repository
        )
        remote_repository_url = kwd.get(
        ...
        avg_rating, num_ratings = self.get_ave_item_rating_data(
            trans.sa_session, repository, webapp_model=trans.model
        )
        ...
        if kwd.get("edit_repository_button", False):
            ...
            repository, message = repository_util.update_repository(
                app=trans.app, trans=trans, id=id, **update_kwds
            )
            ...
            if category_ids:
                ...
                for category_id in category_ids:
                    category = trans.sa_session.query(trans.model.Category).get(
                        trans.security.decode_id(category_id)
                    )
                    rca = trans.app.model.RepositoryCategoryAssociation(
                        repository, category
                    )
        ...
        revision_label = hg_util.get_revision_label(
            trans.app, repository, repository.tip(trans.app), include_date=False
        )
    ...
    def multi_select_email_alerts(self, trans, **kwd):
        ...
        self.email_alerts_repository_grid.title = (
            "Set email alerts for repository changes"
        )
    ...
    def open_folder(self, trans, folder_path, repository_id):
        ...
        return suc.open_repository_files_folder(
            trans.app, folder_path, repository_id, is_admin
        )
    ...
    def preview_tools_in_changeset(self, trans, repository_id, **kwd):
        ...
        if repository_metadata:
            ...
            if metadata:
                if (
                    "repository_dependencies" in metadata
                    and not repository_dependencies
                ):
                    ...
                    for rd_tup in rd_tups:
                        rdtool_shed, rd_name, rd_owner, rd_changeset_revision, rd_prior_installation_required, rd_only_if_compiling_contained_td = common_util.parse_repository_dependency_tuple(
                            rd_tup
                        )
    ...
    def rate_repository(self, trans, **kwd):
        ...
        display_reviews = util.string_as_bool(kwd.get("display_reviews", False))
        rra = self.get_user_item_rating(
            trans.sa_session, trans.user, repository, webapp_model=trans.model
        )
        metadata = metadata_util.get_repository_metadata_by_repository_id_changeset_revision(
            trans.app, id, changeset_revision, metadata_only=True
        )
            ...
            if selected_files_to_delete:
                ...
                for selected_file in selected_files_to_delete:
                    try:
                        ...
                        log.debug(
                            "Error removing the following file using the mercurial API:\n %s"
                            % str(selected_file)
                        )
                        log.debug("The error was: %s" % str(e))
                        log.debug(
                            "Attempting to remove the file using a different approach."
                        )
                        relative_selected_file = selected_file.split(
                            "repo_%d" % repository.id
                        )[1].lstrip("/")
    ...
    def send_to_owner(self, trans, id, message=""):
        ...
        if not message:
            ...
            if smtp_server is None or from_address is None:
                return trans.show_error_message(
                    "Mail is not configured for this Galaxy tool shed instance"
                )
            ...
            try:
                util.send_mail(
                    from_address, to_address, subject, body, trans.app.config
                )
    ...
    def set_email_alerts(self, trans, **kwd):
        ...
        if user:
            ...
            if flush_needed:
                trans.sa_session.flush()
            message = "Total alerts added: %d, total alerts removed: %d" % (
                total_alerts_added,
                total_alerts_removed,
            )
            kwd["message"] = message
            kwd["status"] = "done"
        del kwd["operation"]
        return trans.response.send_redirect(
            web.url_for(controller="repository", action=caller, **kwd)
        )
    ...
    def sharable_owner(self, trans, owner):
        ...
        if user:
            ...
            return trans.response.send_redirect(
                web.url_for(controller="repository", action="index", user_id=user_id)
            )
        else:
            return trans.show_error_message(
    ...
    def sharable_repository(self, trans, owner, name):
        ...
        if repository:
            ...
            return trans.response.send_redirect(
                web.url_for(
                    controller="repository", action="index", repository_id=repository_id
                )
            )
        else:
            ...
            if user:
                user_id = trans.security.encode_id(user.id)
                message = (
                    "This list of repositories owned by <b>%s</b>, does not include one named <b>%s</b>."
                    % (str(owner), str(name))
                )
                return trans.response.send_redirect(
                    web.url_for(
                ...
                return trans.show_error_message(
                    ...
                    % (
                        web.url_for("/", qualified=True).rstrip("/"),
                        str(name),
                        str(owner),
                    )
                )
        ...
        if repository:
            ...
            if not repository_metadata:
                ...
                if (
                    ...
                    and upper_bound_changeset_revision != changeset_revision
                ):
            ...
            return trans.response.send_redirect(
                web.url_for(
                    controller="repository", action="sharable_owner", owner=owner
                )
            )
    ...
    def updated_changeset_revisions(self, trans, **kwd):
        ...
        if name and owner and changeset_revision:
            return metadata_util.get_updated_changeset_revisions(
                trans.app, name, owner, changeset_revision
            )
        return ""
    ...
    def upload_capsule(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/webapps/tool_shed/repository/upload_capsule.mako",
            url=url,
            message=message,
            status=status,
        )
    ...
    def view_changeset(self, trans, id, ctx_str, **kwd):
        ...
        if ctx is None:
            message = "Repository does not include changeset revision '%s'." % str(
                ctx_str
            )
            status = "error"
            return trans.response.send_redirect(
        ...
        for diff in patch.diff(
            repo, node1=ctx_parent.node(), node2=ctx.node(), opts=diffopts
        ):
        ...
        if repository_id:
            ...
            if repository:
                if user is not None and (
                    ...
                    return trans.response.send_redirect(
                        web.url_for(
                            controller="repository", action="manage_repository", **kwd
                        )
                    )
                else:
                    return trans.response.send_redirect(
                        web.url_for(
                            controller="repository", action="view_repository", **kwd
                        )
                    )
            return trans.show_error_message(
                "Invalid repository id '%s' received." % repository_id
            )
    ...
    def view_repository(self, trans, id, **kwd):
        ...
        if repository_metadata:
            ...
            metadata = None
        is_malicious = metadata_util.is_malicious(
            trans.app, id, repository.tip(trans.app)
        )
        if is_malicious:
            if trans.app.security_agent.can_push(trans.app, trans.user, repository):
                message += malicious_error_can_push
    ...
    @web.expose
    def view_tool_metadata(
        self, trans, repository_id, changeset_revision, tool_id, **kwd
    ):
        message = escape(kwd.get("message", ""))
        status = kwd.get("status", "done")
        render_repository_actions_for = kwd.get(
                    ...
                    with ValidationContext.from_app(trans.app) as validation_context:
                        ...
                        for tool_metadata_dict in metadata["tools"]:
                            if tool_metadata_dict["id"] == tool_id:
                                ...
                                relative_path_to_tool_config = tool_metadata_dict[
                                    "tool_config"
                                ]
                                guid = tool_metadata_dict["guid"]
                                full_path_to_tool_config = os.path.abspath(
                                    relative_path_to_tool_config
                                )
                                full_path_to_dir, tool_config_filename = os.path.split(
                                ...
                                if can_use_disk_file:
                                    ...
                                    tool, valid, message, sample_files = tv.handle_sample_files_and_load_tool_from_tmp_config(
                                        repo,
                                        repository_id,
                                        changeset_revision,
                        ...
                        if guid:
                            ...
                            tool_lineage = tvm.get_version_lineage_for_tool(
                                repository_id, repository_metadata, guid
                            )
        else:
            repository_metadata_id = None
            metadata = None
    ...
    def view_workflow(self, trans, workflow_name, repository_metadata_id, **kwd):
        ...
        repository = repository_util.get_repository_in_tool_shed(
            trans.app, trans.security.encode_id(repository_metadata.repository_id)
        )
        changeset_revision = repository_metadata.changeset_revision
        metadata = repository_metadata.metadata
        return trans.fill_template(
            "/webapps/tool_shed/repository/view_workflow.mako",
            repository=repository,
            render_repository_actions_for=render_repository_actions_for,
            changeset_revision=changeset_revision,
            repository_metadata_id=repository_metadata_id,
