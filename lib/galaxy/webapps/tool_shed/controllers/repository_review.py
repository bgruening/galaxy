...
class RepositoryReviewController(BaseUIController, ratings_util.ItemRatings):
    ...
    repositories_ready_for_review_grid = (
        repository_review_grids.RepositoriesReadyForReviewGrid()
    )
    repositories_reviewed_by_me_grid = (
        repository_review_grids.RepositoriesReviewedByMeGrid()
    )
    repositories_with_reviews_grid = (
        repository_review_grids.RepositoriesWithReviewsGrid()
    )
    repositories_without_reviews_grid = (
        repository_review_grids.RepositoriesWithoutReviewsGrid()
    )
    repository_reviews_by_user_grid = (
        repository_review_grids.RepositoryReviewsByUserGrid()
    )
    reviewed_repositories_i_own_grid = (
        repository_review_grids.ReviewedRepositoriesIOwnGrid()
    )
    repositories_with_no_tool_tests_grid = (
        repository_review_grids.RepositoriesWithNoToolTestsGrid()
    )
    ...
    def approve_repository_review(self, trans, **kwd):
        ...
        if kwd.get("approve_repository_review_button", False):
            ...
            message = "Approved value <b>%s</b> saved for this revision." % escape(
    ...
    def browse_components(self, trans, **kwd):
        if "operation" in kwd:
            ...
            if operation == "create":
                return trans.response.send_redirect(
                    web.url_for(
                        controller="repository_review", action="create_component", **kwd
                    )
                )
    ...
    def browse_review(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/webapps/tool_shed/repository_review/browse_review.mako",
    ...
    def copy_review(self, trans, review_to_copy, review):
        for component_review in review_to_copy.component_reviews:
            copied_component_review = trans.model.ComponentReview(
    ...
    def create_component(self, trans, **kwd):
        ...
        if kwd.get("create_component_button", False):
            if not name or not description:
                ...
                component = trans.app.model.Component(
        ...
        return trans.fill_template(
            "/webapps/tool_shed/repository_review/create_component.mako",
            name=name,
    ...
    def create_review(self, trans, **kwd):
        ...
        if repository_id:
            if changeset_revision:
                ...
                if review_util.get_review_by_repository_id_changeset_revision_user_id(
                    app=trans.app,
                    ...
                    message = (
                        "You have already created a review for revision <b>%s</b> of repository <b>%s</b>."
                    ...
                    if (
                        not create_without_copying
                        and not previous_review_id
                        and review_util.has_previous_repository_reviews(
                    ...
                    if repository_metadata:
                        ...
                        if metadata:
                            review = trans.app.model.RepositoryReview(
                            ...
                            if previous_review_id:
                                review_to_copy = review_util.get_review(
                                    trans.app, previous_review_id
                                )
                                self.copy_review(trans, review_to_copy, review)
                            review_id = trans.security.encode_id(review.id)
                            message = (
                                "Begin your review of revision <b>%s</b> of repository <b>%s</b>."
                        ...
                        message = (
                            "A new review cannot be created for revision <b>%s</b> of repository <b>%s</b>.  Select a valid revision and try again."
                            % (changeset_revision, escape(repository.name))
                        )
                        kwd["message"] = message
                        kwd["status"] = "error"
    ...
    def edit_component(self, trans, **kwd):
        ...
        if kwd.get("edit_component_button", False):
            ...
            if component.description != new_description:
                ...
                trans.sa_session.flush()
                message = (
                    "The information has been saved for the component named <b>%s</b>"
        ...
        return trans.fill_template(
            "/webapps/tool_shed/repository_review/edit_component.mako",
            component=component,
            message=message,
            status=status,
        )
    ...
    def edit_review(self, trans, **kwd):
        ...
        for component in review_util.get_components(trans.app):
            components_dict[component.name] = dict(
                component=component, component_review=None
            )
        repository = review.repository
        repo = hg_util.get_repo_for_repository(trans.app, repository=repository)
        ...
        save_buttons = [
            "%s%sreview_button" % (comp_name, STRSEP)
            for comp_name in components_dict.keys()
        ]
        ...
        if save_button_clicked:
            ...
            for component_name in components_dict.keys():
                ...
                component_review = review_util.get_component_review_by_repository_review_id_component_id(
                    trans.app, review_id, component_id
                )
                if component_review:
                    # See if the existing component review should be updated.
                    if (
                        component_review.comment != comment
                        or component_review.private != private
                        or component_review.approved != approved
                        or component_review.rating != rating
                    ):
                        component_review.comment = comment
                    ...
                    if (
                        comment
                        or private
                        or approved != trans.model.ComponentReview.approved_states.NO
                        or rating
                    ):
                ...
                if flushed:
                    ...
                    average_rating = (
                        trans.sa_session.query(
                            func.avg(trans.model.ComponentReview.table.c.rating)
                        )
                        .filter(
                            and_(
                                trans.model.ComponentReview.table.c.repository_review_id
                                == review.id,
                                trans.model.ComponentReview.table.c.deleted == false(),
                                trans.model.ComponentReview.table.c.approved
                                != trans.model.ComponentReview.approved_states.NA,
                            )
                        )
                        .scalar()
                    )
            ...
            if revision_approved_setting_changed:
                message += (
                    "Approved value <b>%s</b> saved for this revision.<br/>"
                    % review.approved
                )
    ...
    def manage_components(self, trans, **kwd):
        if "operation" in kwd:
            ...
            if operation == "create":
                ...
                return trans.response.send_redirect(
                    web.url_for(
                        controller="repository_review", action="edit_component", **kwd
                    )
                )
    ...
    def manage_repositories_ready_for_review(self, trans, **kwd):
        ...
        if "operation" in kwd:
            ...
            if operation == "inspect repository revisions":
                return trans.response.send_redirect(
                    web.url_for(
                        controller="repository_review", action="create_review", **kwd
                    )
                )
    ...
    def manage_repository_reviews(self, trans, mine=False, **kwd):
        ...
        if repository_id:
            ...
            for changeset in hg_util.get_reversed_changelog_changesets(repo):
                ...
                if (
                    changeset_revision in metadata_revision_hashes
                    or changeset_revision in reviewed_revision_hashes
                ):
                    rev, changeset_revision_label = hg_util.get_rev_label_from_changeset_revision(
                    ...
                    if changeset_revision in reviewed_revision_hashes:
                        ...
                        repository_reviews = review_util.get_reviews_by_repository_id_changeset_revision(
                            trans.app, repository_id, changeset_revision
                        )
    ...
    def repository_reviews_by_user(self, trans, **kwd):
        ...
        self.repository_reviews_by_user_grid.title = (
            "All repository revision reviews for user '%s'" % user.username
        )
