...
class User(BaseUser):
        ...
        if user:
            ...
            if trans.response.status == 400:
                ...
                change_password_url = url_for(
                    ...
                    id=response.get("expired_user"),
                )
                message = (
                    "%s<br>Click <a href='%s'>here</a> to change your password."
                    % (response.get("message"), change_password_url)
                )
                status = "warning"
            else:
                success = True
        ...
        if subscribe_checked:
            ...
            if trans.app.config.smtp_server is None:
                ...
                message = (
                    "Now logged in as "
                    + user.email
                    + ". However, subscribing to the mailing list has failed because mail is not configured for this Galaxy instance. <br>Please contact your local Galaxy administrator."
                )
            else:
        ...
        if status != "error":
            ...
            if trans.app.config.user_activation_on:
                ...
                if is_activation_sent:
                    message = (
                        'Now logged in as %s.<br>Verification email has been sent to your email address. Please verify it by clicking the activation link in the email.<br>Please check your spam/trash folder in case you cannot find the message.<br><a target="_top" href="%s">Return to the home page.</a>'
            ...
            message = (
                'Now logged in as %s.<br><a target="_top" href="%s">Return to the home page.</a>'
                % (escape(user.email), url_for("/"))
            )
    ...
    def edit_info(self, trans, cntrller, **kwd):
        ...
        if user_id and is_admin:
            ...
            not trans.user or trans.user.id != trans.security.decode_id(user_id)
        ):
        ...
        if user and params.get("login_info_button", False):
            ...
            if user_type_fd_id not in ["none"]:
                user_type_form_definition = trans.sa_session.query(
                    trans.app.model.FormDefinition
                ).get(trans.security.decode_id(user_type_fd_id))
            elif user.values:
            ...
            if user_type_form_definition:
                values = self.get_form_values(
                    trans, user, user_type_form_definition, **kwd
                )
            else:
                values = {}
            flush_needed = False
        ...
        return trans.response.send_redirect(
            web.url_for(
                controller="user", action="manage_user_info", cntrller=cntrller, **kwd
            )
        )
    ...
    def change_password(self, trans, token=None, id=None, **kwd):
        ...
        return trans.fill_template(
            "/webapps/tool_shed/user/change_password.mako", token=token, id=id
        )
    ...
    def logout(self, trans, logout_all=False, **kwd):
        ...
        message = (
            'You have been logged out.<br>To log in again <a target="_top" href="%s">go to the home page</a>.'
            % (url_for("/"))
        )
        if (
            trans.app.config.use_remote_user
            and trans.app.config.remote_user_logout_href
        ):
    ...
    def __validate(self, trans, email, password, confirm, username):
        ...
        if username in ["repos"]:
            return (
                "The term <b>%s</b> is a reserved word in the tool shed, so it cannot be used as a public user name."
                % escape(username)
            )
