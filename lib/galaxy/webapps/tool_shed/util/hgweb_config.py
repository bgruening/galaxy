...
class HgWebConfigManager(object):
    ...
    def get_entry(self, lhs):
        ...
        try:
            ...
            try:
                ...
                raise Exception(
                    "Entry for repository %s missing in file %s."
    ...
    def make_backup(self):
        ...
        hgweb_config_copy = os.path.join(
            self.hgweb_config_dir, hgweb_config_backup_filename
        )
        shutil.copy(
            os.path.abspath(self.hgweb_config), os.path.abspath(hgweb_config_copy)
        )
