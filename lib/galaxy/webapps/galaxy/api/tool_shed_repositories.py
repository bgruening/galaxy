...
class ToolShedRepositoriesController(BaseAPIController):
    ...
    def __flatten_repository_dependency_list(self, trans, tool_shed_repository):
        ...
        for dependency in tool_shed_repository.repository_dependencies:
            if len(dependency.repository_dependencies) > 0:
                sub_dependencies = self.__flatten_repository_dependency_list(
                    trans, dependency
                )
            ...
            if dependency not in dependencies:
                dependencies.append(
                    dependency.as_dict(
                        value_mapper=self.__get_value_mapper(
    ...
    def __get_repo_info_dict(self, trans, repositories, tool_shed_url):
        ...
        params = dict(
            repository_ids=str(",".join(repo_ids)),
    ...
    def __get_tool_dependencies(self, metadata, tool_dependencies=None):
        ...
        if metadata["has_repository_dependencies"]:
            for dependency in metadata["repository_dependencies"]:
                tool_dependencies = self.__get_tool_dependencies(
                    dependency, tool_dependencies
                )
    ...
    def check_for_updates(self, trans, **kwd):
        ...
        message, status = repository_util.check_for_updates(
            self.app, trans.install_model, repository_id
        )
    ...
    def get_latest_installable_revision(self, trans, payload, **kwd):
        ...
        try:
            ...
            message = (
                "Error attempting to retrieve the latest installable revision from tool shed %s for repository %s owned by %s: %s"
    ...
    def shed_repository(self, trans, **kwd):
        ...
        if tool_ids is not None:
            if len(tool_shed_url) == 0:
                ...
                tool_shed_url = common_util.get_tool_shed_url_from_tool_shed_registry(
                    self.app, tool_ids[0].split("/")[0]
                )
            found_repository = json.loads(
                util.url_get(
                    tool_shed_url,
                    params=dict(tool_ids=",".join(tool_ids)),
                    pathspec=["api", "repositories"],
                )
            )
            ...
            repository_data["repository"] = json.loads(
                util.url_get(tool_shed_url, pathspec=["api", "repositories", tsr_id])
            )
            ...
            repository_data["repository"]["metadata"] = json.loads(
                util.url_get(
                    tool_shed_url, pathspec=["api", "repositories", tsr_id, "metadata"]
                )
            )
        repository_data["shed_conf"] = tool_util.build_shed_tool_conf_select_field(
            trans.app
        ).to_dict()
        ...
        for changeset, metadata in repository_data["repository"]["metadata"].items():
            ...
            if metadata["has_repository_dependencies"]:
                for repository_dependency in metadata["repository_dependencies"]:
                    tool_dependencies[changeset] = self.__get_tool_dependencies(
                        repository_dependency, tool_dependencies[changeset]
                    )
    ...
    def shed_search(self, trans, **kwd):
        ...
        response = json.loads(
            util.url_get(
                tool_shed_url, params=dict(q=q), pathspec=["api", "repositories"]
            )
        )
        return response
    ...
    def import_workflow(self, trans, payload, **kwd):
        ...
        if api_key is None:
            raise HTTPBadRequest(
                detail="Missing required parameter 'key' whose value is the API key for the Galaxy user importing the specified workflow."
            )
    ...
    def index(self, trans, **kwd):
        ...
        if "changeset" in kwd:
            clause_list.append(
                ...
                == kwd.get("changeset", None)
            )
        tool_shed_repository_dicts = []
        query = trans.install_model.context.query(
    ...
    def install(self, trans, **kwd):
        ...
        if new_repositories:
            ...
            encoded_kwd, query, tool_shed_repositories, encoded_repository_ids = irm.initiate_repository_installation(
                installation_dict
            )
            return json.dumps(
                dict(
                    ...
                    repositories=[
                        trans.security.encode_id(repo.id) for repo in new_repositories
                    ],
                )
            )
    ...
    def install_repository_revision(self, trans, payload, **kwd):
        ...
        irm = InstallRepositoryManager(self.app)
        installed_tool_shed_repositories = irm.install(
            tool_shed_url, name, owner, changeset_revision, payload
        )
    ...
    def install_repository_revisions(self, trans, payload, **kwd):
        ...
        install_resolver_dependencies = payload.get(
        ...
        for tool_shed_url, name, owner, changeset_revision in zip(
            tool_shed_urls, names, owners, changeset_revisions
        ):
            ...
            installed_tool_shed_repositories = self.install_repository_revision(
                trans, **current_payload
            )
    ...
    def uninstall_repository(self, trans, id=None, **kwd):
        ...
        if id:
            ...
            try:
                ...
                raise HTTPBadRequest(
                    detail="Missing required parameter '%s'" % e.args[0]
                )
        ...
        errors = irm.uninstall_repository(
            repository=repository, remove_from_disk=kwd.get("remove_from_disk", True)
        )
        if not errors:
            action = "removed" if kwd.get("remove_from_disk", True) else "deactivated"
            return {
                "message": "The repository named %s has been %s."
    ...
    def show(self, trans, id, **kwd):
        ...
        if tool_shed_repository is None:
            log.debug(
                "Unable to locate tool_shed_repository record for id %s." % (str(id))
            )
            return {}
        tool_shed_repository_dict = tool_shed_repository.as_dict(
            value_mapper=self.__get_value_mapper(trans, tool_shed_repository)
    ...
    def status(self, trans, id, **kwd):
        ...
        ] = self.__flatten_repository_dependency_list(trans, tool_shed_repository)
        return tool_shed_repository_dict
