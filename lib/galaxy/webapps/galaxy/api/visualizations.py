...
class VisualizationsController(
    BaseAPIController, UsesVisualizationMixin, SharableMixin, UsesAnnotations
):
    """
    ...
    def show(self, trans, id, **kwargs):
        ...
        if trans.app.visualizations_registry:
            visualization = trans.app.visualizations_registry.get_plugin(
                dictionary["type"]
            )
    ...
    def update(self, trans, id, payload, **kwargs):
        ...
        if (
            ...
            revision = self.add_visualization_revision(
                trans, visualization, config, title, dbkey
            )
    ...
    def _validate_and_parse_payload(self, payload):
        ...
        for key, val in payload.items():
            ...
            if key == "type":
                if not isinstance(val, string_types):
                    raise ValidationError(
                        "%s must be a string or unicode: %s" % (key, str(type(val)))
                    )
                val = util.sanitize_html.sanitize_html(val)
            elif key == "config":
                if not isinstance(val, dict):
                    raise ValidationError(
                        "%s must be a dictionary: %s" % (key, str(type(val)))
                    )
            ...
            #   (they are still valid for create, tho)
                ...
                if not isinstance(val, string_types):
                    raise ValidationError(
                        "%s must be a string: %s" % (key, str(type(val)))
                    )
                val = util.sanitize_html.sanitize_html(val)
            elif key == "dbkey":
                if not isinstance(val, string_types):
