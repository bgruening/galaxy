...
from galaxy import exceptions as galaxy_exceptions, managers, model, util, web
...
class DatasetsController(BaseAPIController, UsesVisualizationMixin):
    def __init__(self, app):
        ...
        self.serializer_by_type = {
            "dataset": self.hda_serializer,
            "dataset_collection": self.hdca_serializer,
        }
        self.ldda_manager = managers.lddas.LDDAManager(app)
        self.history_contents_manager = managers.history_contents.HistoryContentsManager(
            app
        )
        self.history_contents_filters = managers.history_contents.HistoryContentsFilters(
            app
        )
    ...
    def index(self, trans, limit=500, offset=0, history_id=None, **kwd):
        ...
        order_by = self._parse_order_by(
            ...
            order_by_string=kwd.get("order", "create_time-dsc"),
        )
        container = None
        if history_id:
            container = self.history_manager.get_accessible(
                self.decode_id(history_id), trans.user
            )
        ...
        return [
            self.serializer_by_type[content.history_content_type].serialize_to_view(
                content, user=trans.user, trans=trans, view="summary"
            )
    ...
    def _search_features(self, trans, dataset, query):
        ...
        if dataset.can_convert_to("fli"):
            ...
            if converted_dataset:
                data_provider = FeatureLocationIndexDataProvider(
    ...
    def _data(
        self, trans, dataset, chrom, low, high, start_val=0, max_vals=None, **kwargs
    ):
        """
        ...
        if mode == "Coverage":
            ...
            indexer = data_provider_registry.get_data_provider(
                trans, original_dataset=dataset, source="index"
            )
        ...
        result.update(
            {"dataset_type": data_provider.dataset_type, "extra_info": extra_info}
        )
        return result
    ...
    def _raw_data(self, trans, dataset, provider=None, **kwargs):
        ...
        if provider:
            if provider in registry.dataset_type_name_to_data_provider:
                data_provider = registry.dataset_type_name_to_data_provider[provider](
                    dataset
                )

            elif dataset.datatype.has_dataprovider(provider):
                kwargs = dataset.datatype.dataproviders[
                    provider
                ].parse_query_string_settings(kwargs)
                # use dictionary to allow more than the data itself to be returned (data totals, other meta, etc.)
                return {
                    "data": list(
                        dataset.datatype.dataprovider(dataset, provider, **kwargs)
                    )
                }

            else:
                raise dataproviders.exceptions.NoProviderAvailable(
                    dataset.datatype, provider
                )

        # no provider name: look up by datatype
        else:
            data_provider = registry.get_data_provider(
                trans, raw=True, original_dataset=dataset
            )
                ...
                rval.append(
                    {
                        ...
                        "path": os.path.relpath(
                            os.path.join(root, directory), extra_files_path
                        ),
                    }
                )
            for file in files:
                rval.append(
                    {
                        "class": "File",
                        "path": os.path.relpath(
                            os.path.join(root, file), extra_files_path
                        ),
                    }
                )
    ...
    def get_metadata_file(
        ...
        try:
            hda = self.hda_manager.get_accessible(decoded_content_id, trans.user)
            file_ext = hda.metadata.spec.get(metadata_file).get(
                "file_ext", metadata_file
            )
            fname = "".join(
                c in util.FILENAME_VALID_CHARS and c or "_" for c in hda.name
            )[0:150]
            trans.response.headers["Content-Type"] = "application/octet-stream"
            trans.response.headers[
                "Content-Disposition"
            ] = 'attachment; filename="Galaxy%s-[%s].%s"' % (hda.hid, fname, file_ext)
            return open(hda.metadata.get(metadata_file).file_name, "rb")
        except Exception as e:
            log.exception(
                "Error getting metadata_file (%s) for dataset (%s) from history (%s)",
                metadata_file,
                history_content_id,
                history_id,
            )
    ...
    def _get_or_create_converted(self, trans, original, target_ext, **kwargs):
        try:
            ...
            raise galaxy_exceptions.RequestParameterInvalidException(
                "Conversion not possible", **exc_data
            )
