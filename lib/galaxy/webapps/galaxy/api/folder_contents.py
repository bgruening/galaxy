...
class FolderContentsController(
    BaseAPIController, UsesLibraryMixin, UsesLibraryMixinItems
):
    """
    ...
    def index(self, trans, folder_id, **kwd):
        ...
        if (
            trans.app.security_agent.can_access_library(
                current_user_roles, folder.parent_library
            )
            or is_admin
        ):
            pass
            ...
            raise exceptions.ObjectNotFound(
                "Folder with the id provided ( %s ) was not found" % str(folder_id)
            )
            ...
            if content_item.api_type == "folder":
                ...
                can_manage = is_admin or (
                    ...
                    and trans.app.security_agent.can_manage_dataset(
                        current_user_roles,
                        content_item.library_dataset_dataset_association.dataset,
                    )
                )

                nice_size = util.nice_size(
                    int(content_item.library_dataset_dataset_association.get_size())
                )
                ...
                if content_item.library_dataset_dataset_association.message:
                    return_item.update(
                        dict(
                            message=content_item.library_dataset_dataset_association.message
                        )
                    )
        ...
        can_add_library_item = (
            is_admin
            or trans.app.security_agent.can_add_library_item(current_user_roles, folder)
        )
    ...
    def build_path(self, trans, folder):
        ...
        if folder.parent_id is None:
            path_to_root.append(
                ("F" + trans.security.encode_id(folder.id), folder.name)
            )
        else:
            ...
            upper_folder = trans.sa_session.query(trans.app.model.LibraryFolder).get(
                folder.parent_id
            )
