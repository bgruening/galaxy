...
class WorkflowsAPIController(
    BaseAPIController, UsesStoredWorkflowMixin, UsesAnnotations, SharableMixin
):
    ...
    def get_workflows_list(self, trans, kwd):
        ...
        for wf in (
            ...
            .options(
                joinedload("latest_workflow").undefer("step_count").lazyload("steps")
            )
        ...
        for wf_sa in (
            ...
            .options(
                joinedload("stored_workflow")
                .joinedload("latest_workflow")
                .undefer("step_count")
                .lazyload("steps")
            )
            ...
            item["show_in_tool_panel"] = False
            if user is not None:
                item["show_in_tool_panel"] = wf_sa.stored_workflow.show_in_tool_panel(
                    user_id=user.id
                )
            rval.append(item)
        if missing_tools:
                ...
                if "steps" in workflow_details:
                    for step in workflow_details["steps"]:
                        ...
                        if (
                            ...
                            and self.app.toolbox.is_missing_shed_tool(tool_id)
                        ):
            ...
            for workflow in workflows_missing_tools:
                for tool_id in workflow["missing_tools"]:
                    ...
                    if repo_identifier not in workflows_by_toolshed:
                        ...
                        if (
                            tool_id
                            not in workflows_by_toolshed[repo_identifier]["tools"]
                        ):
                            workflows_by_toolshed[repo_identifier]["tools"].append(
                                tool_id
                            )
                        if (
                            workflow["name"]
                            not in workflows_by_toolshed[repo_identifier]["workflows"]
                        ):
                            workflows_by_toolshed[repo_identifier]["workflows"].append(
                                workflow["name"]
                            )
    ...
    def show(self, trans, id, **kwd):
        ...
        if (
            stored_workflow.importable is False
        ...
        if "installed_repository_file" in payload:
            ...
            if not os.path.exists(installed_repository_file):
                raise exceptions.RequestParameterInvalidException(
                    "Workflow file '%s' not found" % installed_repository_file
                )
            ...
            if archive_source:
                if archive_source.startswith("file://"):
                    ...
                    try:
                        ...
                        raise exceptions.MessageException(
                            "Failed to open URL '%s'." % escape(archive_source)
                        )
    ...
    def workflow_dict(self, trans, workflow_id, **kwd):
        ...
        if download_format == "json-download":
            ...
            sname = "".join(c in util.FILENAME_VALID_CHARS and c or "_" for c in sname)[
                0:150
            ]
            ...
            trans.response.headers[
                "Content-Disposition"
            ] = 'attachment; filename="Galaxy-Workflow-%s.%s"' % (sname, extension)
        ...
        if workflow_dict:
            ...
            if "annotation" in workflow_dict:
                ...
                self.add_item_annotation(
                    trans.sa_session, trans.get_user(), stored_workflow, newAnnotation
                )
    ...
    def build_module(self, trans, payload={}):
        ...
        if "tool_state" not in payload:
            module_state = {}
            populate_state(
                trans, module.get_inputs(), inputs, module_state, check=False
            )
    ...
    def __api_import_new_workflow(self, trans, payload, **kwd):
        ...
        workflow, missing_tool_tups = self._workflow_from_dict(
            trans, raw_workflow_description, **from_dict_kwds
        )
        if importable:
        ...
        if import_tools:
            ...
            for k in tools:
                ...
                irm.install(tool_shed_url, name, owner, changeset_revision, payload)
        return item
    ...
    def __import_or_update_kwds(self, payload):
        ...
        return {"exact_tools": exact_tools, "fill_defaults": fill_defaults}
    ...
    def __api_import_shared_workflow(self, trans, workflow_id, payload, **kwd):
        try:
            stored_workflow = self.get_stored_workflow(
    ...
    def invoke(self, trans, workflow_id, payload, **kwd):
        ...
        for run_config in run_configs:
            ...
            invocation = self.encode_all_ids(
    ...
    def index_invocations(self, trans, workflow_id=None, **kwd):
        ...
        if encoded_history_id:
            history = self.history_manager.get_accessible(
                self.decode_id(encoded_history_id),
                trans.user,
    ...
    def show_invocation(self, trans, workflow_id, invocation_id, **kwd):
        ...
        workflow_invocation = self.workflow_manager.get_invocation(
    ...
    def cancel_invocation(self, trans, workflow_id, invocation_id, **kwd):
        ...
        """
        decoded_workflow_invocation_id = self.decode_id(invocation_id)
        workflow_invocation = self.workflow_manager.cancel_invocation(
            trans, decoded_workflow_invocation_id
        )
    ...
    def invocation_step(self, trans, workflow_id, invocation_id, step_id, **kwd):
        ...
        invocation_step = self.workflow_manager.get_invocation_step(
            trans, decoded_invocation_step_id
        )
        return self.__encode_invocation_step(trans, invocation_step)

    @expose_api
    def update_invocation_step(
        self, trans, workflow_id, invocation_id, step_id, payload, **kwd
    ):
        """
        ...
        invocation_step = self.workflow_manager.update_invocation_step(
            trans, decoded_invocation_step_id, action=action
        )
        return self.__encode_invocation_step(trans, invocation_step)
