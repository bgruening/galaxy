...
class DatatypesController(BaseAPIController):
    ...
    def mapping(self, trans, **kwd):
        ...
        try:
            ...
            return dict(
                ext_to_class_name=ext_to_class_name, class_to_classes=class_to_classes
            )

        except Exception as exception:
            log.error(
                "could not get datatype mapping: %s", str(exception), exc_info=True
            )
            if not isinstance(exception, exceptions.MessageException):
                raise exceptions.InternalServerError(str(exception))
            else:
    ...
    def converters(self, trans, **kwd):
        ...
        for (
            source_type,
            targets,
        ) in self._datatypes_registry.datatype_converters.items():
