...
class FoldersController(BaseAPIController, UsesLibraryMixin, UsesLibraryMixinItems):
    ...
    def show(self, trans, id, **kwd):
        ...
        *GET /api/folders/{encoded_folder_id}
        ...
        folder = self.folder_manager.get(
            trans, folder_id, check_manageable=False, check_accessible=True
        )
    ...
    def create(self, trans, encoded_parent_folder_id, payload=None, **kwd):
        ...
        *POST /api/folders/{encoded_parent_folder_id}
        ...
        new_folder = self.folder_manager.create(
            trans, parent_folder.id, name, description
        )
        ...
        *POST /api/folders/{encoded_folder_id}/permissions
        ...
        if action is None:
            ...
            if len(invalid_manage_roles_names) > 0:
                log.warning(
                    "The following roles could not be added to the manage folder permission: "
            ...
            if len(invalid_modify_roles_names) > 0:
                log.warning(
                    "The following roles could not be added to the modify folder permission: "
                    + str(invalid_modify_roles_names)
                )

            permissions = {
                trans.app.security_agent.permitted_actions.LIBRARY_ADD: valid_add_roles
            }
            permissions.update(
                {
                    trans.app.security_agent.permitted_actions.LIBRARY_MANAGE: valid_manage_roles
                }
            )
            permissions.update(
                {
    ...
    def delete(self, trans, encoded_folder_id, **kwd):
        ...
        * DELETE /api/folders/{encoded_folder_id}
        ...
        """
        folder = self.folder_manager.get(
            trans, self.folder_manager.cut_and_decode(trans, encoded_folder_id), True
        )
    ...
    def __decode_id(self, trans, encoded_id, object_name=None):
        ...
        try:
            ...
            raise exceptions.MalformedId(
                "Malformed %s id specified, unable to decode." % object_name
                if object_name is not None
                else ""
            )
        except ValueError:
            raise exceptions.MalformedId(
                "Wrong %s id specified, unable to decode." % object_name
                if object_name is not None
                else ""
            )
