...
class RemoteFilesAPIController(BaseAPIController):
    ...
    def index(self, trans, **kwd):
        ...
        if target == "userdir":
            ...
            if full_import_dir is not None:
                if format == "jstree":
                    ...
                    try:
                        ...
                        log.error(
                            "Could not get user import files: %s",
                            str(exception),
                            exc_info=True,
                        )
                        raise exceptions.InternalServerError(
            ...
            try:
                ...
                log.warning(
                    "Could not get ftp files: %s", str(exception), exc_info=True
                )
                return None
        return response
    ...
    def __load_all_filenames(self, directory, whitelist=None):
        ...
        if self.__safe_directory(directory, whitelist=whitelist):
            for (dirpath, dirnames, filenames) in safe_walk(
                directory, whitelist=whitelist
            ):
            ...
            for (dirpath, dirnames, filenames) in safe_walk(
                ...
                for dirname in dirnames:
                    dir_path = os.path.relpath(
                        os.path.join(dirpath, dirname), directory
                    )
