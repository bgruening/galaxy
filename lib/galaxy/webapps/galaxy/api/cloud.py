    ...
    def get(self, trans, payload, **kwargs):
        ...
        * POST /api/cloud/storage/get
        ...
        if len(missing_arguments) > 0:
            raise ActionInputError(
                "The following required arguments are missing in the payload: {}".format(
                    missing_arguments
                )
            )
        ...
        if not isinstance(objects, list):
            raise ActionInputError(
                "The `objects` should be a list, but received an object of type {} instead.".format(
                    type(objects)
                )
            )
        ...
        for dataset in datasets:
            rtv.append(
                self.datasets_serializer.serialize_to_view(dataset, view="summary")
            )
        return rtv
    ...
    def send(self, trans, payload, **kwargs):
        ...
        * POST /api/cloud/storage/send
        ...
        log.info(
            ...
            "".format(
                "all the dataset in the given history"
                if not dataset_ids
                else len(dataset_ids),
                authz_id,
                history_id,
            )
        )
