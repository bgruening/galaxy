...
from tool_shed.util import common_util, repository_util, tool_util
...
class ToolShedController(BaseAPIController):
    ...
    def __get_repo_dict_by_id(self, id):
        ...
        tool_shed_repository_dict = tool_shed_repository.as_dict(
            value_mapper=self.__get_value_mapper(tool_shed_repository)
        )
        ...
        ] = self.__flatten_repository_dependency_list(tool_shed_repository)
    ...
    def tool_json(self, trans, **kwd):
        ...
        response = json.loads(
            util.url_get(
                tool_shed_url,
                params=dict(
                    tsr_id=tsr_id, guid=guid, changeset=changeset.split(":")[-1]
                ),
                pathspec=["api", "tools", "json"],
            )
        )
        return response
    ...
    def category(self, trans, **kwd):
        ...
        params = dict(
            installable=True, sort_order=sort_order, sort_key=sort_key, page=page
        )
        tool_shed_url = common_util.get_tool_shed_url_from_tool_shed_registry(
            trans.app, tool_shed_url
        )
    ...
    def repository(self, trans, **kwd):
        ...
        if tool_ids is not None:
            if len(tool_shed_url) == 0:
                ...
                tool_shed_url = common_util.get_tool_shed_url_from_tool_shed_registry(
                    self.app, tool_ids[0].split("/repos")[0]
                )
            ...
            repository_data["repository"] = json.loads(
                util.url_get(
                    tool_shed_url, pathspec=["api", "repositories", repository_id]
                )
            )
        ...
        repository_data["repository"]["metadata"] = json.loads(
            util.url_get(
                tool_shed_url,
                pathspec=["api", "repositories", repository_id, "metadata"],
            )
        )
        ...
        for changeset in repository_data["repository"]["metadata"]:
            if changeset in tools:
                repository_data["repository"]["metadata"][changeset]["tools"] = tools[
                    changeset
                ]
            else:
