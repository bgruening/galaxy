...
from galaxy.managers import folders, libraries, roles
...
class LibrariesController(BaseAPIController):
    ...
    def index(self, trans, **kwd):
        ...
        * GET /api/libraries:
        ...
        for library in query:
            libraries.append(
                self.library_manager.get_library_dict(trans, library, prefetched_ids)
            )
        return libraries
    ...
    def update(self, trans, id, payload=None, **kwd):
       ...
       .. note:: Currently, only admin users can update libraries. Also the library must not be `deleted`.
        ...
        updated_library = self.library_manager.update(
            trans, library, name, description, synopsis
        )
    ...
    def set_permissions(self, trans, encoded_library_id, payload=None, **kwd):
        ...
        *POST /api/libraries/{encoded_library_id}/permissions
        ...
        if action is None:
            ...
            permissions = {
                trans.app.security_agent.permitted_actions.LIBRARY_ACCESS: valid_access_roles
            }
            permissions.update(
                {
