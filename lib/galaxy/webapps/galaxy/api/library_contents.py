        ...
        * GET /api/libraries/{library_id}/contents:
        ...
        try:
            ...
            raise exceptions.MalformedId(
                "Malformed library id ( %s ) specified, unable to decode." % library_id
            )
        try:
            library = (
        ...
        if not (
            trans.user_is_admin
            or trans.app.security_agent.can_access_library(current_user_roles, library)
        ):
            raise exceptions.RequestParameterInvalidException(
                "No library found with the id provided."
            )
        ...
        for content in traverse(library.root_folder):
            ...
            rval.append(
                dict(
                    ...
                    url=url_for(
                        "library_content", library_id=library_id, id=encoded_id
                    ),
                )
            )
        return rval
    ...
    def show(self, trans, id, library_id, **kwd):
        ...
        * GET /api/libraries/{library_id}/contents/{id}
        ...
        if class_name == "LibraryFolder":
            content = self.get_library_folder(
                trans, content_id, check_ownership=False, check_accessible=True
            )
            ...
            content = self.get_library_dataset(
                trans, content_id, check_ownership=False, check_accessible=True
            )
    ...
    def create(self, trans, library_id, payload, **kwd):
        ...
        * POST /api/libraries/{library_id}/contents:
        ...
        if create_type not in ("file", "folder", "collection"):
            trans.response.status = 400
            return (
                "Invalid value for 'create_type' parameter ( %s ) specified."
                % create_type
            )
        ...
        try:
            ...
            parent = self.get_library_folder(
                trans, folder_id, check_ownership=False, check_accessible=False
            )
        except Exception as e:
            return str(e)
        ...
        if create_type == "file":
            ...
            status, output = self._create_folder(
                trans, real_folder_id, library_id, **payload
            )
            ...
            self.check_user_can_add_to_library_item(
                trans, parent, check_accessible=True
            )
            ...
            return [
                dictify_dataset_collection_instance(
                    dataset_collection_instance, security=trans.security, parent=parent
                )
            ]
        if status != 200:
    ...
    def update(self, trans, id, library_id, payload, **kwd):
        ...
        * PUT /api/libraries/{library_id}/contents/{id}
        ...
        if "converted_dataset_id" in payload:
            ...
            content = self.get_library_dataset(
                trans, id, check_ownership=False, check_accessible=False
            )
            content_conv = self.get_library_dataset(
                trans, converted_id, check_ownership=False, check_accessible=False
            )
    ...
    def _decode_library_content_id(self, content_id):
        if len(content_id) % 16 == 0:
            ...
            raise HTTPBadRequest(
                "Malformed library content id ( %s ) specified, unable to decode."
                % str(content_id)
            )
        ...
        * DELETE /api/libraries/{library_id}/contents/{id}
        ...
        try:
            ...
            if not (user_is_admin or can_modify):
                ...
                rval.update(
                    {"error": "Unauthorized to delete or purge this library dataset"}
                )
                return rval
            ...
            if purge:
                ...
                can_purge_dataset = (
                    not dataset.purged and no_history_assoc and no_library_assoc
                )
            ...
            log.exception(
                "library_contents API, delete: uncaught exception: %s, %s", id, str(kwd)
            )
