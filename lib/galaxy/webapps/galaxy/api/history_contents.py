...
class HistoryContentsController(
    BaseAPIController, UsesLibraryMixin, UsesLibraryMixinItems, UsesTagsMixin
):
        ...
        for content in history.contents_iter(**contents_kwds):
            ...
            if isinstance(content, trans.app.model.HistoryDatasetAssociation):
                ...
                hda_dict = self.hda_serializer.serialize_to_view(
                    content, view=view, user=trans.user, trans=trans
                )
                rval.append(hda_dict)

            elif isinstance(
                content, trans.app.model.HistoryDatasetCollectionAssociation
            ):
    ...
    def index_jobs_summary(self, trans, history_id, **kwd):
        ...
        * GET /api/histories/{history_id}/jobs_summary
        ...
        if ids is None:
            ...
            types = util.listify(types)
        return [
            self.encode_all_ids(trans, s)
            for s in fetch_job_states(self.app, trans.sa_session, ids, types)
        ]
    ...
    def show_jobs_summary(self, trans, id, history_id, **kwd):
        ...
        * GET /api/histories/{history_id}/contents/{type}/{id}/jobs_summary
        ...
        if contents_type == "dataset":
            ...
            dataset_collection_instance = self.__get_accessible_collection(
                trans, id, history_id
            )
            ...
            if job_source_type == "Job":
                ...
                implicit_collection_jobs = (
                    dataset_collection_instance.implicit_collection_jobs
                )
    ...
    def __show_dataset_collection(self, trans, id, history_id, **kwd):
        ...
        return self.__collection_dict(
            trans, dataset_collection_instance, view=view, fuzzy_count=fuzzy_count
        )
    ...
    def __stream_dataset_collection(self, trans, dataset_collection_instance):
        ...
        for name, hda in zip(names, hdas):
            if hda.state != hda.states.OK:
                continue
            for file_path, relpath in hda.datatype.to_archive(
                trans=trans, dataset=hda, name=name
            ):
                archive.add(file=file_path, relpath=relpath)
        archive_name = "%s: %s.%s" % (
            dataset_collection_instance.hid,
        ...
        * POST /api/histories/{history_id}/contents
        ...
        if type == "dataset":
            ...
            if source == "library_folder":
                return self.__create_datasets_from_library_folder(
                    trans, history, payload, **kwd
                )
            else:
    ...
    def __create_dataset(self, trans, history, payload, **kwd):
        ...
        if content is None:
            raise exceptions.RequestParameterMissingException(
                "'content' id of lda or hda is missing"
            )
        ...
        # copy an existing, accessible hda
            ...
            self.history_manager.error_unless_accessible(
                original.history, trans.user, current_history=trans.history
            )
       ...
       .. note:: Elements may be nested depending on the collection_type
        ...
        if source == "new_collection":
            ...
            dataset_collection_instance = service.create(
                trans, parent=history, **create_params
            )
        elif source == "hdca":
            content = payload.get("content", None)
            if content is None:
        ...
        return self.__collection_dict(
            trans, dataset_collection_instance, view="element"
        )
    ...
    def show_roles(self, trans, encoded_dataset_id, **kwd):
        ...
        * GET /api/histories/{history_id}/contents/datasets/{encoded_dataset_id}/permissions
        ...
        hda = self.hda_manager.get_owned(
            self.decode_id(encoded_dataset_id),
            trans.user,
    ...
    def update_permissions(
        self, trans, history_id, history_content_id, payload=None, **kwd
    ):
        """
        ...
        * PUT /api/histories/{history_id}/contents/datasets/{encoded_dataset_id}/permissions
        ...
        if payload:
            kwd.update(payload)
        hda = self.hda_manager.get_owned(
            self.decode_id(history_content_id),
            trans.user,
            current_history=trans.history,
            trans=trans,
        )
    ...
    def update_batch(self, trans, history_id, payload, **kwd):
        ...
        * PUT /api/histories/{history_id}/contents
        ...
        for hdca_id in hdca_ids:
            ...
            dataset_collection_instance = self.__get_accessible_collection(
                trans, hdca_id, history_id
            )
            rval.append(
                self.__collection_dict(
                    trans, dataset_collection_instance, view="summary"
                )
            )
        return rval
    ...
    def update(self, trans, history_id, id, payload, **kwd):
        ...
        * PUT /api/histories/{history_id}/contents/{id}
        ...
        if contents_type == "dataset":
            ...
            return self.__update_dataset_collection(
                trans, history_id, id, payload, **kwd
            )
        else:
    ...
    def __update_dataset_collection(self, trans, history_id, id, payload, **kwd):
        return trans.app.dataset_collections_service.update(
            trans, "history", id, payload
        )
        ...
        * DELETE /api/histories/{history_id}/contents/{type}s/{id}
        ...
        if contents_type == "dataset":
            ...
            if kwd.get("payload", None):
                ...
                recursive = util.string_as_bool(
                    kwd["payload"].get("recursive", recursive)
                )

            trans.app.dataset_collections_service.delete(
                trans, "history", id, recursive=recursive, purge=purge
            )
    ...
    def __index_v2(self, trans, history_id, **kwd):
        ...
        * GET /api/histories/{history_id}/contents
        ...
        order_by = self._parse_order_by(
            ...
            order_by_string=kwd.get("order", "hid-asc"),
        )
        ...
        contents = self.history_contents_manager.contents(
            history, filters=filters, limit=limit, offset=offset, order_by=order_by
        )
    ...
    @expose_api_raw
    def archive(
        self, trans, history_id, filename="", format="tgz", dry_run=True, **kwd
    ):
        """
        ...
        def build_archive_files_and_paths(content, *parents):
            ...
            if parents:
                ...
                archive_path = os.path.join(
                    archive_path, *build_path_from_parents(parents)[::-1]
                )
            ...
            if self.hda_manager.is_composite(content):
                ...
                paths_and_files.append(
                    (
                        content.file_name,
                        os.path.join(archive_path, content.name + ".html"),
                    )
                )
