...
from galaxy.managers import citations, histories, users, workflows
from galaxy.util import restore_text, string_as_bool
...
class HistoriesController(
    BaseAPIController, ExportsHistoryMixin, ImportsHistoryMixin, SharableMixin
):
    ...
    def index(self, trans, deleted="False", **kwd):
        ...
        * GET /api/histories/deleted:
        ...
        order_by = self._parse_order_by(
            manager=self.manager, order_by_string=kwd.get("order", "create_time-dsc")
        )
    ...
    def citations(self, trans, history_id, **kwd):
        ...
        return [
            citation.to_dict("bibtex")
            for citation in self.citations_manager.citations_for_tool_ids(tool_ids)
        ]
    ...
    def shared_with_me(self, trans, **kwd):
        ...
        * GET /api/histories/shared_with_me:
        ...
        histories = self.manager.list_shared_with(
            current_user, filters=filters, order_by=order_by, limit=limit, offset=offset
        )
        rval = []
        for history in histories:
            history_dict = self.serializer.serialize_to_view(
    ...
    def create(self, trans, payload, **kwd):
        ...
        * POST /api/histories:
        ...
        if "archive_source" in payload:
            ...
            self.queue_history_import(
                trans, archive_type=archive_type, archive_source=archive_source
            )
            return {
                "message": "Importing history from source '%s'. This history will be visible when the import is complete."
                % archive_source
            }
        ...
        if copy_this_history_id:
            ...
            new_history = original_history.copy(
                name=hist_name, target_user=trans.user, all_datasets=all_datasets
            )
    ...
    def delete(self, trans, id, **kwd):
        ...
        * DELETE /api/histories/{id}
        ...
        history = self.manager.get_owned(
            self.decode_id(history_id), trans.user, current_history=trans.history
        )
        if purge:
    ...
    def archive_export(self, trans, id, **kwds):
        ...
        * PUT /api/histories/{id}/exports:
        ...
        if up_to_date and jeha.ready:
            ...
            return dict(
                download_url=url_for("history_archive_download", id=id, jeha_id=jeha_id)
            )
        else:
    ...
    def archive_download(self, trans, id, jeha_id, **kwds):
        ...
        * GET /api/histories/{id}/exports/{jeha_id}:
        ...
        matching_exports = [
            e for e in history.exports if trans.security.encode_id(e.id) == jeha_id
        ]
    ...
    def get_custom_builds_metadata(self, trans, id, payload=None, **kwd):
        ...
        return {
            "installed_builds": [
                {"label": ins, "value": ins} for ins in installed_builds
            ],
            "fasta_hdas": [
                {
                    "label": "%s: %s" % (hda.hid, hda.name),
                    "value": trans.security.encode_id(hda.id),
                }
