...
def app_factory(global_conf, load_app_kwds={}, **kwargs):
    ...
    webapp.add_route(
        "/authnz/{provider}/login", controller="authnz", action="login", provider=None
    )
    webapp.add_route(
        "/authnz/{provider}/callback",
        controller="authnz",
        action="callback",
        provider=None,
    )
    webapp.add_route(
        "/authnz/{provider}/disconnect",
        controller="authnz",
        action="disconnect",
        provider=None,
    )
    ...
    if kwargs.get("middleware", True):
        webapp = wrap_in_middleware(
            webapp, global_conf, app.application_stack, **kwargs
        )
...
def populate_api_routes(webapp, app):
    ...
    valid_history_contents_types = ["dataset", "dataset_collection"]
    ...
    webapp.mapper.connect(
        "/api/histories/shared_with_me", action="shared_with_me", controller="histories"
    )
    ...
    _add_item_tags_controller(
        webapp, name_prefix="history_", path_prefix="/api/histories/{history_id}"
    )
    _add_item_tags_controller(
        webapp, name_prefix="workflow_", path_prefix="/api/workflows/{workflow_id}"
    )
    ...
    webapp.mapper.connect(
        "/api/tool_data/{id:.+?}/fields/{value:.+?}",
        action="show_field",
        controller="tool_data",
    )
    webapp.mapper.connect(
        "/api/tool_data/{id:.+?}/reload", action="reload", controller="tool_data"
    )
    webapp.mapper.resource(
        "dataset_collection", "dataset_collections", path_prefix="/api/"
    )
    ...
    webapp.mapper.connect(
        "/api/tools/all_requirements", action="all_requirements", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/error_stack", action="error_stack", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/build", action="build", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/reload", action="reload", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/tests_summary", action="tests_summary", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/test_data_path",
        action="test_data_path",
        controller="tools",
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/test_data_download",
        action="test_data_download",
        controller="tools",
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/test_data", action="test_data", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/diagnostics", action="diagnostics", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/citations", action="citations", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/download", action="download", controller="tools"
    )
    webapp.mapper.connect(
        "/api/tools/{id:.+?}/requirements", action="requirements", controller="tools"
    )
    webapp.mapper.connect(
    ...
    webapp.mapper.connect(
        ...
        action="manager_requirements",
        controller="tool_dependencies",
    )
    webapp.mapper.connect(
        "/api/dependency_resolvers/{id}/clean",
        action="clean",
        controller="tool_dependencies",
        conditions=dict(method=["POST"]),
    )
    webapp.mapper.connect(
        "/api/dependency_resolvers/{id}/dependency",
    ...
    webapp.mapper.connect(
        "/api/dependency_resolvers/{id}/requirements",
        action="resolver_requirements",
        controller="tool_dependencies",
    )
    webapp.mapper.resource(
        "dependency_resolver",
    ...
    webapp.mapper.connect(
        "/api/genomes/{id}/indexes", controller="genomes", action="indexes"
    )
    webapp.mapper.connect(
        "/api/genomes/{id}/sequences", controller="genomes", action="sequences"
    )
    ...
    webapp.mapper.connect(
        "/api/workflows/build_module", action="build_module", controller="workflows"
    )
    webapp.mapper.connect(
        "/api/workflows/menu",
        action="get_workflow_menu",
        controller="workflows",
        conditions=dict(method=["GET"]),
    )
    webapp.mapper.connect(
        "/api/workflows/menu",
        action="set_workflow_menu",
        controller="workflows",
        conditions=dict(method=["PUT"]),
    )
    ...
    webapp.mapper.connect(
        "/api/histories/{history_id}/contents/{type:%s}s/{id}/jobs_summary"
        % "|".join(valid_history_contents_types),
    ...
    webapp.add_route(
        "/visualization/show/{visualization_name}",
        controller="visualization",
        action="render",
    ...
    invoke_names = {"invocations": "", "usage": "_deprecated"}
    ...
    webapp.mapper.resource(
        ...
        parent_resources=dict(
            member_name="tool_shed_repository", collection_name="tool_shed_repositories"
        ),
    )
...
def _add_item_extended_metadata_controller(webapp, name_prefix, path_prefix, **kwd):
    ...
    webapp.mapper.resource(
        name, "extended_metadata", path_prefix=path_prefix, controller=controller
    )
...
def _add_item_annotation_controller(webapp, name_prefix, path_prefix, **kwd):
    ...
    webapp.mapper.resource(
        name, "annotation", path_prefix=path_prefix, controller=controller
    )
...
def _add_item_provenance_controller(webapp, name_prefix, path_prefix, **kwd):
    ...
    webapp.mapper.resource(
        name, "provenance", path_prefix=path_prefix, controller=controller
    )
...
def wrap_in_middleware(app, global_conf, application_stack, **local_conf):
    ...
    if debug:
        ...
        if asbool(conf.get("use_lint", False)):
            ...
            app = wrap_if_allowed(
                app, stack, lint.make_middleware, name="paste.lint", args=(conf,)
            )
...
def wrap_in_static(app, global_conf, plugin_frameworks=None, **local_conf):
    urlmap, cache_time = galaxy.web.framework.webapp.build_url_map(
