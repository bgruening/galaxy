...
class InteractiveEnvironmentsController(BaseUIController):
    ...
    def ready(self, trans, **kwd):
        ...
        try:
            ...
            log.error(
                "Invalid container interface key: %s", proxy_map.container_interface
            )
            return None
        ...
        if not proxy_map.port > 0:
            ...
            # container port
            log.debug(
                "Container %s (%s) mapping for port %s is not set in the proxy, attempting to get port mapping "
                "now",
                container.name,
                container.id,
                -proxy_map.port,
            ...
            if mapping:
                ...
                self.app.proxy_manager.update_proxy(
                    trans, host=mapping.hostaddr, port=mapping.hostport
                )
            else:
                log.warning(
                    "Container %s (%s) mapping for port %s cannot be determined!",
                    container.name,
                    container.id,
                    -proxy_map.port,
                )
