...
def render_hda_copied_from_history(trans, hda, children):
    ...
    history_id = trans.security.encode_id(
        hda.copied_from_history_dataset_association.history_id
    )
    ...
    template += (
        '<span class="bold">'
        + hda.copied_from_history_dataset_association.name
        + "</span>"
    )
...
def render_hda_copied_from_library(trans, hda, children):
    ...
    template += (
        '<span class="bold">'
        + hda.copied_from_library_dataset_dataset_association.name
        + "</span>"
    )
    template += "</div>"
...
def render_item_wf(trans, wf, children):
    ...
    """
    template = (
        '<div class="workflow"><div class="header"><span class="bold">'
        + wf.workflow.name
        + "</span>"
    )
    ...
    for input_index, input in enumerate(input_params.itervalues()):
        if input.name in param_values:
            if input.type == "repeat":
                ...
                tool_parameter_template += inputs_recursive_indent(
                    text=input.name, depth=depth
                )
                ...
                if is_valid:
                    tool_parameter_template += "<tr>"
                    tool_parameter_template += inputs_recursive_indent(
                        text=input.test_param.label, depth=depth
                    )
                    tool_parameter_template += (
                        "<td>" + input.cases[current_case].value + "</td><td></td></tr>"
                    )
                    inputs_recursive(
                        trans,
                ...
                tool_parameter_template += "<tr>"
                tool_parameter_template += inputs_recursive_indent(
                    text=input.group_title(param_values), depth=depth
                )
                tool_parameter_template += (
                    "<td>"
                    + str(len(param_values[input.name]))
                    + " uploaded datasets</td><td></td></tr>"
                )
            elif input.type == "data":
                tool_parameter_template += "<tr>"
                tool_parameter_template += inputs_recursive_indent(
                    text=input.label, depth=depth
                )
                ...
                for i, element in enumerate(listify(param_values[input.name])):
                    ...
                    if element.history_content_type == "dataset":
                        ...
                        dataset_info_url = url_for(
                            controller="dataset",
                            action="show_params",
                            dataset_id=encoded_id,
                        )
                        tool_parameter_template += (
                            '<a target="galaxy_main" data-hda-id="' + encoded_id + '"'
                        )
                        tool_parameter_template += (
                            'href="'
                            + dataset_info_url
                ...
                tool_parameter_template += "<tr>"
                tool_parameter_template += inputs_recursive_indent(
                    text=label, depth=depth
                )
                tool_parameter_template += (
                    "<td>"
                    + input.value_to_display_text(param_values[input.name])
                    + "</td>"
                )
                tool_parameter_template += (
                    "<td>" + upgrade_messages.get(input.name, "") + "</td></tr>"
                )
        else:
            tool_parameter_template += "<tr>"
