...
def _get_page_identifiers(item_id, app):
    ...
    try:
        ...
        if decoded_id >= PAGE_MAXRAW:
            raise ValueError(
                "Identifier larger than maximum expected raw int, must be already encoded."
            )
...
class PageListGrid(grids.Grid):
    ...
    operations = [
        ...
        grids.GridOperation(
            "Edit content", allow_multiple=False, url_args=dict(action="edit_content")
        ),
        grids.GridOperation(
            "Edit attributes",
            allow_multiple=False,
            url_args=dict(controller="", action="pages/edit"),
        ),
        grids.GridOperation(
            "Share or Publish",
            allow_multiple=False,
        ...
        grids.GridOperation(
            "Delete", confirm="Are you sure you want to delete this page?"
        ),
    ]
...
class HistoryDatasetAssociationSelectionGrid(ItemSelectionGrid):
    ...
    def apply_query_filter(self, trans, query, **kwargs):
        ...
        return query.select_from(
            model.HistoryDatasetAssociation.table.join(model.History.table)
...
class _PageContentProcessor(HTMLParser, object):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        for attribute in attrs:
            if (attribute[0] == "class") and (
                "embedded-item" in attribute[1].split(" ")
            ):
                embedded_item = True
                break
        ...
        if embedded_item:
            ...
            for attribute in attrs:
                ...
                if name == "id":
                    ...
                    embed_html = self.render_embed_html_fn(
                        self.trans, item_class, item_id
                    )
...
class PageController(
        ...
        if trans.request.method == "GET":
            ...
            if not p_title:
                return self.message_exception(
                    trans, "Please provide a page name is required."
                )
            elif not p_slug:
                return self.message_exception(
                ...
                return self.message_exception(
                    trans,
                    "Page identifier can only contain lowercase letters, numbers, and dashes (-).",
                )
            elif (
                trans.sa_session.query(model.Page)
                .filter_by(user=user, slug=p_slug, deleted=False)
                .first()
            ):
            ...
            return {
                "message": "Page '%s' successfully created." % p.title,
                "status": "success",
            }
    ...
    def edit(self, trans, payload=None, **kwd):
        ...
        if trans.request.method == "GET":
            ...
            if not p_title:
                ...
                p_slug != p.slug
                and trans.sa_session.query(model.Page)
                .filter_by(user=p.user, slug=p_slug, deleted=False)
                .first()
            ):
                return self.message_exception(trans, "Page id must be unique.")
            else:
                ...
                trans.sa_session.flush()
            return {
                "message": "Attributes of '%s' successfully saved." % p.title,
                "status": "success",
            }
    ...
    def share(self, trans, id, email="", use_panels=False):
        ...
        if email:
            ...
            if not other:
                ...
                trans.sa_session.query(model.PageUserShareAssociation)
                .filter_by(user=other, page=page)
                .count()
                > 0
            ):
                mtype = "error"
                ...
                trans.set_message(
                    "Page '%s' shared with user '%s'" % (page_title, other_email)
                )
    ...
    def save(self, trans, id, content, annotations):
        ...
        for annotation_dict in annotations:
            ...
            if item_id and item_class and text:
                ...
                annotation_assoc_class = eval(
                    "model.%sAnnotationAssociation" % item_class.__name__
                )
                annotation_assoc = trans.sa_session.query(
                    annotation_assoc_class
                ).filter_by(user=trans.get_user())
    ...
    def display_by_username_and_slug(self, trans, username, slug):
        ...
        if trans.get_user():
            user_item_rating = self.get_user_item_rating(
                trans.sa_session, trans.get_user(), page
            )
            if user_item_rating:
    ...
    def _get_embedded_history_html(self, trans, decoded_id):
        ...
        contents = self.history_serializer.serialize_contents(
            history, "contents", trans=trans, user=trans.user
        )
    ...
    def _get_embedded_visualization_html(self, trans, encoded_id):
        ...
        return trans.fill_template(
            "visualization/embed.mako", item=visualization, item_data=None
        )
    ...
    def _get_embed_html(self, trans, item_class, item_id):
        ...
        if item_class == model.History:
            ...
            if dataset:
                ...
                return trans.fill_template(
                    "dataset/embed.mako", item=dataset, item_data=data
                )
...
def _placeholderRenderForSave(trans, item_class, item_id, encode=False):
    ...
    if item_class == "History":
        ...
        hda = trans.sa_session.query(trans.model.HistoryDatasetAssociation).get(
            decoded_item_id
        )
