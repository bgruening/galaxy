...
class ASync(BaseUIController):
    ...
    def default(self, trans, tool_id=None, data_id=None, data_secret=None, **kwd):
        ...
        return self.index(
            trans, tool_id=tool_id, data_id=data_id, data_secret=data_secret, **kwd
        )
    ...
    def index(self, trans, tool_id=None, data_secret=None, **kwd):
        ...
        if data_id:
            ...
            data = trans.sa_session.query(trans.model.HistoryDatasetAssociation).get(
                data_id
            )
            ...
            if STATUS == "OK":
                key = hmac_new(
                    trans.app.config.tool_secret, "%d:%d" % (data.id, data.history_id)
                )
                if key != data_secret:
                ...
                original_history = trans.sa_session.query(trans.app.model.History).get(
                    data.history_id
                )
            ...
            if params.data_type:
                ...
                params.galaxyFileFormat == "wig"
            ):  # this is an undocumented legacy special case
                GALAXY_TYPE = "wig"
            ...
            GALAXY_INFO = (
                params.info or params.GALAXY_INFO or params.galaxyDescription or ""
            )
            GALAXY_BUILD = (
                params.dbkey or params.GALAXY_BUILD or params.galaxyFreeze or "?"
            )
        ...
        return trans.fill_template(
            "root/tool_runner.mako", out_data={}, num_jobs=1, job_errors=[]
        )
