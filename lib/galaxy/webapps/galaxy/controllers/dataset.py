...
from galaxy import datatypes, managers, model, util, web
...
class HistoryDatasetAssociationListGrid(grids.Grid):
    ...
    columns = [
        grids.TextColumn(
            ...
            link=(
                lambda item: iff(
                    item.history.deleted, None, dict(operation="switch", id=item.id)
                )
            ),
            filterable="advanced",
            attach_popup=True,
    ...
    def build_initial_query(self, trans, **kwargs):
        ...
        return (
            ...
            .select_from(self.model_class.table.join(model.History.table))
...
class DatasetInterface(
    BaseUIController, UsesAnnotations, UsesItemRatings, UsesExtendedMetadataMixin
):
    ...
    def _get_job_for_dataset(self, trans, dataset_id):
        ...
        """
        hda = trans.sa_session.query(trans.app.model.HistoryDatasetAssociation).get(
            self.decode_id(dataset_id)
        )
        assert hda and self._can_access_dataset(trans, hda)
        return hda.creating_job

    def _can_access_dataset(
        self, trans, dataset_association, allow_admin=True, additional_roles=None
    ):
        roles = trans.get_current_user_roles()
        if additional_roles:
            roles = roles + additional_roles
        return (
            allow_admin and trans.user_is_admin
    ...
    def errors(self, trans, id):
        hda = trans.sa_session.query(model.HistoryDatasetAssociation).get(
            self.decode_id(id)
        )
    ...
    def exit_code(self, trans, dataset_id=None, **kwargs):
        ...
        try:
            ...
            exit_code = (
                "Invalid dataset ID or you are not allowed to access this dataset"
            )
        return exit_code
    ...
    def get_metadata_file(self, trans, hda_id, metadata_name):
        ...
        data = trans.sa_session.query(trans.app.model.HistoryDatasetAssociation).get(
            self.decode_id(hda_id)
        )
        ...
        fname = "".join(c in util.FILENAME_VALID_CHARS and c or "_" for c in data.name)[
            0:150
        ]
        ...
        trans.response.headers[
            "Content-Disposition"
        ] = 'attachment; filename="Galaxy%s-[%s].%s"' % (data.hid, fname, file_ext)
    ...
    def _check_dataset(self, trans, hda_id):
        ...
        if not data:
            raise paste.httpexceptions.HTTPRequestRangeNotSatisfiable(
                "Invalid reference dataset id: %s." % str(hda_id)
            )
        ...
        if data.purged:
            return trans.show_error_message(
                "The dataset you are attempting to view has been purged."
            )
        if data.deleted and not (
            trans.user_is_admin
            or (data.history and trans.get_user() == data.history.user)
        ):
            return trans.show_error_message(
                "The dataset you are attempting to view has been deleted."
            )
        ...
        **kwd
        ...
        if "hdca" in kwd:
            raise RequestParameterInvalidException(
                "Invalid request parameter 'hdca' encountered."
            )
        hdca_id = kwd.get("hdca_id", None)
        if hdca_id:
            hdca = self.app.dataset_collections_service.get_dataset_collection_instance(
        ...
        if ck_size:
            ck_size = int(ck_size)
        return data.datatype.display_data(
            trans,
            data,
            preview,
            filename,
    ...
    def get_edit(self, trans, dataset_id=None, **kwd):
        ...
        if self._can_access_dataset(trans, data):
            ...
            all_roles = [
                ...
                for r in trans.app.security_agent.get_legitimate_roles(
                    trans, data.dataset, "root"
                )
            ]
            data_metadata = [(name, spec) for name, spec in data.metadata.spec.items()]
            converters_collection = [
                (key, value.name) for key, value in data.get_converter_types().items()
            ]
            ...
            attribute_inputs = [
                ...
                },
                {
                    ...
                    "area": True,
                    "label": "Annotation",
                    "value": self.get_item_annotation_str(
                        trans.sa_session, trans.user, data
                    ),
            ...
            datatype_inputs = [
                {
                    ...
                    "value": [
                        ext_id for ext_id, ext_name in ldatatypes if ext_id == data.ext
                    ],
            ...
            if trans.user:
                if data.dataset.actions:
                    in_roles = {}
                    for action, roles in trans.app.security_agent.get_permissions(
                        data.dataset
                    ).items():
                        in_roles[action.action] = [
                            trans.security.encode_id(role.id) for role in roles
                        ]
                    for (
                        index,
                        action,
                    ) in trans.app.model.Dataset.permitted_actions.items():
                        if (
                            action
                            ...
                            help_text = (
                                action.description
                                + "<br/>NOTE: Users must have every role associated with this dataset in order to access it."
                            )
                        else:
            ...
            return self.message_exception(
                trans,
                "You do not have permission to edit this dataset's ( id: %s ) information."
                % str(dataset_id),
            )
    ...
    def set_edit(self, trans, payload=None, **kwd):
        ...
        def __ok_to_edit_metadata(dataset_id):
            ...
            for job_to_dataset_association in (
                trans.sa_session.query(self.app.model.JobToInputDatasetAssociation)
        ...
        if operation == "attributes":
            ...
            if __ok_to_edit_metadata(data.id):
                ...
                for name, spec in data.datatype.metadata_spec.items():
                    if not spec.get("readonly"):
                        setattr(
                            data.metadata, name, spec.unwrap(payload.get(name) or None)
                        )
                ...
                if payload.get("annotation"):
                    ...
                    self.add_item_annotation(
                        trans.sa_session, trans.get_user(), data, annotation
                    )
                # if setting metadata previously failed and all required elements have now been set, clear the failed state.
                if (
                    data._state == trans.model.Dataset.states.FAILED_METADATA
                    and not data.missing_meta()
                ):
                    data._state = None
                message = (
                    "Attributes updated. %s" % message
                    if message
                    else "Attributes updated."
                )
            else:
            ...
            if (
                data.datatype.allow_datatype_change
                and trans.app.datatypes_registry.get_datatype_by_extension(
                    datatype
                ...
                return self.message_exception(
                    trans,
                    "You are unable to change datatypes in this manner. Changing %s to %s is not allowed."
                    % (data.extension, datatype),
                )
            ...
            if data.datatype.allow_datatype_change:
                ...
                if not __ok_to_edit_metadata(data.id):
                    ...
                    datatype = sniff.guess_ext(
                        path,
                        trans.app.datatypes_registry.sniff_order,
                        is_binary=is_binary,
                    )
                    trans.app.datatypes_registry.change_datatype(data, datatype)
                    trans.sa_session.flush()
                    ...
                    message = (
                        "Detection was finished and changed the datatype to %s."
                        % datatype
                    )
            else:
                return self.message_exception(
                    trans, 'Changing datatype "%s" is not allowed.' % (data.extension)
                )
            ...
            if not __ok_to_edit_metadata(data.id):
                ...
                for name, spec in data.metadata.spec.items():
                    ...
                    if name not in ["name", "info", "dbkey", "base_name"]:
                        if spec.get("default"):
                            setattr(
                                data.metadata, name, spec.unwrap(spec.get("default"))
                            )
            ...
            if not trans.user:
                return self.message_exception(
                    trans, "You must be logged in if you want to change permissions."
                )
            if trans.app.security_agent.can_manage_dataset(
                trans.get_current_user_roles(), data.dataset
            ):
                payload_permissions = {}
                for action in trans.app.model.Dataset.permitted_actions.keys():
                    payload_permissions[action] = [
                        trans.security.decode_id(role_id)
                        for role_id in util.listify(payload.get(action))
                    ]
                ...
                permissions, in_roles, error, message = trans.app.security_agent.derive_roles_from_access(
                    trans, data.dataset.id, "root", **payload_permissions
                )
                if error:
                ...
                return self.message_exception(
                    trans,
                    "You are not authorized to change this dataset's permissions.",
                )
        else:
            return self.message_exception(
                trans, "Invalid operation identifier (%s)." % operation
            )
    ...
    def _get_dataset_for_edit(self, trans, dataset_id):
        if dataset_id is not None:
            ...
            data = trans.sa_session.query(self.app.model.HistoryDatasetAssociation).get(
                id
            )
        else:
            ...
            return (
                ...
                self.message_exception(
                    trans, "You must provide a dataset id to edit attributes."
                ),
            )
        if data is None:
        ...
        if (
            ...
            and data.history.user is not None
            and data.history.user != trans.user
        ):
            trans.log_event(
                "User attempted to edit a dataset they do not own (encoded: %s, decoded: %s)."
                % (dataset_id, id)
            )
            return None, self.message_exception(trans, "The dataset id is invalid.")
        if data.history.user and not data.dataset.has_manage_permissions_roles(trans):
            ...
            permissions = {
                manage_permissions_action: [
                    trans.app.security_agent.get_private_user_role(data.history.user)
                ]
            }
        ...
        if "operation" in kwargs:
            ...
            for encoded_hda_id in hda_ids:
                ...
                if hda:
                    ...
                    if hda.history.user_id is not None and trans.user:
                        assert (
                            trans.user.id == hda.history.user_id
                        ), "HistoryDatasetAssocation does not belong to current user"
                    hdas.append(hda)
                else:
                    log.warning(
                        "Invalid history_dataset_association id '%r' passed to list",
                        hda_id,
                    )
            ...
            if hdas:
                if operation == "switch" or operation == "switch_history":
                    ...
                    status, message = trans.webapp.controllers["history"]._list_switch(
                        trans, histories
                    )
                    ...
                    hda_ids.reverse()
                    status, message = self._copy_datasets(
                        trans, hda_ids, target_histories
                    )
    ...
    def imp(self, trans, dataset_id=None, **kwd):
        ...
        if referer:
            referer_message = "<a href='%s'>return to the previous page</a>" % escape(
                referer
            )
        else:
        ...
        if not dataset_id:
            return trans.show_error_message(
                "You must specify a dataset to import. You can %s." % referer_message,
                use_panels=True,
            )
        # Do import.
        cur_history = trans.get_history(create=True)
        status, message = self._copy_datasets(
            trans, [dataset_id], [cur_history], imported=True
        )
        message = (
            "Dataset imported. <br>You can <a href='%s'>start using the dataset</a> or %s."
    ...
    @web.expose
    def display_by_username_and_slug(
        self, trans, username, slug, filename=None, preview=True
    ):
            ...
            if (
                ...
                if trans.get_user():
                    user_item_rating = self.get_user_item_rating(
                        trans.sa_session, trans.get_user(), dataset
                    )
                    if user_item_rating:
    ...
    def get_item_content_async(self, trans, id):
        ...
        # Get annotation.
        dataset.annotation = self.get_item_annotation_str(
            trans.sa_session, trans.user, dataset
        )
        return trans.stream_template_mako(
            "/dataset/item_content.mako",
            item=dataset,
            item_data=dataset_data,
            truncated=truncated,
        )
    ...
    def annotate_async(self, trans, id, new_annotation=None, **kwargs):
        ...
        if dataset and new_annotation:
            ...
            self.add_item_annotation(
                trans.sa_session, trans.get_user(), dataset, new_annotation
            )
            trans.sa_session.flush()
    ...
    def display_at(self, trans, dataset_id, filename=None, **kwd):
        ...
        if not data:
            raise paste.httpexceptions.HTTPRequestRangeNotSatisfiable(
                "Invalid reference dataset id: %s." % str(dataset_id)
            )
        ...
        if self._can_access_dataset(trans, data):
            trans.app.host_security_agent.set_dataset_permissions(
                data, trans.user, site
            )
    ...
    def display_application(
        ...
        **kwds
        ...
        for name, value in dict(
            kwds
        ).items():  # clone kwds because we remove stuff as we go.
            if name.startswith("app_"):
                app_kwds[name[len("app_") :]] = value
                del kwds[name]
        if kwds:
            log.debug(
                "Unexpected Keywords passed to display_application: %s" % kwds
            )  # route memory?
        # decode ids
        ...
        if None in [app_name, link_name]:
            return trans.show_error_message(
                "A display application name and link name must be provided."
            )
        if self._can_access_dataset(trans, data, additional_roles=user_roles):
            msg = []
            preparable_steps = []
            refresh = False
            display_app = trans.app.datatypes_registry.display_applications.get(
                app_name
            )
            if not display_app:
                log.debug(
                    "Unknown display application has been requested: %s", app_name
                )
                return paste.httpexceptions.HTTPNotFound(
                    "The requested display application (%s) is not available."
                    % (app_name)
                )
            dataset_hash, user_hash = encode_dataset_user(trans, data, user)
            try:
                display_link = display_app.get_link(
                    link_name, data, dataset_hash, user_hash, trans, app_kwds
                )
            except Exception as e:
                ...
                return paste.httpexceptions.HTTPConflict(
                    "Error generating display_link: %s" % e
                )
            if not display_link:
                log.debug("Unknown display link has been requested: %s", link_name)
                return paste.httpexceptions.HTTPNotFound(
                    "Unknown display link has been requested: %s" % link_name
                )
            if data.state == data.states.ERROR:
                ...
                if display_link.display_ready():
                    if app_action in ["data", "param"]:
                        assert (
                            action_param
                        ), "An action param must be provided for a data or param action"
                        ...
                        try:
                            action_param = display_link.get_param_name_by_url(
                                action_param
                            )
                        ...
                        assert value, (
                            "An invalid parameter name was provided: %s" % action_param
                        )
                        assert (
                            value.parameter.viewable
                        ), "This parameter is not viewable."
                        if value.parameter.type == "data":
                            try:
                                if action_param_extra:
                                    assert value.parameter.allow_extra_files_access, (
                                        "Extra file content requested (%s), but allow_extra_files_access is False."
                                        % (action_param_extra)
                                    )
                                    file_name = os.path.join(
                                        value.extra_files_path, action_param_extra
                                    )
                                else:
                                ...
                                log.debug(
                                    "Unable to access requested file in display application: %s",
                                    e,
                                )
                                return paste.httpexceptions.HTTPNotFound(
                        ...
                        trans.response.set_content_type(
                            value.mime_type(action_param_extra=action_param_extra)
                        )
                        ...
                        trans.response.headers["Cache-Control"] = [
                            "no-cache",
                            "max-age=0",
                            "no-store",
                            "must-revalidate",
                        ...
                        msg.append(
                            ("Invalid action provided: %s" % app_action, "error")
                        )
                else:
                    if app_action is None:
                        ...
                        raise Exception(
                            "Attempted a view action (%s) on a non-ready display application"
                            % app_action
                        )
            return trans.fill_template_mako(
    ...
    def _delete(self, trans, dataset_id):
        ...
        try:
            ...
            assert (
                topmost_parent in trans.history.datasets
            ), "Data does not belong to current history"
    ...
    def _undelete(self, trans, dataset_id):
        ...
        try:
            ...
            while topmost_parent.parent:
                topmost_parent = topmost_parent.parent
            assert (
                topmost_parent in history.datasets
            ), "Data does not belong to current history"
            # Mark undeleted
            hda.mark_undeleted()
            trans.sa_session.flush()
    ...
    def _purge(self, trans, dataset_id):
        ...
        try:
            ...
            if not user:
                ...
                assert (
                    hda.history.id == current_history_id
                ), "Data does not belong to current user"
            ...
            if hda.dataset.user_can_purge:
                try:
                    ...
                    trans.log_event(
                        "Dataset id %s has been purged upon the the purge of HDA id %s"
                        % (hda.dataset.id, hda.id)
                    )
                    trans.sa_session.add(hda.dataset)
                except Exception:
                    log.exception(
                        "Unable to purge dataset (%s) on purge of HDA (%s):"
                        % (hda.dataset.id, hda.id)
                    )
            trans.sa_session.flush()
        except Exception:
    ...
    def unhide(self, trans, dataset_id, filename):
        if self._unhide(trans, dataset_id):
            return trans.response.send_redirect(
                web.url_for(controller="root", action="history", show_hidden=True)
            )
    ...
    def show_params(self, trans, dataset_id=None, from_noframe=None, **kwd):
        ...
        if job_dataset_association.creating_job_associations:
            ...
            if job:
                ...
                try:
                    ...
                    try:
                        params_objects = job.get_param_values(
                            trans.app, ignore_errors=False
                        )
                    except Exception:
                        params_objects = job.get_param_values(
        ...
        if job is None:
            return trans.show_error_message(
                "Job information is not available for this dataset."
            )
        ...
        **kwd
        ...
        if source_content_ids:
            ...
            encoded_dataset_collection_ids = [
                s[len("dataset_collection|") :]
                for s in source_content_ids
                if s.startswith("dataset_collection|")
            ]
            encoded_dataset_ids = [
                s[len("dataset|") :]
                for s in source_content_ids
            ...
            decoded_dataset_collection_ids = set(
                map(self.decode_id, encoded_dataset_collection_ids)
            )
        ...
        if new_history_name:
            ...
            if target_history_id:
                ...
                target_history_ids = list(
                    set([self.decode_id(h) for h in target_history_ids if h])
                )
            else:
                target_history_ids = []
        done_msg = error_msg = ""
        ...
        if do_copy:
            ...
            if not (decoded_dataset_ids or decoded_dataset_collection_ids) or not (
                ...
                error_msg = (
                    "You must provide both source datasets and target histories. "
                )
            else:
                if new_history_name:
                ...
                if user:
                    target_histories = [
                        hist
                        for hist in map(
                            trans.sa_session.query(trans.app.model.History).get,
                            target_history_ids,
                        )
                ...
                for content in source_contents:
                    if content is None:
                        error_msg = (
                            error_msg
                            + "You tried to copy a dataset that does not exist. "
                        )
                        invalid_contents += 1
                    elif content.history != history:
                        error_msg = (
                            error_msg
                            + "You tried to copy a dataset which is not in your current history. "
                        )
                        invalid_contents += 1
                    else:
                    ...
                    [
                        ...
                        % (
                            url_for(
                                ...
                                hist_id=trans.security.encode_id(hist.id),
                            ),
                            escape(hist.name),
                        )
                        for hist in target_histories
        ...
        if not dataset_ids or not target_histories:
            ...
            for dataset_id in dataset_ids:
                ...
                if data is None:
                    error_msg = (
                        error_msg
                        + "You tried to copy a dataset that does not exist or that you do not have access to.  "
                    )
                    invalid_datasets += 1
                else:
                    for hist in target_histories:
