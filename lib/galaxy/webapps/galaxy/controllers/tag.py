...
class TagsController(BaseUIController, UsesTagsMixin):
    ...
    def get_tagging_elt_async(self, trans, item_id, item_class, elt_context=""):
        ...
        if not item:
            return trans.show_error_message(
                "No item of class %s with id %s " % (item_class, item_id)
            )
    ...
    def add_tag_async(
        self, trans, item_id=None, item_class=None, new_tag=None, context=None
    ):
        """
        Add tag to an item.
        """
    ...
    def remove_tag_async(
        self, trans, item_id=None, item_class=None, tag_name=None, context=None
    ):
        """
        ...
        item = self._get_item(trans, item_class, trans.security.decode_id(item_id))
        user = trans.user
        self.get_tag_handler(trans).remove_item_tag(
            user, item, tag_name.encode("utf-8")
        )
        trans.sa_session.flush()
        # Log.
    ...
    def retag_async(self, trans, item_id=None, item_class=None, new_tags=None):
        ...
        self.get_tag_handler(trans).apply_item_tags(
            user, item, new_tags.encode("utf-8")
        )
        trans.sa_session.flush()

    @web.expose
    @web.require_login("get autocomplete data for an item's tags")
    def tag_autocomplete_data(
        self, trans, q=None, limit=None, timestamp=None, item_id=None, item_class=None
    ):
        """
        ...
        if q.find(":") == -1:
            return self._get_tag_autocomplete_names(
                trans, q, limit, timestamp, user, item, item_class
            )
        else:
            return self._get_tag_autocomplete_values(
                trans, q, limit, timestamp, user, item, item_class
            )

    def _get_tag_autocomplete_names(
        self, trans, q, limit, timestamp, user=None, item=None, item_class=None
    ):
        """
        ...
        item_tag_assoc_class = self.get_tag_handler(trans).get_tag_assoc_class(
            item_class
        )
        ...
        from_obj = item_tag_assoc_class.table.join(item_class.table).join(
            trans.app.model.Tag.table
        )
        where_clause = and_(
            trans.app.model.Tag.table.c.name.like(q + "%"),
        ...
        for row in result_set:
            ...
            if (item is not None) and (
                self.get_tag_handler(trans).item_has_tag(trans.user, item, tag)
            ):
                continue
            # Add tag to autocomplete data. Use the most frequent name that user
            # has employed for the tag.
            tag_names = self._get_usernames_for_tag(
                trans, trans.user, tag, item_class, item_tag_assoc_class
            )
    ...
    def _get_tag_autocomplete_values(
        ...
        tag_uname = self._get_usernames_for_tag(
            trans, trans.user, tag, item_class, item_tag_assoc_class
        )[0]
        for row in result_set:
    ...
    def _get_usernames_for_tag(
        self, trans, user, tag, item_class, item_tag_assoc_class
    ):
        """
    ...
    def _get_item(self, trans, item_class_name, id):
        ...
        """
        item_class = (
            self.get_tag_handler(trans).item_tag_assoc_info[item_class_name].item_class
        )
