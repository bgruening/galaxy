...
from tool_shed.galaxy_install import dependency_display, install_manager
from tool_shed.galaxy_install.grids import admin_toolshed_grids
from tool_shed.galaxy_install.installed_repository_manager import (
    InstalledRepositoryManager,
)
from tool_shed.galaxy_install.metadata.installed_repository_metadata_manager import (
    InstalledRepositoryMetadataManager,
)
from tool_shed.galaxy_install.repository_dependencies import (
    repository_dependency_manager,
)
...
class AdminToolshed(AdminGalaxy):
    ...
    def activate_repository(self, trans, **kwd):
        ...
        try:
            ...
            message = "The <b>%s</b> repository has been activated." % escape(
                repository.name
            )
            status = "done"
    ...
    def browse_repository(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/admin/tool_shed_repository/browse_repository.mako",
        ...
        if "operation" in kwd:
            ...
            if operation == "update tool shed status":
                message, status = repository_util.check_for_updates(
                    trans.app, trans.install_model, kwd.get("id")
                )
        if message and status:
            kwd["message"] = util.sanitize_text(message)
            kwd["status"] = (
                "success" if status in ["ok", "done", "success"] else "error"
            )
        ...
        if repository.uninstalled:
            ...
            if current_changeset_revision and current_ctx_rev:
                if current_ctx_rev == repository.ctx_rev:
                    ...
                    encoded_repo_info_dict = encoding_util.tool_shed_encode(
                        json_repo_info_dict
                    )
                ...
                message = (
                    "Unable to get latest revision for repository <b>%s</b> from "
            ...
            return trans.response.send_redirect(
                web.url_for(
                    controller="admin_toolshed", action="activate_repository", **kwd
                )
            )
    ...
    def browse_tool_dependency(self, trans, **kwd):
        ...
        tool_dependency = tool_dependency_util.get_tool_dependency(
            trans.app, tool_dependency_ids[0]
        )
        ...
        return trans.fill_template(
            "/admin/tool_shed_repository/browse_tool_dependency.mako",
            repository=repository,
    ...
    def browse_tool_shed(self, trans, **kwd):
        ...
        url = util.build_url(
            tool_shed_url,
            pathspec=["repository", "browse_valid_categories"],
            params=params,
        )
    ...
    def browse_toolsheds(self, trans, **kwd):
        app = {"jscript": "adminToolshed"}
        return trans.fill_template(
            "galaxy.panels.mako",
            config={"title": "Galaxy Tool Sheds", "app": app, "bundle": "extended"},
        )
    ...
    def browse_tool_sheds(self, trans, **kwd):
        message = escape(kwd.get("message", ""))
        return trans.fill_template(
            "/webapps/galaxy/admin/tool_sheds.mako", message=message, status="error"
        )
    ...
    def deactivate_or_uninstall_repository(self, trans, **kwd):
        ...
        tool_shed_repositories = repository_util.get_installed_tool_shed_repository(
            trans.app, kwd["id"]
        )
        ...
        for tool_shed_repository in tool_shed_repositories:
            if kwd.get("deactivate_or_uninstall_repository_button", False):
                errors = irm.uninstall_repository(
                    repository=tool_shed_repository,
                    remove_from_disk=remove_from_disk_checked,
                )
                action = "uninstalled" if remove_from_disk_checked else "deactivated"
                status = max(status, statuses.index("done"))
                message += "The repository named <b>%s</b> has been %s.  " % (
                    escape(tool_shed_repository.name),
                    action,
                )
                if remove_from_disk_checked:
                    if errors:
                        message += (
                            "Attempting to uninstall tool dependencies resulted in errors: %s"
                            % errors
                        )
            ...
            if repository:
                ...
                if path_to_file and os.path.exists(path_to_file):
                    ...
                    try:
                        ...
                        extension = None
                    if extension:
                        mimetype = trans.app.datatypes_registry.get_mimetype_by_extension(
                            extension
                        )
                        if mimetype:
    ...
    def find_tools_in_tool_shed(self, trans, **kwd):
        ...
        params = dict(galaxy_url=web.url_for("/", qualified=True))
        url = util.build_url(
            tool_shed_url, pathspec=["repository", "find_tools"], params=params
        )
        return trans.response.send_redirect(url)
    ...
    def find_workflows_in_tool_shed(self, trans, **kwd):
        ...
        params = dict(galaxy_url=web.url_for("/", qualified=True))
        url = util.build_url(
            tool_shed_url, pathspec=["repository", "find_workflows"], params=params
        )
        return trans.response.send_redirect(url)
    ...
    def generate_workflow_image(self, trans, workflow_name, repository_id=None):
        ...
        return workflow_util.generate_workflow_image(
            trans,
            workflow_name,
    ...
    def get_file_contents(self, trans, file_path, repository_id):
        ...
        return suc.get_repository_file_contents(
            trans.app, file_path, repository_id, is_admin=True
        )
    ...
    def get_tool_dependencies(
        ...
        if (
            tool_shed_url is None
            or repository_name is None
            or repository_owner is None
    ...
    def import_workflow(self, trans, workflow_name, repository_id, **kwd):
        ...
        if workflow_name:
            ...
            if repository:
                workflow, status, message = workflow_util.import_workflow(
                ...
                if workflow:
                    ...
                    message += (
                        "Unable to locate a workflow named <b>%s</b> within the installed tool shed repository named <b>%s</b>"
                        % (escape(str(workflow_name)), escape(str(repository.name)))
                    )
                    status = "error"
            else:
                message = "Invalid repository id <b>%s</b> received." % str(
                    repository_id
                )
                status = "error"
        else:
    ...
    def initiate_tool_dependency_installation(self, trans, tool_dependencies, **kwd):
        ...
        for installed_tool_dependency in installed_tool_dependencies:
            if (
                installed_tool_dependency.status
        ...
        if err_msg:
            message += err_msg
            status = "error"
        message += "Installed tool dependencies: %s" % ", ".join(
            td.name for td in installed_tool_dependencies
        )
        td_ids = [
            trans.security.encode_id(td.id)
    ...
    def install_latest_repository_revision(self, trans, **kwd):
        ...
        if repository_id is not None:
            ...
            if repository is not None:
                ...
                owner = str(repository.owner)
                params = dict(
                    galaxy_url=web.url_for("/", qualified=True), name=name, owner=owner
                )
                ...
                if latest_downloadable_revision == hg_util.INITIAL_CHANGELOG_HASH:
                    return trans.show_error_message(
                        "Error retrieving the latest downloadable revision for this repository via the url <b>%s</b>."
                        % url
                    )
                else:
                    ...
                    tmp_clone_url = util.build_url(
                        tool_shed_url, pathspec=["repos", owner, name]
                    )
                    tmp_repo_info_tuple = (
                        None,
                        tmp_clone_url,
                    ...
                    installed_repository, installed_changeset_revision = repository_util.repository_was_previously_installed(
                        trans.app,
                        tool_shed_url,
                        name,
                        tmp_repo_info_tuple,
                    ...
                    if installed_repository:
                        current_changeset_revision = str(
                        ...
                        if current_changeset_revision != latest_downloadable_revision:
                            message += (
                                "  The current changeset revision is <b>%s</b>."
                ...
                return trans.show_error_message(
                    "Cannot locate installed tool shed repository with encoded id <b>%s</b>."
                    % str(repository_id)
                )
        else:
            return trans.show_error_message(
    ...
    def install_tool_dependencies_with_update(self, trans, **kwd):
        ...
        repository = repository_util.get_installed_tool_shed_repository(
            trans.app, updating_repository_id
        )
        ...
        if encoded_relative_install_dir is not None:
            relative_install_dir = encoding_util.tool_shed_decode(
        ...
        if "install_tool_dependencies_with_update_button" in kwd:
            ...
            if encoded_updated_metadata:
                updated_metadata = encoding_util.tool_shed_decode(
                    encoded_updated_metadata
                )
            else:
            ...
            if tool_dependencies_dict:
                ...
                message = (
                    "The installed repository named '%s' has been updated to change set revision '%s'.  "
                    % (escape(str(repository.name)), updating_to_changeset_revision)
                )
                self.initiate_tool_dependency_installation(
                    trans, tool_dependencies, message=message, status=status
                )
        # Handle tool dependencies check box.
        if trans.app.config.tool_dependency_dir is None:
            if tool_dependencies_dict:
                message = (
                    "Tool dependencies defined in this repository can be automatically installed if you set "
        ...
        if view.installable_resolvers:
            install_resolver_dependencies_check_box = CheckboxField(
                "install_resolver_dependencies", value=True
            )
        else:
    ...
    def install_repositories(self, trans, **kwd):
        ...
        install_resolver_dependencies = CheckboxField.is_checked(
            decoded_kwd.get("install_resolver_dependencies", "")
        )
        install_tool_dependencies = CheckboxField.is_checked(
            decoded_kwd.get("install_tool_dependencies", "")
        )
        ...
        try:
            tool_shed_repositories = irm.install_repositories(
                tsr_ids=tsr_ids, decoded_kwd=decoded_kwd, reinstalling=reinstalling
            )
            tsr_ids_for_monitoring = [
                trans.security.encode_id(tsr.id) for tsr in tool_shed_repositories
            ]
    ...
    def manage_repository(self, trans, **kwd):
        ...
        if repository_id and isinstance(repository_id, list):
            ...
            # this should be OK.
            repository_id = [r for r in repository_id if "=" not in r][
                0
            ]  # This method only work for a single repo id
        ...
        if relative_install_dir:
            repo_files_dir = os.path.abspath(
                os.path.join(tool_path, relative_install_dir, name)
            )
        else:
            repo_files_dir = None
        ...
        requirements_status = view.get_requirements_status(
            tool_requirements_d, repository.installed_tool_dependencies
        )
        return trans.fill_template(
    ...
    def manage_repository_tool_dependencies(self, trans, **kwd):
            ...
            if operation == "browse":
                ...
                for tool_dependency_id in tool_dependency_ids:
                    ...
                    if tool_dependency.status in [
                        trans.install_model.ToolDependency.installation_status.INSTALLED,
                ...
                if trans.app.config.tool_dependency_dir:
                    ...
                    if tool_dependencies_for_installation:
                        ...
                        message = (
                            "All selected tool dependencies are already installed."
                        )
                        status = "error"
                else:
                    message = "Set the value of your <b>tool_dependency_dir</b> setting in your Galaxy config file (galaxy.ini) "
                    message += (
                        " and restart your Galaxy server to install tool dependencies."
                    )
                    status = "error"
        installed_tool_dependencies_select_field = tool_dependency_util.build_tool_dependencies_select_field(
            trans.app,
            tool_shed_repository=tool_shed_repository,
            name="inst_td_ids",
            uninstalled_only=False,
        )
        uninstalled_tool_dependencies_select_field = tool_dependency_util.build_tool_dependencies_select_field(
            trans.app,
            tool_shed_repository=tool_shed_repository,
        ...
        return trans.fill_template(
            "/admin/tool_shed_repository/manage_repository_tool_dependencies.mako",
            repository=tool_shed_repository,
    ...
    def monitor_repository_installation(self, trans, **kwd):
        ...
        if not tsr_ids:
            tsr_ids = repository_util.get_ids_of_tool_shed_repositories_being_installed(
                trans.app, as_string=False
            )
        tsr_ids = [trans.security.decode_id(tsr_id) for tsr_id in tsr_ids]
        tool_shed_repositories = []
        for tsr_id in tsr_ids:
            tsr = trans.install_model.context.query(
                trans.install_model.ToolShedRepository
            ).get(tsr_id)
            tool_shed_repositories.append(tsr)
        clause_list = []
        for tsr_id in tsr_ids:
            clause_list.append(
                trans.install_model.ToolShedRepository.table.c.id == tsr_id
            )
        query = trans.install_model.context.current.query(
            trans.install_model.ToolShedRepository
    ...
    def open_folder(self, trans, folder_path, repository_id):
        ...
        trans.response.headers["Pragma"] = "no-cache"
        trans.response.headers["Expires"] = "0"
        return suc.open_repository_files_folder(
            trans.app, folder_path, repository_id, is_admin=True
        )
    ...
    def prepare_for_install(self, trans, **kwd):
        ...
        has_repository_dependencies = util.string_as_bool(
            kwd.get("has_repository_dependencies", False)
        )
        ...
        includes_tool_dependencies = util.string_as_bool(
            kwd.get("includes_tool_dependencies", False)
        )
        ...
        if encoded_repo_info_dicts:
            encoded_repo_info_dicts = encoded_repo_info_dicts.split(
                encoding_util.encoding_sep
            )
        if not encoded_repo_info_dicts:
            ...
            if updating:
                ...
                try:
                    params = dict(
                        name=str(repository.name), owner=str(repository.owner)
                    )
                    ...
                    message = (
                        "The updates available for the repository <b>%s</b> "
                        % escape(str(repository.name))
                    )
            ...
            params = dict(
                repository_ids=str(repository_ids),
                changeset_revisions=str(changeset_revisions),
            )
            ...
            repo_information_dict = json.loads(raw_text)
            for encoded_repo_info_dict in repo_information_dict.get(
                "repo_info_dicts", []
            ):
            ...
            encoded_repo_info_dicts = util.listify(
                repo_information_dict.get("repo_info_dicts", [])
            )
        repo_info_dicts = [
        ...
        if kwd.get("select_tool_panel_section_button", False):
            if updating:
                ...
                decoded_updated_metadata = encoding_util.tool_shed_decode(
                    encoded_updated_metadata
                )
            ...
            created_or_updated_tool_shed_repositories, tool_panel_section_keys, repo_info_dicts, filtered_repo_info_dicts = install_repository_manager.handle_tool_shed_repositories(
                installation_dict
            )
            if created_or_updated_tool_shed_repositories:
                ...
                encoded_kwd, query, tool_shed_repositories, encoded_repository_ids = install_repository_manager.initiate_repository_installation(
                    installation_dict
                )
                return trans.fill_template(
                    "admin/tool_shed_repository/monitor_repository_installation.mako",
                ...
                kwd["message"] = (
                    message if message else "Repository has already been installed."
                )
                kwd["status"] = status
                return trans.response.send_redirect(
                    web.url_for(
        ...
        shed_tool_conf_select_field = tool_util.build_shed_tool_conf_select_field(
            trans.app
        )
        tool_path = suc.get_tool_path_by_shed_tool_conf_filename(
            trans.app, shed_tool_conf
        ...
        if len(repo_info_dicts) == 1:
            ...
            if not includes_tools:
                includes_tools = dependencies_for_repository_dict.get(
            ...
            installed_tool_dependencies = dependencies_for_repository_dict.get(
                "installed_tool_dependencies", None
            )
            missing_repository_dependencies = dependencies_for_repository_dict.get(
                "missing_repository_dependencies", None
            )
            missing_tool_dependencies = dependencies_for_repository_dict.get(
                "missing_tool_dependencies", None
            )
            readme_files_dict = readme_util.get_readme_files_dict_for_display(
                trans.app, tool_shed_url, repo_info_dict
            )
            ...
            containers_dict = dd.merge_containers_dicts_for_new_install(
                containers_dicts
            )
        # Handle tool dependencies check box.
        if trans.app.config.tool_dependency_dir is None:
        ...
        install_repository_dependencies_check_box = CheckboxField(
            "install_repository_dependencies", value=True
        )
        view = views.DependencyResolversView(self.app)
        if view.installable_resolvers:
            install_resolver_dependencies_check_box = CheckboxField(
            ...
            install_resolver_dependencies_check_box = None
        encoded_repo_info_dicts = encoding_util.encoding_sep.join(
            encoded_repo_info_dicts
        )
    ...
    def purge_repository(self, trans, **kwd):
        ...
        if repository_id is not None:
            ...
            if repository:
                if repository.is_new:
                    if kwd.get("purge_repository_button", False):
                        ...
                        return trans.fill_template(
                            "admin/tool_shed_repository/purge_repository_confirmation.mako",
                            repository=repository,
                        )
                else:
                    new_kwd["status"] = "error"
                    new_kwd[
                        "message"
                    ] = "Repositories must have a <b>New</b> status in order to be purged."
            else:
                new_kwd["status"] = "error"
                new_kwd["message"] = (
                    "Cannot locate the database record for the repository with encoded id %s."
                    % str(repository_id)
                )
        else:
            new_kwd["status"] = "error"
            new_kwd[
                "message"
            ] = 'Invalid repository id value "None" received for repository to be purged.'
    ...
    def reinstall_repository(self, trans, **kwd):
        ...
        status = kwd.get("status", "done")
        repository_id = kwd["id"]
        tool_shed_repository = repository_util.get_installed_tool_shed_repository(
            trans.app, repository_id
        )
        no_changes = kwd.get("no_changes", "")
        no_changes_checked = CheckboxField.is_checked(no_changes)
        install_repository_dependencies = CheckboxField.is_checked(
            kwd.get("install_repository_dependencies", "")
        )
        install_tool_dependencies = CheckboxField.is_checked(
            kwd.get("install_tool_dependencies", "")
        )
        install_resolver_dependencies = CheckboxField.is_checked(
            kwd.get("install_resolver_dependencies", "")
        )
        shed_tool_conf, tool_path, relative_install_dir = suc.get_tool_panel_config_tool_path_install_dir(
        ...
        if (
            ...
            if repository_type == rt_util.TOOL_DEPENDENCY_DEFINITION:
                ...
                suc.clean_dependency_relationships(
                    trans, new_meta, tool_shed_repository, tool_shed_url
                )
        ...
        created_or_updated_tool_shed_repositories, tool_panel_section_keys, repo_info_dicts, filtered_repo_info_dicts = rdim.create_repository_dependency_objects(
            tool_path=tool_path,
            tool_shed_url=tool_shed_url,
            repo_info_dicts=repo_info_dicts,
            install_repository_dependencies=install_repository_dependencies,
        ...
        encoded_repository_ids = [
            trans.security.encode_id(r.id)
            for r in created_or_updated_tool_shed_repositories
        ]
        new_kwd = dict(
            includes_tool_dependencies=includes_tool_dependencies,
    ...
    def reselect_tool_panel_section(self, trans, **kwd):
        ...
        if latest_changeset_revision and latest_ctx_rev:
            ...
            updated_repo_info_dict = encoding_util.tool_shed_decode(
                encoded_updated_repo_info_dict
            )
            readme_files_dict = updated_repo_info_dict.get("readme_files_dict", None)
            includes_data_managers = updated_repo_info_dict.get(
            ...
            includes_tool_dependencies = updated_repo_info_dict.get(
                "includes_tool_dependencies", False
            )
        ...
        if includes_tools_for_display_in_tool_panel:
            ...
            if "tool_panel_section" in metadata:
                ...
                if tool_panel_dict:
                    if tool_util.panel_entry_per_tool(tool_panel_dict):
                        ...
                        tool_section_dicts = tool_panel_dict[
                            next(iter(tool_panel_dict.keys()))
                        ]
            ...
            if original_section_name:
                ...
                message += (
                    "The tools contained in your <b>%s</b> repository were last loaded into the tool panel outside of any sections.  "
                    % escape(tool_shed_repository.name)
                )
        ...
        containers_dict = dd.merge_missing_repository_dependencies_to_installed_container(
            containers_dict
        )
        # Handle repository dependencies check box.
        install_repository_dependencies_check_box = CheckboxField(
    ...
    def reset_metadata_on_selected_installed_repositories(self, trans, **kwd):
        ...
        if "reset_metadata_on_selected_repositories_button" in kwd:
            message, status = irmm.reset_metadata_on_selected_repositories(
                trans.user, **kwd
            )
        else:
            message = escape(kwd.get("message", ""))
        ...
        return trans.fill_template(
            "/admin/tool_shed_repository/reset_metadata_on_selected_repositories.mako",
    ...
    def reset_repository_metadata(self, trans, id):
        ...
        if relative_install_dir:
            ...
            if irmm_metadata_dict != original_metadata_dict:
                ...
                trans.install_model.context.add(repository)
                trans.install_model.context.flush()
                message = "Metadata has been reset on repository <b>%s</b>." % escape(
                    repository.name
                )
                status = "done"
            else:
                message = (
                    "Metadata did not need to be reset on repository <b>%s</b>."
                    % escape(repository.name)
                )
                status = "done"
        else:
            message = (
                "Error locating installation directory for repository <b>%s</b>."
                % escape(repository.name)
            )
            status = "error"
        return trans.response.send_redirect(
    ...
    def reset_to_install(self, trans, **kwd):
        ...
        if kwd.get("reset_repository", False):
            ...
            new_kwd = {}
            new_kwd["message"] = (
                "You can now attempt to install the repository named <b>%s</b> again."
                % escape(str(repository.name))
            )
            new_kwd["status"] = "done"
        return trans.response.send_redirect(
    ...
    def tool_dependency_status_updates(self, trans, ids=None, status_list=None):
        ...
        if ids is not None and status_list is not None:
            ...
            for tup in zip(ids, status_list):
                id, status = tup
                tool_dependency = trans.install_model.context.query(
                    trans.install_model.ToolDependency
                ).get(trans.security.decode_id(id))
                if tool_dependency.status != status:
                    rval.append(
                        dict(
                            ...
                            html_status=unicodify(
                                trans.fill_template(
                                    "admin/tool_shed_repository/tool_dependency_installation_status.mako",
                                    tool_dependency=tool_dependency,
                                ),
                                "utf-8",
                            ),
    ...
    def uninstall_tool_dependencies(self, trans, **kwd):
        ...
        if kwd.get("uninstall_tool_dependencies_button", False):
            ...
            for tool_dependency in tool_dependencies_for_uninstallation:
                uninstalled, error_message = tool_dependency_util.remove_tool_dependency(
                    trans.app, tool_dependency
                )
                if error_message:
                    errors = True
            ...
            if errors:
                message = (
                    "Error attempting to uninstall tool dependencies: %s" % message
                )
                status = "error"
            else:
                message = (
                    "These tool dependencies have been uninstalled: %s"
    ...
    def update_to_changeset_revision(self, trans, **kwd):
        ...
        if changeset_revision and latest_changeset_revision and latest_ctx_rev:
            if changeset_revision == latest_changeset_revision:
                message = (
                    "The installed repository named '%s' is current, there are no updates available.  "
                    % name
                )
            else:
                shed_tool_conf, tool_path, relative_install_dir = suc.get_tool_panel_config_tool_path_install_dir(
                ...
                if relative_install_dir:
                    ...
                    repository_clone_url = os.path.join(
                        tool_shed_url, "repos", owner, name
                    )
                    hg_util.pull_repository(
                        repo_files_dir, repository_clone_url, latest_ctx_rev
                    )
                    ...
                    if "tools" in irmm_metadata_dict:
                        tool_panel_dict = irmm_metadata_dict.get(
                    ...
                    if (
                        ...
                        new_tool_dependencies_dict = irmm_metadata_dict.get(
                            "tool_dependencies", {}
                        )
                        if new_repository_dependencies:
                            ...
                            if (
                                ...
                                == original_repository_dependencies
                            ):
                                for new_repository_tup in new_repository_dependencies:
                                    ...
                                    new_tool_shed, new_name, new_owner, new_changeset_revision, new_pir, new_oicct = common_util.parse_repository_dependency_tuple(
                                        new_repository_tup
                                    )
                                    ...
                                    # from the Tool Shed.
                                    new_repository_db_record, installed_changeset_revision = repository_util.repository_was_previously_installed(
                                        trans.app,
                                        tool_shed_url,
                                        new_name,
                                        repo_info_tuple,
                                    ...
                                    if new_repository_db_record:
                                        if new_repository_db_record.status in [
                            ...
                            if proceed_to_install:
                                ...
                                new_kwd = dict(
                                    tool_shed_url=tool_shed_url,
                                    updating_repository_id=trans.security.encode_id(
                                        repository.id
                                    ),
                                    updating_to_ctx_rev=latest_ctx_rev,
                                    updating_to_changeset_revision=latest_changeset_revision,
                                    encoded_updated_metadata=encoding_util.tool_shed_encode(
                                        irmm_metadata_dict
                                    ),
                                    updating=True,
                                )
                        ...
                        if proceed_to_install:
                            ...
                            encoded_relative_install_dir = encoding_util.tool_shed_encode(
                                relative_install_dir
                            )
                            new_kwd = dict(
                                updating_repository_id=trans.security.encode_id(
                            ...
                            return self.install_tool_dependencies_with_update(
                                trans, **new_kwd
                            )
                    ...
                    message = (
                        "The directory containing the installed repository named '%s' cannot be found.  "
                        % name
                    )
                    status = "error"
        else:
            message = (
                "The latest changeset revision could not be retrieved for the installed repository named '%s'.  "
                % name
            )
            status = "error"
        return trans.response.send_redirect(
                ...
                if tool_metadata_dict["id"] == tool_id:
                    ...
                    if shed_config_dict and shed_config_dict.get("tool_path"):
                        tool_config = os.path.join(
                            shed_config_dict.get("tool_path"), tool_config
                        )
                    tool = trans.app.toolbox.get_tool(
                        tool_id=tool_metadata["guid"], exact=True
                    )
                    if not tool:
                        tool = trans.app.toolbox.load_tool(
                            os.path.abspath(tool_config), guid=tool_metadata["guid"]
                        )
                        if tool:
                            tool._lineage = trans.app.toolbox._lineage_map.register(
                                tool
                            )
                    if tool:
    ...
    def update_tool_shed_status_for_installed_repository(self, trans, **kwd):
        message, status = repository_util.check_for_updates(
            trans.app, trans.install_model, kwd.get("id", None)
        )
        return trans.response.send_redirect(
            web.url_for(
                controller="admin",
    ...
    def view_workflow(self, trans, workflow_name=None, repository_id=None, **kwd):
        ...
        return trans.fill_template(
            "/admin/tool_shed_repository/view_workflow.mako",
            repository=repository,
            changeset_revision=changeset_revision,
            repository_id=repository_id,
            workflow_name=workflow_name,
