...
class DataManager(BaseUIController):
    ...
    def data_managers_list(self, trans, **kwd):
        ...
        for data_manager_id, data_manager in sorted(
            trans.app.data_managers.data_managers.iteritems(),
            key=lambda data_manager: data_manager[1].name,
        ):
            data_managers.append(
                {
                    "toolUrl": web.url_for(
                        controller="root", tool_id=data_manager.tool.id
                    ),
                    "id": data_manager_id,
    ...
    def jobs_list(self, trans, **kwd):
        ...
        if data_manager is None:
            return {
                "message": "Invalid Data Manager (%s) was requested" % data_manager_id,
                "status": "error",
            }
        jobs = []
        for assoc in trans.sa_session.query(
            trans.app.model.DataManagerJobAssociation
        ).filter_by(data_manager_id=data_manager_id):
            j = assoc.job
            jobs.append(
                {
                    "id": j.id,
                    ...
                    "runUrl": web.url_for(
                        ...
                        action="rerun",
                        job_id=trans.security.encode_id(j.id),
                    ),
                    "user": j.history.user.email
                    if j.history and j.history.user
                    else "anonymous",
        ...
        for hda in hdas:
            ...
            try:
                ...
                error_messages.append(
                    "Unable to obtain data_table info for hda (%s): %s" % (hda.id, e)
                )
            values = []
    ...
    def tool_data_table_info_1(self, trans, **kwd):
        ...
        if data_table is None:
            return {
                "message": "Invalid data table '%s' was requested." % data_table_name,
                "status": "error",
            }
        return {
            "dataTable": {
    ...
    def reload_tool_data_tables(self, trans, table_name=None, **kwd):
        ...
        data = None
        if table_names:
            message = "Reloaded data table%s '%s'." % (
                "s"[len(table_names) == 1 :],
                ", ".join(table_names),
            )
            data = self.tool_data_table_info_1(
                trans, table_name=table_names[0], message=message, status="done"
            )
        else:
    ...
    def reload_tool_data_table(self, trans, **kwd):
        ...
        if not table_name:
            return {"message": "No data table has been reloaded.", "status": "error"}
