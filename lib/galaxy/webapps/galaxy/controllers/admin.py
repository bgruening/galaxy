...
from tool_shed.util import common_util, encoding_util, repository_util
...
class UserListGrid(grids.Grid):
    ...
    columns = [
        EmailColumn(
            ...
            link=(
                lambda item: dict(
                    controller="user", action="information", id=item.id, webapp="galaxy"
                )
            ),
            attach_popup=True,
            filterable="advanced",
    ...
    operations = [
        ...
        grids.GridOperation(
            "Generate New API Key", allow_multiple=False, async_compatible=True
        ),
    ]
    standard_filters = [
...
class RoleListGrid(grids.Grid):
    ...
    columns = [
        NameColumn(
            ...
            link=(
                lambda item: dict(
                    action="form/manage_users_and_groups_for_role",
                    id=item.id,
                    webapp="galaxy",
                )
            ),
    ...
    operations = [
        ...
        grids.GridOperation(
            ...
            allow_multiple=False,
            url_args=dict(
                action="form/manage_users_and_groups_for_role", webapp="galaxy"
            ),
        ),
        grids.GridOperation(
            "Delete", condition=(lambda item: not item.deleted), allow_multiple=True
        ),
        grids.GridOperation(
            "Undelete", condition=(lambda item: item.deleted), allow_multiple=True
        ),
        grids.GridOperation(
            "Purge", condition=(lambda item: item.deleted), allow_multiple=True
        ),
    ]
    standard_filters = [
        grids.GridColumnFilter("Active", args=dict(deleted=False)),
...
class GroupListGrid(grids.Grid):
    ...
    columns = [
        NameColumn(
            ...
            link=(
                lambda item: dict(
                    action="form/manage_users_and_roles_for_group",
                    id=item.id,
                    webapp="galaxy",
                )
            ),
    ...
    operations = [
        ...
        grids.GridOperation(
            ...
            allow_multiple=False,
            url_args=dict(
                action="form/manage_users_and_roles_for_group", webapp="galaxy"
            ),
        ),
        grids.GridOperation(
            "Delete", condition=(lambda item: not item.deleted), allow_multiple=True
...
class QuotaListGrid(grids.Grid):
    ...
    columns = [
        ...
        AmountColumn(
            "Amount", key="amount", model_class=model.Quota, attach_popup=False
        ),
        UsersColumn("Users", attach_popup=False),
        GroupsColumn("Groups", attach_popup=False),
        StatusColumn("Status", attach_popup=False),
            ...
            if tool:
                for tool_id in tool.lineage.tool_ids:
                    if toolbox.has_tool(tool_id, exact=True):
                        ...
                        link_str = '<a target="_blank" href="%s">' % link
                        tool_ids_str += (
                            '<div class="count-box state-color-ok">%s%s</a></div><br/>'
                            % (link_str, tool_id)
                        )
                    else:
...
class AdminGalaxy(
    controller.JSAppLauncher, AdminActions, UsesQuotaMixin, QuotaParamParser
):
    ...
    def client(self, trans, **kwd):
        ...
        settings = {
            "is_repo_installed": trans.install_model.context.query(
                trans.install_model.ToolShedRepository
            ).first()
            is not None,
            "installing_repository_ids": repository_util.get_ids_of_tool_shed_repositories_being_installed(
                trans.app, as_string=True
            ),
            "is_tool_shed_installed": bool(
                trans.app.tool_shed_registry and trans.app.tool_shed_registry.tool_sheds
            ),
        }
        return self._bootstrapped_client(
            trans, app_name="admin", settings=settings, message=message, status=status
        )
    ...
    def data_types_list(self, trans, **kwd):
        ...
        status = kwd.get("status", "done")
        for dtype in sorted(
            trans.app.datatypes_registry.datatype_elems,
        ...
        if "operation" in kwargs:
            ...
            if not id:
                return self.message_exception(
                    trans, "Invalid quota id (%s) received." % str(id)
                )
            quotas = []
    ...
    def create_quota(self, trans, payload=None, **kwd):
        if trans.request.method == "GET":
            ...
            try:
                quota, message = self._create_quota(
                    util.Params(payload), decode_id=trans.security.decode_id
                )
    ...
    def rename_quota(self, trans, payload=None, **kwd):
        ...
        if trans.request.method == "GET":
            return {
                "title": "Change quota name and description for '%s'"
                % util.sanitize_text(quota.name),
                "inputs": [
    ...
    def manage_users_and_groups_for_quota(self, trans, payload=None, **kwd):
        ...
        if not quota_id:
            return self.message_exception(
                trans, "Invalid quota id (%s) received" % str(quota_id)
            )
        ...
        if trans.request.method == "GET":
            ...
            try:
                return {
                    "message": self._manage_users_and_groups_for_quota(
    ...
    def impersonate(self, trans, **kwd):
        if not trans.app.config.allow_user_impersonation:
            return trans.show_error_message(
                "User impersonation is not enabled in this instance of Galaxy."
            )
        user = None
            ...
            try:
                ...
                if user:
                    ...
                    return trans.show_message(
                        'You are now logged in as %s, <a target="_top" href="%s">return to the home page</a>'
                        % (user.email, url_for(controller="root")),
                        use_panels=True,
                    )
            except Exception:
    ...
    def check_for_tool_dependencies(self, trans, migration_stage):
        ...
        shed_url = common_util.get_tool_shed_url_from_tool_shed_registry(
            trans.app, tool_shed
        )
        ...
        if shed_url:
            for elem in root:
                if elem.tag == "repository":
                    ...
                    text = url_get(
                        shed_url,
                        password_mgr=self.app.tool_shed_registry.url_auth(shed_url),
                        pathspec=pathspec,
                        params=params,
                    )
                    if text:
                        ...
                        for (
                            dependency_key,
                            requirements_dict,
                        ) in tool_dependencies_dict.items():
    ...
    def review_tool_migration_stages(self, trans, **kwd):
        ...
        migration_scripts_dir = os.path.abspath(
            os.path.join(
                trans.app.config.root,
                "lib",
                "tool_shed",
        ...
        for item in modules:
            ...
            repo_name_dependency_tups = self.check_for_tool_dependencies(
                trans, migration_stage
            )
            open_file_obj, file_name, description = imp.find_module(
                module, [migration_scripts_dir]
            )
            imported_module = imp.load_module(
                "upgrade", open_file_obj, file_name, description
            )
            migration_info = imported_module.__doc__
            open_file_obj.close()
            migration_stages_dict[migration_stage] = (
                migration_info,
                repo_name_dependency_tups,
            )
        return trans.fill_template(
            "admin/review_tool_migration_stages.mako",
    ...
    def center(self, trans, **kwd):
        ...
        status = kwd.get("status", "done")
        is_repo_installed = (
            trans.install_model.context.query(
                trans.install_model.ToolShedRepository
            ).first()
            is not None
        )
        ...
        return trans.fill_template(
            "/webapps/galaxy/admin/center.mako",
    ...
    def create_role(self, trans, payload=None, **kwd):
        if trans.request.method == "GET":
            ...
            in_users = [
                trans.sa_session.query(trans.app.model.User).get(
                    trans.security.decode_id(x)
                )
                for x in util.listify(payload.get("in_users"))
            ]
            in_groups = [
                trans.sa_session.query(trans.app.model.Group).get(
                    trans.security.decode_id(x)
                )
            ...
            if not name or not description:
                return self.message_exception(
                    trans, "Enter a valid name and a description."
                )
            elif (
                trans.sa_session.query(trans.app.model.Role)
                .filter(trans.app.model.Role.table.c.name == name)
                .first()
            ):
                return self.message_exception(
                    trans,
                    "Role names must be unique and a role with that name already exists, so choose another name.",
                )
            elif None in in_users or None in in_groups:
                return self.message_exception(
                    trans, "One or more invalid user/group id has been provided."
                )
            else:
                # Create the role
                ...
                if auto_create_checked:
                    ...
                    if (
                        trans.sa_session.query(trans.app.model.Group)
                        .filter(trans.app.model.Group.table.c.name == name)
                        .first()
                    ):
                        return self.message_exception(
                            trans,
    ...
    def rename_role(self, trans, payload=None, **kwd):
        ...
        if trans.request.method == "GET":
            return {
                "title": "Change role name and description for '%s'"
                % util.sanitize_text(role.name),
                "inputs": [
            ...
            if not new_name:
                ...
                if existing_role and existing_role.id != role.id:
                    return self.message_exception(
                        trans, "A role with that name already exists."
                    )
                else:
                    if not (
                        role.name == new_name and role.description == new_description
                    ):
                        role.name = new_name
    ...
    def manage_users_and_groups_for_role(self, trans, payload=None, **kwd):
        ...
        if not role_id:
            return self.message_exception(
                trans, "Invalid role id (%s) received" % str(role_id)
            )
        ...
        if trans.request.method == "GET":
            ...
            for ura in role.users:
                ...
                if user not in in_users:
                    ...
                    trans.sa_session.flush()
            trans.app.security_agent.set_entity_role_associations(
                roles=[role], users=in_users, groups=in_groups
            )
    ...
    def _undelete_role(self, trans, ids):
        ...
        for role_id in ids:
            ...
            if not role.deleted:
                return (
                    "Role '%s' has not been deleted, so it cannot be undeleted."
                    % role.name,
                    "error",
                )
            role.deleted = False
    ...
    def _purge_role(self, trans, ids):
        ...
        for role_id in ids:
            ...
            if not role.deleted:
                return (
                    "Role '%s' has not been deleted, so it cannot be purged."
                    % role.name,
                    "error",
                )
    ...
    def groups_list(self, trans, **kwargs):
        ...
        if "operation" in kwargs:
            ...
            if not id:
                return self.message_exception(
                    trans, "Invalid group id (%s) received." % str(id)
                )
            ids = util.listify(id)
            operation = kwargs["operation"].lower().replace("+", " ")
            if operation == "delete":
        ...
        if trans.request.method == "GET":
            ...
            if not new_name:
                ...
                if existing_group and existing_group.id != group.id:
                    return self.message_exception(
                        trans, "A group with that name already exists."
                    )
                else:
    ...
    def manage_users_and_roles_for_group(self, trans, payload=None, **kwd):
        ...
        if not group_id:
            return self.message_exception(
                trans, "Invalid group id (%s) received" % str(group_id)
            )
        ...
        if trans.request.method == "GET":
            ...
            if None in in_users or None in in_roles:
                return self.message_exception(
                    trans, "One or more invalid user/role id has been provided."
                )
            trans.app.security_agent.set_entity_group_associations(
                groups=[group], users=in_users, roles=in_roles
            )
            trans.sa_session.refresh(group)
            return {
                "message": "Group '%s' has been updated with %d associated users and %d associated roles."
                % (group.name, len(in_users), len(in_roles))
            }
    ...
    def _undelete_group(self, trans, ids):
        ...
        for group_id in ids:
            ...
            if not group.deleted:
                return (
                    "Group '%s' has not been deleted, so it cannot be undeleted."
                    % group.name,
                    "error",
                )
            group.deleted = False
    ...
    def _purge_group(self, trans, ids):
        ...
        for group_id in ids:
            ...
            if not group.deleted:
                return (
                    "Group '%s' has not been deleted, so it cannot be purged."
                    % group.name,
                    "error",
                )
    ...
    def create_new_user(self, trans, **kwd):
        return trans.response.send_redirect(
            web.url_for(controller="user", action="create", cntrller="admin")
        )
    ...
    def reset_user_password(self, trans, payload=None, **kwd):
        users = {
            user_id: get_user(trans, user_id) for user_id in util.listify(kwd.get("id"))
        }
        if users:
            if trans.request.method == "GET":
                return {
                    "message": "Changes password(s) for: %s."
                    % ", ".join(user.email for user in users.values()),
                    "status": "info",
                    "inputs": [
                        {
                            "name": "password",
                ...
                if len(password) < 6:
                    return self.message_exception(
                        trans, "Use a password of at least 6 characters."
                    )
    ...
    def _delete_user(self, trans, ids):
        ...
        for user_id in ids:
            ...
            if self.app.config.redact_user_address_during_deletion:
                user_addresses = (
                    trans.sa_session.query(trans.app.model.UserAddress)
                    .filter(trans.app.model.UserAddress.user_id == user.id)
                    .all()
                )
                ...
                for addr in user_addresses:
                    ...
                    addr.institution = new_secure_hash(
                        addr.institution + pseudorandom_value
                    )
                    ...
                    addr.postal_code = new_secure_hash(
                        addr.postal_code + pseudorandom_value
                    )
    ...
    def _undelete_user(self, trans, ids):
        ...
        for user_id in ids:
            ...
            if not user.deleted:
                message = (
                    "User '%s' has not been deleted, so it cannot be undeleted."
                    % user.email
                )
    ...
    def _purge_user(self, trans, ids):
        ...
        for user_id in ids:
            ...
            if not user.deleted:
                return (
                    "User '%s' has not been deleted, so it cannot be purged."
                    % user.email,
                    "error",
                )
    ...
    def _recalculate_user(self, trans, user_id):
        ...
        if new in (current, None):
            ...
            message = "Usage has changed by %s to %s." % (
                nice_size(new - current),
                nice_size(new),
            )
        return (message, "done")
    ...
    def _new_user_apikey(self, trans, user_id):
        ...
        trans.sa_session.flush()
        return (
            "New key '%s' generated for requested user '%s'."
            % (new_key.key, user.email),
            "done",
        )
    ...
    def manage_roles_and_groups_for_user(self, trans, payload=None, **kwd):
        ...
        if not user_id:
            return self.message_exception(
                trans, "Invalid user id (%s) received" % str(user_id)
            )
        user = get_user(trans, user_id)
        if trans.request.method == "GET":
            in_roles = []
            ...
            for group in (
                ...
                all_groups.append((group.name, trans.security.encode_id(group.id)))
            return {
                "title": "Roles and groups for '%s'" % user.email,
            ...
            if None in in_groups or None in in_roles:
                return self.message_exception(
                    trans, "One or more invalid role/group id has been provided."
                )
            ...
            trans.app.security_agent.set_entity_user_associations(
                users=[user], roles=in_roles, groups=in_groups
            )
    ...
    def jobs_list(self, trans, stop=[], stop_msg=None, cutoff=180, **kwd):
        ...
        if job_ids and stop_msg in [None, ""]:
            ...
            for job_id in job_ids:
                error_msg = (
                    "This job was stopped by an administrator: %s  <a href='%s' target='_blank'>Contact support</a> for additional help."
                    % (
                        stop_msg,
                        self.app.config.get(
        ...
        jobs = (
            trans.sa_session.query(trans.app.model.Job)
            .filter(
                and_(
        ...
        recent_jobs = (
            ...
            .filter(
                and_(
                    trans.app.model.Job.table.c.update_time > cutoff_time,
                    or_(
                        trans.app.model.Job.state == trans.app.model.Job.states.ERROR,
        ...
        def prepare_jobs_list(jobs):
            ...
            for job in jobs:
                ...
                update_time = ""
                if delta.days > 0:
                    update_time = "%s hours ago" % (
                        delta.days * 24 + int(delta.seconds / 60 / 60)
                    )
                ...
                try:
                    inputs = ", ".join(
                        [
                            "{} {}".format(da.dataset.id, da.dataset.state)
                ...
                res.append(
                    {
                        "job_info": {
                            "id": job.id,
                            "info_url": "{}?jobid={}".format(
                                web.url_for(controller="admin", action="job_info"),
                                job.id,
                            ),
                        },
                        "user": job.history.user.email
    ...
    def job_info(self, trans, jobid=None):
        ...
        return trans.fill_template(
            "/webapps/reports/job_info.mako", job=job, message="<a href='jobs'>Back</a>"
        )
    ...
    def manage_tool_dependencies(
        ...
        if selected_tool_ids:
            ...
            requirements = set(
                [tools_by_id[tid].tool_requirements for tid in selected_tool_ids]
            )
            if install_dependencies:
                [view.install_dependencies(r) for r in requirements]
            elif uninstall_dependencies:
                [
                    view.uninstall_dependencies(index=None, requirements=r)
    ...
    def sanitize_whitelist(self, trans, submit_whitelist=False, tools_to_whitelist=[]):
        if submit_whitelist:
            ...
            with open(trans.app.config.sanitize_whitelist_file, "wt") as f:
                ...
                new_whitelist = sorted(
                    [
                        tid
                        for tid in tools_to_whitelist
                        if tid in trans.app.toolbox.tools_by_id
                    ]
                )
                f.write("\n".join(new_whitelist))
            trans.app.config.sanitize_whitelist = new_whitelist
            galaxy.queue_worker.send_control_task(
                trans.app, "reload_sanitize_whitelist", noop_self=True
            )
            # dispatch a message to reload list for other processes
        return trans.fill_template(
            "/webapps/galaxy/admin/sanitize_whitelist.mako",
