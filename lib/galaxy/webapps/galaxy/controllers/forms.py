...
class FormsGrid(grids.Grid):
        ...
        NameColumn(
            ...
            link=(
                lambda item: iff(
                    item.deleted,
                    None,
                    dict(controller="admin", action="form/edit_form", id=item.id),
                )
            ),
            attach_popup=True,
            filterable="advanced",
    ...
    operations = [
        grids.GridOperation(
            "Delete", allow_multiple=True, condition=(lambda item: not item.deleted)
        ),
        grids.GridOperation("Undelete", condition=(lambda item: item.deleted)),
    ]
    global_actions = [
        grids.GridAction(
            "Create new form", dict(controller="admin", action="form/create_form")
        )
    ]
    ...
    def build_initial_query(self, trans, **kwargs):
        return trans.sa_session.query(self.model_class).join(
            ...
            self.model_class.latest_form_id == model.FormDefinition.id,
        )
...
class Forms(BaseUIController):
        ...
        if "operation" in kwd:
            ...
            if not id:
                return self.message_exception(
                    trans, "Invalid form id (%s) received." % str(id)
                )
            ids = util.listify(id)
    ...
    def create_form(self, trans, payload=None, **kwd):
        if trans.request.method == "GET":
            ...
            if csv_file:
                ...
                for line in lines:
                    ...
                    if len(row) >= 6:
                        ...
                        payload["%s%s" % (prefix, "name")] = "%i_imported_field" % (
                            index + 1
                        )
            ...
            message = "The form '%s' has been created%s." % (
                payload.get("name"),
                imported,
            )
    ...
    def edit_form(self, trans, payload=None, **kwd):
        ...
        if trans.request.method == "GET":
            fd_types = sorted(trans.app.model.FormDefinition.types.items())
            ff_types = [
                (t.__name__.replace("Field", ""), t.__name__)
                for t in trans.model.FormDefinition.supported_field_types
            ]
            field_cache = []
            field_inputs = [
                {
    ...
    def get_current_form(self, trans, payload=None, **kwd):
        ...
        while True:
            ...
            if "%s%s" % (prefix, "label") in payload:
                ...
                field_dict = {
                    attr: payload.get("%s%s" % (prefix, attr))
    ...
    def save_form_definition(self, trans, form_id=None, payload=None, **kwd):
        ...
        for field in current_form["fields"]:
            ...
            if field["name"] in field_names_dict:
                return (
                    None,
                    "Each field name must be unique in the form definition. %s is not unique."
                    % field["name"],
                )
            else:
        ...
        if form_id:
            form_definition_current = trans.sa_session.query(
                trans.app.model.FormDefinitionCurrent
            ).get(trans.security.decode_id(form_id))
            if form_definition_current is None:
                return (
                    None,
                    "Invalid form id (%s) provided. Cannot save form." % form_id,
                )
        else:
...
def get_form(trans, form_id):
    ...
    form = trans.sa_session.query(trans.app.model.FormDefinitionCurrent).get(
