...
class OpenID(BaseUIController):
    def __init__(self, app):
        ...
        if app.config.enable_openid:
            ...
            self.openid_providers = OpenIDProviders.from_file(
                "lib/galaxy/openid/openid_conf.xml"
            )
    ...
    def openid_auth(self, trans, **kwd):
        ...
        if not trans.app.config.enable_openid:
            return trans.show_error_message(
                "OpenID authentication is not enabled in this instance of Galaxy."
            )
        ...
        try:
            ...
            if request is None:
                return trans.show_error_message(
                    "No OpenID services are available at %s."
                    % openid_provider_obj.op_endpoint_url
                )
        except Exception as e:
            return trans.show_error_message(
                "Failed to begin OpenID authentication: %s." % str(e)
            )
        if request is not None:
            self.openid_manager.add_sreg(
                trans,
                request,
                required=openid_provider_obj.sreg_required,
                optional=openid_provider_obj.sreg_optional,
            )
            if request.shouldSendRedirect():
                ...
                form = request.htmlMarkup(
                    trans.request.base,
                    process_url,
                    form_tag_attrs={"id": "openid_message", "target": "_top"},
                )
                self.openid_manager.persist_session(trans, consumer)
                return form
    ...
    def openid_process(self, trans, **kwd):
        ...
        if not trans.app.config.enable_openid:
            return trans.show_error_message(
                "OpenID authentication is not enabled in this instance of Galaxy. %s"
                % return_link
            )
        consumer = self.openid_manager.get_consumer(trans)
        ...
        if info.status == self.openid_manager.SUCCESS:
            ...
            return trans.show_message(
                "Processed OpenID authentication. %s" % return_link
            )
        else:
            return trans.show_error_message(
                "Authentication via OpenID failed: %s. %s" % (info.message, return_link)
            )
