...
class ToolRunner(BaseUIController):
    ...
    def index(self, trans, tool_id=None, from_noframe=None, **kwd):
        def __tool_404__():
            ...
            return trans.show_error_message(
                "Tool '%s' does not exist." % (escape(tool_id))
            )
        ...
        if tool.require_login and not trans.user:
            redirect = url_for(
                controller="tool_runner", action="index", tool_id=tool_id, **kwd
            )
            return trans.response.send_redirect(
                url_for(
                    controller="user",
        ...
        if "runtool_btn" not in params.__dict__ and "URL" not in params.__dict__:
            error(
                "Tool execution through the `tool_runner` requires a `runtool_btn` flag or `URL` parameter."
            )
    ...
    def data_source_redirect(self, trans, tool_id=None):
        ...
        if not tool:
            log.error(
                "data_source_redirect called with tool id '%s' but no such tool exists",
                tool_id,
            )
            trans.log_event("Tool id '%s' does not exist" % tool_id)
            trans.response.status = 404
