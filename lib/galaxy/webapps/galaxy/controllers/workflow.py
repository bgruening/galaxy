...
class StoredWorkflowListGrid(grids.Grid):
    ...
    operations = [
        ...
        grids.GridOperation(
            "Run", condition=(lambda item: not item.deleted), async_compatible=False
        ),
        grids.GridOperation(
            "Copy", condition=(lambda item: not item.deleted), async_compatible=False
        ),
        grids.GridOperation(
            "Rename", condition=(lambda item: not item.deleted), async_compatible=False
        ),
        grids.GridOperation(
            "Sharing", condition=(lambda item: not item.deleted), async_compatible=False
        ),
        grids.GridOperation(
            "Delete", condition=(lambda item: item.deleted), async_compatible=True
        ),
    ]
...
class StoredWorkflowAllPublishedGrid(grids.Grid):
    ...
    columns = [
        grids.PublicURLColumn(
            "Name", key="name", filterable="advanced", attach_popup=True
        ),
...
class WorkflowController(
    BaseUIController, SharableMixin, UsesStoredWorkflowMixin, UsesItemRatings
):
    ...
    def _display(self, trans, stored_workflow):
        ...
        for step in stored_workflow.latest_workflow.steps:
            step.annotation = self.get_item_annotation_str(
                trans.sa_session, stored_workflow.user, step
            )
        # Get rating data.
        user_item_rating = 0
        if trans.get_user():
            user_item_rating = self.get_user_item_rating(
                trans.sa_session, trans.get_user(), stored_workflow
            )
            if user_item_rating:
        ...
        ave_item_rating, num_ratings = self.get_ave_item_rating_data(
            trans.sa_session, stored_workflow
        )
    ...
    def get_item_content_async(self, trans, id):
        ...
        # Get annotations.
        stored.annotation = self.get_item_annotation_str(
            trans.sa_session, stored.user, stored
        )
        for step in stored.latest_workflow.steps:
            step.annotation = self.get_item_annotation_str(
                trans.sa_session, stored.user, step
            )
        return trans.stream_template_mako(
            "/workflow/item_content.mako",
            item=stored,
            item_data=stored.latest_workflow.steps,
        )
    ...
    def share(self, trans, id, email="", use_panels=False):
        ...
        if email:
            ...
            if not other:
                ...
                session.flush()
                trans.set_message(
                    "Workflow '%s' shared with user '%s'"
                    % (escape(stored.name), escape(other.email))
                )
                return trans.response.send_redirect(
                    url_for(controller="workflow", action="sharing", id=id)
                )
    ...
    def sharing(self, trans, id, **kwargs):
        ...
        if "unshare_me" in kwargs:
            ...
            session.flush()
            return trans.fill_template(
                "/workflow/sharing.mako", use_panels=True, item=stored
            )
    ...
    def imp(self, trans, id, **kwargs):
        ...
        if stored.importable is False:
            return trans.show_error_message(
                "The owner of this workflow has disabled imports via this link.<br>You can %s"
                % referer_message,
                use_panels=True,
            )
        elif stored.deleted:
            return trans.show_error_message(
                "You can't import this workflow because it has been deleted.<br>You can %s"
                % referer_message,
                use_panels=True,
            )
    ...
    def annotate_async(self, trans, id, new_annotation=None, **kwargs):
        ...
        if new_annotation:
            ...
            self.add_item_annotation(
                trans.sa_session, trans.get_user(), stored, new_annotation
            )
            trans.sa_session.flush()
    ...
    def rate_async(self, trans, id, rating):
        ...
        stored = self.get_stored_workflow(
    ...
    def copy(self, trans, id, save_as_name=None):
        ...
        # Copy annotation.
        annotation_obj = self.get_item_annotation_obj(
            trans.sa_session, stored.user, stored
        )
        if annotation_obj:
            self.add_item_annotation(
    ...
    def create(self, trans, payload=None, **kwd):
        if trans.request.method == "GET":
            ...
            self.add_item_annotation(
                trans.sa_session, trans.get_user(), stored_workflow, workflow_annotation
            )
            # Persist
            session = trans.sa_session
            ...
            return {
                ...
                "message": "Workflow %s has been created." % workflow_name,
            }

    @web.json
    def save_workflow_as(
        self, trans, workflow_name, workflow_data, workflow_annotation=""
    ):
        """
        ...
        if workflow_name is not None:
            ...
            try:
                workflow, errors = workflow_contents_manager.update_workflow_from_raw_description(
                    trans, stored_workflow, workflow_data
                )
    ...
    def delete(self, trans, id=None):
        ...
        trans.user.stored_workflow_menu_entries = [
            entry
            for entry in trans.user.stored_workflow_menu_entries
            if entry.stored_workflow != stored
        ]
        ...
        trans.set_message(message)
        return trans.response.send_redirect(
            url_for("/") + "workflow?status=done&message=%s" % escape(message)
        )
    ...
    def export_to_myexp(self, trans, id, myexp_username, myexp_password):
        ...
        workflow_list_str = " <br>Return to <a href='%s'>workflow list." % url_for(
            controller="workflows", action="list"
        )
    ...
    def export_to_file(self, trans, id):
        ...
        trans.response.headers[
            "Content-Disposition"
        ] = 'attachment; filename="Galaxy-Workflow-%s.ga"' % (sname)
            ...
            for step in workflow.steps:
                if step.type == "tool":
                    ...
                    for outputname in outputs:
                        m = model.WorkflowOutput(
                            workflow_step_id=int(step.id), output_name=outputname
                        )
    ...
    def _workflow_to_svg_canvas(self, trans, stored):
        ...
        for step in workflow.steps:
            ...
            workflow_canvas.populate_data_for_step(
                step, module_name, module_data_inputs, module_data_outputs
            )
...
def _build_workflow_on_str(instance_ds_names):
    ...
    if num_multi_inputs == 0:
        ...
        return " on %s and %s" % (
            ", ".join(instance_ds_names[0:-1]),
            instance_ds_names[-1],
        )


def _expand_multiple_inputs(kwargs):
    (single_inputs, matched_multi_inputs, multiplied_multi_inputs) = _split_inputs(
        kwargs
    )
    ...
    multi_input_keys = list(matched_multi_inputs.keys()) + list(
        multiplied_multi_inputs.keys()
    )
...
def _extend_with_matched_combos(single_inputs, multi_inputs):
    ...
    for value in first_multi_value:
        new_inputs = _copy_and_extend_inputs(
            single_inputs, first_multi_input_key, value
        )
    ...
    for multi_input_key, multi_input_value in multi_inputs.items():
        ...
        for combo in combos:
            for input_value in multi_input_value:
                iter_combos.append(
                    _copy_and_extend_inputs(combo, multi_input_key, input_value)
                )
