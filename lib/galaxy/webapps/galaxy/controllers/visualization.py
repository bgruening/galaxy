...
class HistoryDatasetsSelectionGrid(grids.Grid):
    ...
    class HistoryColumn(grids.GridColumn):
        ...
        def sort(self, trans, query, ascending, column_name=None):
            ...
            return grids.GridColumn.sort(
                self, trans, query, ascending, column_name="history_id"
            )
    ...
    columns = [
        ...
        grids.TextColumn(
            "Name", key="name", model_class=model.HistoryDatasetAssociation
        ),
        grids.TextColumn(
            "Type", key="extension", model_class=model.HistoryDatasetAssociation
        ),
        grids.TextColumn(
            "history_id",
            key="history_id",
    ...
    def apply_query_filter(self, trans, query, **kwargs):
        ...
        return (
            ...
            .filter(
                model.HistoryDatasetAssociation.extension.in_(self.available_tracks)
            )
...
class LibraryDatasetsSelectionGrid(grids.Grid):
    ...
    columns = [
        ...
        grids.TextColumn(
            "Name", key="name", model_class=model.LibraryDatasetDatasetAssociation
        ),
        grids.TextColumn(
            "Type", key="extension", model_class=model.LibraryDatasetDatasetAssociation
        ),
    ]
    columns.append(
        grids.MulticolFilterColumn(
    ...
    def apply_query_filter(self, trans, query, **kwargs):
        ...
        return (
            ...
            .filter(
                model.LibraryDatasetDatasetAssociation.extension.in_(
...
class TracksterSelectionGrid(grids.Grid):
    ...
    columns = [
        grids.TextColumn(
            "Title", key="title", model_class=model.Visualization, filterable="standard"
        ),
...
class VisualizationListGrid(grids.Grid):
    ...
    def get_display_name(self, trans, item):
        if (
            trans.app.visualizations_registry
            and item.type in trans.app.visualizations_registry.plugins
        ):
    ...
    columns = [
        ...
        grids.SharingStatusColumn(
            "Sharing", key="sharing", filterable="advanced", sortable=False
        ),
    ...
    operations = [
        ...
        grids.GridOperation(
            ...
            url_args=dict(controller="", action="visualizations/edit"),
        ),
        grids.GridOperation(
            "Copy", allow_multiple=False, condition=(lambda item: not item.deleted)
        ),
        grids.GridOperation(
            "Share or Publish",
            allow_multiple=False,
...
class VisualizationController(
    ...
    def rate_async(self, trans, id, rating):
        ...
        if not visualization:
            return trans.show_error_message(
                "The specified visualization does not exist."
            )
    ...
    def imp(self, trans, id):
        ...
        if referer:
            ...
            referer_message = (
                "<a href='%s'>go to Galaxy's start page</a>" % web.url_for("/")
            )
        ...
        if visualization.importable is False:
            return trans.show_error_message(
                "The owner of this visualization has disabled imports via this link.<br>You can %s"
                % referer_message,
                use_panels=True,
            )
        elif visualization.deleted:
            return trans.show_error_message(
                "You can't import this visualization because it has been deleted.<br>You can %s"
                % referer_message,
                use_panels=True,
            )
        else:
            ...
            imported_visualization = visualization.copy(
                user=trans.user, title="imported: " + visualization.title
            )
            ...
            return trans.show_ok_message(
                ...
                % (
                    visualization.title,
                    web.url_for("/visualizations/list"),
                    referer_message,
                ),
                use_panels=True,
            )
    ...
    def share(self, trans, id=None, email="", use_panels=False):
        ...
        if email:
            ...
            if not other:
                ...
                trans.sa_session.query(model.VisualizationUserShareAssociation)
                .filter_by(user=other, visualization=visualization)
                .count()
                > 0
            ):
                mtype = "error"
                ...
                trans.set_message(
                    "Visualization '%s' shared with user '%s'"
                    % (viz_title, other_email)
                )
                return trans.response.send_redirect(
                    web.url_for("/visualizations/sharing?id=%s" % id)
                )
    ...
    def display_by_username_and_slug(self, trans, username, slug):
        ...
        visualization = (
            trans.sa_session.query(model.Visualization)
        ...
        self.security_check(
            trans, visualization, check_ownership=False, check_accessible=True
        )
        ...
        if trans.get_user():
            user_item_rating = self.get_user_item_rating(
                trans.sa_session, trans.get_user(), visualization
            )
            if user_item_rating:
        ...
        ave_item_rating, num_ratings = self.get_ave_item_rating_data(
            trans.sa_session, visualization
        )

        # Fork to template based on visualization.type (registry or builtin).
        if (
            trans.app.visualizations_registry
            and visualization.type in trans.app.visualizations_registry.plugins
        ) and (
            visualization.type
            not in trans.app.visualizations_registry.BUILT_IN_VISUALIZATIONS
        ):
    ...
    def save(
        ...
        return self.save_visualization(
            trans, vis_config, vis_type, vis_id, vis_title, vis_dbkey, vis_annotation
        )
    ...
    def edit(self, trans, payload=None, **kwd):
        ...
        if not id:
            return self.message_exception(
                trans, "No visualization id received for editing."
            )
        ...
        if trans.request.method == "GET":
            ...
            if not v_title:
                return self.message_exception(
                    trans, "Please provide a visualization name is required."
                )
            elif not v_slug:
                return self.message_exception(
                    trans, "Please provide a unique identifier."
                )
            elif not self._is_valid_slug(v_slug):
                return self.message_exception(
                    trans,
                    "Visualization identifier can only contain lowercase letters, numbers, and dashes (-).",
                )
            elif (
                v_slug != v.slug
                ...
                if v_annotation:
                    ...
                    self.add_item_annotation(
                        trans.sa_session, trans.get_user(), v, v_annotation
                    )
                trans.sa_session.add(v)
                trans.sa_session.flush()
            return {
                "message": "Attributes of '%s' successfully saved." % v.title,
                "status": "success",
            }
    ...
    def _get_plugin_from_registry(self, trans, visualization_name):
        ...
        if not trans.app.visualizations_registry:
            raise HTTPNotFound(
                "No visualization registry (possibly disabled in galaxy.ini)"
            )
        ...
        **kwargs
        ...
        if id is None:
            return HTTPBadRequest(
                "A valid visualization id is required to load a visualization"
            )
        visualization = self.get_visualization(
            trans, id, check_ownership=False, check_accessible=True
        )
    ...
    def _POST_to_saved(
        ...
        **kwargs
        ...
        if type is None or config is None:
            return HTTPBadRequest(
                "A visualization type and config are required to save a visualization"
            )
        ...
        if id is not None:
            self.get_visualization(
                trans, id, check_ownership=True, check_accessible=False
            )
        ...
        render_url = web.url_for(
            controller="visualization", action="saved", id=returned.get("vis_id")
        )
    ...
    def trackster(self, trans, **kwargs):
        ...
        if not id:
            ...
            if new_dataset_id:
                ...
                hda = self.hda_manager.get_owned(
                    decoded_id, trans.user, current_history=trans.user
                )
                dbkey = hda.dbkey
                if dbkey == "?":
        ...
        return trans.fill_template(
            "galaxy.panels.mako",
            config={"right_panel": True, "app": app, "bundle": "extended"},
        )
    ...
    def circster(self, trans, id=None, hda_ldda=None, dataset_id=None, dbkey=None):
        ...
        if id:
            ...
            vis = self.create_visualization(
                trans, type="genome", dbkey=dbkey, save=False
            )
        ...
        for track in tracks:
            ...
            dataset = self.get_hda_or_ldda(
                trans, dataset_dict["hda_ldda"], dataset_dict["id"]
            )
        ...
        # fill template
        return trans.fill_template(
            "galaxy.panels.mako", config={"app": app, "bundle": "extended"}
        )
    ...
    def sweepster(self, trans, id=None, hda_ldda=None, dataset_id=None, regions=None):
        ...
        if id:
            ...
            dataset = self.hda_manager.get_owned(
        ...
        return trans.fill_template_mako(
            "visualization/sweepster.mako", config=viz_config
        )
    ...
    def phyloviz(self, trans, id=None, dataset_id=None, tree_index=0, **kwargs):
        ...
        return trans.fill_template_mako(
            "visualization/phyloviz.mako", data=data, config=config
        )
        ...
        if not hasattr(self, "gie_image_map"):
            ...
            for gie_dir in self.app.config.gie_dirs:
                ...
                for gie in gie_list:
                    ...
                    if os.path.exists(
                        self._gie_config_dir(gie_path, "allowed_images.yml")
                    ):
                        image_file = self._gie_config_dir(
                            gie_path, "allowed_images.yml"
                        )
                    elif os.path.exists(
                        self._gie_config_dir(gie_path, "allowed_images.yml.sample")
                    ):
                        image_file = self._gie_config_dir(
                            gie_path, "allowed_images.yml.sample"
                        )
                    else:
                        continue
