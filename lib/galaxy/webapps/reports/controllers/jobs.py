...
class SpecifiedDateListGrid(grids.Grid):
    ...
    class EmailColumn(grids.GridColumn):
        def filter(self, trans, user, query, column_filter):
            ...
            return query.filter(
                and_(
                    model.Job.table.c.user_id == model.User.table.c.id,
    ...
    columns = [
        JobIdColumn(
            ...
            key="id",
            link=(
                lambda item: dict(operation="job_info", id=item.id, webapp="reports")
            ),
            attach_popup=False,
            filterable="advanced",
        ),
...
class Jobs(BaseUIController, ReportQueryBuilder):
        ...
        if "operation" in kwd:
            ...
            if operation == "job_info":
                return trans.response.send_redirect(
                    web.url_for(controller="jobs", action="job_info", **kwd)
                )
                ...
                return trans.response.send_redirect(
                    web.url_for(controller="jobs", action="tool_per_month", **kwd)
                )
                ...
                return trans.response.send_redirect(
                    web.url_for(controller="jobs", action="user_per_month", **kwd)
                )
    ...
    def specified_month_all(self, trans, **kwd):
        ...
        specified_date = kwd.get(
            "specified_date", datetime.utcnow().strftime("%Y-%m-%d")
        )
    ...
    def per_month_all(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/webapps/reports/jobs_per_month_all.mako",
            order=order,
            arrow=arrow,
            sort_id=sort_id,
            trends=trends,
    ...
    def per_user(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/webapps/reports/jobs_per_user.mako",
            order=order,
            arrow=arrow,
            sort_id=sort_id,
    ...
    def user_per_month(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/webapps/reports/jobs_user_per_month.mako",
            order=order,
            arrow=arrow,
            sort_id=sort_id,
            id=kwd.get("id"),
    ...
    def tool_per_month(self, trans, **kwd):
        ...
        q = sa.select(
            ...
            whereclause=sa.and_(
                model.Job.table.c.tool_id == tool_id,
                model.Job.table.c.user_id != monitor_user_id,
            ),
            from_obj=[model.Job.table],
            group_by=self.group_by_month(model.Job.table.c.create_time),
    ...
    def job_info(self, trans, **kwd):
        message = ""
        job = trans.sa_session.query(model.Job).get(
            trans.security.decode_id(kwd.get("id", ""))
        )
        return trans.fill_template(
            "/webapps/reports/job_info.mako", job=job, message=message
        )
...
def get_monitor_id(trans, monitor_email):
    ...
    monitor_row = (
        trans.sa_session.query(trans.model.User.table.c.id)
