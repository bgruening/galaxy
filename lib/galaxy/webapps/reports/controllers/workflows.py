...
from galaxy.webapps.reports.controllers.jobs import get_spark_time, sorter
...
class SpecifiedDateListGrid(grids.Grid):
    ...
    columns = [
        WorkflowNameColumn(
            "Name", key="name", attach_popup=False, filterable="advanced"
        ),
...
class Workflows(BaseUIController, ReportQueryBuilder):
        ...
        if "operation" in kwd:
            ...
            if operation == "workflow_per_month":
                ...
                return trans.response.send_redirect(
                    web.url_for(
                        controller="workflows", action="workflow_per_month", **kwd
                    )
                )
                ...
                if workflow.user:
                    ...
                    kwd[
                        "email"
                    ] = None  # For anonymous users ( shouldn't happen with workflows )
                return trans.response.send_redirect(
                    web.url_for(controller="workflows", action="user_per_month", **kwd)
                )
    ...
    def per_month_all(self, trans, **kwd):
        ...
        q = sa.select(
            (
                self.select_month(model.StoredWorkflow.table.c.create_time).label(
                    "date"
                ),
        ...
        for row in q.execute():
            ...
            workflows.append(
                (row.date.strftime("%Y-%m"), row.total_workflows, month_name, year)
            )
    ...
    def per_workflow(self, trans, **kwd):
        ...
        q = sa.select(
            ...
            whereclause=sa.and_(
                model.WorkflowInvocation.table.c.workflow_id
        ...
        all_runs_per_workflow = sa.select(
            (
                ...
                self.select_day(model.WorkflowInvocation.table.c.create_time).label(
                    "date"
                ),
            ),
            from_obj=[model.Workflow.table, model.WorkflowInvocation.table],
...
def get_workflow(trans, id):
    return trans.sa_session.query(trans.model.Workflow).get(
