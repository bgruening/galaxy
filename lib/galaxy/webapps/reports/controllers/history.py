...
class History(BaseUIController):
    ...
    def history_and_dataset_per_user(self, trans, **kwd):
        ...
        histories = dict(
            [
                (_.email if _.email is not None else "Unknown", int(_.history))
        ...
        datasets = dict(
            [
                (
                    _.email if _.email is not None else "Unknown",
    ...
    def history_and_dataset_type(self, trans, **kwd):
        ...
        # execute requests, replace None fields by "Unknown"
        data = [
            (_.name if _.name is not None else "NoNamedHistory", _.state)
            for _ in histories.execute()
        ]
        ...
        number_of_possible_status = (
            len(possible_status) + 1
        )  # + 1 to handle unknown status!
