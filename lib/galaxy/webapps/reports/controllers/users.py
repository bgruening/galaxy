...
class Users(BaseUIController, ReportQueryBuilder):
    ...
    def registered_users(self, trans, **kwd):
        ...
        return trans.fill_template(
            "/webapps/reports/registered_users.mako",
            num_users=num_users,
            message=message,
        )
    ...
    def last_access_date(self, trans, **kwd):
        ...
        users = sorted(
            users, key=operator.itemgetter(name_to_num(sort_id)), reverse=_order
        )
        return trans.fill_template(
    ...
    def user_disk_usage(self, trans, **kwd):
        ...
        users = sorted(
            trans.sa_session.query(galaxy.model.User).all(),
    ...
    def history_per_user(self, trans, **kwd):
        ...
        req = sa.select(
            ...
            from_obj=[
                sa.outerjoin(galaxy.model.History.table, galaxy.model.User.table)
            ],
        ...
        histories = [
            (_.username if _.username is not None else "Unknown", _.history)
            for _ in req.execute()
        ]
