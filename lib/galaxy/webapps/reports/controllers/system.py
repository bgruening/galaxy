        ...
        if params.deleted_histories_days:
            ...
            message = (
                ...
                % (
                    history_count,
                    dataset_count,
                    deleted_histories_days,
                    nice_size(disk_space, True),
                )
            )
        else:
            message = "Enter the number of days."
    ...
    def deleted_datasets(self, trans, **kwd):
        ...
        if params.deleted_datasets_days:
            ...
            message = (
                ...
                " disk space: %s."
                % (dataset_count, deleted_datasets_days, nice_size(disk_space, True))
            )
        else:
            message = "Enter the number of days."
        ...
        associated_lddas = (
            ...
            .filter(
                and_(
                    ...
                    trans.model.LibraryDatasetDatasetAssociation.dataset_id
                    == dataset.id,
                )
            )
            .all()
            ...
            if df_line:
                ...
                if "filesystem" in df_line or "proc" in df_line:
                    ...
                    if ":" in df_line and "/" in df_line:
                        ...
                        try:
                            disk_size, disk_used, disk_avail, disk_cap_pct, file_system = (
                                df_line.split()
                            )
                            break
                        except Exception:
                    ...
                    try:
                        file_system, disk_size, disk_used, disk_avail, disk_cap_pct, mount = (
                            df_line.split()
                        )
                        break
                    except Exception:
