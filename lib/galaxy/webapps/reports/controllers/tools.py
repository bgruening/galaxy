...
def int_to_octet(size):
    ...
    try:
        ...
        return "%.0f %s" % (size * ((no_unit - len(units) + 1) * 1000.0), units[-1])
...
class Tools(BaseUIController):
    ...
    def tools_and_job_state(self, trans, **kwd):
        ...
        tools_and_jobs_ok = sa.select(
            (
                galaxy.model.Job.table.c.tool_id.label("tool"),
        ...
        for tool in tools:
            data[tool] = (
                str(tools_and_jobs_ok.get(tool, "-")),
                str(tools_and_jobs_error.get(tool, "-")),
            )
    ...
    def tool_execution_time(self, trans, **kwd):
        ...
        jobs_times = [
            (name, (create, update, time))
            for name, create, update, time in jobs_times.execute()
        ]
        ...
        for tool in data:
            ...
            data[tool]["avg"] = sum(data[tool]["avg"], timedelta()) / len(
                data[tool]["avg"]
            )
    ...
    def tool_execution_time_per_month(self, trans, **kwd):
        ...
        jobs_times = sa.select(
            (
                sa.func.date_trunc("month", galaxy.model.Job.table.c.create_time).label(
                    "date"
                ),
                sa.func.max(
                    galaxy.model.Job.table.c.update_time
    ...
    def tool_error_messages(self, trans, **kwd):
        ...
        tool_errors = [
            [unicodify(a), b]
            for a, b in sa.select(
                (galaxy.model.Job.table.c.stderr, galaxy.model.Job.table.c.create_time),
                from_obj=[galaxy.model.Job.table],
                whereclause=and_(
                ...
                for word in to_replace:
                    ...
                    if sentence in new_key:
                        new_key = new_key.replace(
                            sentence,
                            "</br>" + word + " [this line in %d times]" % (count),
                        )
