...
class OpenIDProvider(object):
    ...
    def from_elem(cls, xml_root):
        ...
        assert provider_id and provider_name and op_endpoint_url, Exception(
        ...
        for elem in provider_elem.findall("sreg"):
            ...
            for field_elem in elem.findall("field"):
                ...
                for store_user_preference_elem in field_elem.findall(
                    ...
                    store_user_preference[
                        store_user_preference_elem.get("name")
                    ] = sreg_name
        if use_default_sreg:
            sreg_required = None
            sreg_optional = None
    ...
    def post_authentication(self, trans, openid_manager, info):
        ...
        ) in self.store_user_preference.items():
        ...
        for elem in oid_elem.findall("provider"):
            try:
                provider = OpenIDProvider.from_file(
                    os.path.join("lib/galaxy/openid", elem.get("file"))
                )
                providers[provider.id] = provider
                log.debug(
                    "Loaded OpenID provider: %s (%s)" % (provider.name, provider.id)
                )
            except Exception as e:
