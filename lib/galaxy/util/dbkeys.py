...
class GenomeBuilds(object):
    ...
    def get_genome_build_names(self, trans=None):
        ...
        if trans is not None:
            if trans.history:
                ...
                datasets = trans.sa_session.query(
                    self._app.model.HistoryDatasetAssociation
    ...
    def get_chrom_info(
        self, dbkey, trans=None, custom_build_hack_get_len_from_fasta_conversion=True
    ):
        ...
        if trans:
            ...
            if db_dataset:
                ...
                if (
                    ...
                    if (
                        "fasta" in custom_build_dict
                        and custom_build_hack_get_len_from_fasta_conversion
                    ):
        ...
        if not chrom_info:
            dbkey_table = self._app.tool_data_tables.get(self._data_table_name, None)
            if dbkey_table is not None:
                chrom_info = dbkey_table.get_entry(
                    "value", dbkey, "len_path", default=None
                )
        ...
        if not chrom_info:
            ...
            chrom_info = os.path.join(
                self._static_chrom_info_path, "%s.len" % sanitize_lists_to_string(dbkey)
            )
