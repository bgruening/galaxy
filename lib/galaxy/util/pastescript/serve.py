...
class Command(object):
    ...
    def run(self, args):
        ...
        if os.environ.get("PASTE_CONFIG_FILE") and self.takes_config_file is not None:
            ...
            if take == 1:
                ...
                assert 0, (
                    "Value takes_config_file must be None, 1, or -1 (not %r)" % take
                )
        ...
        if self.min_args is not None and len(self.args) < self.min_args:
            raise BadCommand(
                self.min_args_error
                % {"min_args": self.min_args, "actual_args": len(self.args)}
            )
        if self.max_args is not None and len(self.args) > self.max_args:
            raise BadCommand(
                self.max_args_error
                % {"max_args": self.max_args, "actual_args": len(self.args)}
            )
    ...
    def standard_parser(
        ...
        if no_interactive:
            parser.add_option(
                "--no-interactive", action="count", dest="no_interactive", default=0
            )
        if interactive:
            parser.add_option(
                "-i", "--interactive", action="count", dest="interactive", default=0
            )
        if simulate:
            parser.add_option(
                "-n", "--simulate", action="store_true", dest="simulate", default=False
            )
        if overwrite:
            parser.add_option(
                "-f",
...
class NotFoundCommand(Command):
    def run(self, args):
        print(
            "Command %r not known (you may need to run setup.py egg_info)"
            % self.command_name
        )
        commands = list()
        commands.sort()
...
class ServeCommand(Command):
    ...
    def command(self):
        ...
        if getattr(self.options, "daemon", False) and getattr(
        ...
        if self.options.monitor_restart and not os.environ.get(
...
def _remove_pid_file(written_pid, filename, verbosity):
    ...
    try:
        ...
        if pid_in_file != current_pid:
            print(
                "PID file %s contains %s, not expected PID %s"
...
def ensure_port_cleanup(bound_addresses, maxtries=30, sleeptime=2):
    ...
    """
    atexit.register(
        _cleanup_ports, bound_addresses, maxtries=maxtries, sleeptime=sleeptime
    )
...
parser.add_option(
    "-h", "--help", action="store_true", dest="do_help", help="Show this help message"
)
