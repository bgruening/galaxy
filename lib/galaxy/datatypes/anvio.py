...
class AnvioComposite(Html):
    ...
    def generate_primary_file(self, dataset=None):
        ...
        rval = [
            "<html><head><title>Files for Anvi'o Composite Dataset (%s)</title></head>"
            % (self.file_ext)
        ]
        if defined_files:
            rval.append(
                "<p/>This composite dataset is composed of the following defined files:<p/><ul>"
            )
            for composite_name, composite_file in defined_files:
                ...
                if not os.path.exists(
                    os.path.join(dataset.extra_files_path, composite_name)
                ):
                    missing_text = " (missing)"
                rval.append(
                    '<li><a href="%s">%s</a>%s%s</li>'
                    % (composite_name, composite_name, opt_text, missing_text)
                )
            rval.append("</ul>")
        defined_files = map(lambda x: x[0], defined_files)
        extra_files = []
        for (dirpath, dirnames, filenames) in os.walk(
            dataset.extra_files_path, followlinks=True
        ):
            for filename in filenames:
                rel_path = os.path.relpath(
                    os.path.join(dirpath, filename), dataset.extra_files_path
                )
                if rel_path not in defined_files:
                    extra_files.append(rel_path)
        if extra_files:
            rval.append(
                "<p/>This composite dataset contains these undefined files:<p/><ul>"
            )
        ...
        if not (defined_files or extra_files):
            rval.append(
                "<p/>This composite dataset does not contain any files!<p/><ul>"
            )
        rval.append("</html>")
        return "\n".join(rval)
...
class AnvioDB(AnvioComposite):
    ...
    MetadataElement(
        name="anvio_basename", default=_anvio_basename, desc="Basename", readonly=True
    )
    file_ext = "anvio_db"
    composite_type = "auto_primary_file"
    ...
    def __init__(self, *args, **kwd):
        ...
        if self._anvio_basename is not None:
            self.add_composite_file(
                self._anvio_basename, is_binary=True, optional=False
            )
    ...
    def set_meta(self, dataset, **kwd):
        ...
        for basename in [dataset.metadata.anvio_basename, self._anvio_basename]:
            if found:
                break
            if basename is not None and not os.path.exists(
                os.path.join(dataset.extra_files_path, basename)
            ):
                for name in glob.glob(
                    os.path.join(dataset.extra_files_path, "*%s" % (basename))
                ):
