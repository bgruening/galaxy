...
def __main__():
        ...
        with open(input_name) as fh, open(output_name, "w") as out:
            for count, line in enumerate(fh):
                ...
                try:
                    assert (
                        len(fields) >= 3
                    ), "A BED file requires at least 3 columns"  # we can't fix this
                    if len(fields) > 12:
                        strict_bed = False
                        break
                                ...
                                if len(fields) > 7:
                                    ...
                                    if len(fields) > 8:
                                        if (
                                            ...
                                            assert (
                                                len(fields2) == 3
                                            ), "RGB value must be 0 or have length of 3"
                ...
                if force_num_columns is not None and len(fields) != force_num_columns:
                    line = "\t".join(
                        force_bed_field_count(fields, count, force_num_columns)
                    )
                out.write("%s\n" % line)
    else:
        strict_bed = False
        ...
        with open(input_name, "r") as fh, open(output_name, "w") as out:
            for count, region in enumerate(
                ...
                try:
                    ...
                    if (
                        force_num_columns is not None
                        and len(fields) != force_num_columns
                    ):
    ...
    if skipped_lines > 0:
        print(
            "Skipped %d blank or invalid lines starting with line # %d."
