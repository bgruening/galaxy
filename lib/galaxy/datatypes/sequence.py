...
class Sequence(data.Text):
    ...
    def do_slow_split(cls, input_datasets, subdir_generator_function, split_params):
        ...
        if (
            input_datasets[0].metadata is not None
            and input_datasets[0].metadata.sequences is not None
        ):
        ...
        return cls.write_split_files(
            input_datasets, None, subdir_generator_function, sequences_per_file
        )

    do_slow_split = classmethod(do_slow_split)

    def do_fast_split(
        cls, input_datasets, toc_file_datasets, subdir_generator_function, split_params
    ):
    ...
    def write_split_files(
        ...
        for part_no in range(len(sequences_per_file)):
            ...
            for ds_no in range(len(input_datasets)):
                ...
                with open(
                    os.path.join(dir, "split_info_%s.json" % base_name), "w"
                ) as f:
    ...
    def get_split_commands_with_toc(
        ...
        if i == len(sections):  # bad input data!
            raise Exception(
                "No FQTOC section contains starting sequence %s" % start_sequence
            )
        ...
        if start_chunk > -1:
            result.append(
                copy_chunk_cmd
                % (start_chunk, end_chunk - start_chunk, input_name, output_name)
            )
    ...
    def get_split_commands_sequential(
        is_compressed, input_name, output_name, start_sequence, sequence_count
    ):
        """
...
class Fasta(Sequence):
    ...
    def split(cls, input_datasets, subdir_generator_function, split_params):
        ...
        if "split_mode" not in split_params:
            ...
            log.debug(
                "Split %s into batches of %i records..." % (input_file, batch_size)
            )
            cls._count_split(input_file, batch_size, subdir_generator_function)
        else:
            raise Exception("Unsupported split mode %s" % split_params["split_mode"])
    ...
    def _size_split(cls, input_file, chunk_size, subdir_generator_function):
        ...
        """
        log.debug(
            "Attemping to split FASTA file %s into chunks of %i bytes"
    ...
    def _count_split(cls, input_file, chunk_size, subdir_generator_function):
        ...
        log.debug(
            "Attemping to split FASTA file %s into chunks of %i sequences"
            % (input_file, chunk_size)
        )
        f = open(input_file, "rU")
        part_file = None
...
class BaseFastq(Sequence):
    ...
    def display_data(
        self, trans, dataset, preview=False, filename=None, to_ext=None, **kwd
    ):
        if preview:
            with compression_utils.get_fileobj(dataset.file_name) as fh:
            ...
            return Sequence.display_data(
                self, trans, dataset, preview, filename, to_ext, **kwd
            )
    ...
    def split(cls, input_datasets, subdir_generator_function, split_params):
        ...
        if len(toc_file_datasets) == len(input_datasets):
            return cls.do_fast_split(
                input_datasets,
                toc_file_datasets,
                subdir_generator_function,
                split_params,
            )
        return cls.do_slow_split(
            input_datasets, subdir_generator_function, split_params
        )
            ...
            for i, block in enumerate(bx.align.maf.Reader(maf_file)):
                ...
                if ref_comp:
                    ...
                    if chrom is None:
                        chrom = ref_chrom
                    if chrom == ref_chrom:
                        forward_strand_start = min(
                            forward_strand_start, ref_comp.forward_strand_start
                        )
                        forward_strand_end = max(
                            forward_strand_end, ref_comp.forward_strand_end
                        )
