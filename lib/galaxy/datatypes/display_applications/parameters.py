...
class DisplayApplicationDataParameter(DisplayApplicationParameter):
    ...
    def __init__(self, elem, link):
        ...
        self.viewable = string_as_bool(
            elem.get("viewable", "True")
        )  # data params should be viewable
    ...
    def _get_dataset_like_object(self, other_values):
        ...
        assert (
            data
        ), "Base dataset could not be found in values provided to DisplayApplicationDataParameter"
        if isinstance(data, DisplayDataValueWrapper):
            data = data.value
        if self.metadata:
            rval = getattr(data.metadata, self.metadata, None)
            assert rval, (
                "Unknown metadata name (%s) provided for dataset type (%s)."
        ...
        elif self.extensions and (
            self.force_conversion or not isinstance(data.datatype, self.formats)
        ):
            for ext in self.extensions:
                rval = data.get_converted_files_by_type(ext)
                if rval:
                    return rval
            assert data.find_conversion_destination(self.formats)[0] is not None, (
                "No conversion path found for data param: %s" % self.name
            )
            return None
        return data
    ...
    def get_value(self, other_values, dataset_hash, user_hash, trans):
        ...
        if data:
            return DisplayDataValueWrapper(
                data, self, other_values, dataset_hash, user_hash, trans
            )
        return None
    ...
    def prepare(self, other_values, dataset_hash, user_hash, trans):
        ...
        if not data and self.formats:
            ...
            if target_ext and not converted_dataset:
                ...
                converted_dataset
                and converted_dataset.state == converted_dataset.states.ERROR
            ):
                raise Exception(
                    "Dataset conversion failed for data parameter: %s" % self.name
                )
        return self.get_value(other_values, dataset_hash, user_hash, trans)

    def is_preparing(self, other_values):
        value = self._get_dataset_like_object(other_values)
        if value and value.state in (
            value.states.NEW,
            value.states.UPLOAD,
            value.states.QUEUED,
    ...
    def ready(self, other_values):
        ...
        if value:
            if value.state == value.states.OK:
                ...
                raise Exception(
                    "A data display parameter is in the error state: %s" % (self.name)
                )
        return False
...
class DisplayApplicationTemplateParameter(DisplayApplicationParameter):
    ...
    def get_value(self, other_values, dataset_hash, user_hash, trans):
        ...
        if self.strip:
            value = value.strip()
        return DisplayParameterValueWrapper(
            value, self, other_values, dataset_hash, user_hash, trans
        )
