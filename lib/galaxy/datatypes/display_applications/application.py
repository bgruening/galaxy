...
class DisplayApplicationLink(object):
    ...
    def get_inital_values(self, data, trans):
        ...
        rval[
            DEFAULT_DATASET_NAME
        ] = data  # always have the display dataset name available
        return rval
    ...
    def build_parameter_dict(self, data, dataset_hash, user_hash, trans, app_kwds):
        ...
        for name, param in self.parameters.items():
            assert name not in other_values, (
                "The display parameter '%s' has been defined more than once." % name
            )
    ...
    def filter_by_dataset(self, data, trans):
        ...
        for filter_elem in self.filters:
            if fill_template(filter_elem.text, context=context) != filter_elem.get(
                "value", "True"
            ):
                return False
        return True
        ...
        if filename is None:
            ...
            if data_table_name:
                ...
                assert data_table is not None, (
                    'Unable to find data table named "%s".' % data_table_name
                )

        assert (
            filename is not None or data_table is not None
        ), "Filename or data Table is required for dynamic_links."
            ...
            if len(fields) > max_col:
                ...
                links.append(
                    DisplayApplicationLink.from_elem(
                        new_elem, display_application, other_values=dynamic_values
                    )
                )
            else:
                log.warning(
...
class PopulatedDisplayApplicationLink(object):
    def __init__(
        self, display_application_link, data, dataset_hash, user_hash, trans, app_kwds
    ):
    ...
    def preparing_display(self):
        if not self.ready:
            return self.link.parameters[list(self.parameters.keys())[-1]].is_preparing(
                self.parameters
            )
        return False
    ...
    def prepare_display(self):
        ...
        if not self.ready and not self.preparing_display():
            ...
            for name, param in self.link.parameters.items():
                if (
                    found_last or list(other_values.keys())[-1] == name
                ):  # found last parameter to be populated
                    found_last = True
                    value = param.prepare(
                        other_values, self.dataset_hash, self.user_hash, self.trans
                    )
...
class DisplayApplication(object):
    ...
    def get_link(self, link_name, data, dataset_hash, user_hash, trans, app_kwds):
        ...
        return PopulatedDisplayApplicationLink(
            self.links[link_name], data, dataset_hash, user_hash, trans, app_kwds
        )
