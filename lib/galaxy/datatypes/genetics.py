...
VALID_GENOTYPES_LINE = re.compile(
    r"^([a-zA-Z0-9]+)(\s([0-9]{2}|[A-Z]{2}|NC|\?\?))+\s*$"
)
...
class GenomeGraphs(Tabular):
    ...
    def __init__(self, **kwd):
        ...
        self.add_display_app(
            "ucsc", "Genome Graph", "as_ucsc_display_file", "ucsc_links"
        )
    ...
    def sniff_prefix(self, file_prefix):
        ...
        rows = [
            l.split() for l in buf.splitlines()[1:4]
...
class Rgenetics(Html):
    ...
    def set_meta(self, dataset, **kwd):
        ...
        try:
            ...
            if verbose:
                gal_Log.debug(
                    "@@@rgenetics set_meta failed %s - dataset %s has no efp ?"
                    % (sys.exc_info()[0], dataset.name)
                )
            return False
        try:
            flist = os.listdir(efp)
        ...
        if len(flist) == 0:
            if verbose:
                gal_Log.debug(
                    "@@@rgenetics set_meta failed - %s efp %s is empty?"
                    % (dataset.name, efp)
                )
            return False
...
class Fped(Rgenetics):
    ...
    def __init__(self, **kwd):
        ...
        self.add_composite_file(
            "%s.fped",
            description="FBAT format pedfile",
            substitute_name_with_metadata="base_name",
            is_binary=False,
        )
...
class Pbed(Rgenetics):
    ...
    def __init__(self, **kwd):
        Rgenetics.__init__(self, **kwd)
        self.add_composite_file(
            "%s.bim", substitute_name_with_metadata="base_name", is_binary=False
        )
        self.add_composite_file(
            "%s.bed", substitute_name_with_metadata="base_name", is_binary=True
        )
        self.add_composite_file(
            "%s.fam", substitute_name_with_metadata="base_name", is_binary=False
        )
...
class Eigenstratgeno(Rgenetics):
    ...
    def __init__(self, **kwd):
        ...
        self.add_composite_file(
            "%s.ind", substitute_name_with_metadata="base_name", is_binary=False
        )
        self.add_composite_file(
            "%s.map", substitute_name_with_metadata="base_name", is_binary=False
        )
...
class Eigenstratpca(Rgenetics):
    ...
    def __init__(self, **kwd):
        ...
        self.add_composite_file(
            "%s.eigenstratpca",
            description="Eigenstrat PCA file",
            substitute_name_with_metadata="base_name",
        )
...
class IdeasPre(Html):
    ...
    MetadataElement(
        name="input_config", desc="IDEAS input config", default=None, readonly=True
    )
    MetadataElement(
        name="tmp_archive",
    ...
    def __init__(self, **kwd):
        ...
        self.add_composite_file(
            "IDEAS_input_config.txt", description="IDEAS input config", is_binary=False
        )
        self.add_composite_file(
            "tmp.tar.gz",
...
class RexpBase(Html):
    ...
    def get_phecols(self, phenolist=[], maxConc=20):
        ...
        for c, conc in enumerate(concordance):  # c is column number
            if (len(conc) > 1) and (
                len(conc) < min(nrows, maxConc)
            ):  # not all same and not all different!!
        ...
        phe = [
            [x[i] for i in useCols] for x in plist if len(x) >= totcols
        ]  # strip unused data
        for i in range(0, (nuse - 1)):  # for each interesting column
            for j in range(i + 1, nuse):
                ...
                for row in phe:  # row is a list of lists
                    ...
                    kdict[k] = k
                if len(kdict.keys()) == len(
                    concordance[useCols[j]]
        ...
        for i, conc in enumerate(
            useConc
        ):  # these are all unique columns for the design matrix
            ccounts = sorted(
                (conc.get(code, 0), code) for code in conc.keys()
            )  # decorate
    ...
    def set_peek(self, dataset, **kwd):
        ...
        if not dataset.dataset.purged:
            pp = os.path.join(
                dataset.extra_files_path, "%s.pheno" % dataset.metadata.base_name
            )
            try:
    ...
    def set_meta(self, dataset, **kwd):
        ...
        try:
            pf = open(
                pp, "r"
            ).readlines()  # read the basename.phenodata in the extra_files_path
        except Exception:
            pf = None
        if pf:
            h = pf[0].strip()
        ...
        if not dataset.blurb:
            dataset.blurb = (
                "R loadable BioC expression object for the Rexpression Galaxy toolkit"
            )
        return True
