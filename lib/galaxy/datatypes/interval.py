...
from galaxy.datatypes.sniff import build_sniff_from_prefix, get_headers, iter_headers
...
class Interval(Tabular):
    ...
    MetadataElement(
        name="chromCol", default=1, desc="Chrom column", param=metadata.ColumnParameter
    )
    MetadataElement(
        name="startCol", default=2, desc="Start column", param=metadata.ColumnParameter
    )
    MetadataElement(
        name="endCol", default=3, desc="End column", param=metadata.ColumnParameter
    )
    MetadataElement(
        name="strandCol",
    ...
    def __init__(self, **kwd):
        ...
        self.add_display_app(
            "ucsc", "display at UCSC", "as_ucsc_display_file", "ucsc_links"
        )
            ...
            for i, line in enumerate(open(dataset.file_name)):
                ...
                if line:
                    if first_line_is_header or line[0] == "#":
                        ...
                        elems = line.split("\t")
                        if len(elems) > 2:
                            if overwrite or not dataset.metadata.element_is_set(
                                "chromCol"
                            ):
    ...
    def get_estimated_display_viewport(
        self, dataset, chrom_col=None, start_col=None, end_col=None
    ):
        """Return a chrom, start, stop tuple for viewing a file."""
        viewport_feature_count = (
            100
        )  # viewport should check at least 100 features; excludes comment lines
        max_line_count = max(
            viewport_feature_count, 500
        )  # maximum number of lines to check; includes comment lines
        ...
        try:
            ...
            while True:
                ...
                while line.rstrip("\n\r") == line:
                    assert readline_count > 0, Exception(
                        "Viewport readline count exceeded for dataset %s." % dataset.id
                    )
                    line = fh.readline(VIEWPORT_READLINE_BUFFER_SIZE)
                    if not line:
                        break  # EOF
            ...
            log.exception(
                "Exception caught attempting to generate viewport for dataset '%d'",
                dataset.id,
            )
        return (None, None, None)
    ...
    def ucsc_links(self, dataset, type, app, base_url):
        ...
        valid_sites = [
            (name, url)
            for name, url in app.datatypes_registry.get_legacy_sites_by_build(
                "ucsc", dataset.dbkey
            )
    ...
    def validate(self, dataset):
        ...
        with open(dataset.file_name, "r") as infile:
            reader = GenomicIntervalReader(
                infile, chrom_col=c, start_col=s, end_col=e, strand_col=t
            )
    ...
    @dataproviders.decorators.dataprovider_factory(
        "interval", dataproviders.dataset.IntervalDataProvider.settings
    )
    def interval_dataprovider(self, dataset, **settings):
        return dataproviders.dataset.IntervalDataProvider(dataset, **settings)

    @dataproviders.decorators.dataprovider_factory(
        "interval-dict", dataproviders.dataset.IntervalDataProvider.settings
    )
...
class BedGraph(Interval):
    ...
    def get_estimated_display_viewport(
        self, dataset, chrom_col=0, start_col=1, end_col=2
    ):
        """
...
class Bed(Interval):
    ...
    def as_ucsc_display_file(self, dataset, **kwd):
        ...
        for line in open(dataset.file_name):
            ...
            try:
                if len(fields) > 12:
                    return Interval.as_ucsc_display_file(
                        self, dataset
                    )  # too many fields
                if len(fields) > 6:
                    int(fields[6])
            ...
            for hdr in headers:
                ...
                if len(hdr) > 11:
                    ...
                    if (
                        ...
                        or len(block_starts) != block_count
                    ):
                        return False
            return True
        except Exception:
...
class BedStrict(Bed):
    ...
    def set_meta(self, dataset, overwrite=True, **kwd):
        Tabular.set_meta(
            self, dataset, overwrite=overwrite, **kwd
        )  # need column count first
...
class _RemoteCallMixin(object):
    def _get_remote_call_url(
        self, redirect_url, site_name, dataset, type, app, base_url
    ):
...
class Gff(Tabular, _RemoteCallMixin):
    ...
    def __init__(self, **kwd):
        ...
        self.add_display_app(
            "gbrowse", "display in Gbrowse", "as_gbrowse_display_file", "gbrowse_links"
        )
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        if line.startswith(
                            "##sequence-region"
                        ):  # ##sequence-region IV 6000000 6030000
                            elems = line.rstrip("\n\r").split()
                            if len(elems) > 3:
                                # line looks like:
                                ...
                                log.exception(
                                    "line (%s) uses an unsupported ##sequence-region definition."
                                    % str(line)
                                )
                            ...
                            break  # use location declared in file
                        elif True not in map(
                            line.startswith, ("#", "track", "browser")
                        ):  # line.startswith() does not accept iterator in python2.4
                            viewport_feature_count -= 1
                            elems = line.rstrip("\n\r").split("\t")
                            if len(elems) > 3:
                                if not seqid:
                    ...
                    while line.rstrip("\n\r") == line:
                        assert readline_count > 0, Exception(
                            "Viewport readline count exceeded for dataset %s."
                            % dataset.id
                        )
                        line = fh.readline(VIEWPORT_READLINE_BUFFER_SIZE)
                        if not line:
        ...
        if seqid is not None:
            for site_name, site_url in app.datatypes_registry.get_legacy_sites_by_build(
                ...
                if site_name in app.datatypes_registry.get_display_sites("ucsc"):
                    ...
                    link = self._get_remote_call_url(
                        redirect_url, site_name, dataset, type, app, base_url
                    )
                    ret_val.append((site_name, link))
        return ret_val
    ...
    def genomic_region_dataprovider(self, dataset, **settings):
        return dataproviders.dataset.GenomicRegionDataProvider(
            dataset, 0, 3, 4, **settings
        )
    ...
    def interval_dataprovider(self, dataset, **settings):
        return dataproviders.dataset.IntervalDataProvider(
            dataset, 0, 3, 4, 6, 2, **settings
        )
        ...
        try:
            ...
            for hdr in headers:
                if hdr and hdr[0].startswith("##gff-version") and hdr[0].find("3") >= 0:
                    ...
                    hdr and hdr[0].startswith("##gff-version") and hdr[0].find("3") < 0
                ):
                    return False
...
class Wiggle(Tabular, _RemoteCallMixin):
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        if line and (
                            line.lower().startswith("variablestep")
                            or line.lower().startswith("fixedstep")
                        ):
                            if chrom is not None:
            ...
            for site_name, site_url in app.datatypes_registry.get_legacy_sites_by_build(
                ...
                if site_name in app.datatypes_registry.get_display_sites("gbrowse"):
                    if chrom.startswith("chr") and len(chrom) > 3:
                        chrom = chrom[3:]
                    redirect_url = quote_plus(
                        "%s/?q=%s:%s..%s&eurl=%%s" % (site_url, chrom, start, stop)
                    )
                    link = self._get_remote_call_url(
                        redirect_url, site_name, dataset, type, app, base_url
                    )
    ...
    # ------------- Dataproviders
    @dataproviders.decorators.dataprovider_factory(
        "wiggle", dataproviders.dataset.WiggleDataProvider.settings
    )
    ...
    @dataproviders.decorators.dataprovider_factory(
        "wiggle-dict", dataproviders.dataset.WiggleDataProvider.settings
    )
...
class ChromatinInteractions(Interval):
    ...
    MetadataElement(
        name="end1Col", default=3, desc="End1 column", param=metadata.ColumnParameter
    )
    MetadataElement(
        name="chrom2Col",
        default=4,
    ...
    MetadataElement(
        name="end2Col", default=6, desc="End2 column", param=metadata.ColumnParameter
    )
    MetadataElement(
        name="valueCol", default=7, desc="Value column", param=metadata.ColumnParameter
    )
