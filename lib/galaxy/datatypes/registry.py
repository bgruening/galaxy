...
class Registry(object):
    ...
    def load_datatypes(
        self, root_dir=None, config=None, deactivate=False, override=True
    ):
        """
        ...
        def __import_module(full_path, datatype_module, datatype_class_name):
            open_file_obj, file_name, description = imp.find_module(
                datatype_module, [full_path]
            )
            imported_module = imp.load_module(
                datatype_class_name, open_file_obj, file_name, description
            )
        ...
        if root_dir and config:
            ...
            if not self.converters_path:
                self.converters_path_attr = registration.get(
                ...
                if not os.path.isdir(self.converters_path):
                    raise ConfigurationError(
                        "Directory does not exist: %s" % self.converters_path
                    )
            if not self.display_applications_path:
                self.display_path_attr = registration.get(
                    "display_path", "display_applications"
                )
                self.display_applications_path = os.path.join(
                    root_dir, self.display_path_attr
                )
            ...
            proprietary_converter_path = registration.get(
                "proprietary_converter_path", None
            )
            proprietary_display_path = registration.get(
                "proprietary_display_path", None
            )
            if (
            ...
            for elem in registration.findall("datatype"):
                ...
                auto_compressed_types = galaxy.util.listify(
                    elem.get("auto_compressed_types", "")
                )
                sniff_compressed_types = galaxy.util.string_as_bool_or_none(
                    elem.get("sniff_compressed_types", "None")
                )
                if sniff_compressed_types is None:
                    sniff_compressed_types = getattr(
                        self.config, "sniff_compressed_dynamic_datatypes_default", True
                    )
                ...
                display_in_upload = galaxy.util.string_as_bool(
                ...
                if edam_format and not make_subclass:
                    self.log.warning(
                        "Cannot specify edam_format without setting subclass to True, skipping datatype."
                    )
                    continue
                edam_data = elem.get("edam_data", None)
                if edam_data and not make_subclass:
                    self.log.warning(
                        "Cannot specify edam_data without setting subclass to True, skipping datatype."
                    )
                    continue
                ...
                proprietary_datatype_module = elem.get(
                    "proprietary_datatype_module", None
                )
                if (
                ...
                if deactivate:
                    ...
                    for in_memory_elem in self.datatype_elems:
                        ...
                        if in_memory_extension == extension:
                            ...
                            in_memory_make_subclass = galaxy.util.string_as_bool(
                                elem.get("subclass", False)
                            )
                            if (
                    ...
                    if (
                        ...
                        and extension in self.datatypes_by_extension
                    ):
                        # We are deactivating or uninstalling an installed tool shed repository, so eliminate the datatype
                        ...
                        self.log.debug(
                            "Removed datatype with extension '%s' from the registry."
                            % extension
                        )
                else:
                    ...
                    if can_process_datatype:
                        if dtype is not None:
                            try:
                                ...
                                self.log.exception(
                                    "Error parsing datatype definition for dtype %s",
                                    str(dtype),
                                )
                                ok = False
                            if ok:
                                ...
                                if (
                                    proprietary_path
                                    and proprietary_datatype_module
                                    ...
                                    try:
                                        ...
                                        if hasattr(
                                            imported_module, datatype_class_name
                                        ):
                                            datatype_class = getattr(
                                                imported_module, datatype_class_name
                                            )
                                ...
                                if datatype_class is None:
                                    try:
                                        ...
                                        datatype_class = getattr(
                                            module, datatype_class_name
                                        )
                                        self.log.debug(
                                            "Retrieved datatype module %s:%s from the datatype registry."
                                            % (
                                                str(datatype_module),
                                                datatype_class_name,
                                            )
                                        ...
                                        self.log.exception(
                                            "Error importing datatype module %s",
                                            str(datatype_module),
                                        )
                                        ok = False
                        elif type_extension is not None:
                            try:
                                datatype_class = self.datatypes_by_extension[
                                    type_extension
                                ].__class__
                            except Exception:
                                self.log.exception(
                                    "Error determining datatype_class for type_extension %s",
                                    str(type_extension),
                                )
                                ok = False
                        if ok:
                            if not deactivate:
                                ...
                                if make_subclass:
                                    datatype_class = type(
                                        datatype_class_name, (datatype_class,), {}
                                    )
                                    if edam_format:
                                ...
                                if mimetype is None:
                                    ...
                                    mimetype = self.datatypes_by_extension[
                                        extension
                                    ].get_mime()
                                ...
                                if (
                                    display_in_upload
                                    and extension not in self.upload_file_formats
                                ):
                                    self.upload_file_formats.append(extension)
                                # Max file size cut off for setting optional metadata.
                                self.datatypes_by_extension[
                                    extension
                                ].max_optional_metadata_filesize = elem.get(
                                ...
                                for composite_file in elem.findall("composite_file"):
                                    ...
                                    if name is None:
                                        self.log.warning(
                                            "You must provide a name for your composite_file (%s)."
                                            % composite_file
                                        )
                                ...
                                for display_app in elem.findall("display"):
                                    if proprietary_display_path:
                                        if (
                                            elem
                                            not in self.proprietary_display_app_containers
                                        ):
                                            self.proprietary_display_app_containers.append(
                                                elem
                                            )
                                    else:
                                ...
                                if composite_files:
                                    datatype_info_dict["composite_files"] = [
                                        _.dict() for _ in composite_files.values()
                                    ]
                                self.datatype_info_dicts.append(datatype_info_dict)

                                for auto_compressed_type in auto_compressed_types:
                                    compressed_extension = "%s.%s" % (
                                        extension,
                                        auto_compressed_type,
                                    )
                                    upper_compressed_type = (
                                        auto_compressed_type[0].upper()
                                        + auto_compressed_type[1:]
                                    )
                                    auto_compressed_type_name = (
                                        datatype_class_name + upper_compressed_type
                                    )
                                    attributes = {}
                                    if auto_compressed_type == "gz":
                                        ...
                                        dynamic_parent = (
                                            binary.Bz2DynamicCompressedArchive
                                        )
                                    else:
                                        raise Exception(
                                            "Unknown auto compression type [%s]"
                                            % auto_compressed_type
                                        )
                                    attributes["file_ext"] = compressed_extension
                                    attributes[
                                        "uncompressed_datatype_instance"
                                    ] = datatype_instance
                                    ...
                                    if (
                                        display_in_upload
                                        and compressed_extension
                                        not in self.upload_file_formats
                                    ):
                                        self.upload_file_formats.append(
                                            compressed_extension
                                        )
                                    ...
                                    if sniff_compressed_types:
                                        compressed_sniffers[datatype_class].append(
                                            compressed_datatype_instance
                                        )
                            ...
                            if dtype is not None or type_extension is not None:
                                if extension in self.datatypes_by_extension:
                                    if not override:
                                        ...
                                        # to override.
                                        self.log.debug(
                                            "Ignoring conflicting datatype with extension '%s' from %s."
                                            % (extension, config)
                                        )
    ...
    def _load_build_sites(self, root):
        def load_build_site(build_site_config):
            ...
            if not (build_site_config.get("type") and build_site_config.get("file")):
                self.log.exception(
                    "Site is missing required 'type' and 'file' attributes"
                )
                return
            ...
            if not os.path.exists(path):
                ...
                if os.path.exists(sample_path):
                    self.log.debug(
                        "Build site file [%s] not found using sample [%s]."
                        % (path, sample_path)
                    )
                    path = sample_path
            ...
            if build_site_config.get("display", None):
                ...
                self.log.debug(
                    "Loaded build site '%s': %s with display sites: %s",
                    site_type,
                    path,
                    display,
                )
        ...
        if root.find("build_sites") is not None:
            ...
            build_sites_config_file = getattr(
                self.config, "build_sites_config_file", None
            )
            if build_sites_config_file and os.path.exists(build_sites_config_file):
                ...
                if not isinstance(build_sites_config, list):
                    self.log.exception(
                        "Build sites configuration YAML file does not declare list of sites."
                    )
                    return
            ...
            for elem in sniffers.findall("sniffer"):
                ...
                if dtype is not None:
                    try:
                        ...
                        self.log.exception(
                            "Error determining datatype class or module for dtype %s",
                            str(dtype),
                        )
                        ok = False
                    if ok:
                        ...
                        if module is None:
                            try:
                                ...
                                self.log.exception(
                                    "Error importing datatype class for '%s'",
                                    str(dtype),
                                )
                                ok = False
                        if ok:
                            try:
                                ...
                                self.log.exception(
                                    "Error calling method %s from class %s",
                                    str(datatype_class_name),
                                    str(module),
                                )
                                ok = False
                            if ok:
                                if deactivate:
                                    ...
                                    if sniffer_class is not None:
                                        for index, s_e_c in enumerate(
                                            ...
                                            if sniffer_class == s_e_c:
                                                ...
                                                self.log.debug(
                                                    "Removed sniffer element for datatype '%s'"
                                                    % str(dtype)
                                                )
                                                break
                                        for sniffer_class in self.sniff_order:
                                            if (
                                                sniffer_class.__class__
                                                == aclass.__class__
                                            ):
                                                self.sniff_order.remove(sniffer_class)
                                                self.log.debug(
                                                    "Removed sniffer class for datatype '%s' from sniff order"
                                                    % str(dtype)
                                                )
                                                break
                                else:
                                    ...
                                    conflict = False
                                    for conflict_loc, sniffer_class in enumerate(
                                        self.sniff_order
                                    ):
                                        if sniffer_class.__class__ == aclass.__class__:
                                            ...
                                            if override:
                                                ...
                                                self.log.debug(
                                                    "Removed conflicting sniffer for datatype '%s'"
                                                    % dtype
                                                )
                                            break
                                    if not conflict or override:
                                        if (
                                            compressed_sniffers
                                            and aclass.__class__ in compressed_sniffers
                                        ):
                                            for (
                                                compressed_sniffer
                                            ) in compressed_sniffers[aclass.__class__]:
                                                self.sniff_order.append(
                                                    compressed_sniffer
                                                )
                                        self.sniff_order.append(aclass)
                                        self.log.debug(
                                            "Loaded sniffer for datatype '%s'" % dtype
                                        )
    ...
    def is_extension_unsniffable_binary(self, ext):
        ...
        return (
            ...
            and isinstance(datatype, binary.Binary)
            and not hasattr(datatype, "sniff")
        )
    ...
    def load_datatype_converters(
        self,
        toolbox,
        installed_repository_dict=None,
        deactivate=False,
        use_cached=False,
    ):
        """
        ...
        for elem in converters:
            ...
            try:
                ...
                if installed_repository_dict:
                    ...
                    converter.repository_name = installed_repository_dict[
                        "repository_name"
                    ]
                    converter.repository_owner = installed_repository_dict[
                        "repository_owner"
                    ]
                    converter.installed_changeset_revision = installed_repository_dict[
                ...
                if deactivate:
                    ...
                    if source_datatype in self.datatype_converters:
                        if target_datatype in self.datatype_converters[source_datatype]:
                            del self.datatype_converters[source_datatype][
                                target_datatype
                            ]
                    ...
                    if (
                        not hasattr(toolbox.app, "tool_cache")
                        or converter.id in toolbox.app.tool_cache._new_tool_ids
                    ):
                        self.log.debug("Loaded converter: %s", converter.id)
            except Exception:
                if deactivate:
                    self.log.exception(
                        "Error deactivating converter from (%s)" % converter_path
                    )
                else:
                    self.log.exception("Error loading converter (%s)" % converter_path)

    def load_display_applications(
        self, app, installed_repository_dict=None, deactivate=False
    ):
        """
            ...
            for display_app in elem.findall("display"):
                ...
                if installed_repository_dict:
                    ...
                    config_path = os.path.join(
                        self.display_applications_path, display_file
                    )
                try:
                    inherit = galaxy.util.string_as_bool(
                        display_app.get("inherit", "False")
                    )
                    ...
                    if display_app:
                        if display_app.id in self.display_applications:
                            ...
                            display_app.tool_shed = installed_repository_dict[
                                "tool_shed"
                            ]
                            display_app.repository_name = installed_repository_dict[
                                "repository_name"
                            ]
                            display_app.repository_owner = installed_repository_dict[
                                "repository_owner"
                            ]
                        ...
                        if deactivate:
                            ...
                            if extension in self.datatypes_by_extension:
                                if (
                                    display_app.id
                                    in self.datatypes_by_extension[
                                        extension
                                    ].display_applications
                                ):
                                    del self.datatypes_by_extension[
                                        extension
                                    ].display_applications[display_app.id]
                            if (
                                inherit
                                and (
                                    self.datatypes_by_extension[extension],
                                    display_app,
                                )
                            ...
                            self.log.debug(
                                "Deactivated display application '%s' for datatype '%s'."
                                % (display_app.id, extension)
                            )
                        else:
                            self.display_applications[display_app.id] = display_app
                            self.datatypes_by_extension[
                                extension
                            ].add_display_application(display_app)
                            if (
                                inherit
                                and (
                                    self.datatypes_by_extension[extension],
                            ...
                            self.log.debug(
                                "Loaded display application '%s' for datatype '%s', inherit=%s."
                                % (display_app.id, extension, inherit)
                            )
                except Exception:
                    if deactivate:
                        self.log.exception(
                            "Error deactivating display application (%s)" % config_path
                        )
                    else:
                        self.log.exception(
                            "Error loading display application (%s)" % config_path
                        )
        ...
        for extension, d_type1 in self.datatypes_by_extension.items():
            for d_type2, display_app in self.inherit_display_application_by_class:
                ...
                if current_app is None and isinstance(d_type1, type(d_type2)):
                    self.log.debug(
                        "Adding inherited display application '%s' to datatype '%s'"
                        % (display_app.id, extension)
                    )
    ...
    def reload_display_applications(self, display_application_ids=None):
        ...
        for display_application_id in display_application_ids:
            try:
                ...
                self.log.debug(
                    'Requested to reload display application "%s", but failed: %s.',
                    display_application_id,
                    e,
                )
    ...
    def load_external_metadata_tool(self, toolbox):
        ...
        self.log.debug(
            "Loaded external metadata tool: %s", self.set_external_metadata_tool.id
        )
    ...
    def find_conversion_destination_for_dataset_by_extensions(
        self, dataset_or_ext, accepted_formats, converter_safe=True
    ):
        ...
        for convert_ext in self.get_converters_by_datatype(ext):
            ...
            if convert_ext_datatype is None:
                self.log.warning(
                    "Datatype class not found for extension '%s', which is used as target for conversion from datatype '%s'"
            ...
            for meta_name, meta_spec in d_type.metadata_spec.items():
                if meta_spec.set_in_upload:
                    ...
                    if not help_txt or help_txt == meta_name:
                        help_txt = ""
                    inputs.append(
                        '<param type="text" name="%s" label="Set metadata value for &quot;%s&quot;" value="%s" help="%s"/>'
    ...
    def edam_formats(self):
        ...
        if not self._edam_formats_mapping:
            self._edam_formats_mapping = dict(
                (k, v.edam_format) for k, v in self.datatypes_by_extension.items()
            )
    ...
    def edam_data(self):
        ...
        if not self._edam_data_mapping:
            self._edam_data_mapping = dict(
                (k, v.edam_data) for k, v in self.datatypes_by_extension.items()
            )
    ...
    def to_xml_file(self, path):
        if not self._registry_xml_string:
            ...
            datatype_elems = "".join(
                (galaxy.util.xml_to_string(elem) for elem in self.datatype_elems)
            )
            sniffer_elems = "".join(
                (galaxy.util.xml_to_string(elem) for elem in self.sniffer_elems)
            )
