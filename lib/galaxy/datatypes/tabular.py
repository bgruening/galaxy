...
class TabularData(data.Text):
    ...
    def set_peek(
        self, dataset, line_count=None, is_multi_byte=False, WIDTH=256, skipchars=None
    ):
        ...
        if offset is not None:
            ...
            if os.stat(dataset.file_name).st_size < max_peek_size:
                ...
                return trans.stream_template_mako(
                    ...
                    truncated_data=open(dataset.file_name, mode="r").read(
                        max_peek_size
                    ),
                    data=dataset,
                )
        ...
        try:
            ...
            for line in dataset.peek.splitlines():
                if line.startswith(tuple(skipchars)):
                    ...
                    if len(elems) != columns:
                        out.append(
                            '<tr><td colspan="100%%">%s</td></tr>' % escape(line)
                        )
                    else:
                        out.append("<tr>")
                        for elem in elems:
    ...
    def column_dataprovider(self, dataset, **settings):
        ...
        return dataproviders.column.ColumnarDataProvider(
            dataset_source, deliminator=delimiter, **settings
        )

    @dataproviders.decorators.dataprovider_factory(
        "dataset-column", dataproviders.column.ColumnarDataProvider.settings
    )
    def dataset_column_dataprovider(self, dataset, **settings):
        """Attempts to get column settings from dataset.metadata"""
        delimiter = dataset.metadata.delimiter
        return dataproviders.dataset.DatasetColumnarDataProvider(
    ...
    def dict_dataprovider(self, dataset, **settings):
        ...
        dataset_source = dataproviders.dataset.DatasetDataProvider(dataset)
        delimiter = dataset.metadata.delimiter
        return dataproviders.column.DictDataProvider(
            dataset_source, deliminator=delimiter, **settings
        )

    @dataproviders.decorators.dataprovider_factory(
        "dataset-dict", dataproviders.column.DictDataProvider.settings
    )
    def dataset_dict_dataprovider(self, dataset, **settings):
        """Attempts to get column settings from dataset.metadata"""
        delimiter = dataset.metadata.delimiter
        return dataproviders.dataset.DatasetDictDataProvider(
            dataset, deliminator=delimiter, **settings
        )
...
class Tabular(TabularData):
    ...
    def set_meta(
        ...
        **kwd
        ...
        if skip is None:
            skip = 0
        column_type_set_order = [
            "int",
            "float",
            "list",
            "str",
        ]  # Order to set column types in
        default_column_type = column_type_set_order[
            -1
        ]  # Default column type is lowest in list
        column_type_compare_order = list(
            column_type_set_order
        )  # Order to compare column types
        ...
        def type_overrules_type(column_type1, column_type2):
            ...
            raise ValueError(
                "Tried to compare unknown column types: %s and %s"
                % (column_type1, column_type2)
            )
        ...
        first_line_column_types = [
            default_column_type
        ]  # default value is one column of type str
        if dataset.has_data():
            ...
            with compression_utils.get_fileobj(dataset.file_name) as dataset_fh:
                ...
                while True:
                    ...
                    if i < skip or not line or line.startswith("#"):
                        ...
                        data_lines += 1
                        if (
                            max_guess_type_data_lines is None
                            or data_lines <= max_guess_type_data_lines
                        ):
                            fields = line.split("\t")
                            for field_count, field in enumerate(fields):
                                ...
                                if type_overrules_type(
                                    column_type, column_types[field_count]
                                ):
        ...
        for i in range(len(column_types)):
            if column_types[i] is None:
                if (
                    len(first_line_column_types) <= i
                    or first_line_column_types[i] is None
                ):
...
class Sam(Tabular):
    ...
    def header_dataprovider(self, dataset, **settings):
        dataset_source = dataproviders.dataset.DatasetDataProvider(dataset)
        headers_source = dataproviders.line.RegexLineDataProvider(
            dataset_source, regex_list=["^@"]
        )
        return dataproviders.line.RegexLineDataProvider(headers_source, **settings)

    @dataproviders.decorators.dataprovider_factory(
        "id-seq-qual", dict_dataprovider.settings
    )
...
class Pileup(Tabular):
    ...
    MetadataElement(
        name="endCol", default=2, desc="End column", param=metadata.ColumnParameter
    )
    MetadataElement(
        name="baseCol",
        default=3,
        desc="Reference base column",
...
class VcfGz(BaseVcf, binary.Binary):
    ...
    def sniff(self, filename):
        ...
        with open(filename, "rb") as fh:
            ...
            last28 = fh.read()
            return (
                binascii.hexlify(last28)
                == b"1f8b08040000000000ff0600424302001b0003000000000000000000"
            )
    ...
    def set_meta(self, dataset, **kwd):
        ...
        try:
            pysam.tabix_index(
                dataset.file_name, index=index_file.file_name, preset="vcf", force=True
            )
...
class Eland(Tabular):
    ...
    def set_meta(self, dataset, overwrite=True, skip=None, max_data_lines=5, **kwd):
        if dataset.has_data():
            with compression_utils.get_fileobj(
                dataset.file_name, compressed_formats=["gzip"]
            ) as dataset_fh:
                lanes = {}
                tiles = {}
                barcodes = {}
                ...
                for i, line in enumerate(dataset_fh):
                    if line:
                        ...
                        if len(line_pieces) != 22:
                            raise Exception(
                                "%s:%d:Corrupt line!" % (dataset.file_name, i)
                            )
...
class BaseCSV(TabularData):
    ...
    def sniff(self, filename):
        ...
        try:
            ...
            auto_dialect = csv.Sniffer().sniff(
                open(filename, "r").read(self.big_peek_size)
            )
        ...
        if dataset.has_data():
            with open(dataset.file_name, "r") as csvfile:
                ...
                try:
                    ...
                    raise Exception(
                        "CSV reader error - line %d: %s" % (reader.line_num, e)
                    )
                else:
...
class ConnectivityTable(Tabular):
                ...
                if i == 0:
                    ...
                    if not self.structure_regexp.match(line.upper()):
                        ...
                        if j != int(re.split(r"\W+", line, 1)[0]):
                            ...
                            j == length
                        ):  # Last line of first sequence has been recheached
                            return True
                        else:
                            j += 1
                i += 1
    ...
    def get_chunk(self, trans, dataset, chunk):
        ...
        return dumps(
            {
                "ck_data": util.unicodify(ck_data_header + "\n" + ck_data_body),
                "ck_index": ck_index + 1,
            }
        )
