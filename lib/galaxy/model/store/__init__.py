...
class ModelImportStore(object):
        ...
        if self.defines_new_history():
            ...
            if self.user:
                add_item_annotation(
                    ...
                    self.user,
                    new_history,
                    history_properties.get("annotation"),
                )
    ...
    def perform_import(self, history=None, new_history=False, job=None):
        ...
        self._import_datasets(
            object_import_tracker, datasets_attrs, history, new_history, job
        )
        ...
        self._import_collection_instances(
            object_import_tracker, collections_attrs, history, new_history
        )
        self._import_collection_implicit_input_associations(
            object_import_tracker, collections_attrs
        )
        self._import_collection_copied_associations(
            object_import_tracker, collections_attrs
        )
    ...
    def _import_datasets(
        self, object_import_tracker, datasets_attrs, history, new_history, job
    ):
        ...
        for dataset_attrs in datasets_attrs:
                    ...
                    for attribute in dataset_attributes:
                        if attribute in dataset_attrs["dataset"]:
                            setattr(
                                ...
                                attribute,
                                dataset_attrs["dataset"][attribute],
                            )
            ...
            if (
                ...
                hda = self.sa_session.query(model.HistoryDatasetAssociation).get(
                    dataset_attrs["id"]
                )
                attributes = [
                    "name",
                    ...
                    if attribute in dataset_attrs:
                        ...
                        if attribute == "metadata":
                            ...
                            def remap_objects(p, k, obj):
                                if (
                                    isinstance(obj, dict)
                                    and "model_class" in obj
                                    and obj["model_class"] == "MetadataFile"
                                ):
                                    return (
                                        k,
                                        model.MetadataFile(
                                            dataset=hda, uuid=obj["uuid"]
                                        ),
                                    )
                                return (k, obj)
                ...
                if "dataset" in dataset_attrs:
                    ...
                    if file_name:
                        ...
                        archive_path = os.path.abspath(
                            os.path.join(self.archive_dir, file_name)
                        )
                        if os.path.islink(archive_path):
                            raise MalformedContents(
                                "Invalid dataset path: %s" % archive_path
                            )
                        ...
                        if not in_directory(temp_dataset_file_name, self.archive_dir):
                            raise MalformedContents(
                                "Invalid dataset path: %s" % temp_dataset_file_name
                            )
                    ...
                    if not file_name or not os.path.exists(temp_dataset_file_name):
                        ...
                        if dataset_extra_files_path:
                            ...
                            dir_name = "dataset_%s_files" % getattr(
                                dataset_instance.dataset, store_by
                            )
                            dataset_extra_files_path = os.path.join(
                                self.archive_dir, dataset_extra_files_path
                            )
                ...
                if model_class == "HistoryDatasetAssociation" and self.user:
                    ...
                    if tag_list:
                        tag_handler = model.tags.GalaxyTagHandler(
                            sa_session=self.sa_session
                        )
                ...
                if model_class == "HistoryDatasetAssociation":
                    if object_key in dataset_attrs:
                        object_import_tracker.hdas_by_key[
                            dataset_attrs[object_key]
                        ] = dataset_instance
                    else:
                        assert "id" in dataset_attrs
                        object_import_tracker.hdas_by_id[
                            dataset_attrs["id"]
                        ] = dataset_instance
                else:
                    object_import_tracker.lddas_by_key[
                        dataset_attrs[object_key]
                    ] = dataset_instance
        ...
        for library_attrs in libraries_attrs:
            ...
            def import_folder(folder_attrs):
                ...
                library_folder = model.LibraryFolder(
                    name=name, description=description, genome_build=genome_build
                )
                ...
                for ld_attrs in folder_attrs.get("datasets", []):
                    ...
                    if "ldda" in ld_attrs:
                        ldda = object_import_tracker.lddas_by_key[
                            ld_attrs["ldda"][object_key]
                        ]
    ...
    def _import_collection_instances(
        self, object_import_tracker, collections_attrs, history, new_history
    ):
        object_key = self.object_key
        ...
        def import_collection(collection_attrs):
                    ...
                    if "hda" in element_attrs:
                        ...
                        if object_key in hda_attrs:
                            ...
                            if hda_key in hdas_by_key:
                                ...
                                raise KeyError(
                                    "Failed to find exported hda with key [%s] of type [%s] in [%s]"
                                    % (hda_key, object_key, hdas_by_key)
                                )
                        else:
                            hda_id = hda_attrs["id"]
                            ...
                            if hda_id not in hdas_by_id:
                                raise Exception(
                                    "Failed to find HDA with id [%s] in [%s]"
                                    % (hda_id, hdas_by_id)
                                )
                            hda = hdas_by_id[hda_id]
                        dce.hda = hda
                    elif "child_collection" in element_attrs:
                        dce.child_collection = import_collection(
                            element_attrs["child_collection"]
                        )
                    else:
            ...
            if (
                "id" in collection_attrs
                and self.import_options.allow_edit
                and not self.sessionless
            ):
                dc = self.sa_session.query(model.DatasetCollection).get(
                    collection_attrs["id"]
                )
                attributes = ["collection_type", "populated_state", "element_count"]
                for attribute in attributes:
        ...
        for collection_attrs in collections_attrs:
            ...
            if (
                ...
                hdca = self.sa_session.query(
                    model.HistoryDatasetCollectionAssociation
                ).get(collection_attrs["id"])
    ...
    def _import_collection_implicit_input_associations(
        self, object_import_tracker, collections_attrs
    ):
        object_key = self.object_key
        ...
        for collection_attrs in collections_attrs:
            ...
            if "implicit_input_collections" in collection_attrs:
                implicit_input_collections = collection_attrs[
                    "implicit_input_collections"
                ]
                for implicit_input_collection in implicit_input_collections:
                    name = implicit_input_collection["name"]
                    input_collection_identifier = implicit_input_collection[
                        "input_dataset_collection"
                    ]
                    if (
                        input_collection_identifier
                        in object_import_tracker.hdcas_by_key
                    ):
                        input_dataset_collection = object_import_tracker.hdcas_by_key[
                            input_collection_identifier
                        ]
                        hdca.add_implicit_input_collection(
                            name, input_dataset_collection
                        )

    def _import_dataset_copied_associations(
        self, object_import_tracker, datasets_attrs
    ):
        object_key = self.object_key
        ...
        for dataset_attrs in datasets_attrs:
            ...
            copied_from_chain = dataset_attrs.get(
    ...
    def _import_collection_copied_associations(
        ...
        for collection_attrs in collections_attrs:
            ...
            copied_from_chain = collection_attrs.get(
                "copied_from_history_dataset_collection_association_id_chain", []
            )
            copied_from_object_key = _copied_from_object_key(
                copied_from_chain, object_import_tracker.hdcas_by_key
            )
            if not copied_from_object_key:
                continue
    ...
    def _import_jobs(self, object_import_tracker, history):
        ...
        def _find_hda(input_key):
            ...
            if input_key in object_import_tracker.hda_copied_from_sinks:
                hda = object_import_tracker.hdas_by_key[
        ...
        for job_attrs in jobs_attrs:
            ...
            params = self._normalize_job_parameters(
                imported_job, job_attrs, _find_hda, _find_hdca
            )
...
def _copied_from_object_key(copied_from_chain, objects_by_key):
    ...
    for i, copied_from_key in enumerate(copied_from_chain):
        filter_id = (i != len(copied_from_chain) - 1) and (
            copied_from_key not in objects_by_key
        )
        if not filter_id:
...
class BaseDirectoryImportModelStore(ModelImportStore):
    ...
    def datasets_properties(self):
        datasets_attrs_file_name = os.path.join(
            self.archive_dir, ATTRS_FILENAME_DATASETS
        )
    ...
    def collections_properties(self):
        collections_attrs_file_name = os.path.join(
            self.archive_dir, ATTRS_FILENAME_COLLECTIONS
        )
    ...
    def library_properties(self):
        libraries_attrs_file_name = os.path.join(
            self.archive_dir, ATTRS_FILENAME_LIBRARIES
        )
    ...
    def implicit_collection_jobs_properties(self):
        ...
        if os.path.exists(implicit_collection_jobs_attrs_file_name):
            implicit_collection_jobs_attrs = load(
                open(implicit_collection_jobs_attrs_file_name)
            )
        else:
    ...
    def _normalize_job_parameters(self, imported_job, job_attrs, _find_hda, _find_hdca):
        def remap_objects(p, k, obj):
            if isinstance(obj, dict) and obj.get(
                "__HistoryDatasetAssociation__", False
            ):
...
class DirectoryImportModelStoreLatest(BaseDirectoryImportModelStore):
    ...
    def __init__(self, archive_dir, **kwd):
        ...
        self.archive_dir = archive_dir
        if self.defines_new_history():
            self.import_history_encoded_id = self.new_history_properties().get(
                "encoded_id"
            )
        else:
    ...
    def _connect_job_io(self, imported_job, job_attrs, _find_hda, _find_hdca):
        ...
        if "input_dataset_collection_mapping" in job_attrs:
            for input_name, input_keys in job_attrs[
                ...
                for input_key in input_keys:
                    ...
                    if input_hdca:
                        imported_job.add_input_dataset_collection(
                            input_name, input_hdca
                        )
        ...
        if "output_dataset_collection_mapping" in job_attrs:
            for output_name, output_keys in job_attrs[
                ...
                for output_key in output_keys:
                    ...
                    if output_hdca:
                        imported_job.add_output_dataset_collection(
                            output_name, output_hdca
                        )

    def trust_hid(self, obj_attrs):
        return (
            self.import_history_encoded_id
...
class DirectoryModelExportStore(ModelExportStore):
    ...
    def serialize_files(self, dataset, as_dict):
        ...
        if file_name:
            ...
            target_filename = get_export_dataset_filename(
                as_dict["name"], as_dict["extension"], dataset_hid
            )
        ...
        self.dataset_id_to_path[dataset.dataset.id] = (
            as_dict.get("file_name"),
            as_dict.get("extra_files_path"),
        )
    ...
    def export_history(self, history, include_hidden=False, include_deleted=False):
        ...
        query = (
            ...
            .filter(
                model.HistoryDatasetCollectionAssociation.deleted == expression.false()
            )
        )
        ...
        for dataset in datasets:
            dataset.annotation = get_item_annotation_str(
                sa_session, history.user, dataset
            )
            add_dataset = (not dataset.visible or not include_hidden) and (
                not dataset.deleted or include_deleted
            )
    ...
    def export_library(self, library, include_hidden=False, include_deleted=False):
        ...
        def collect_datasets(library_folder):
            for library_dataset in library_folder.datasets:
                ...
                add_dataset = (not ldda.visible or not include_hidden) and (
                    not ldda.deleted or include_deleted
                )
    ...
    def _finalize(self):
        ...
        datasets_attrs_filename = os.path.join(
            export_directory, ATTRS_FILENAME_DATASETS
        )
        with open(datasets_attrs_filename, "w") as datasets_attrs_out:
            dump(
                list(
                    map(
                        lambda d: d.serialize(
                            self.security, self.serialization_options
                        ),
                        datasets_attrs,
                    )
                ),
        ...
        with open(datasets_attrs_filename + ".provenance", "w") as provenance_attrs_out:
            dump(
                ...
                provenance_attrs_out,
            )

        libraries_attrs_filename = os.path.join(
            export_directory, ATTRS_FILENAME_LIBRARIES
        )
        with open(libraries_attrs_filename, "w") as librariess_attrs_out:
            dump(
                ...
                librariess_attrs_out,
            )

        collections_attrs_filename = os.path.join(
            export_directory, ATTRS_FILENAME_COLLECTIONS
        )
        ...
        for hda_id, (hda, include_files) in self.included_datasets.items():
            ...
            job_hda = hda
            while (
                job_hda.copied_from_history_dataset_association
        ...
        for id, job in jobs_dict.items():
            ...
            for assoc in job.output_datasets:
                ...
                if assoc.dataset:
                    ...
                    output_dataset_mapping[name].append(
                        self.exported_key(assoc.dataset)
                    )
            ...
            for assoc in job.input_dataset_collections:
                ...
                if assoc.dataset_collection:
                    ...
                    input_dataset_collection_mapping[name].append(
        ...
        for (
            job_id,
            job_output_dataset_associations,
        ) in self.job_output_dataset_associations.items():
            output_dataset_mapping = {}
            ...
            jobs_attrs.append(
                {"id": job_id, "output_dataset_mapping": output_dataset_mapping}
            )
        ...
        icjs_attrs_filename = os.path.join(
            export_directory, ATTRS_FILENAME_IMPLICIT_COLLECTION_JOBS
        )
...
def tar_export_directory(export_directory, out_file, gzip):
    ...
    with tarfile.open(out_file, tarfile_mode, dereference=True) as history_archive:
        for export_path in os.listdir(export_directory):
            history_archive.add(
                os.path.join(export_directory, export_path), arcname=export_path
            )
