...
DatasetPermissions_table = Table(
    "dataset_permissions",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
...
LibraryPermissions_table = Table(
    "library_permissions",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
...
LibraryFolderPermissions_table = Table(
...
LibraryDatasetPermissions_table = Table(
...
LibraryDatasetDatasetAssociationPermissions_table = Table(
...
Index(
    ...
    LibraryDatasetDatasetAssociationPermissions_table.c.library_dataset_dataset_association_id,
)

LibraryItemInfoPermissions_table = Table(
    ...
    Column("action", TEXT),
    Column(
        "library_item_info_id",
        Integer,
        ForeignKey("library_item_info.id"),
        nullable=True,
        index=True,
    ),
    Column("role_id", Integer, ForeignKey("role.id"), index=True),
...
LibraryItemInfoTemplatePermissions_table = Table(
    ...
    Column("action", TEXT),
    Column(
        "library_item_info_template_id",
        Integer,
        ForeignKey("library_item_info_template.id"),
        nullable=True,
    ),
    Column("role_id", Integer, ForeignKey("role.id"), index=True),
)
Index(
    "ix_liitp_library_item_info_template_id",
    LibraryItemInfoTemplatePermissions_table.c.library_item_info_template_id,
)

DefaultUserPermissions_table = Table(
...
DefaultHistoryPermissions_table = Table(
...
LibraryItemInfoTemplateElement_table = Table(
...
Index(
    "ix_liite_library_item_info_template_id",
    LibraryItemInfoTemplateElement_table.c.library_item_info_template_id,
)

LibraryItemInfoTemplate_table = Table(
...
LibraryInfoTemplateAssociation_table = Table(
...
Index(
    "ix_lita_library_item_info_template_id",
    LibraryInfoTemplateAssociation_table.c.library_item_info_template_id,
)

LibraryFolderInfoTemplateAssociation_table = Table(
...
Index(
    "ix_lfita_library_item_info_template_id",
    LibraryFolderInfoTemplateAssociation_table.c.library_item_info_template_id,
)

LibraryDatasetInfoTemplateAssociation_table = Table(
...
Index(
    "ix_ldita_library_item_info_template_id",
    LibraryDatasetInfoTemplateAssociation_table.c.library_item_info_template_id,
)
...
Index(
    "ix_lddita_library_dataset_dataset_association_id",
    LibraryDatasetDatasetInfoTemplateAssociation_table.c.library_dataset_dataset_association_id,
)
Index(
    "ix_lddita_library_item_info_template_id",
    LibraryDatasetDatasetInfoTemplateAssociation_table.c.library_item_info_template_id,
)

LibraryItemInfoElement_table = Table(
    ...
    Column(
        "library_item_info_id", Integer, ForeignKey("library_item_info.id"), index=True
    ),
    Column(
        "library_item_info_template_element_id",
        Integer,
        ForeignKey("library_item_info_template_element.id"),
    ),
)
Index(
    "ix_liie_library_item_info_template_element_id",
    LibraryItemInfoElement_table.c.library_item_info_template_element_id,
)

LibraryItemInfo_table = Table(
    "library_item_info",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
...
LibraryDatasetInfoAssociation_table = Table(
...
Index(
    "ix_lddia_library_dataset_dataset_association_id",
    LibraryDatasetDatasetInfoAssociation_table.c.library_dataset_dataset_association_id,
)
Index(
    "ix_lddia_library_item_info_id",
    LibraryDatasetDatasetInfoAssociation_table.c.library_item_info_id,
)

JobExternalOutputMetadata_table = Table(
...
Index(
    ...
    JobExternalOutputMetadata_table.c.library_dataset_dataset_association_id,
)
...
def upgrade(migrate_engine):
    ...
    try:
        HistoryDatasetAssociation_table = Table(
            "history_dataset_association", metadata, autoload=True
        )
    except NoSuchTableError:
    ...
    if HistoryDatasetAssociation_table is not None:
        try:
            ...
            assert (
                col
                is HistoryDatasetAssociation_table.c.copied_from_library_dataset_dataset_association_id
            )
        except Exception:
            log.exception(
    ...
    if (
        HistoryDatasetAssociation_table is not None
        and LibraryDatasetDatasetAssociation_table is not None
    ):
        try:
    ...
    if migrate_engine.name != "sqlite":
        ...
        if (
            MetadataFile_table is not None
            and LibraryDatasetDatasetAssociation_table is not None
        ):
            try:
                cons = ForeignKeyConstraint(
    ...
    if users:
        ...
        if not roles:
            ...
            if migrate_engine.name in ["postgres", "postgresql", "sqlite"]:
                cmd = (
                    "UPDATE role SET description = 'Private role for ' || description;"
                )
            ...
            cmd = cmd % (
                nextval(migrate_engine, "user_role_association"),
                localtimestamp(migrate_engine),
                localtimestamp(migrate_engine),
            )
            migrate_engine.execute(cmd)
            ...
            cmd = cmd % (
                nextval(migrate_engine, "default_history_permissions"),
                engine_false(migrate_engine),
            )
            migrate_engine.execute(cmd)
            ...
            cmd = cmd % (
                nextval(migrate_engine, "dataset_permissions"),
                localtimestamp(migrate_engine),
                localtimestamp(migrate_engine),
                engine_false(migrate_engine),
            )
