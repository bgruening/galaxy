...
def upgrade(migrate_engine):
    ...
    add_column(
        ...
        index_name="ix_history_dataset_association_purged",
    )
    try:
        migrate_engine.execute(
            HistoryDatasetAssociation_table.update().values(purged=False)
        )
    except Exception:
        log.exception(
            "Updating column 'purged' of table 'history_dataset_association' failed."
        )
