...
def upgrade(migrate_engine):
    ...
    if migrate_engine.name != "sqlite":
        col = Column(
            ...
            Integer,
            ForeignKey(
                "external_service.id", name="sample_dataset_external_services_id_fk"
            ),
            index=True,
        )
    else:
    ...
    add_column(
        col, SampleDataset_table, index_name="ix_sample_dataset_external_service_id"
    )

    # populate the column
    cmd = (
        "SELECT sample_dataset.id, request_type.sequencer_id "
    ...
    for row in results_list:
        ...
        if not sequencer_id:
            sequencer_id = "null"
        cmd = (
            "INSERT INTO request_type_external_service_association VALUES ( %s, %s, %s )"
            % (
...
def downgrade(migrate_engine):
    ...
    Sequencer_table = Table("sequencer", metadata, autoload=True)
    col = Column(
        "sequencer_type_id", TrimmedString(255)
    )  # should also have nullable=False
