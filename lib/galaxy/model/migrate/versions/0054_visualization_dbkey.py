...
def upgrade(migrate_engine):
    ...
    Visualization_revision_table = Table(
        "visualization_revision", metadata, autoload=True
    )
    ...
    for viz in all_viz:
        ...
        if viz[Visualization_revision_table.c.config]:
            dbkey = (
                loads(viz[Visualization_revision_table.c.config])
                .get("dbkey", "")
                .replace("'", "\\'")
            )
            ...
            migrate_engine.execute(
                "UPDATE visualization SET dbkey='%s' WHERE id=%s" % (dbkey, viz_id)
            )
