...
WorkflowInvocationToSubworkflowInvocationAssociation_table = Table(
    ...
    ForeignKeyConstraint(
        ["workflow_invocation_id"], ["workflow_invocation.id"], name="fk_wfi_swi_wfi"
    ),
    ForeignKeyConstraint(
        ["subworkflow_invocation_id"], ["workflow_invocation.id"], name="fk_wfi_swi_swi"
    ),
    ForeignKeyConstraint(
...
INDEXES = [
    Index(
        "ix_wfinv_swfinv_wfi",
        WorkflowInvocationToSubworkflowInvocationAssociation_table.c.workflow_invocation_id,
    ),
    Index(
        "ix_wfinv_swfinv_swfi",
        WorkflowInvocationToSubworkflowInvocationAssociation_table.c.subworkflow_invocation_id,
    ),
    Index(
...
def upgrade(migrate_engine):
    ...
    if migrate_engine.name in ["postgres", "postgresql"]:
        subworkflow_id_column = Column(
            "subworkflow_id", Integer, ForeignKey("workflow.id"), nullable=True
        )
        ...
        parent_workflow_id_column = Column(
            "parent_workflow_id", Integer, ForeignKey("workflow.id"), nullable=True
        )
    else:
