...
def create_sequencer_form_definition(migrate_engine):
    ...
    form_definition_current_id = get_latest_id(
        migrate_engine, "form_definition_current"
    )
    ...
    cmd = cmd % (
        nextval(migrate_engine, "form_definition"),
        localtimestamp(migrate_engine),
        localtimestamp(migrate_engine),
    ...
    cmd = "UPDATE form_definition_current SET latest_form_id=%i WHERE id=%i" % (
        form_definition_id,
        form_definition_current_id,
    )
    migrate_engine.execute(cmd)
...
def get_sequencer_id(migrate_engine, sequencer_info):
    ...
    for row in result:
        ...
        if values and isinstance(values, dict):
            if (
                sequencer_info.get("host", "") == values.get("field_0", "")
                ...
                and sequencer_info.get("rename_dataset", "")
                == values.get("field_4", "")
            ):
                return sequencer_id
    return None
...
def add_sequencer(
    migrate_engine, sequencer_index, sequencer_form_definition_id, sequencer_info
):
    ...
    values = dumps(
        {
            "field_0": sequencer_info.get("host", ""),
    ...
    cmd = "INSERT INTO form_values VALUES ( %s, %s, %s, %s, '%s' )" % (
        nextval(migrate_engine, "form_values"),
        localtimestamp(migrate_engine),
        localtimestamp(migrate_engine),
    ...
    if len(results_list):
        ...
        for row in results_list:
            ...
            if sequencer_info and isinstance(sequencer_info, dict):
                ...
                update_sequencer_id_in_request_type(
                    migrate_engine, request_type_id, sequencer_id
                )
...
def downgrade(migrate_engine):
    ...
    if RequestType_table is not None:
        ...
        cmd = (
            "SELECT request_type.id, form_values.content "
