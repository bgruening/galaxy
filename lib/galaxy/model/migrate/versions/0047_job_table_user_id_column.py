...
def upgrade(migrate_engine):
    ...
    if Job_table is not None:
        ...
        try:
            cmd = (
                "SELECT job.id AS galaxy_job_id, "
            ...
            print(
                "Updating user_id column in job table for ", len(job_users), " rows..."
            )
            print("")
            update_count = 0
            for row in job_users:
                if row.galaxy_user_id:
                    cmd = "UPDATE job SET user_id = %d WHERE id = %d" % (
                        int(row.galaxy_user_id),
                        int(row.galaxy_job_id),
                    )
                    update_count += 1
                migrate_engine.execute(cmd)
            print(
                "Updated the user_id column for ",
                update_count,
