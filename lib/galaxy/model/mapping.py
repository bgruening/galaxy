...
model.ValidationError.table = Table(
    ...
    Column("id", Integer, primary_key=True),
    Column(
        "dataset_id", Integer, ForeignKey("history_dataset_association.id"), index=True
    ),
...
model.LibraryFolder.table = Table(
    ...
    Column("id", Integer, primary_key=True),
    Column(
        "parent_id", Integer, ForeignKey("library_folder.id"), nullable=True, index=True
    ),
    Column("create_time", DateTime, default=now),
    Column("update_time", DateTime, default=now, onupdate=now),
...
model.ImplicitCollectionJobsJobAssociation.table = Table(
    ...
    Column(
        ...
        Integer,
        ForeignKey("implicit_collection_jobs.id"),
        index=True,
    ),
    Column(
        "job_id", Integer, ForeignKey("job.id"), index=True
...
model.StoredWorkflow.table = Table(
    ...
    Column("update_time", DateTime, default=now, onupdate=now),
    Column(
        "user_id", Integer, ForeignKey("galaxy_user.id"), index=True, nullable=False
    ),
    Column(
        "latest_workflow_id",
        Integer,
        ForeignKey(
            "workflow.id", use_alter=True, name="stored_workflow_latest_workflow_id_fk"
        ),
        index=True,
    ),
    Column("name", TEXT),
...
model.WorkflowStep.table = Table(
    ...
    Column("update_time", DateTime, default=now, onupdate=now),
    Column(
        "workflow_id", Integer, ForeignKey("workflow.id"), index=True, nullable=False
    ),
    Column(
        "subworkflow_id", Integer, ForeignKey("workflow.id"), index=True, nullable=True
    ),
    Column(
        "dynamic_tool_id",
        Integer,
...
model.WorkflowStepConnection.table = Table(
    ...
    Column(
        "input_step_input_id", Integer, ForeignKey("workflow_step_input.id"), index=True
    ),
    Column("output_name", TEXT),
    Column(
        "input_subworkflow_step_id", Integer, ForeignKey("workflow_step.id"), index=True
    ),
)
...
model.WorkflowInvocationStepOutputDatasetAssociation.table = Table(
    ...
    Column(
        ...
        Integer,
        ForeignKey("workflow_invocation_step.id"),
        index=True,
    ),
    Column(
        "dataset_id", Integer, ForeignKey("history_dataset_association.id"), index=True
...
model.Page.table = Table(
    ...
    Column(
        ...
        Integer,
        ForeignKey(
            "page_revision.id", use_alter=True, name="page_latest_revision_id_fk"
        ),
        index=True,
    ),
    Column("title", TEXT),
...
model.Visualization.table = Table(
    ...
    Column(
        ...
        Integer,
        ForeignKey(
            "visualization_revision.id",
            use_alter=True,
            name="visualization_latest_revision_id_fk",
        ),
        index=True,
    ),
    Column("title", TEXT),
...
model.LibraryDatasetCollectionTagAssociation.table = Table(
    ...
    Column(
        ...
        Integer,
        ForeignKey("library_dataset_collection_association.id"),
        index=True,
    ),
    Column("tag_id", Integer, ForeignKey("tag.id"), index=True),
    Column("user_id", Integer, ForeignKey("galaxy_user.id"), index=True),
...
# Ratings tables.
model.HistoryRatingAssociation.table = Table(
...
CleanupEventDatasetAssociation_table = Table(
    "cleanup_event_dataset_association",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column(
...
CleanupEventMetadataFileAssociation_table = Table(
    "cleanup_event_metadata_file_association",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column(
...
CleanupEventHistoryAssociation_table = Table(
...
CleanupEventHistoryDatasetAssociationAssociation_table = Table(
    "cleanup_event_hda_association",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column(
...
CleanupEventLibraryAssociation_table = Table(
    "cleanup_event_library_association",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column(
...
CleanupEventLibraryFolderAssociation_table = Table(
    "cleanup_event_library_folder_association",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column(
...
CleanupEventLibraryDatasetAssociation_table = Table(
    "cleanup_event_library_dataset_association",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column(
...
CleanupEventLibraryDatasetDatasetAssociationAssociation_table = Table(
    "cleanup_event_ldda_association",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column(
...
CleanupEventImplicitlyConvertedDatasetAssociationAssociation_table = Table(
    "cleanup_event_icda_association",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
    Column(
...
mapper(
    model.FormValues,
    model.FormValues.table,
    properties=dict(
        form_definition=relation(
            model.FormDefinition,
            primaryjoin=(
                model.FormValues.table.c.form_definition_id
...
mapper(
    model.FormDefinition,
    model.FormDefinition.table,
    properties=dict(
        current=relation(
            model.FormDefinitionCurrent,
            primaryjoin=(
                model.FormDefinition.table.c.form_definition_current_id
                == model.FormDefinitionCurrent.table.c.id
            ),
        )
    ),
)
...
mapper(
    model.FormDefinitionCurrent,
    model.FormDefinitionCurrent.table,
    properties=dict(
        forms=relation(
            model.FormDefinition,
        ...
        latest_form=relation(
            model.FormDefinition,
            post_update=True,
            primaryjoin=(
                model.FormDefinitionCurrent.table.c.latest_form_id
                == model.FormDefinition.table.c.id
            ),
        ),
    ),
...
mapper(
    ...
    model.UserAuthnzToken.table,
    properties=dict(
        user=relation(
            model.User,
            primaryjoin=(
                model.UserAuthnzToken.table.c.user_id == model.User.table.c.id
            ),
            backref="social_auth",
        )
    ),
...
mapper(
    model.CustosAuthnzToken,
    model.CustosAuthnzToken.table,
    properties=dict(
        user=relation(
            model.User,
            primaryjoin=(
                model.CustosAuthnzToken.table.c.user_id == model.User.table.c.id
            ),
            backref="custos_auth",
        )
    ),
...
mapper(
    ...
    properties=dict(
        ...
        authn=relation(
            model.UserAuthnzToken,
            primaryjoin=(
                model.CloudAuthz.table.c.authn_id == model.UserAuthnzToken.table.c.id
            ),
            backref="cloudauthz",
        ),
    ),
...
simple_mapping(
    ...
    dataset=relation(
        model.Dataset,
        primaryjoin=(
            model.Dataset.table.c.id
            == model.HistoryDatasetAssociation.table.c.dataset_id
        ),
        lazy=False,
    ),
    ...
    copied_from_history_dataset_association=relation(
        model.HistoryDatasetAssociation,
        primaryjoin=(
            model.HistoryDatasetAssociation.table.c.copied_from_history_dataset_association_id
    ...
    implicitly_converted_datasets=relation(
        ...
        primaryjoin=(
            model.ImplicitlyConvertedDatasetAssociation.table.c.hda_parent_id
            == model.HistoryDatasetAssociation.table.c.id
        ),
    ),
    tags=relation(
    ...
    ratings=relation(
        model.HistoryDatasetAssociationRatingAssociation,
    ...
    extended_metadata=relation(
        ...
        primaryjoin=(
            (
                model.HistoryDatasetAssociation.table.c.extended_metadata_id
    ...
    hidden_beneath_collection_instance=relation(
        ...
        primaryjoin=(
            (
                model.HistoryDatasetAssociation.table.c.hidden_beneath_collection_instance_id
...
simple_mapping(
    ...
    library_associations=relation(
        ...
        primaryjoin=(
            model.Dataset.table.c.id
            == model.LibraryDatasetDatasetAssociation.table.c.dataset_id
        ),
    ),
...
mapper(
    model.DatasetHash,
    model.DatasetHash.table,
    properties=dict(dataset=relation(model.Dataset, backref="hashes")),
)

mapper(
    model.DatasetSource,
    model.DatasetSource.table,
    properties=dict(dataset=relation(model.Dataset, backref="sources")),
)

mapper(
    model.DatasetSourceHash,
    model.DatasetSourceHash.table,
    properties=dict(source=relation(model.DatasetSource, backref="hashes")),
)

mapper(
    model.HistoryDatasetAssociationHistory, model.HistoryDatasetAssociationHistory.table
)

mapper(
    model.HistoryDatasetAssociationDisplayAtAuthorization,
...
mapper(
    model.HistoryDatasetAssociationSubset,
    model.HistoryDatasetAssociationSubset.table,
    properties=dict(
        hda=relation(
            model.HistoryDatasetAssociation,
            primaryjoin=(
                model.HistoryDatasetAssociationSubset.table.c.history_dataset_association_id
                == model.HistoryDatasetAssociation.table.c.id
            ),
        ),
        subset=relation(
            model.HistoryDatasetAssociation,
            primaryjoin=(
                model.HistoryDatasetAssociationSubset.table.c.history_dataset_association_subset_id
                == model.HistoryDatasetAssociation.table.c.id
            ),
        ),
    ),
...
mapper(
    model.ImplicitlyConvertedDatasetAssociation,
    model.ImplicitlyConvertedDatasetAssociation.table,
    properties=dict(
        parent_hda=relation(
            model.HistoryDatasetAssociation,
            primaryjoin=(
        ...
        parent_ldda=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(
                model.ImplicitlyConvertedDatasetAssociation.table.c.ldda_parent_id
                == model.LibraryDatasetDatasetAssociation.table.c.id
            ),
        ),
        dataset_ldda=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(
                model.ImplicitlyConvertedDatasetAssociation.table.c.ldda_id
                == model.LibraryDatasetDatasetAssociation.table.c.id
            ),
        ...
        dataset=relation(
            model.HistoryDatasetAssociation,
            primaryjoin=(
                model.ImplicitlyConvertedDatasetAssociation.table.c.hda_id
                == model.HistoryDatasetAssociation.table.c.id
            ),
            backref="implicitly_converted_parent_datasets",
        ),
...
mapper(
    ...
    properties=dict(
        ...
        exports=relation(
            ...
            primaryjoin=(
                model.JobExportHistoryArchive.table.c.history_id
        ...
        active_datasets=relation(
            ...
            primaryjoin=(
                (
                    model.HistoryDatasetAssociation.table.c.history_id
                    == model.History.table.c.id
                )
        ...
        active_dataset_collections=relation(
            ...
            primaryjoin=(
                (
                    model.HistoryDatasetCollectionAssociation.table.c.history_id
                    == model.History.table.c.id
                )
        ...
        average_rating=column_property(
            select([func.avg(model.HistoryRatingAssociation.table.c.rating)]).where(
                model.HistoryRatingAssociation.table.c.history_id
                == model.History.table.c.id
            ),
            deferred=True,
        ),
        users_shared_with_count=column_property(
            select([func.count(model.HistoryUserShareAssociation.table.c.id)]).where(
                model.History.table.c.id
                == model.HistoryUserShareAssociation.table.c.history_id
            ),
            deferred=True,
        ),
    ),
...
model.History.users_shared_with_dot_users = association_proxy(
    "users_shared_with", "user"
)

mapper(
    model.HistoryUserShareAssociation,
    model.HistoryUserShareAssociation.table,
    properties=dict(
...
mapper(
    ...
    properties=dict(
        ...
        galaxy_sessions=relation(
            model.GalaxySession, order_by=desc(model.GalaxySession.table.c.update_time)
        ),
        stored_workflow_menu_entries=relation(
            ...
            primaryjoin=(
                ...
                & (
                    model.StoredWorkflowMenuEntry.table.c.stored_workflow_id
        ...
        values=relation(
            model.FormValues,
            primaryjoin=(
                model.User.table.c.form_values_id == model.FormValues.table.c.id
            ),
        ),
        api_keys=relation(
            model.APIKeys,
...
mapper(
    ...
    model.PasswordResetToken.table,
...
model.User.preferences = association_proxy(
    "_preferences", "value", creator=model.UserPreference
)
...
mapper(
    ...
    model.UserGroupAssociation.table,
    properties=dict(
...
mapper(
    model.DefaultUserPermissions,
    model.DefaultUserPermissions.table,
    properties=dict(
...
mapper(
    model.DefaultHistoryPermissions,
    model.DefaultHistoryPermissions.table,
    properties=dict(
...
mapper(
    ...
    model.UserRoleAssociation.table,
    properties=dict(
...
mapper(
    model.GroupRoleAssociation,
    model.GroupRoleAssociation.table,
    properties=dict(
        group=relation(model.Group, backref="roles"), role=relation(model.Role)
    ),
)
...
mapper(
    model.UserQuotaAssociation,
    model.UserQuotaAssociation.table,
    properties=dict(
        user=relation(model.User, backref="quotas"), quota=relation(model.Quota)
    ),
)

mapper(
    model.GroupQuotaAssociation,
    model.GroupQuotaAssociation.table,
    properties=dict(
        group=relation(model.Group, backref="quotas"), quota=relation(model.Quota)
    ),
)

mapper(
    model.DefaultQuotaAssociation,
    model.DefaultQuotaAssociation.table,
    properties=dict(quota=relation(model.Quota, backref="default")),
)

mapper(
    model.DatasetPermissions,
    model.DatasetPermissions.table,
    properties=dict(
...
mapper(
    model.LibraryPermissions,
    model.LibraryPermissions.table,
    properties=dict(
...
mapper(
    model.LibraryFolderPermissions,
    model.LibraryFolderPermissions.table,
    properties=dict(
...
mapper(
    model.LibraryDatasetPermissions,
    model.LibraryDatasetPermissions.table,
    properties=dict(
...
mapper(
    ...
    model.LibraryDatasetDatasetAssociationPermissions.table,
    properties=dict(
        library_dataset_dataset_association=relation(
            model.LibraryDatasetDatasetAssociation, backref="actions"
        ),
...
mapper(
    model.ExtendedMetadata,
    model.ExtendedMetadata.table,
    properties=dict(
        children=relation(
            model.ExtendedMetadataIndex,
            primaryjoin=(
                model.ExtendedMetadataIndex.table.c.extended_metadata_id
                == model.ExtendedMetadata.table.c.id
            ),
            backref=backref(
                "parent",
...
mapper(
    model.ExtendedMetadataIndex,
    model.ExtendedMetadataIndex.table,
    properties=dict(
        extended_metadata=relation(
            model.ExtendedMetadata,
            primaryjoin=(
...
mapper(
    model.LibraryInfoAssociation,
    model.LibraryInfoAssociation.table,
    properties=dict(
        library=relation(
            model.Library,
            primaryjoin=(
                (
                    model.LibraryInfoAssociation.table.c.library_id
        ...
        template=relation(
            model.FormDefinition,
            primaryjoin=(
                model.LibraryInfoAssociation.table.c.form_definition_id
                == model.FormDefinition.table.c.id
            ),
        ),
        info=relation(
            model.FormValues,
            primaryjoin=(
                model.LibraryInfoAssociation.table.c.form_values_id
...
mapper(
    ...
    properties=dict(
        ...
        datasets=relation(
            ...
            primaryjoin=(
                (
                    model.LibraryDataset.table.c.folder_id
                    == model.LibraryFolder.table.c.id
                )
            ),
...
mapper(
    model.LibraryFolderInfoAssociation,
    model.LibraryFolderInfoAssociation.table,
    properties=dict(
        folder=relation(
            model.LibraryFolder,
            primaryjoin=(
                (
                    model.LibraryFolderInfoAssociation.table.c.library_folder_id
                    == model.LibraryFolder.table.c.id
                )
        ...
        template=relation(
            model.FormDefinition,
            primaryjoin=(
                model.LibraryFolderInfoAssociation.table.c.form_definition_id
                == model.FormDefinition.table.c.id
            ),
        ),
        info=relation(
            model.FormValues,
            primaryjoin=(
                model.LibraryFolderInfoAssociation.table.c.form_values_id
                == model.FormValues.table.c.id
            ),
        ),
    ),
...
mapper(
    model.LibraryDataset,
    model.LibraryDataset.table,
    properties=dict(
        folder=relation(model.LibraryFolder),
        library_dataset_dataset_association=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(
                model.LibraryDataset.table.c.library_dataset_dataset_association_id
                == model.LibraryDatasetDatasetAssociation.table.c.id
            ),
        ),
        expired_datasets=relation(
            model.LibraryDatasetDatasetAssociation,
            foreign_keys=[
                model.LibraryDataset.table.c.id,
                model.LibraryDataset.table.c.library_dataset_dataset_association_id,
            ],
            primaryjoin=(
                (
                    model.LibraryDataset.table.c.id
                    == model.LibraryDatasetDatasetAssociation.table.c.library_dataset_id
                )
                & (
                    not_(
                        model.LibraryDataset.table.c.library_dataset_dataset_association_id
...
mapper(
    model.LibraryDatasetDatasetAssociation,
    model.LibraryDatasetDatasetAssociation.table,
    properties=dict(
        ...
        copied_from_library_dataset_dataset_association=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(
                model.LibraryDatasetDatasetAssociation.table.c.copied_from_library_dataset_dataset_association_id
                == model.LibraryDatasetDatasetAssociation.table.c.id
            ),
        ...
        extended_metadata=relation(
            ...
            primaryjoin=(
                (
                    model.LibraryDatasetDatasetAssociation.table.c.extended_metadata_id
                    == model.ExtendedMetadata.table.c.id
                )
            ),
        ),
...
mapper(
    model.LibraryDatasetDatasetInfoAssociation,
    model.LibraryDatasetDatasetInfoAssociation.table,
    properties=dict(
        library_dataset_dataset_association=relation(
            model.LibraryDatasetDatasetAssociation,
            primaryjoin=(
        ...
        template=relation(
            model.FormDefinition,
            primaryjoin=(
                model.LibraryDatasetDatasetInfoAssociation.table.c.form_definition_id
                == model.FormDefinition.table.c.id
            ),
        ),
        info=relation(
            model.FormValues,
            primaryjoin=(
                model.LibraryDatasetDatasetInfoAssociation.table.c.form_values_id
                == model.FormValues.table.c.id
            ),
        ),
    ),
...
mapper(
    ...
    model.JobToInputDatasetAssociation.table,
    properties=dict(
        job=relation(model.Job),
        dataset=relation(
            model.HistoryDatasetAssociation, lazy=False, backref="dependent_jobs"
        ),
    ),
)
...
mapper(
    ...
    model.JobToOutputDatasetAssociation.table,
    properties=dict(
        job=relation(model.Job),
...
mapper(
    ...
    model.JobToInputDatasetCollectionAssociation.table,
    properties=dict(
        job=relation(model.Job),
        dataset_collection=relation(
...
mapper(
    ...
    model.JobToOutputDatasetCollectionAssociation.table,
    properties=dict(
        job=relation(model.Job),
        dataset_collection_instance=relation(
            model.HistoryDatasetCollectionAssociation,
            lazy=False,
...
mapper(
    model.JobToImplicitOutputDatasetCollectionAssociation,
    model.JobToImplicitOutputDatasetCollectionAssociation.table,
    properties=dict(
        job=relation(model.Job),
        dataset_collection=relation(
...
mapper(
    ...
    model.JobToInputLibraryDatasetAssociation.table,
    properties=dict(
        job=relation(model.Job),
        dataset=relation(
            model.LibraryDatasetDatasetAssociation, lazy=False, backref="dependent_jobs"
        ),
    ),
)
...
mapper(
    ...
    model.JobToOutputLibraryDatasetAssociation.table,
    properties=dict(
        job=relation(model.Job),
        dataset=relation(model.LibraryDatasetDatasetAssociation, lazy=False),
    ),
)

simple_mapping(model.JobStateHistory, job=relation(model.Job, backref="state_history"))

simple_mapping(model.JobMetricText, job=relation(model.Job, backref="text_metrics"))

simple_mapping(model.TaskMetricText, task=relation(model.Task, backref="text_metrics"))

simple_mapping(
    model.JobMetricNumeric, job=relation(model.Job, backref="numeric_metrics")
)
...
mapper(
    model.JobExternalOutputMetadata,
    model.JobExternalOutputMetadata.table,
    properties=dict(
        job=relation(model.Job),
        history_dataset_association=relation(
...
mapper(
    model.JobExportHistoryArchive,
    model.JobExportHistoryArchive.table,
    properties=dict(
        job=relation(model.Job),
...
mapper(
    model.JobImportHistoryArchive,
    model.JobImportHistoryArchive.table,
    properties=dict(job=relation(model.Job), history=relation(model.History)),
)

mapper(
    model.GenomeIndexToolData,
    model.GenomeIndexToolData.table,
    properties=dict(
...
mapper(
    model.PostJobAction,
    model.PostJobAction.table,
    properties=dict(
        workflow_step=relation(
            model.WorkflowStep,
            backref="post_job_actions",
            primaryjoin=(
                model.WorkflowStep.table.c.id
                == model.PostJobAction.table.c.workflow_step_id
            ),
        )
    ),
)
...
mapper(
    model.PostJobActionAssociation,
    model.PostJobActionAssociation.table,
    properties=dict(
        job=relation(model.Job), post_job_action=relation(model.PostJobAction)
    ),
)
...
mapper(
    ...
    properties=dict(
        ...
        input_dataset_collections=relation(
            model.JobToInputDatasetCollectionAssociation, lazy=True
        ),
        ...
        any_output_dataset_collection_instances_deleted=column_property(
            exists(
                ...
                and_(
                    model.Job.table.c.id
                    == model.JobToOutputDatasetCollectionAssociation.table.c.job_id,
                    model.HistoryDatasetCollectionAssociation.table.c.id
                    == model.JobToOutputDatasetCollectionAssociation.table.c.dataset_collection_id,
                    model.HistoryDatasetCollectionAssociation.table.c.deleted == true(),
                ),
            )
        ),
        output_dataset_collection_instances=relation(
            model.JobToOutputDatasetCollectionAssociation, lazy=True
        ),
        output_dataset_collections=relation(
            model.JobToImplicitOutputDatasetCollectionAssociation, lazy=True
        ),
        ...
        output_library_datasets=relation(
            model.JobToOutputLibraryDatasetAssociation, lazy=True
        ),
...
mapper(model.Task, model.Task.table, properties=dict(job=relation(model.Job)))
...
simple_mapping(
    ...
    elements=relation(
        ...
        primaryjoin=(
            model.DatasetCollection.table.c.id
            == model.DatasetCollectionElement.table.c.dataset_collection_id
        ),
...
simple_mapping(
    ...
    copied_from_history_dataset_collection_association=relation(
        model.HistoryDatasetCollectionAssociation,
        primaryjoin=(
            model.HistoryDatasetCollectionAssociation.table.c.copied_from_history_dataset_collection_association_id
            == model.HistoryDatasetCollectionAssociation.table.c.id
        ),
...
simple_mapping(
    ...
    hda=relation(
        model.HistoryDatasetAssociation,
        primaryjoin=(
            model.DatasetCollectionElement.table.c.hda_id
            == model.HistoryDatasetAssociation.table.c.id
        ),
    ),
    ldda=relation(
        model.LibraryDatasetDatasetAssociation,
        primaryjoin=(
            model.DatasetCollectionElement.table.c.ldda_id
            == model.LibraryDatasetDatasetAssociation.table.c.id
        ),
    ),
    child_collection=relation(
        model.DatasetCollection,
        primaryjoin=(
            model.DatasetCollectionElement.table.c.child_collection_id
            == model.DatasetCollection.table.c.id
        ),
    ),
)
...
mapper(
    model.GalaxySessionToHistoryAssociation,
    model.GalaxySessionToHistoryAssociation.table,
    properties=dict(
        galaxy_session=relation(model.GalaxySession), history=relation(model.History)
    ),
)
...
mapper(
    ...
    properties=dict(
        steps=relation(
            ...
            backref="workflow",
            primaryjoin=(
                (model.Workflow.table.c.id == model.WorkflowStep.table.c.workflow_id)
            ),
        ...
        step_count=column_property(
            select([func.count(model.WorkflowStep.table.c.id)]).where(
                model.Workflow.table.c.id == model.WorkflowStep.table.c.workflow_id
            ),
            deferred=True,
        ),
...
mapper(
    model.WorkflowStep,
    model.WorkflowStep.table,
    properties=dict(
        subworkflow=relation(
            model.Workflow,
            primaryjoin=(
                model.Workflow.table.c.id == model.WorkflowStep.table.c.subworkflow_id
            ),
            backref="parent_workflow_steps",
        ),
        dynamic_tool=relation(
            model.DynamicTool,
            primaryjoin=(
                model.DynamicTool.table.c.id
                == model.WorkflowStep.table.c.dynamic_tool_id
            ),
            backref="workflow_steps",
        ),
        tags=relation(
            model.WorkflowStepTagAssociation,
...
mapper(
    ...
    properties=dict(
        workflow_step=relation(
            ...
            cascade="all",
            primaryjoin=(
                model.WorkflowStepInput.table.c.workflow_step_id
                == model.WorkflowStep.table.c.id
            ),
        )
    ),
...
mapper(
    ...
    properties=dict(
        workflow_step=relation(
            ...
            primaryjoin=(
                model.WorkflowStep.table.c.id
                == model.WorkflowOutput.table.c.workflow_step_id
            ),
        )
    ),
)
...
mapper(
    model.WorkflowStepConnection,
    model.WorkflowStepConnection.table,
    properties=dict(
        input_step_input=relation(
            model.WorkflowStepInput,
            backref="connections",
            cascade="all",
            primaryjoin=(
                model.WorkflowStepConnection.table.c.input_step_input_id
                == model.WorkflowStepInput.table.c.id
            ),
        ),
        input_subworkflow_step=relation(
            model.WorkflowStep,
            backref=backref("parent_workflow_input_connections", uselist=True),
            primaryjoin=(
                model.WorkflowStepConnection.table.c.input_subworkflow_step_id
                == model.WorkflowStep.table.c.id
            ),
        ),
        output_step=relation(
            model.WorkflowStep,
            backref="output_connections",
            cascade="all",
            primaryjoin=(
                model.WorkflowStepConnection.table.c.output_step_id
                == model.WorkflowStep.table.c.id
            ),
        ),
    ),
...
mapper(
    ...
    properties=dict(
        ...
        latest_workflow=relation(
            model.Workflow,
            post_update=True,
            primaryjoin=(
                model.StoredWorkflow.table.c.latest_workflow_id
        ...
        ratings=relation(
            model.StoredWorkflowRatingAssociation,
            order_by=model.StoredWorkflowRatingAssociation.table.c.id,
            backref="stored_workflows",
        ),
        average_rating=column_property(
            select(
                [func.avg(model.StoredWorkflowRatingAssociation.table.c.rating)]
            ).where(
                model.StoredWorkflowRatingAssociation.table.c.stored_workflow_id
                == model.StoredWorkflow.table.c.id
            ),
            deferred=True,
        ),
...
model.StoredWorkflow.users_shared_with_dot_users = association_proxy(
    "users_shared_with", "user"
)

mapper(
    model.StoredWorkflowUserShareAssociation,
    model.StoredWorkflowUserShareAssociation.table,
    properties=dict(
        user=relation(model.User, backref="workflows_shared_by_others"),
...
mapper(
    model.StoredWorkflowMenuEntry,
    model.StoredWorkflowMenuEntry.table,
...
mapper(
    ...
    properties=dict(
        history=relation(
            model.History, backref=backref("workflow_invocations", uselist=True)
        ),
        ...
        subworkflow_invocations=relation(
            model.WorkflowInvocationToSubworkflowInvocationAssociation,
            primaryjoin=(
                (
                    model.WorkflowInvocationToSubworkflowInvocationAssociation.table.c.workflow_invocation_id
                    == model.WorkflowInvocation.table.c.id
                )
            ),
            backref=backref("parent_workflow_invocation", uselist=False),
            uselist=True,
        ),
        steps=relation(model.WorkflowInvocationStep, backref="workflow_invocation"),
...
mapper(
    model.WorkflowInvocationToSubworkflowInvocationAssociation,
    model.WorkflowInvocationToSubworkflowInvocationAssociation.table,
    properties=dict(
        subworkflow_invocation=relation(
            model.WorkflowInvocation,
            primaryjoin=(
                (
                    model.WorkflowInvocationToSubworkflowInvocationAssociation.table.c.subworkflow_invocation_id
                    == model.WorkflowInvocation.table.c.id
                )
            ),
...
simple_mapping(
    ...
    job=relation(
        model.Job,
        backref=backref("workflow_invocation_step", uselist=False),
        uselist=False,
    ),
    implicit_collection_jobs=relation(
        model.ImplicitCollectionJobs,
...
simple_mapping(
    model.WorkflowRequestInputStepParameter,
    workflow_invocation=relation(model.WorkflowInvocation),
    workflow_step=relation(model.WorkflowStep),
)
...
simple_mapping(
    ...
    workflow_invocation=relation(
        model.WorkflowInvocation, backref="output_dataset_collections"
    ),
    workflow_step=relation(model.WorkflowStep),
    dataset_collection=relation(model.HistoryDatasetCollectionAssociation),
    workflow_output=relation(model.WorkflowOutput),
...
simple_mapping(
    ...
    workflow_invocation_step=relation(
        model.WorkflowInvocationStep, backref="output_datasets"
    ),
    dataset=relation(model.HistoryDatasetAssociation),
)
...
simple_mapping(
    ...
    workflow_invocation_step=relation(
        model.WorkflowInvocationStep, backref="output_dataset_collections"
    ),
    dataset_collection=relation(model.HistoryDatasetCollectionAssociation),
)
...
mapper(
    ...
    properties=dict(
        ...
        latest_revision=relation(
            model.PageRevision,
            post_update=True,
            primaryjoin=(
                model.Page.table.c.latest_revision_id == model.PageRevision.table.c.id
            ),
            lazy=False,
        ),
        tags=relation(
        ...
        ratings=relation(
            model.PageRatingAssociation,
            order_by=model.PageRatingAssociation.table.c.id,
            backref="pages",
        ),
        average_rating=column_property(
            select([func.avg(model.PageRatingAssociation.table.c.rating)]).where(
                model.PageRatingAssociation.table.c.page_id == model.Page.table.c.id
            ),
            deferred=True,
        ),
    ),
...
mapper(
    model.PageUserShareAssociation,
    model.PageUserShareAssociation.table,
    properties=dict(
        user=relation(model.User, backref="pages_shared_by_others"),
...
mapper(
    ...
    properties=dict(
        user=relation(model.User),
        revisions=relation(
            model.VisualizationRevision,
            backref="visualization",
            cascade="all, delete-orphan",
            primaryjoin=(
                model.Visualization.table.c.id
        ...
        latest_revision=relation(
            ...
            post_update=True,
            primaryjoin=(
                model.Visualization.table.c.latest_revision_id
                == model.VisualizationRevision.table.c.id
            ),
            lazy=False,
        ),
        tags=relation(
        ...
        ratings=relation(
            model.VisualizationRatingAssociation,
            order_by=model.VisualizationRatingAssociation.table.c.id,
            backref="visualizations",
        ),
        average_rating=column_property(
            select(
                [func.avg(model.VisualizationRatingAssociation.table.c.rating)]
            ).where(
                model.VisualizationRatingAssociation.table.c.visualization_id
                == model.Visualization.table.c.id
            ),
            deferred=True,
        ),
...
model.Visualization.users_shared_with_dot_users = association_proxy(
    "users_shared_with", "user"
)

mapper(
    model.VisualizationUserShareAssociation,
    model.VisualizationUserShareAssociation.table,
    properties=dict(
        user=relation(model.User, backref="visualizations_shared_by_others"),
...
simple_mapping(
    model.Tag,
    children=relation(
        model.Tag, backref=backref("parent", remote_side=[model.Tag.table.c.id])
    ),
)
...
def tag_mapping(tag_association_class, backref_name):
    simple_mapping(
        ...
        tag=relation(model.Tag, backref=backref_name),
        user=relation(model.User),
    )
...
tag_mapping(
    model.HistoryDatasetAssociationTagAssociation, "tagged_history_dataset_associations"
)
tag_mapping(
    model.LibraryDatasetDatasetAssociationTagAssociation,
...
tag_mapping(
    model.HistoryDatasetCollectionTagAssociation, "tagged_history_dataset_collections"
)
tag_mapping(
    model.LibraryDatasetCollectionTagAssociation, "tagged_library_dataset_collections"
)
...
annotation_mapping(
    model.StoredWorkflowAnnotationAssociation, stored_workflow=model.StoredWorkflow
)
annotation_mapping(
    model.WorkflowStepAnnotationAssociation, workflow_step=model.WorkflowStep
)
annotation_mapping(model.PageAnnotationAssociation, page=model.Page)
annotation_mapping(
    model.VisualizationAnnotationAssociation, visualization=model.Visualization
)
annotation_mapping(
    model.HistoryDatasetCollectionAssociationAnnotationAssociation,
...
rating_mapping(
    model.StoredWorkflowRatingAssociation, stored_workflow=model.StoredWorkflow
)
...
# Data Manager tables
mapper(
    model.DataManagerHistoryAssociation,
    model.DataManagerHistoryAssociation.table,
    properties=dict(
        history=relation(model.History),
...
mapper(
    model.DataManagerJobAssociation,
    model.DataManagerJobAssociation.table,
    properties=dict(
        job=relation(
            model.Job,
...
def db_next_hid(self, n=1):
    ...
    try:
        if "postgres" not in session.bind.dialect.name:
            next_hid = select(
                [table.c.hid_counter],
                table.c.id == model.cached_id(self),
                for_update=True,
            ).scalar()
            ...
            stmt = (
                table.update()
                .where(table.c.id == model.cached_id(self))
                .values(hid_counter=(table.c.hid_counter + n))
...
def _workflow_invocation_update(self):
    ...
    stmt = (
        table.update()
        .values(update_time=now_val)
        .where(and_(table.c.id == self.id, table.c.update_time < now_val))
    )
