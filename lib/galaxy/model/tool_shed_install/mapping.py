...
install_model.ToolShedRepository.table = Table(
...
install_model.RepositoryRepositoryDependencyAssociation.table = Table(
...
install_model.RepositoryDependency.table = Table(
...
install_model.ToolDependency.table = Table(
    "tool_dependency",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
...
install_model.ToolVersion.table = Table(
    "tool_version",
    metadata,
    Column("id", Integer, primary_key=True),
    Column("create_time", DateTime, default=now),
...
install_model.ToolVersionAssociation.table = Table(
...
install_model.MigrateTools.table = Table(
    "migrate_tools",
    metadata,
...
mapper(
    ...
    install_model.ToolShedRepository.table,
    properties=dict(
        tool_versions=relation(
            install_model.ToolVersion,
            primaryjoin=(
                install_model.ToolShedRepository.table.c.id
                == install_model.ToolVersion.table.c.tool_shed_repository_id
            ),
            backref="tool_shed_repository",
        ),
        tool_dependencies=relation(
            install_model.ToolDependency,
            primaryjoin=(
                install_model.ToolShedRepository.table.c.id
                == install_model.ToolDependency.table.c.tool_shed_repository_id
            ),
            order_by=install_model.ToolDependency.table.c.name,
            backref="tool_shed_repository",
        ),
        required_repositories=relation(
            install_model.RepositoryRepositoryDependencyAssociation,
            primaryjoin=(
                install_model.ToolShedRepository.table.c.id
                == install_model.RepositoryRepositoryDependencyAssociation.table.c.tool_shed_repository_id
            ),
        ),
    ),
)
...
mapper(
    install_model.RepositoryRepositoryDependencyAssociation,
    install_model.RepositoryRepositoryDependencyAssociation.table,
    properties=dict(
        repository=relation(
            install_model.ToolShedRepository,
            primaryjoin=(
        ...
        repository_dependency=relation(
            ...
            primaryjoin=(
                ...
                == install_model.RepositoryDependency.table.c.id
            ),
        ),
    ),
)
...
mapper(
    install_model.RepositoryDependency,
    install_model.RepositoryDependency.table,
    properties=dict(
        repository=relation(
            install_model.ToolShedRepository,
            primaryjoin=(
...
mapper(
    install_model.ToolVersion,
    install_model.ToolVersion.table,
    properties=dict(
        parent_tool_association=relation(
            install_model.ToolVersionAssociation,
            primaryjoin=(
                install_model.ToolVersion.table.c.id
                == install_model.ToolVersionAssociation.table.c.tool_id
            ),
        ),
        child_tool_association=relation(
            install_model.ToolVersionAssociation,
            primaryjoin=(
                install_model.ToolVersion.table.c.id
                == install_model.ToolVersionAssociation.table.c.parent_id
            ),
        ),
    ),
)
