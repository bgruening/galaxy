...
class ToolShedRepository(object):
    ...
    def get_sharable_url(self, app):
        return common_util.get_tool_shed_repository_url(
            app, self.tool_shed, self.owner, self.name
        )

    def get_shed_config_filename(self):
        shed_config_filename = None
        if self.metadata:
            shed_config_filename = self.metadata.get(
                "shed_config_filename", shed_config_filename
            )
    ...
    def includes_data_managers(self):
        if self.metadata:
            return bool(
                len(self.metadata.get("data_manager", {}).get("data_managers", {}))
            )
        return False
    ...
    def is_latest_installable_revision(self):
        if self.tool_shed_status:
            return asbool(
                self.tool_shed_status.get("latest_installable_revision", False)
            )
        return False
...
class ToolVersion(Dictifiable):
    ...
    def __init__(
        self, id=None, create_time=None, tool_id=None, tool_shed_repository=None
    ):
        self.id = id
        self.create_time = create_time
