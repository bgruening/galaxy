                ...
                if field.sqlalchemy_field is not None:
                    ...
                    if operator == "=":
                        ...
                        self.query = self.query.filter(
                            sqlalchemy_field_value.like(right)
                        )
                    else:
                        raise GalaxyParseError(
                            "Invalid comparison operator: %s" % (operator)
                        )
...
class LibraryDatasetDatasetView(ViewQueryBaseClass):
    ...
    FIELDS = {
        "extended_metadata": ViewField(
            "extended_metadata", handler=library_extended_metadata_filter
        ),
        "name": ViewField(
            "name", sqlalchemy_field=(LibraryDatasetDatasetAssociation, "name")
        ),
        "id": ViewField(
            "id",
            sqlalchemy_field=(LibraryDatasetDatasetAssociation, "id"),
            id_decode=True,
        ),
        "deleted": ViewField(
            "deleted", sqlalchemy_field=(LibraryDatasetDatasetAssociation, "deleted")
        ),
        "parent_library_id": ViewField(
            "parent_library_id", id_decode=True, post_filter=ldda_parent_library_filter
        ),
        "data_type": ViewField(
            "data_type",
            sqlalchemy_field=(LibraryDatasetDatasetAssociation, "extension"),
        ),
    }
...
class LibraryFolderView(ViewQueryBaseClass):
    ...
    FIELDS = {
        ...
        "parent_id": ViewField(
            "parent_id", sqlalchemy_field=(LibraryFolder, "parent_id"), id_decode=True
        ),
        "parent_library_id": ViewField(
            "parent_library_id",
            post_filter=library_folder_parent_library_id_filter,
            id_decode=True,
        ),
...
class LibraryDatasetView(ViewQueryBaseClass):
    ...
    FIELDS = {
        ...
        "folder_id": ViewField(
            "folder_id", sqlalchemy_field=(LibraryDataset, "folder_id"), id_decode=True
        ),
    }
...
class HistoryDatasetView(ViewQueryBaseClass):
    ...
    FIELDS = {
        ...
        "id": ViewField(
            "id", sqlalchemy_field=(HistoryDatasetAssociation, "id"), id_decode=True
        ),
        "history_id": ViewField(
            "history_id",
            sqlalchemy_field=(HistoryDatasetAssociation, "history_id"),
            id_decode=True,
        ),
        ...
        "deleted": ViewField(
            "deleted", sqlalchemy_field=(HistoryDatasetAssociation, "deleted")
        ),
        "extended_metadata": ViewField(
            "extended_metadata", handler=history_dataset_extended_metadata_filter
        ),
    }
...
def workflow_tag_handler(view, left, operator, right):
    if operator == "=":
        ...
        tmp = right.split(":")
        view.query = view.query.filter(
            StoredWorkflowTagAssociation.user_tname == tmp[0]
        )
        if len(tmp) > 1:
            view.query = view.query.filter(
                StoredWorkflowTagAssociation.user_value == tmp[1]
            )
    else:
        raise GalaxyParseError("Invalid comparison operator: %s" % (operator))
...
def job_input_hda_filter(view, left, operator, right):
    ...
    view.query = view.query.filter(
        and_(
            Job.id == alias.job_id, alias.name == param_name, alias.dataset_id == right
        )
    )
...
def job_input_ldda_filter(view, left, operator, right):
    ...
    view.query = view.query.filter(
        and_(Job.id == alias.job_id, alias.name == param_name, alias.ldda_id == right)
    )
...
class JobView(ViewQueryBaseClass):
    ...
    FIELDS = {
        ...
        "input_ldda": ViewField(
            "input_ldda", handler=job_input_ldda_filter, id_decode=True
        ),
        "input_hda": ViewField(
            "input_hda", handler=job_input_hda_filter, id_decode=True
        ),
        "output_hda": ViewField(
            "output_hda", handler=job_output_hda_filter, id_decode=True
        ),
    }
...
class PageRevisionView(ViewQueryBaseClass):
    ...
    FIELDS = {
        ...
        "page_id": ViewField(
            "page_id", sqlalchemy_field=(PageRevision, "page_id"), id_decode=True
        ),
    }
