...
class UninitializedTree(BaseTree):
    ...
    def multiply(self, other_structure):
        ...
        new_collection_type = self.collection_type_description.multiply(
            other_structure.collection_type_description
        )
        return UninitializedTree(new_collection_type)

    def __str__(self):
        return (
            "UninitializedTree[collection_type=%s]" % self.collection_type_description
        )
...
class Tree(BaseTree):
    ...
    def _walk_collections(self, collection_dict):
        for index, (identifier, substructure) in enumerate(self.children):
            ...
            if substructure.is_leaf:
                ...
                sub_collections = dict_map(
                    lambda collection: element(collection).child_collection,
                    collection_dict,
                )
    ...
    def can_match(self, other_structure):
        if not self.collection_type_description.can_match_type(
            other_structure.collection_type_description
        ):
            return False
    ...
    def __str__(self):
        return "Tree[collection_type=%s,children=%s]" % (
            self.collection_type_description,
            ",".join(
                map(
                    lambda identifier_and_element: "%s=%s"
                    % (identifier_and_element[0], identifier_and_element[1]),
                    self.children,
                )
            ),
        )
...
def tool_output_to_structure(
    get_sliced_input_collection_structure, tool_output, collections_manager
):
    if not tool_output.collection:
        ...
        if structured_like:
            ...
            if (
                collection_type
                and tree.collection_type_description.collection_type != collection_type
            ):
                # See tool paired_collection_map_over_structured_like - type should
                # override structured_like if they disagree.
                tree = UninitializedTree(
                    collection_type_descriptions.for_collection_type(collection_type)
                )
        else:
            ...
            if not collection_type:
                raise Exception(
                    "Failed to determine collection type for mapping over output %s"
                    % tool_output.name
                )
    ...
    if (
        not tree.children_known
        and tree.collection_type_description.collection_type == "paired"
    ):
...
def get_structure(
    ...
    if leaf_subcollection_type:
        ...
        if hasattr(dataset_collection_instance, "child_collection"):
            collection_type_description = collection_type_description.collection_type_description_factory.for_collection_type(
                leaf_subcollection_type
            )
