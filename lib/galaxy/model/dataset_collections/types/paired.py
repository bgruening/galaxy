...
INVALID_IDENTIFIERS_MESSAGE = "Paired instance must define '%s' and '%s' datasets ." % (
    FORWARD_IDENTIFIER,
    REVERSE_IDENTIFIER,
)
...
class PairedDatasetCollectionType(BaseDatasetCollectionType):
    ...
    def generate_elements(self, elements):
        ...
        left_association = DatasetCollectionElement(
            element=forward_dataset, element_identifier=FORWARD_IDENTIFIER
        )
        right_association = DatasetCollectionElement(
            element=reverse_dataset, element_identifier=REVERSE_IDENTIFIER
        )
    ...
    def prototype_elements(self):
        left_association = DatasetCollectionElement(
            element=HistoryDatasetAssociation(), element_identifier=FORWARD_IDENTIFIER
        )
        right_association = DatasetCollectionElement(
            element=HistoryDatasetAssociation(), element_identifier=REVERSE_IDENTIFIER
        )
        yield left_association
        yield right_association
