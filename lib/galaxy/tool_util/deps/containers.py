...
class ContainerFinder(object):
    ...
    def find_container(self, tool_info, destination_info, job_info):
        ...
        def __destination_container(
            container_description=None, container_id=None, container_type=None
        ):
        ...
        if "container_override" in destination_info:
            container = container_from_description_from_dicts(
                destination_info["container_override"]
            )
            if container:
                return container
    ...
    def __build_container_id_from_parts(self, container_type, destination_info, mode):
        ...
        cont_id = (
            repo + owner + destination_info["%s_image_%s" % (container_type, mode)]
        )
...
class ContainerRegistry(object):
    ...
    def __resolvers_dict(self):
        ...
        return plugin_config.plugins_dict(
            galaxy.tool_util.deps.container_resolvers, "resolver_type"
        )
