    ...
    def __init__(self, tool_requirements=None):
        if tool_requirements:
            ...
            self.tool_requirements = OrderedSet(
                [
                    r
                    if isinstance(r, ToolRequirement)
                    else ToolRequirement.from_dict(r)
                    for r in tool_requirements
                ]
            )
        else:
            self.tool_requirements = OrderedSet()
...
DEFAULT_CONTAINER_SHELL = (
    "/bin/sh"
)  # Galaxy assumes bash, but containers are usually thinner.
