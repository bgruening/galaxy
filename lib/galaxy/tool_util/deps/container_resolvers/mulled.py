...
CachedMulledImageSingleTarget = collections.namedtuple(
    "CachedMulledImageSingleTarget",
    ["package_name", "version", "build", "image_identifier"],
)
CachedV1MulledImageMultiTarget = collections.namedtuple(
    "CachedV1MulledImageMultiTarget", ["hash", "build", "image_identifier"]
)
CachedV2MulledImageMultiTarget = collections.namedtuple(
...
def list_docker_cached_mulled_images(namespace=None, hash_func="v2"):
    ...
    images_and_versions = (
        unicodify(subprocess.check_output(command)).strip().splitlines()
    )
    ...
    # TODO: Sort on build ...
    raw_images = [
        output_line_to_image(_) for _ in filter(name_filter, images_and_versions)
    ]
...
def identifier_to_cached_target(identifier, hash_func, namespace=None):
    ...
    if image_name.startswith(prefix + "mulled-v1-"):
        ...
        image = CachedV2MulledImageMultiTarget(
            image_name, version_hash, build, identifier
        )
    else:
        build = None
        if version and "--" in version:
...
def list_cached_mulled_images_from_path(directory, hash_func="v2"):
    ...
    raw_images = map(
        lambda name: identifier_to_cached_target(name, hash_func), contents
    )
    return [i for i in raw_images if i is not None]
    ...
    if len(targets) == 1:
        ...
        for cached_image in cached_images:
            ...
            if version_hash is None:
                ...
                if (
                    package_hash == cached_image.package_hash
                    and version_hash == cached_image.version_hash
                ):
                    image = cached_image
                    break
...
def docker_cached_container_description(
    targets, namespace, hash_func="v2", shell=DEFAULT_CONTAINER_SHELL
):
    if len(targets) == 0:
        return None
    ...
    if image:
        container = ContainerDescription(
            image.image_identifier, type="docker", shell=shell
        )
...
def singularity_cached_container_description(
    targets, cache_directory, hash_func="v2", shell=DEFAULT_CONTAINER_SHELL
):
    if len(targets) == 0:
        return None
    ...
    cached_images = list_cached_mulled_images_from_path(
        cache_directory, hash_func=hash_func
    )
    image = find_best_matching_cached_image(targets, cached_images, hash_func)
...
class CachedMulledDockerContainerResolver(ContainerResolver):
    ...
    def __init__(
        self, app_info=None, namespace="biocontainers", hash_func="v2", **kwds
    ):
...
class CachedMulledSingularityContainerResolver(ContainerResolver):
    ...
    def __init__(self, app_info=None, hash_func="v2", **kwds):
        ...
        self.cache_directory = kwds.get(
            "cache_directory",
            os.path.join(app_info.container_image_cache_path, "singularity", "mulled"),
        )
        self.hash_func = hash_func
    ...
    def __str__(self):
        return (
            "CachedMulledSingularityContainerResolver[cache_directory=%s]"
...
class MulledDockerContainerResolver(ContainerResolver):
    ...
    def resolve(self, enabled_container_types, tool_info, install=False, **kwds):
        ...
        name = targets_to_mulled_name(
            targets=targets, hash_func=self.hash_func, namespace=self.namespace
        )
        if name:
            ...
            container_description = ContainerDescription(
                container_id, type=self.container_type, shell=self.shell
            )
            destination_for_container_type = kwds.get("destination_for_container_type")
            if (
                install
                and destination_for_container_type
                and not self.cached_container_description(
                    targets, namespace=self.namespace, hash_func=self.hash_func
                )
            ):
...
class BuildMulledSingularityContainerResolver(ContainerResolver):
    ...
    def __str__(self):
        return (
            "BuildSingularityContainerResolver[cache_directory=%s]"
            % self.cache_directory
        )
