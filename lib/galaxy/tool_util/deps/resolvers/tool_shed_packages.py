...
class ToolShedPackageDependencyResolver(
    BaseGalaxyPackageDependencyResolver, UsesInstalledRepositoriesMixin
):
    ...
    def __init__(self, dependency_manager, **kwds):
        super(ToolShedPackageDependencyResolver, self).__init__(
    ...
    def _find_dep_versioned(self, name, version, type="package", **kwds):
        ...
        if installed_tool_dependency:
            path = self._get_package_installed_dependency_path(
                installed_tool_dependency, name, version
            )
    ...
    def _get_package_installed_dependency_path(
        self, installed_tool_dependency, name, version
    ):
    ...
    def _get_set_environment_installed_dependency_script_path(
        self, installed_tool_dependency, name
    ):
        tool_shed_repository = installed_tool_dependency.tool_shed_repository
        base_path = self.base_path
        path = abspath(
