...
def involucro_link():
    if IS_OS_X:
        url = (
            "https://github.com/mvdbeek/involucro/releases/download/v%s/involucro.darwin"
            % INVOLUCRO_VERSION
        )
    else:
        url = (
            "https://github.com/involucro/involucro/releases/download/v%s/involucro"
            % INVOLUCRO_VERSION
        )
    return url
...
def get_affected_packages(args):
    ...
    pkg_list = set(
        [
            ...
            if x.startswith("recipes/") and x.endswith("meta.yaml")
        ]
    )
    for pkg in pkg_list:
...
def mull_targets(
        ...
        if not rebuild:
            ...
            if ":" in repo_template_kwds["image"]:
                ...
                assert len(image_name_parts) == 2, (
                    ": not allowed in image name [%s]" % repo_template_kwds["image"]
                )
    ...
    if singularity:
        ...
        involucro_args.extend(
            ["-set", "SINGULARITY_IMAGE_NAME='%s'" % singularity_image_name]
        )
        involucro_args.extend(
            ["-set", "SINGULARITY_IMAGE_DIR='%s'" % singularity_image_dir]
        )
        ...
        for test_file in test_files:
            if ":" not in test_file:
                if os.path.exists(test_file):
                    test_bind.append(
                        "%s:%s/%s" % (test_file, DEFAULT_WORKING_DIR, test_file)
                    )
            else:
    ...
    if not dry_run:
        ...
        if singularity:
            ...
            with open(
                os.path.join(singularity_image_dir, "Singularity.def"), "w+"
            ) as sin_def:
...
def install_involucro(involucro_context=None, to_path=None):
    ...
    download_cmd = " ".join(
        commands.download_command(involucro_link(), to=install_path, quote_url=True)
    )
...
def main(argv=None):
    ...
    parser.add_argument(
        "command", metavar="COMMAND", help="Command (build-and-test, build, all)"
    )
    parser.add_argument(
        "targets",
        metavar="TARGETS",
