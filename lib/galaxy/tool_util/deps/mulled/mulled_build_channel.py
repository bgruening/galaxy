    ...
    if not os.path.exists(repo_data):
        ...
        subprocess.check_call(
            [
                "wget",
                "--quiet",
                "https://conda.anaconda.org/%s/%s/repodata.json.bz2"
                % (channel, platform_tag),
                "-O",
                "%s.bz2" % repo_data,
            ]
        )
        subprocess.check_call(["bzip2", "-d", "%s.bz2" % repo_data])
    return repo_data
...
def main(argv=None):
    ...
    parser.add_argument(
        "command", metavar="COMMAND", help="Command (list, build-and-test, build, all)"
    )
    parser.add_argument(
        "--targets",
        dest="targets",
