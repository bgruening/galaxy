...
class ColumnDataProvider(BaseDataProvider):
    ...
    def get_data(
        self, columns=None, start_val=0, max_vals=None, skip_comments=True, **kwargs
    ):
        """
        ...
        response["meta"] = meta = [
            {"min": None, "max": None, "count": 0, "sum": 0} for column in columns
        ]

        column_types = [
            self.original_dataset.metadata.column_types[column] for column in columns
        ]
                ...
                if column < fields_len:
                    ...
                    if column_val is not None:
                        ...
                        if column_type == "float" or column_type == "int":
                            if (meta[index]["min"] is None) or (
                                column_val < meta[index]["min"]
                            ):
                                meta[index]["min"] = column_val

                            if (meta[index]["max"] is None) or (
                                column_val > meta[index]["max"]
                            ):
        ...
        for index, meta in enumerate(response["meta"]):
            ...
            if (column_type == "float" or column_type == "int") and count:
                ...
                if count % 2 == 0:
                    meta["median"] = (
                        sorted_data[middle_index] + sorted_data[(middle_index + 1)]
                    ) / 2.0
