...
PYSAM_INDEX_SYMLINK_NECESSARY = packaging.version.parse(
    pysam.__version__
        ...
        **kwargs
        ...
        with self.open_data_file() as data_file:
            ...
            data = self.process_data(
                iterator, start_val, max_vals, start=start, end=end, **kwargs
            )
        return data
        ...
        if isinstance(self.original_dataset.datatype, Gff):
            ...
            if isinstance(self.original_dataset.datatype, Gtf):
                ...
                for (
                    name,
                    a_type,
                ) in self.original_dataset.metadata.attribute_types.items():
...
class TabixDataProvider(GenomeDataProvider, FilterableMixin):
    ...
    def open_data_file(self):
        ...
        with pysam.TabixFile(
            self.dependencies["bgzip"].file_name, index=index_path
        ) as f:
            yield f
        if PYSAM_INDEX_SYMLINK_NECESSARY:
            os.unlink(index_path)
        ...
        with self.open_data_file() as data_file, open(filename, "w") as out:
            for region in regions:
                ...
                iterator = self.get_iterator(
                    data_file, region.chrom, region.start, region.end
                )
        ...
        for count, line in enumerate(iterator):
            ...
            if length >= 12:
                ...
                payload.append(
                    [
                        (
                            int(feature[1]) + block[1],
                            int(feature[1]) + block[1] + block[0],
                        )
                        for block in blocks
                    ]
                )
    ...
    def process_data(self, iterator, start_val=0, max_vals=None, **kwargs):
        ...
        def get_mapping(ref, alt):
            ...
            if alt_in_ref_index != -1:
                return (
                    alt_in_ref_index,
                    ref[alt_in_ref_index + 1 :],
                    [[cig_ops.find("D"), ref_len - alt_len]],
                )
            ...
            if ref_in_alt_index != -1:
                return (
                    ref_in_alt_index,
                    alt[ref_in_alt_index + 1 :],
                    [[cig_ops.find("I"), alt_len - ref_len]],
                )
...
class BamDataProvider(GenomeDataProvider, FilterableMixin):
    ...
    def get_filters(self):
        ...
        filters = []
        filters.append(
            {"name": "Mapping Quality", "type": "number", "index": filter_col}
        )
        return filters
    ...
    def write_data_to_file(self, regions, filename):
        ...
        bamfile = pysam.AlignmentFile(
            self.original_dataset.file_name,
            mode="rb",
        ...
        **kwargs
        ...
        for read in read_iterator:
            ...
            if read.cigar is not None:
                read_len = sum(
                    [cig[1] for cig in read.cigar]
                )  # Use cigar to determine length
            else:
...
class SamDataProvider(BamDataProvider):
    ...
    def __init__(
        self, converted_dataset=None, original_dataset=None, dependencies=None
    ):
...
class BBIDataProvider(GenomeDataProvider):
    ...
    def get_data(
        self, chrom, start, end, start_val=0, max_vals=None, num_samples=1000, **kwargs
    ):
        start = int(start)
        end = int(end)
        ...
        if "stats" in kwargs:
            ...
            if summary is not None:
                ...
                if summary.valid_count > 0:
                    ...
                    var = max(
                        summary.sum_squares[0] - mean, 0
                    )  # Prevent variance underflow.
                    if valid_count > 1:
                        var /= valid_count - 1
                    sd = math.sqrt(var)
        ...
        def summarize_region(bbi, chrom, start, end, num_points):
            ...
            if summary:
                ...
                for i in range(num_points):
                    result.append(
                        (pos, float_nan(summary.sum_data[i] / summary.valid_count[i]))
                    )
                    pos += step_size
...
class IntervalIndexDataProvider(GenomeDataProvider, FilterableMixin):
    ...
    def write_data_to_file(self, regions, filename):
        ...
        with open(self.original_dataset.file_name) as source, open(
            filename, "w"
        ) as out:
            for region in regions:
                # Write data from region.
...
class RawGFFDataProvider(GenomeDataProvider):
    ...
    def get_iterator(self, data_file, chrom, start, end, **kwargs):
        ...
        def features_in_region_iter():
            ...
            for feature in GFFReaderWrapper(source, fix_strand=True):
                ...
                feature_start, feature_end = convert_gff_coords_to_bed(
                    [feature.start, feature.end]
                )
                if (
                    feature.chrom == chrom
                    and feature_end > start
                    and feature_start < end
                ):
    ...
    def process_data(self, iterator, start_val=0, max_vals=None, **kwargs):
        ...
        for count, (feature, offset) in enumerate(iterator):
            ...
            payload = package_gff_feature(
                feature, no_detail=no_detail, filter_cols=filter_cols
            )
            payload.insert(0, offset)
            results.append(payload)
...
class ChromatinInteractionsTabixDataProvider(
    ...
    def get_iterator(
        ...
        **kwargs
        ...
        return filter(
            TabixDataProvider.get_iterator(self, data_file, chrom, filter_start, end)
        )
...
def package_gff_feature(feature, no_detail=False, filter_cols=[]):
    ...
    payload.append(
        [
            (feature.start + block[1], feature.start + block[1] + block[0])
            for block in blocks
        ]
    )
