...
class Newick_Parser(Base_Parser):
                ...
                if re.match(r"[,(]", newickString[i + 1 :]):
                    ...
                    for j in range(i + 1, len(newickString)):
                        ...
                        if (
                            ...
                            newString += (
                                newickString[start:end] + nameMap[termToReplace]
                            )  # + "'"  "'" +
                            start = j
                            break
    ...
    def parseNode(self, string, depth):
        ...
        for j in range(len(string)):
            ...
            if (
                string[j] == ")"
            ):  # finding the positions of all the closed brackets to extract claude
                i = bracketStack.pop()
                ...
                if len(bracketStack) == 0:  # is child of current node
                    ...
                    if j + 1 < len(string):
                        stringRightOfBracket = string[
                            j + 1 :
                        ]  # Eg. '(b:0.4,a:0.3)c:0.3, stringRightOfBracket = c:0.3
