...
class Nexus_Parser(Newick_Parser):
    ...
    def parseNexus(self, filename):
        ...
        intranslateBlock = (
            False
        )  # sentinel to check if we are in the translate region of the tree. Stores synonyms of the labellings
        ...
        for line in nexlines:
            ...
            if inTreeBlock:
                ...
                if "tree" in lline:
                    ...
                    treeNames.append(
                        (treeName, treeIndex)
                    )  # appending name of tree, and its index
                    continue
        ...
        if not self.inCommentBlock:
            if "[" in line:
                if "]" not in line:
                    ...
                    return (
                        "Nextline"
                    )  # need to move on to the nextline after getting out of comment
        else:
            if "]" in line:
