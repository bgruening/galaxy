...
class InteractiveEnvironmentRequest(object):
    def __init__(self, trans, plugin):
        ...
        self.attr.app_root = (
            self.attr.root
            + "static/plugins/interactive_environments/"
            + self.attr.viz_id
            + "/static/"
        )
        ...
        self.attr.docker_connect_port = (
            int(raw_docker_connect_port) if raw_docker_connect_port else None
        )
        ...
        ie_parent_temp_dir = (
            self.attr.viz_config.get("docker", "docker_galaxy_temp_dir") or None
        )
        ...
        assert (
            not self.attr.container_interface
            or not self.attr.container_interface.publish_port_list_required
            or (
                self.attr.container_interface.publish_port_list_required
                and self.attr.docker_connect_port is not None
            )
    ...
    def load_allowed_images(self):
        if os.path.exists(os.path.join(self.attr.our_config_dir, "allowed_images.yml")):
            ...
            os.path.join(self.attr.our_config_dir, "allowed_images.yml.sample")
        ):
            ...
            try:
                ...
                raise Exception(
                    "[{0}] Could not find allowed_images.yml, or image tag in {0}.ini file for ".format(
                        self.attr.viz_id
                    )
                )
    ...
    def load_container_interface(self):
        ...
        if (
            string_as_bool_or_none(
                self.attr.viz_config.get("main", "container_interface")
            )
            is not None
        ):
    ...
    def get_conf_dict(self):
        ...
        conf_file["galaxy_web_port"] = (
            web_port or self.attr.galaxy_config.guess_galaxy_port()
        )
        ...
        if self.attr.viz_config.has_option("docker", "galaxy_url"):
            ...
            conf_file["galaxy_url"] = (
                self.attr.galaxy_config.galaxy_infrastructure_url.rstrip("/") + "/"
            )
        else:
    ...
    def url_template(self, url_template):
        ...
        url_template = url_template.replace("${PROTO}", protocol).replace(
            "${HOST}", self.attr.HOST
        )

        # Only the following replacements are used with Galaxy dynamic proxy
        # URLs
        url = url_template.replace("${PROXY_URL}", str(self.attr.proxy_url)).replace(
            "${PROXY_PREFIX}", str(self.attr.proxy_prefix.replace("/", "%2F"))
        )
        return url
    ...
    def volume(self, container_path, host_path, **kwds):
        if self.attr.container_interface is None:
            ...
            return self.attr.container_interface.volume_class(
                container_path, host_path=host_path, mode=kwds.get("mode", "ro")
            )
    ...
    def base_docker_cmd(self, subcmd=None):
        ...
        base = shlex.split(
            self.attr.viz_config.get("docker", "command").format(docker_args="").strip()
        )
        if subcmd:
            base.append(subcmd)
        return base
    ...
    def docker_cmd(self, image, env_override=None, volumes=None):
        ...
        def _check_uid_and_gid(cmd_inject):
            ...
            uid_gid_subs = {
                "$(id -u)": "{}".format(os.geteuid()),
                "$(id -g)": "{}".format(os.getgid()),
            }
    ...
    def use_volumes(self):
        if (
            self.attr.container_interface
            and not self.attr.container_interface.supports_volumes
        ):
            return False
    ...
    def _get_command_inject_env(self):
        ...
        envsets = []
        command_inject = (
            self.attr.viz_config.get("docker", "command_inject").strip().split()
        )
    ...
    def _ids_to_volumes(self, ids):
        ...
        for id in ids:
            ...
            dataset = self.trans.sa_session.query(model.HistoryDatasetAssociation).get(
                decoded_id
            )
            ...
            volumes.append(
                self.volume(
                    "/import/[{0}] {1}.{2}".format(
                        dataset.id, dataset.name, dataset.ext
                    ),
    ...
    def _launch_legacy(self, image, env_override, volumes):
        ...
        log.info(
            "Starting docker container for IE {0} with command [{1}]".format(
                self.attr.viz_id, " ".join([shlex_quote(x) for x in redacted_command])
            )
        )
        ...
        if p.returncode != 0:
            ...
            log.debug(
                "Container host/port: %s:%s", self.attr.docker_hostname, host_port
            )
    ...
    def _launch_container_interface(self, image, env_override, volumes):
        ...
        if not container_port:
            log.warning(
                "Container %s (%s) created but no port information available, readiness check will determine "
                "ports",
                container.name,
                container.id,
            )
            container_port = ContainerPort(
                self.attr.docker_connect_port, None, None, None
            )
        ...
        log.debug(
            "Container %s (%s) port %s accessible at: %s:%s",
            container.name,
            container.id,
            container_port.port,
            container_port.hostaddr,
    ...
    def inspect_container(self, container_id):
        ...
        log.info(
            "Inspecting docker container {0} with command [{1}]".format(
                container_id, " ".join([shlex_quote(x) for x in raw_cmd])
            )
        )
