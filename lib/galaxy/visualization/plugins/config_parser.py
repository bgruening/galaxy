...
class VisualizationsConfigParser(object):
    ...
    def parse_visualization(self, xml_tree):
        ...
        if "disabled" in xml_tree.attrib:
            log.info(
                "Visualizations plugin disabled: %s. Skipping...", returned["name"]
            )
            return None
        ...
        returned["description"] = (
            description.text.strip() if description is not None else None
        )
        ...
        param_modifiers = {}
        param_modifier_elements = (
            param_confs.findall("param_modifier") if param_confs is not None else []
        )
...
class DataSourceParser(object):
    ...
    def parse_model_class(self, xml_tree):
        ...
        if xml_tree.text not in self.ALLOWED_MODEL_CLASSES:
            ...
            raise ParsingException(
                "Invalid data_source model_class: %s" % (xml_tree.text)
            )
    ...
    def _build_getattr_lambda(self, attr_name_list):
        ...
        # recursive case
        return lambda o: getattr(
            self._build_getattr_lambda(attr_name_list[:-1])(o), next_attr_name
        )
    ...
    def parse_tests(self, xml_tree_list):
        ...
        for test_elem in xml_tree_list:
            ...
            test_attr = (
                test_attr.split(self.ATTRIBUTE_SPLIT_CHAR)
                if isinstance(test_attr, string_types)
                else []
            )
            ...
            if test_type == "isinstance":
                ...
                def test_fn(o, result):
                    return hasattr(getter(o), "has_dataprovider") and getter(
                        o
                    ).has_dataprovider(result)
