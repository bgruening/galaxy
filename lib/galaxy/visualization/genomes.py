...
class GenomeRegion(object):
    ...
    def from_dict(obj_dict):
        return GenomeRegion(
            chrom=obj_dict["chrom"], start=obj_dict["start"], end=obj_dict["end"]
        )
    ...
    def from_str(obj_str):
        ...
        if len(gene_region) == 2:
            ...
            if len(gene_interval) == 2:
                return GenomeRegion(
                    chrom=gene_region[0], start=gene_interval[0], end=gene_interval[1]
                )
...
class Genomes(object):
    ...
    def reload_genomes(self):
        ...
        if twobit_table is None:
            ...
            try:
                for line in open(
                    os.path.join(self.app.config.tool_data_path, "twobit.loc")
                ):
                    if line.startswith("#"):
                        continue
                    val = line.split()
    ...
    def chroms(self, trans, dbkey=None, num=None, chrom=None, low=None):
        ...
        if dbkey_owner:
            dbkey_user = (
                ...
                .filter_by(username=dbkey_owner)
                .first()
            )
        else:
        ...
        if dbkey_user and "dbkeys" in dbkey_user.preferences:
            ...
            if dbkey in user_keys:
                ...
                if len_file:
                    genome = Genome(
                        dbkey, dbkey_name, len_file=len_file, twobit_file=twobit_file
                    )
