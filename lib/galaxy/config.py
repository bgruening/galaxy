...
PATH_DEFAULTS = dict(
    ...
    data_manager_config_file=[
        ...
        "config/data_manager_conf.xml.sample",
    ],
    ...
    tool_destinations_config_file=[
        ...
        "config/tool_destinations.yml.sample",
    ],
    ...
    oidc_backends_config_file=[
        "config/oidc_backends_config.yml",
        "config/oidc_backends_config.yml.sample",
    ],
    ...
    workflow_resource_params_file=[
        "config/workflow_resource_params_conf.xml",
    ...
    workflow_schedulers_config_file=[
        ...
        "config/workflow_schedulers_conf.xml.sample",
    ],
    modules_mapping_files=[
        "config/environment_modules_mapping.yml",
        "config/environment_modules_mapping.yml.sample",
    ],
    local_conda_mapping_file=[
        "config/local_conda_mapping.yml",
        "config/local_conda_mapping.yml.sample",
    ],
...
PATH_LIST_DEFAULTS = dict(
    ...
    # [3]: fresh install
    tool_config_file=[
        "config/tool_conf.xml,shed_tool_conf.xml",
...
LOGGING_CONFIG_DEFAULT = {
    ...
    "loggers": {
        ...
        "routes.middleware": {"level": "WARN", "qualname": "routes.middleware"},
    },
    "filters": {"stack": {"()": "galaxy.web.stack.application_stack_log_filter"}},
    "handlers": {
        "console": {
    ...
    "formatters": {"stack": {"()": "galaxy.web.stack.application_stack_log_formatter"}},
}
...
class Configuration(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.database_encoding = kwargs.get(
            "database_encoding", None
        )  # Create new databases with this encoding.
        ...
        self.openid_consumer_cache_path = resolve_path(
            kwargs.get("openid_consumer_cache_path", "database/openid_consumer_cache"),
            self.root,
        )
        ...
        self.builds_file_path = resolve_path(
            kwargs.get(
                "builds_file_path",
                os.path.join(self.tool_data_path, "shared", "ucsc", "builds.txt"),
            ),
            self.root,
        )
        ...
        self.oidc_backends_config = kwargs.get(
            "oidc_backends_config_file", self.oidc_backends_config_file
        )
        self.oidc = []
        ...
        integrated_tool_panel_tracking_directory = kwargs.get(
            "integrated_tool_panel_tracking_directory", None
        )
        if integrated_tool_panel_tracking_directory:
            self.integrated_tool_panel_tracking_directory = resolve_path(
                integrated_tool_panel_tracking_directory, self.root
            )
        else:
        ...
        self.tool_label_filters = listify(
            kwargs.get("tool_label_filters", []), do_strip=True
        )
        self.tool_section_filters = listify(
            kwargs.get("tool_section_filters", []), do_strip=True
        )

        self.user_tool_filters = listify(
            kwargs.get("user_tool_filters", []), do_strip=True
        )
        self.user_tool_label_filters = listify(
            kwargs.get("user_tool_label_filters", []), do_strip=True
        )
        self.user_tool_section_filters = listify(
            kwargs.get("user_tool_section_filters", []), do_strip=True
        )
        self.has_user_tool_filters = bool(
            self.user_tool_filters
            or self.user_tool_label_filters
            or self.user_tool_section_filters
        )

        self.tour_config_dir = resolve_path(
            kwargs.get("tour_config_dir", "config/plugins/tours"), self.root
        )
        self.webhooks_dirs = resolve_path(
            kwargs.get("webhooks_dir", "config/plugins/webhooks"), self.root
        )
        ...
        self.password_expiration_period = timedelta(
            days=int(kwargs.get("password_expiration_period", 0))
        )
        ...
        self.check_migrate_tools = string_as_bool(
            kwargs.get("check_migrate_tools", False)
        )
        ...
        self.enable_tool_shed_check = string_as_bool(
            kwargs.get("enable_tool_shed_check", False)
        )
        ...
        try:
            if isinstance(self.hours_between_check, int):
                ...
                if self.running_functional_tests:
                    if (
                        self.hours_between_check < 0.001
                        or self.hours_between_check > 24.0
                    ):
                        self.hours_between_check = 12.0
                else:
                    if (
                        self.hours_between_check < 1.0
                        or self.hours_between_check > 24.0
                    ):
                        self.hours_between_check = 12.0
            else:
        ...
        self.max_metadata_value_size = int(
            kwargs.get("max_metadata_value_size", 5242880)
        )
        self.metadata_strategy = kwargs.get("metadata_strategy", "directory")
        self.single_user = kwargs.get("single_user", None)
        self.use_remote_user = (
            string_as_bool(kwargs.get("use_remote_user", "False")) or self.single_user
        )
        ...
        self.fetch_url_whitelist_ips = [
            ipaddress.ip_network(
                unicodify(ip.strip())
            )  # If it has a slash, assume 127.0.0.1/24 notation
            if "/" in ip
        ...
        self.allow_user_dataset_purge = string_as_bool(
            kwargs.get("allow_user_dataset_purge", "True")
        )
        self.allow_user_impersonation = string_as_bool(
            kwargs.get("allow_user_impersonation", "False")
        )
        self.show_user_prepopulate_form = string_as_bool(
            kwargs.get("show_user_prepopulate_form", "False")
        )
        ...
        self.template_cache = resolve_path(
            kwargs.get("template_cache_path", "database/compiled_templates"), self.root
        )
        ...
        default_jobs_directory = kwargs.get(
            "job_working_directory", "database/jobs_directory"
        )
        self.jobs_directory = resolve_path(
            kwargs.get("jobs_directory", default_jobs_directory), self.root
        )
        self.default_job_shell = kwargs.get("default_job_shell", "/bin/bash")
        self.cleanup_job = kwargs.get("cleanup_job", "always")
        preserve_python_environment = kwargs.get(
            "preserve_python_environment", "legacy_only"
        )
        if preserve_python_environment not in [
            "legacy_only",
            "legacy_and_local",
            "always",
        ]:
            log.warning(
                "preserve_python_environment set to unknown value [%s], defaulting to legacy_only"
            )
            preserve_python_environment = "legacy_only"
        ...
        self.container_image_cache_path = self.resolve_path(
            kwargs.get("container_image_cache_path", "database/container_images")
        )
        if not os.path.exists(self.container_image_cache_path):
            self.container_image_cache_path = self.resolve_path(
                kwargs.get("container_image_cache_path", "database/container_cache")
            )
        ...
        self.check_job_script_integrity = string_as_bool(
            kwargs.get("check_job_script_integrity", True)
        )
        self.admin_users = kwargs.get("admin_users", "")
        self.admin_users_list = [u.strip() for u in self.admin_users.split(",") if u]
        self.mailing_join_addr = kwargs.get(
        ...
        self.inactivity_box_content = kwargs.get(
            "inactivity_box_content", default_inactivity_box_content
        )
        self.terms_url = kwargs.get("terms_url", None)
        self.myexperiment_target_url = kwargs.get(
            "my_experiment_target_url", "www.myexperiment.org"
        )
        ...
        self.track_jobs_in_database = string_as_bool(
            kwargs.get("track_jobs_in_database", "True")
        )
        self.expose_dataset_path = string_as_bool(
            kwargs.get("expose_dataset_path", "False")
        )
        self.expose_potentially_sensitive_job_metrics = string_as_bool(
            kwargs.get("expose_potentially_sensitive_job_metrics", "False")
        )
        self.enable_communication_server = string_as_bool(
            kwargs.get("enable_communication_server", "False")
        )
        self.communication_server_host = kwargs.get(
            "communication_server_host", "http://localhost"
        )
        self.communication_server_port = int(
            kwargs.get("communication_server_port", "7070")
        )
        self.persistent_communication_rooms = listify(
            kwargs.get("persistent_communication_rooms", []), do_strip=True
        )
        ...
        self.enable_beta_ts_api_install = string_as_bool(
            kwargs.get("enable_beta_ts_api_install", "True")
        )
        ...
        # in the job handler.
        self.legacy_eager_objectstore_initialization = string_as_bool(
            kwargs.get("legacy_eager_objectstore_initialization", "False")
        )
        ...
        # future.
        self.enable_beta_workflow_modules = string_as_bool(
            kwargs.get("enable_beta_workflow_modules", "False")
        )
        # Default format for the export of workflows.
        self.default_workflow_export_format = kwargs.get(
        ...
        self.enable_beta_tool_formats = string_as_bool(
            kwargs.get("enable_beta_tool_formats", "False")
        )
        # Beta containers interface used by GIEs
        self.enable_beta_containers_interface = string_as_bool(
            kwargs.get("enable_beta_containers_interface", "False")
        )
        ...
        self.force_beta_workflow_scheduled_min_steps = int(
            kwargs.get("force_beta_workflow_scheduled_min_steps", "250")
        )
        ...
        self.history_local_serial_workflow_scheduling = string_as_bool(
            kwargs.get("history_local_serial_workflow_scheduling", "False")
        )
        ...
        self.maximum_workflow_invocation_duration = int(
            kwargs.get("maximum_workflow_invocation_duration", 2678400)
        )
        self.maximum_workflow_jobs_per_scheduling_iteration = int(
            kwargs.get("maximum_workflow_jobs_per_scheduling_iteration", -1)
        )

        workflow_resource_params_mapper = kwargs.get(
        ...
        if not workflow_resource_params_mapper:
            ...
            workflow_resource_params_mapper = self.resolve_path(
        ...
        self.monitor_thread_join_timeout = int(
            kwargs.get("monitor_thread_join_timeout", 30)
        )
        self.log_actions = string_as_bool(kwargs.get("log_actions", "False"))
        self.log_events = string_as_bool(kwargs.get("log_events", "False"))
        self.sanitize_all_html = string_as_bool(kwargs.get("sanitize_all_html", True))
        self.sanitize_whitelist_file = resolve_path(
            kwargs.get("sanitize_whitelist_file", "config/sanitize_whitelist.txt"),
            self.root,
        )
        ...
        if "trust_jupyter_notebook_conversion" in kwargs:
            trust_jupyter_notebook_conversion = string_as_bool(
                kwargs.get("trust_jupyter_notebook_conversion", False)
            )
        else:
            trust_jupyter_notebook_conversion = string_as_bool(
                kwargs.get("trust_ipython_notebook_conversion", False)
            )
        ...
        self.show_welcome_with_login = string_as_bool(
            kwargs.get("show_welcome_with_login", "False")
        )
        ...
        self.genomespace_ui_url = kwargs.get(
        ...
        self.user_library_import_symlink_whitelist = listify(
            kwargs.get("user_library_import_symlink_whitelist", []), do_strip=True
        )
        self.user_library_import_check_permissions = string_as_bool(
            kwargs.get("user_library_import_check_permissions", False)
        )
        self.user_library_import_dir_auto_creation = (
            string_as_bool(kwargs.get("user_library_import_dir_auto_creation", False))
            if self.user_library_import_dir
            else False
        )
        ...
        )  # attribute on user - email, username, id, etc...
        ...
        self.allow_library_path_paste = string_as_bool(
            kwargs.get("allow_library_path_paste", self.allow_path_paste)
        )
        self.disable_library_comptypes = (
            kwargs.get("disable_library_comptypes", "").lower().split(",")
        )
        self.sniff_compressed_dynamic_datatypes_default = string_as_bool(
            kwargs.get("sniff_compressed_dynamic_datatypes_default", True)
        )
        self.check_upload_content = string_as_bool(
            kwargs.get("check_upload_content", True)
        )
        ...
        default_tool_test_data_directories = os.environ.get(
            "GALAXY_TEST_FILE_DIR", resolve_path("test-data", self.root)
        )
        self.tool_test_data_directories = kwargs.get(
            "tool_test_data_directories", default_tool_test_data_directories
        )
        # Location for tool dependencies.
        use_tool_dependencies, tool_dependency_dir, use_cached_dependency_manager, tool_dependency_cache_dir, precache_dependencies = parse_dependency_options(
            kwargs, self.root, self.dependency_resolvers_config_file
        )
        ...
        self.enable_beta_mulled_containers = string_as_bool(
            kwargs.get("enable_beta_mulled_containers", "False")
        )
        containers_resolvers_config_file = kwargs.get(
            "containers_resolvers_config_file", None
        )
        if containers_resolvers_config_file:
            containers_resolvers_config_file = resolve_path(
                containers_resolvers_config_file, self.root
            )
        ...
        if involucro_path is None:
            involucro_path = os.path.join(
                tool_dependency_dir or "database", "involucro"
            )
        self.involucro_path = resolve_path(involucro_path, self.root)
        self.involucro_auto_init = string_as_bool(
            kwargs.get("involucro_auto_init", True)
        )
        ...
        default_job_resubmission_condition = kwargs.get(
        ...
        self.nginx_upload_job_files_store = kwargs.get(
            "nginx_upload_job_files_store", False
        )
        self.nginx_upload_job_files_path = kwargs.get(
        ...
        self.object_store_cache_path = resolve_path(
            kwargs.get("object_store_cache_path", "database/object_store_cache"),
            self.root,
        )
        ...
        if (
            global_conf
            and "__file__" in global_conf
            and ".yml" not in global_conf["__file__"]
        ):
        ...
        if "HOST_IP" in galaxy_infrastructure_url:
            galaxy_infrastructure_url = string.Template(
                galaxy_infrastructure_url
            ).safe_substitute({"HOST_IP": socket.gethostbyname(socket.gethostname())})
        ...
        self.job_handlers = [
            x.strip() for x in kwargs.get("job_handlers", self.server_name).split(",")
        ]
        self.default_job_handlers = [
            x.strip()
            for x in kwargs.get(
                "default_job_handlers", ",".join(self.job_handlers)
            ).split(",")
        ]
        ...
        if "amqp_internal_connection" in kwargs:
            ...
            self.amqp_internal_connection = (
                "sqlalchemy+sqlite:///%s?isolation_level=IMMEDIATE"
                % resolve_path("database/control.sqlite", self.root)
            )
        self.pretty_datetime_format = expand_pretty_datetime_format(
            kwargs.get("pretty_datetime_format", "$locale (UTC)")
        )
        self.user_preferences_extra_config_file = kwargs.get(
        ...
        try:
            ...
            log.warning(
                "Config file (%s) could not be found or is malformed."
                % self.user_preferences_extra_config_file
            )
        ...
        self.gie_swarm_mode = string_as_bool(
            kwargs.get("interactive_environment_swarm_mode", False)
        )

        self.proxy_session_map = self.resolve_path(
            kwargs.get("dynamic_proxy_session_map", "database/session_map.sqlite")
        )
        self.manage_dynamic_proxy = string_as_bool(
            kwargs.get("dynamic_proxy_manage", "True")
        )  # Set to false if being launched externally
        self.dynamic_proxy_debug = string_as_bool(
            kwargs.get("dynamic_proxy_debug", "False")
        )
        self.dynamic_proxy_bind_port = int(
            kwargs.get("dynamic_proxy_bind_port", "8800")
        )
        self.dynamic_proxy_bind_ip = kwargs.get("dynamic_proxy_bind_ip", "0.0.0.0")
        self.dynamic_proxy_external_proxy = string_as_bool(
            kwargs.get("dynamic_proxy_external_proxy", "False")
        )
        self.dynamic_proxy_prefix = kwargs.get("dynamic_proxy_prefix", "gie_proxy")

        self.dynamic_proxy = kwargs.get("dynamic_proxy", "node")
        self.dynamic_proxy_golang_noaccess = kwargs.get(
            "dynamic_proxy_golang_noaccess", 60
        )
        self.dynamic_proxy_golang_clean_interval = kwargs.get(
            "dynamic_proxy_golang_clean_interval", 10
        )
        self.dynamic_proxy_golang_docker_address = kwargs.get(
            "dynamic_proxy_golang_docker_address", "unix:///var/run/docker.sock"
        )
        self.dynamic_proxy_golang_api_key = kwargs.get(
    ...
    def reload_sanitize_whitelist(self, explicit=True):
        ...
        try:
            ...
            if explicit:
                log.warning(
                    "Sanitize log file explicitly specified as '%s' but does not exist, continuing with no tools whitelisted.",
    ...
    def check(self):
        ...
        for datatypes_config in listify(self.datatypes_config):
            if not os.path.isfile(datatypes_config):
                raise ConfigurationError(
                    "Datatypes config file not found: %s" % datatypes_config
                )
...
def parse_dependency_options(kwargs, root, dependency_resolvers_config_file):
    ...
    if tool_dependency_dir is not None:
        ...
        tool_dependency_cache_dir = kwargs.get(
            "tool_dependency_cache_dir", os.path.join(tool_dependency_dir, "_cache")
        )
        use_cached_dependency_manager = string_as_bool(
            kwargs.get("use_cached_dependency_manager", "False")
        )
        precache_dependencies = string_as_bool(
            kwargs.get("precache_dependencies", "True")
        )
    else:
        tool_dependency_dir = None
...
def init_models_from_config(
    config, map_install_models=False, object_store=None, trace_logger=None
):
    ...
    if auto_configure_logging:
        ...
        if logging_conf is None:
            ...
            if config.get("log_level", "DEBUG") != "DEBUG":
                logging_conf["handlers"]["console"]["level"] = config.get(
                    "log_level", "DEBUG"
                )
        ...
        for name, conf in logging_conf.get("handlers", {}).items():
            if (
                ...
                conf["filename"] = conf.pop("filename_template").format(
                    **get_stack_facts(config=config)
                )
...
class ConfiguresGalaxyMixin(object):
    ...
    def _configure_genome_builds(
        self, data_table_name="__dbkeys__", load_old_style=True
    ):
        self.genome_builds = GenomeBuilds(
    ...
    def _set_enabled_container_types(self):
        ...
        for destinations in self.job_config.destinations.values():
            for destination in destinations:
                for (
                    enabled_container_type
                ) in self.container_finder._enabled_container_types(destination.params):
                    container_types_to_destinations[enabled_container_type].append(
                        destination
                    )
        ...
        self.toolbox.dependency_manager.resolver_classes.update(
            self.container_finder.container_registry.resolver_classes
        )
        self.toolbox.dependency_manager.dependency_resolvers.extend(
            self.container_finder.container_registry.container_resolvers
        )
    ...
    def _configure_datatypes_registry(self, installed_repository_manager=None):
        ...
        for datatypes_config in listify(datatypes_configs):
            ...
            self.datatypes_registry.load_datatypes(
                self.config.root, datatypes_config, override=True
            )
    ...
    def _configure_models(
        self, check_migrate_databases=False, check_migrate_tools=False, config_file=None
    ):
        """
        ...
        if check_migrate_databases:
            ...
            create_or_verify_database(
                db_url, config_file, self.config.database_engine_options, app=self
            )
            if not combined_install_database:
                from galaxy.model.tool_shed_install.migrate.check import (
                    create_or_verify_database as tsi_create_or_verify_database,
                )
