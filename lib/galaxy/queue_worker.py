...
def send_control_task(
    app, task, noop_self=False, get_response=False, routing_key="control.*", kwargs={}
):
    """
...
def reload_display_application(app, **kwargs):
    ...
    log.debug(
        "Executing display application reload task for %s" % display_application_ids
    )
...
def reload_tool_data_tables(app, **kwargs):
    params = util.Params(kwargs)
    log.debug(
        "Executing tool data table reload for %s"
...
def reload_job_rules(app, **kwargs):
    ...
    for module in job_rule_modules(app):
        ...
        for name, module in sys.modules.items():
            if (
                name == rules_module_name or name.startswith(rules_module_name + ".")
            ) and ismodule(module):
...
def admin_job_lock(app, **kwargs):
    ...
    log.info(
        "Administrative Job Lock is now set to %s. Jobs will %s dispatch."
