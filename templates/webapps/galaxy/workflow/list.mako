...
<%def name="stylesheets()">
    ${parent.stylesheets()}
    ${h.css( "jquery-ui/jquery.webui-popover.min" )}
...
<%def name="javascripts()">
    ${parent.javascripts()}
    ${h.js( "libs/jquery/jquery.webui-popover.min" )}
</%def>
    ...
    <script type="text/javascript">
        ...
        if ($tags.length > 3) {
            ...
            $tags.each(function(idx, elem) {
                if (idx > 2) {
                    popupTags.push($(elem).text());
                    $(elem).remove();
                }
            });

            $('.workflow-tags').append('<div class="popup-tags">' + popupTags[0].substr(0,2) + '...</div>');
            $('.popup-tags').webuiPopover({
                content: '<p>' + popupTags.join(', ') + '</p>',
                placement: 'right',
                trigger: 'hover',
                width: 200
            });
                            ...
                            <td>
                                <div class="workflow-tags">
                                    %for tag in workflow.w_tags:
                                        <div class="tag">${h.to_unicode( tag ) | h}</div>
                                    %endfor
                                </div>
                            </td>
                            <td>
